...
function SelectMatcherMode(t0) {
    ...
    if (matchersForSelectedEvent.length === 0) {
        ...
        if ($[7] === Symbol.for("react.memo_cache_sentinel")) {
            t3 = React.createElement(ink_js_1.Box, { flexDirection: "column", gap: 1 },
        ...
        if ($[8] !== eventDescription || $[9] !== onCancel || $[10] !== t2) {
            t4 = React.createElement(Dialog_js_1.Dialog, { title: t2, subtitle: eventDescription, onCancel: onCancel, inputGuide: _temp2 }, t3);
            $[8] = eventDescription;
            $[9] = onCancel;
            $[10] = t2;
            $[11] = t4;
    ...
    if ($[16] !== onCancel || $[17] !== t3 || $[18] !== t4) {
        t5 = React.createElement(ink_js_1.Box, { flexDirection: "column" },
            React.createElement(select_js_1.Select, { options: t3, onChange: t4, onCancel: onCancel }));
        $[16] = onCancel;
        $[17] = t3;
        $[18] = t4;
        $[19] = t5;
    ...
    if ($[20] !== eventDescription || $[21] !== onCancel || $[22] !== t2 || $[23] !== t5) {
        t6 = React.createElement(Dialog_js_1.Dialog, { title: t2, subtitle: eventDescription, onCancel: onCancel }, t5);
        $[20] = eventDescription;
        $[21] = onCancel;
        $[22] = t2;
        $[23] = t5;
...
function _temp3(item) {
    const sourceText = item.sources.map(hooksSettings_js_1.hookSourceInlineDisplayString).join(", ");
    const matcherLabel = item.matcher || "(all)";
    return {
        label: `[${sourceText}] ${matcherLabel}`,
        value: item.matcher,
        description: `${item.hookCount} ${(0, stringUtils_js_1.plural)(item.hookCount, "hook")}`
    };
}
function _temp2() {
