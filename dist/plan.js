...
const staticRender_js_1 = require("../../utils/staticRender.js");
function PlanDisplay(t0) {
    const $ = (0, compiler_runtime_1.c)(11);
    const { planContent, planPath, editorName } = t0;
    let t1;
    if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
        t1 = React.createElement(ink_js_1.Text, { bold: true }, "Current Plan");
        $[0] = t1;
    }
    else {
        t1 = $[0];
    ...
    let t2;
    if ($[1] !== planPath) {
        t2 = React.createElement(ink_js_1.Text, { dimColor: true }, planPath);
        $[1] = planPath;
        $[2] = t2;
    }
    else {
    ...
    if ($[3] !== planContent) {
        t3 = React.createElement(ink_js_1.Box, { marginTop: 1 },
            React.createElement(ink_js_1.Text, null, planContent));
        $[3] = planContent;
        $[4] = t3;
    }
    else {
    ...
    let t4;
    if ($[5] !== editorName) {
        t4 = editorName && React.createElement(ink_js_1.Box, { marginTop: 1 },
            React.createElement(ink_js_1.Text, { dimColor: true }, "\"/plan open\""),
            React.createElement(ink_js_1.Text, { dimColor: true }, " to edit this plan in "),
            React.createElement(ink_js_1.Text, { bold: true, dimColor: true }, editorName));
        $[5] = editorName;
        $[6] = t4;
    }
    else {
...
async function call(onDone, context, args) {
    ...
    if (currentMode !== 'plan') {
        ...
        setAppState(prev => ({
            ...prev,
            toolPermissionContext: (0, PermissionUpdate_js_1.applyPermissionUpdate)((0, permissionSetup_js_1.prepareContextForPlanMode)(prev.toolPermissionContext), {
                type: 'setMode',
                mode: 'plan',
                destination: 'session'
            })
    ...
    const planContent = (0, plans_js_1.getPlan)();
    const planPath = (0, plans_js_1.getPlanFilePath)();
    if (!planContent) {
    ...
    if (argList[0] === 'open') {
        const result = await (0, promptEditor_js_1.editFileInEditor)(planPath);
        if (result.error) {
    ...
    const editorName = editor ? (0, ide_js_1.toIDEDisplayName)(editor) : undefined;
    ...
    const output = await (0, staticRender_js_1.renderToString)(display);
    onDone(output);
    return null;
}
