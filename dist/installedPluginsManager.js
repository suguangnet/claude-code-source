...
function migrateToSinglePluginFile() {
    ...
    try {
        ...
        const mainData = (0, slowOperations_js_1.jsonParse)(mainContent);
        const version = typeof mainData?.version === 'number' ? mainData.version : 1;
        if (version === 1) {
            // Convert V1 to V2 format in-place
            const v1Data = (0, schemas_js_1.InstalledPluginsFileSchemaV1)().parse(mainData);
            ...
            (0, debug_js_1.logForDebugging)(`Converted installed_plugins.json from V1 to V2 format (${Object.keys(v1Data.plugins).length} plugins)`);
            // Clean up legacy cache directories
            cleanupLegacyCache(v2Data);
        }
        ...
        (0, debug_js_1.logForDebugging)(`Failed to migrate plugin files: ${errorMsg}`, {
            level: 'error',
        });
...
function cleanupLegacyCache(v2Data) {
    ...
    try {
        ...
        for (const dirent of entries) {
            ...
            const entryPath = (0, path_1.join)(cachePath, entry);
            ...
            const hasVersionedStructure = subEntries.some(subDirent => {
                if (!subDirent.isDirectory())
                    return false;
                const subPath = (0, path_1.join)(entryPath, subDirent.name);
            ...
            if (!referencedPaths.has(entryPath)) {
                ...
                (0, debug_js_1.logForDebugging)(`Cleaned up legacy cache directory: ${entry}`);
            }
        }
    }
    catch (error) {
        const errorMsg = (0, errors_js_1.errorMessage)(error);
        (0, debug_js_1.logForDebugging)(`Failed to clean up legacy cache: ${errorMsg}`, {
            level: 'warn',
        });
    }
}
...
function readInstalledPluginsFileRaw() {
    ...
    const data = (0, slowOperations_js_1.jsonParse)(fileContent);
...
function migrateV1ToV2(v1Data) {
    ...
    for (const [pluginId, plugin] of Object.entries(v1Data.plugins)) {
        ...
        const versionedCachePath = (0, pluginLoader_js_1.getVersionedCachePath)(pluginId, plugin.version);
        v2Plugins[pluginId] = [
            {
    ...
    try {
        ...
        if (rawData) {
            if (rawData.version === 2) {
                ...
                const validated = (0, schemas_js_1.InstalledPluginsFileSchemaV2)().parse(rawData.data);
                installedPluginsCacheV2 = validated;
                (0, debug_js_1.logForDebugging)(`Loaded ${Object.keys(validated.plugins).length} installed plugins from ${filePath}`);
                return validated;
            }
            // V1 format - convert to V2
            const v1Validated = (0, schemas_js_1.InstalledPluginsFileSchemaV1)().parse(rawData.data);
            const v2Data = migrateV1ToV2(v1Validated);
            installedPluginsCacheV2 = v2Data;
            (0, debug_js_1.logForDebugging)(`Loaded and converted ${Object.keys(v1Validated.plugins).length} plugins from V1 format`);
            return v2Data;
        }
        ...
        const errorMsg = (0, errors_js_1.errorMessage)(error);
        (0, debug_js_1.logForDebugging)(`Failed to load installed_plugins.json: ${errorMsg}. Starting with empty state.`, { level: 'error' });
        (0, log_js_1.logError)((0, errors_js_1.toError)(error));
        installedPluginsCacheV2 = { version: 2, plugins: {} };
        return installedPluginsCacheV2;
    }
...
function saveInstalledPluginsV2(data) {
    ...
    try {
        ...
        const jsonContent = (0, slowOperations_js_1.jsonStringify)(data, null, 2);
        ...
        (0, debug_js_1.logForDebugging)(`Saved ${Object.keys(data.plugins).length} installed plugins to ${filePath}`);
    }
    catch (error) {
        const _errorMsg = (0, errors_js_1.errorMessage)(error);
        (0, log_js_1.logError)((0, errors_js_1.toError)(error));
        throw error;
    }
}
...
function addPluginInstallation(pluginId, scope, installPath, metadata, projectPath) {
    ...
    if (existingIndex >= 0) {
        ...
        (0, debug_js_1.logForDebugging)(`Updated installation for ${pluginId} at scope ${scope}`);
    }
    else {
        installations.push(newEntry);
        (0, debug_js_1.logForDebugging)(`Added installation for ${pluginId} at scope ${scope}`);
    }
...
function removePluginInstallation(pluginId, scope, projectPath) {
    ...
    saveInstalledPluginsV2(data);
    (0, debug_js_1.logForDebugging)(`Removed installation for ${pluginId} at scope ${scope}`);
}
// =============================================================================
...
function loadInstalledPluginsFromDisk() {
    try {
        ...
        if (rawData) {
            ...
            // V1 format - convert to V2
            const v1Data = (0, schemas_js_1.InstalledPluginsFileSchemaV1)().parse(rawData.data);
        ...
        const errorMsg = (0, errors_js_1.errorMessage)(error);
        (0, debug_js_1.logForDebugging)(`Failed to load installed plugins from disk: ${errorMsg}`, {
            level: 'error',
        });
        return { version: 2, plugins: {} };
    }
...
function updateInstallationPathOnDisk(pluginId, scope, projectPath, newPath, newVersion, gitCommitSha) {
    ...
    if (!installations) {
        (0, debug_js_1.logForDebugging)(`Cannot update ${pluginId} on disk: plugin not found in installed plugins`);
        return;
    }
    ...
    if (entry) {
        ...
        installedPluginsCacheV2 = null;
        (0, debug_js_1.logForDebugging)(`Updated ${pluginId} on disk to version ${newVersion} at ${newPath}`);
    }
    else {
        (0, debug_js_1.logForDebugging)(`Cannot update ${pluginId} on disk: no installation for scope ${scope}`);
    }
...
async function initializeVersionedPlugins() {
    ...
    (0, debug_js_1.logForDebugging)(`Initialized versioned plugins system with ${Object.keys(data.plugins).length} plugins`);
}
/**
...
function removeAllPluginsForMarketplace(marketplaceName) {
    ...
    for (const pluginId of Object.keys(data.plugins)) {
        ...
        (0, debug_js_1.logForDebugging)(`Removed installed plugin for marketplace removal: ${pluginId}`);
    }
...
function isPluginInstalled(pluginId) {
    ...
    return (0, settings_js_1.getSettings_DEPRECATED)().enabledPlugins?.[pluginId] !== undefined;
}
/**
...
function addInstalledPlugin(pluginId, metadata, scope = 'user', projectPath) {
    ...
    (0, debug_js_1.logForDebugging)(`${isUpdate ? 'Updated' : 'Added'} installed plugin: ${pluginId} (scope: ${scope})`);
}
/**
...
function removeInstalledPlugin(pluginId) {
    ...
    saveInstalledPluginsV2(v2Data);
    (0, debug_js_1.logForDebugging)(`Removed installed plugin: ${pluginId}`);
    return metadata;
}
function deletePluginCache(installPath) {
    ...
    try {
        ...
        (0, debug_js_1.logForDebugging)(`Deleted plugin cache at ${installPath}`);
        ...
        if (installPath.includes('/cache/') && installPath.startsWith(cachePath)) {
            const pluginDir = (0, path_1.dirname)(installPath); // e.g., cache/{marketplace}/{plugin}
            if (pluginDir !== cachePath && pluginDir.startsWith(cachePath)) {
                try {
                    const contents = fs.readdirSync(pluginDir);
                    if (contents.length === 0) {
                        fs.rmdirSync(pluginDir);
                        (0, debug_js_1.logForDebugging)(`Deleted empty plugin directory at ${pluginDir}`);
                    }
                }
                catch {
...
async function getGitCommitSha(dirPath) {
    const sha = await (0, gitFilesystem_js_1.getHeadForDir)(dirPath);
...
function getPluginVersionFromManifest(pluginCachePath, pluginId) {
    const fs = (0, fsOperations_js_1.getFsImplementation)();
    const manifestPath = (0, path_1.join)(pluginCachePath, '.claude-plugin', 'plugin.json');
    try {
        const manifestContent = fs.readFileSync(manifestPath, { encoding: 'utf-8' });
        const manifest = (0, slowOperations_js_1.jsonParse)(manifestContent);
        return manifest.version || 'unknown';
    }
    catch {
        (0, debug_js_1.logForDebugging)(`Could not read version from manifest for ${pluginId}`);
        return 'unknown';
    }
}
/**
...
async function migrateFromEnabledPlugins() {
    ...
    if (isV2Format && rawFileData) {
        ...
        const existingData = (0, schemas_js_1.InstalledPluginsFileSchemaV2)().safeParse(rawFileData.data);
    ...
    const projectPath = (0, cwd_js_1.getCwd)();
    ...
    for (const source of settingSources) {
        ...
        for (const pluginId of Object.keys(sourceEnabledPlugins)) {
            ...
            const scope = (0, pluginIdentifier_js_1.settingSourceToScope)(source);
    ...
    for (const [pluginId, scopeInfo] of pluginScopeFromSettings) {
        ...
        if (existingInstallations && existingInstallations.length > 0) {
            ...
            if (existingEntry &&
                ...
                updatedCount++;
                (0, debug_js_1.logForDebugging)(`Updated ${pluginId} scope to ${scopeInfo.scope} (settings.json is source of truth)`);
            }
        }
        else {
            ...
            try {
                (0, debug_js_1.logForDebugging)(`Looking up plugin ${pluginId} in marketplace ${marketplace}`);
                const pluginInfo = await (0, marketplaceManager_js_1.getPluginById)(pluginId);
                if (!pluginInfo) {
                    (0, debug_js_1.logForDebugging)(`Plugin ${pluginId} not found in any marketplace, skipping`);
                    continue;
                }
                ...
                if (typeof entry.source === 'string') {
                    ...
                    const pluginCachePath = (0, path_1.join)(cachePath, sanitizedName);
                    ...
                    let dirEntries;
                    try {
                        dirEntries = (await (0, fsOperations_js_1.getFsImplementation)().readdir(pluginCachePath)).map(e => (typeof e === 'string' ? e : e.name));
                    }
                    catch (e) {
                        if (!(0, errors_js_1.isENOENT)(e))
                            throw e;
                        (0, debug_js_1.logForDebugging)(`External plugin ${pluginId} not in cache, skipping`);
                        continue;
                    }
                ...
                v2Plugins[pluginId] = [
                    {
                        ...
                        installPath: (0, pluginLoader_js_1.getVersionedCachePath)(pluginId, version),
                        version,
                        installedAt: now,
                        lastUpdated: now,
                        gitCommitSha,
                ...
                addedCount++;
                (0, debug_js_1.logForDebugging)(`Added ${pluginId} with scope ${scopeInfo.scope}`);
            }
            catch (error) {
                (0, debug_js_1.logForDebugging)(`Failed to add plugin ${pluginId}: ${error}`);
            }
        }
    }
    ...
    if (!fileExists || updatedCount > 0 || addedCount > 0) {
        ...
        saveInstalledPluginsV2(v2Data);
        (0, debug_js_1.logForDebugging)(`Sync completed: ${addedCount} added, ${updatedCount} updated in installed_plugins.json`);
    }
}
