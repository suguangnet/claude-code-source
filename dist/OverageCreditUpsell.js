...
function OverageCreditUpsell(t0) {
    ...
    if ($[0] !== maxWidth || $[1] !== twoLine) {
        ...
        bb0: {
            ...
            const amount = (0, overageCreditGrant_js_1.formatGrantAmount)(info);
            if (!amount) {
                t2 = null;
                break bb0;
            }
            if (twoLine) {
                ...
                let t4;
                if ($[6] !== t3) {
                    t4 = React.createElement(ink_js_1.Text, { dimColor: true }, t3);
                    $[6] = t3;
                    $[7] = t4;
                }
                else {
                ...
                t2 = React.createElement(React.Fragment, null,
                    React.createElement(ink_js_1.Text, { color: "claude" }, maxWidth ? (0, format_js_1.truncate)(title, maxWidth) : title),
                    t4);
                break bb0;
            }
            const text = getUsageText(amount);
            const display = maxWidth ? (0, format_js_1.truncate)(text, maxWidth) : text;
            const highlightLen = Math.min(getFeedTitle(amount).length, display.length);
            t1 = React.createElement(ink_js_1.Text, { dimColor: true },
                React.createElement(ink_js_1.Text, { color: "claude" }, display.slice(0, highlightLen)),
...
function createOverageCreditFeed() {
    ...
    return {
        ...
        lines: [],
        customContent: {
            content: React.createElement(ink_js_1.Text, { dimColor: true }, FEED_SUBTITLE),
