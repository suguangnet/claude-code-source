...
const teamDiscovery_js_1 = require("../../utils/teamDiscovery.js");
...
function TeamsDialog({ initialTeams, onDone }) {
    ...
    const [dialogLevel, setDialogLevel] = (0, react_1.useState)({
        type: 'teammateList',
    ...
    const teammateStatuses = (0, react_1.useMemo)(() => {
    ...
    const currentTeammate = (0, react_1.useMemo)(() => {
    ...
    const isBypassAvailable = (0, AppState_js_1.useAppState)(s => s.toolPermissionContext.isBypassPermissionsModeAvailable);
    const goBackToList = () => {
        setDialogLevel({
            type: 'teammateList',
    ...
    (0, ink_js_1.useInput)((input, key) => {
        ...
        if (input === 'h') {
            const backend = (0, registry_js_1.getCachedBackend)();
...
function TeamDetailView(t0) {
    ...
    const supportsHideShow = (0, registry_js_1.getCachedBackend)()?.supportsHideShow ?? false;
    const cycleModeShortcut = (0, useShortcutDisplay_js_1.useShortcutDisplay)("confirm:cycleMode", "Confirmation", "shift+tab");
    const t1 = `Team ${teamName}`;
    let t2;
    if ($[0] !== selectedIndex || $[1] !== teammates) {
        t2 = teammates.length === 0 ? React.createElement(ink_js_1.Text, { dimColor: true }, "No teammates") : React.createElement(ink_js_1.Box, { flexDirection: "column" }, teammates.map((teammate, index) => React.createElement(TeammateListItem, { key: teammate.agentId, teammate: teammate, isSelected: index === selectedIndex })));
        $[0] = selectedIndex;
        $[1] = teammates;
        $[2] = t2;
    }
    ...
    if ($[3] !== onCancel || $[4] !== subtitle || $[5] !== t1 || $[6] !== t2) {
        t3 = React.createElement(Dialog_js_1.Dialog, { title: t1, subtitle: subtitle, onCancel: onCancel, color: "background", hideInputGuide: true }, t2);
        $[3] = onCancel;
        $[4] = subtitle;
        $[5] = t1;
        $[6] = t2;
    ...
    if ($[8] !== cycleModeShortcut) {
        t4 = React.createElement(ink_js_1.Box, { marginLeft: 1 },
...
function TeammateListItem(t0) {
    ...
    let t1;
    if ($[0] !== teammate.mode) {
        const mode = teammate.mode ? (0, PermissionMode_js_1.permissionModeFromString)(teammate.mode) : "default";
    ...
    if ($[3] !== teammate.isHidden) {
        t4 = teammate.isHidden && React.createElement(ink_js_1.Text, { dimColor: true }, "[hidden] ");
        $[3] = teammate.isHidden;
        $[4] = t4;
    }
    else {
    ...
    let t5;
    if ($[5] !== isIdle) {
        t5 = isIdle && React.createElement(ink_js_1.Text, { dimColor: true }, "[idle] ");
        $[5] = isIdle;
        $[6] = t5;
    }
    else {
    ...
    if ($[7] !== modeColor || $[8] !== modeSymbol) {
        t6 = modeSymbol && React.createElement(ink_js_1.Text, { color: modeColor },
            modeSymbol,
            " ");
        $[7] = modeColor;
        $[8] = modeSymbol;
    ...
    if ($[10] !== teammate.model) {
        t7 = teammate.model && React.createElement(ink_js_1.Text, { dimColor: true },
            " (",
            teammate.model,
            ")");
        $[10] = teammate.model;
    ...
    if ($[12] !== shouldDim || $[13] !== t2 || $[14] !== t3 || $[15] !== t4 || $[16] !== t5 || $[17] !== t6 || $[18] !== t7 || $[19] !== teammate.name) {
        t8 = React.createElement(ink_js_1.Text, { color: t2, dimColor: shouldDim },
            t3,
            t4,
            t5,
            t6,
...
function TeammateDetailView(t0) {
    const $ = (0, compiler_runtime_1.c)(39);
    const { teammate, teamName, onCancel } = t0;
    const [promptExpanded, setPromptExpanded] = (0, react_1.useState)(false);
    const cycleModeShortcut = (0, useShortcutDisplay_js_1.useShortcutDisplay)("confirm:cycleMode", "Confirmation", "shift+tab");
    const themeColor = teammate.color ? agentColorManager_js_1.AGENT_COLOR_TO_THEME_COLOR[teammate.color] : undefined;
    let t1;
    ...
    const [teammateTasks, setTeammateTasks] = (0, react_1.useState)(t1);
    let t2;
    let t3;
    if ($[1] !== teamName || $[2] !== teammate.agentId || $[3] !== teammate.name) {
        t2 = () => {
            let cancelled = false;
            (0, tasks_js_1.listTasks)(teamName).then(allTasks => {
                if (cancelled) {
                    return;
                }
    ...
    if ($[17] !== teammate.name || $[18] !== themeColor) {
        t7 = themeColor ? React.createElement(ThemedText_js_1.default, { color: themeColor }, `@${teammate.name}`) : `@${teammate.name}`;
        $[17] = teammate.name;
        $[18] = themeColor;
        $[19] = t7;
    }
    ...
    let t9;
    if ($[23] !== teammateTasks) {
        t9 = teammateTasks.length > 0 && React.createElement(ink_js_1.Box, { flexDirection: "column" },
            React.createElement(ink_js_1.Text, { bold: true }, "Tasks"),
    ...
    if ($[25] !== promptExpanded || $[26] !== teammate.prompt) {
        t10 = teammate.prompt && React.createElement(ink_js_1.Box, { flexDirection: "column" },
            React.createElement(ink_js_1.Text, { bold: true }, "Prompt"),
            React.createElement(ink_js_1.Text, null,
                promptExpanded ? teammate.prompt : (0, format_js_1.truncateToWidth)(teammate.prompt, 80),
                (0, stringWidth_js_1.stringWidth)(teammate.prompt) > 80 && !promptExpanded && React.createElement(ink_js_1.Text, { dimColor: true }, " (p to expand)")));
        $[25] = promptExpanded;
        $[26] = teammate.prompt;
        $[27] = t10;
    }
    ...
    if ($[28] !== onCancel || $[29] !== subtitle || $[30] !== t10 || $[31] !== t9 || $[32] !== title) {
        t11 = React.createElement(Dialog_js_1.Dialog, { title: title, subtitle: subtitle, onCancel: onCancel, color: "background", hideInputGuide: true },
            t9,
            t10);
        $[28] = onCancel;
        $[29] = subtitle;
    ...
    if ($[34] !== cycleModeShortcut) {
        t12 = React.createElement(ink_js_1.Box, { marginLeft: 1 },
            React.createElement(ink_js_1.Text, { dimColor: true },
                figures_1.default.arrowLeft,
                " back \u00B7 Esc close \u00B7 k kill \u00B7 s shutdown",
                (0, registry_js_1.getCachedBackend)()?.supportsHideShow && " \xB7 h hide/show",
                " \xB7 ",
                cycleModeShortcut,
                " cycle mode"));
...
function _temp2(task_0) {
    return React.createElement(ink_js_1.Text, { key: task_0.id, color: task_0.status === "completed" ? "success" : undefined },
        task_0.status === "completed" ? figures_1.default.tick : "\u25FC",
        " ",
        task_0.subject);
}
function _temp(prev) {
...
async function killTeammate(paneId, backendType, teamName, teammateId, teammateName, setAppState) {
    ...
    if (backendType) {
        try {
            ...
            await (0, registry_js_1.getBackendByType)(backendType).killPane(paneId, !(0, detection_js_1.isInsideTmuxSync)());
        }
        catch (error) {
            (0, debug_js_1.logForDebugging)(`[TeamsDialog] Failed to kill pane ${paneId}: ${error}`);
        }
    }
    else {
        ...
        (0, debug_js_1.logForDebugging)(`[TeamsDialog] Skipping pane kill for ${paneId}: no backendType recorded`);
    }
    ...
    const { notificationMessage } = await (0, tasks_js_1.unassignTeammateTasks)(teamName, teammateId, teammateName, 'terminated');
    ...
    (0, debug_js_1.logForDebugging)(`[TeamsDialog] Removed ${teammateId} from teamContext`);
}
async function viewTeammateOutput(paneId, backendType) {
    if (backendType === 'iterm2') {
        ...
        await (0, execFileNoThrow_js_1.execFileNoThrow)(detection_js_1.IT2_COMMAND, ['session', 'focus', '-s', paneId]);
    }
    else {
        ...
        const args = (0, detection_js_1.isInsideTmuxSync)() ? ['select-pane', '-t', paneId] : ['-L', (0, constants_js_1.getSwarmSocketName)(), 'select-pane', '-t', paneId];
        await (0, execFileNoThrow_js_1.execFileNoThrow)(constants_js_1.TMUX_COMMAND, args);
    }
}
/**
...
function sendModeChangeToTeammate(teammateName, teamName, targetMode) {
    ...
    const message = (0, teammateMailbox_js_1.createModeSetRequestMessage)({
        mode: targetMode,
        from: 'team-lead'
    });
    ...
    (0, debug_js_1.logForDebugging)(`[TeamsDialog] Sent mode change to ${teammateName}: ${targetMode}`);
}
/**
 * Cycle a single teammate's mode
 */
function cycleTeammateMode(teammate, teamName, isBypassAvailable) {
    const currentMode = teammate.mode ? (0, PermissionMode_js_1.permissionModeFromString)(teammate.mode) : 'default';
    const context = {
        ...(0, Tool_js_1.getEmptyToolPermissionContext)(),
        mode: currentMode,
        isBypassPermissionsModeAvailable: isBypassAvailable
    };
    const nextMode = (0, getNextPermissionMode_js_1.getNextPermissionMode)(context);
...
function cycleAllTeammateModes(teammates, teamName, isBypassAvailable) {
    if (teammates.length === 0)
        return;
    const modes = teammates.map(t => t.mode ? (0, PermissionMode_js_1.permissionModeFromString)(t.mode) : 'default');
    const allSame = modes.every(m => m === modes[0]);
    // Determine target mode for all teammates
    const targetMode = !allSame ? 'default' : (0, getNextPermissionMode_js_1.getNextPermissionMode)({
        ...(0, Tool_js_1.getEmptyToolPermissionContext)(),
    ...
    (0, debug_js_1.logForDebugging)(`[TeamsDialog] Sent mode change to all ${teammates.length} teammates: ${targetMode}`);
}
