...
const toolValidationConfig_js_1 = require("./toolValidationConfig.js");
/**
...
function validatePermissionRule(rule) {
    ...
    // Parse the rule
    const parsed = (0, permissionRuleParser_js_1.permissionRuleValueFromString)(rule);
    // MCP validation - must be done before general tool validation
    const mcpInfo = (0, mcpStringUtils_js_1.mcpInfoFromString)(parsed.toolName);
    if (mcpInfo) {
    ...
    if (parsed.toolName[0] !== parsed.toolName[0]?.toUpperCase()) {
        return {
            ...
            suggestion: `Use "${(0, stringUtils_js_1.capitalize)(String(parsed.toolName))}"`,
        };
    }
    // Check for custom validation rules first
    const customValidation = (0, toolValidationConfig_js_1.getCustomValidation)(parsed.toolName);
 ...
 */
exports.PermissionRuleSchema = (0, lazySchema_js_1.lazySchema)(() => v4_1.z.string().superRefine((val, ctx) => {
