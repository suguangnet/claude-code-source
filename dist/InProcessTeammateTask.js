...
function injectUserMessageToTeammate(taskId, message, setAppState) {
    (0, framework_js_1.updateTaskState)(taskId, setAppState, task => {
        ...
        if ((0, Task_js_1.isTerminalTaskStatus)(task.status)) {
            (0, debug_js_1.logForDebugging)(`Dropping message for teammate task ${taskId}: task status is "${task.status}"`);
            return task;
        }
        return {
            ...task,
            pendingUserMessages: [...task.pendingUserMessages, message],
            messages: (0, types_js_1.appendCappedMessage)(task.messages, (0, messages_js_1.createUserMessage)({
                content: message
            }))
        };
    });
