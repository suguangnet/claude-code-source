...
const getRipgrepConfig = (0, memoize_js_1.default)(() => {
    const userWantsSystemRipgrep = (0, envUtils_js_1.isEnvDefinedFalsy)(process.env.USE_BUILTIN_RIPGREP);
    // Try system ripgrep if user wants it
    if (userWantsSystemRipgrep) {
        const { cmd: systemPath } = (0, findExecutable_js_1.findExecutable)('rg', []);
        if (systemPath !== 'rg') {
...
function ripGrepRaw(args, target, abortSignal, callback, singleThread = false) {
    ...
    const defaultTimeout = (0, platform_js_1.getPlatform)() === 'wsl' ? 60000 : 20000;
    ...
    if (argv0) {
        const child = (0, child_process_1.spawn)(rgPath, fullArgs, {
            argv0,
            signal: abortSignal,
...
async function ripGrepFileCount(args, target, abortSignal) {
    ...
    return new Promise((resolve, reject) => {
        const child = (0, child_process_1.spawn)(rgPath, [...rgArgs, ...args, target], {
            argv0,
            signal: abortSignal,
            windowsHide: true,
...
async function ripGrep(args, target, abortSignal) {
    ...
    return new Promise((resolve, reject) => {
        const handleResult = (error, stdout, stderr, isRetry) => {
            ...
            (0, debug_js_1.logForDebugging)(`rg error (signal=${error.signal}, code=${error.code}, stderr: ${stderr}), ${lines.length} results`);
            ...
            if (isTimeout && lines.length === 0) {
                reject(new RipgrepTimeoutError(`Ripgrep search timed out after ${(0, platform_js_1.getPlatform)() === 'wsl' ? 60 : 20} seconds. The search may have matched files but did not complete in time. Try searching a more specific path or pattern.`, lines));
                return;
            }
            resolve(lines);
        };
 ...
 */
exports.countFilesRoundedRg = (0, memoize_js_1.default)(async (dirPath, abortSignal, ignorePatterns = []) => {
 ...
 */
const testRipgrepOnFirstUse = (0, memoize_js_1.default)(async () => {
    // Already tested
    ...
    try {
        ...
        if (config.argv0) {
            ...
            test = await (0, execFileNoThrow_js_1.execFileNoThrow)(config.command, [...config.args, '--version'], {
                timeout: 5000,
            });
        }
        ...
        (0, debug_js_1.logForDebugging)(`Ripgrep first use test: ${working ? 'PASSED' : 'FAILED'} (mode=${config.mode}, path=${config.command})`);
...
async function codesignRipgrepIfNecessary() {
    ...
    const lines = (await (0, execFileNoThrow_js_1.execFileNoThrow)('codesign', ['-vv', '-d', builtinPath], {
    ...
    try {
        const signResult = await (0, execFileNoThrow_js_1.execFileNoThrow)('codesign', [
            '--sign',
            '-',
            '--force',
        ...
        if (signResult.code !== 0) {
            (0, log_js_1.logError)(new Error(`Failed to sign ripgrep: ${signResult.stdout} ${signResult.stderr}`));
        }
        const quarantineResult = await (0, execFileNoThrow_js_1.execFileNoThrow)('xattr', [
            '-d',
            'com.apple.quarantine',
            builtinPath,
        ]);
        if (quarantineResult.code !== 0) {
            (0, log_js_1.logError)(new Error(`Failed to remove quarantine: ${quarantineResult.stdout} ${quarantineResult.stderr}`));
        }
    }
    catch (e) {
