...
const heredoc_js_1 = require("../../utils/bash/heredoc.js");
...
function validateMalformedTokenInjection(context) {
    const { originalCommand } = context;
    const parseResult = (0, shellQuote_js_1.tryParseShellCommand)(originalCommand);
...
function bashCommandIsSafe_DEPRECATED(command) {
    ...
    const { processedCommand } = (0, heredoc_js_1.extractHeredocs)(command, { quotedOnly: true });
...
async function bashCommandIsSafeAsync_DEPRECATED(command, onDivergence) {
    ...
    const parsed = await ParsedCommand_js_1.ParsedCommand.parse(command);
