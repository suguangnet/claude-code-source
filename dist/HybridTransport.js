 ...
 */
class HybridTransport extends WebSocketTransport_js_1.WebSocketTransport {
    constructor(url, headers = {}, sessionId, refreshHeaders, options) {
        ...
        this.uploader = new SerialBatchEventUploader_js_1.SerialBatchEventUploader({
        ...
        (0, debug_js_1.logForDebugging)(`HybridTransport: POST URL = ${this.postUrl}`);
    ...
    async postOnce(events) {
        ...
        let response;
        try {
            response = await axios_1.default.post(this.postUrl, { events }, {
                headers,
                validateStatus: () => true,
            ...
            (0, debug_js_1.logForDebugging)(`HybridTransport: POST error: ${axiosError.message}`);
            (0, diagLogs_js_1.logForDiagnosticsNoPII)('warn', 'cli_hybrid_post_network_error');
            throw error;
        }
        if (response.status >= 200 && response.status < 300) {
            (0, debug_js_1.logForDebugging)(`HybridTransport: POST success count=${events.length}`);
            return;
        }
        ...
        if (response.status >= 400 &&
            response.status < 500 &&
            response.status !== 429) {
            (0, debug_js_1.logForDebugging)(`HybridTransport: POST returned ${response.status} (permanent), dropping`);
        ...
        (0, debug_js_1.logForDebugging)(`HybridTransport: POST returned ${response.status} (retryable)`);
