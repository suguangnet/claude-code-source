"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.TeammateViewHeader = TeammateViewHeader;
const compiler_runtime_1 = require("react/compiler-runtime");
const React = __importStar(require("react"));
const ink_js_1 = require("../ink.js");
const AppState_js_1 = require("../state/AppState.js");
const selectors_js_1 = require("../state/selectors.js");
const ink_js_2 = require("../utils/ink.js");
const KeyboardShortcutHint_js_1 = require("./design-system/KeyboardShortcutHint.js");
const OffscreenFreeze_js_1 = require("./OffscreenFreeze.js");
/**
 * Header shown when viewing a teammate's transcript.
 * Displays teammate name (colored), task description, and exit hint.
 */
function TeammateViewHeader() {
    const $ = (0, compiler_runtime_1.c)(14);
    const viewedTeammate = (0, AppState_js_1.useAppState)(_temp);
    if (!viewedTeammate) {
        return null;
    }
    let t0;
    if ($[0] !== viewedTeammate.identity.color) {
        t0 = (0, ink_js_2.toInkColor)(viewedTeammate.identity.color);
        $[0] = viewedTeammate.identity.color;
        $[1] = t0;
    }
    else {
        t0 = $[1];
    }
    const nameColor = t0;
    let t1;
    if ($[2] === Symbol.for("react.memo_cache_sentinel")) {
        t1 = React.createElement(ink_js_1.Text, null, "Viewing ");
        $[2] = t1;
    }
    else {
        t1 = $[2];
    }
    let t2;
    if ($[3] !== nameColor || $[4] !== viewedTeammate.identity.agentName) {
        t2 = React.createElement(ink_js_1.Text, { color: nameColor, bold: true },
            "@",
            viewedTeammate.identity.agentName);
        $[3] = nameColor;
        $[4] = viewedTeammate.identity.agentName;
        $[5] = t2;
    }
    else {
        t2 = $[5];
    }
    let t3;
    if ($[6] === Symbol.for("react.memo_cache_sentinel")) {
        t3 = React.createElement(ink_js_1.Text, { dimColor: true },
            " \xB7 ",
            React.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "esc", action: "return" }));
        $[6] = t3;
    }
    else {
        t3 = $[6];
    }
    let t4;
    if ($[7] !== t2) {
        t4 = React.createElement(ink_js_1.Box, null,
            t1,
            t2,
            t3);
        $[7] = t2;
        $[8] = t4;
    }
    else {
        t4 = $[8];
    }
    let t5;
    if ($[9] !== viewedTeammate.prompt) {
        t5 = React.createElement(ink_js_1.Text, { dimColor: true }, viewedTeammate.prompt);
        $[9] = viewedTeammate.prompt;
        $[10] = t5;
    }
    else {
        t5 = $[10];
    }
    let t6;
    if ($[11] !== t4 || $[12] !== t5) {
        t6 = React.createElement(OffscreenFreeze_js_1.OffscreenFreeze, null,
            React.createElement(ink_js_1.Box, { flexDirection: "column", marginBottom: 1 },
                t4,
                t5));
        $[11] = t4;
        $[12] = t5;
        $[13] = t6;
    }
    else {
        t6 = $[13];
    }
    return t6;
}
function _temp(s) {
    return (0, selectors_js_1.getViewedTeammateTask)(s);
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
