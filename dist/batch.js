...
const constants_js_2 = require("../../tools/EnterPlanModeTool/constants.js");
const constants_js_3 = require("../../tools/ExitPlanModeTool/constants.js");
...
1. **Simplify** — Invoke the \`${constants_js_4.SKILL_TOOL_NAME}\` tool with \`skill: "simplify"\` to review and clean up your changes.
...
Call the \`${constants_js_2.ENTER_PLAN_MODE_TOOL_NAME}\` tool now to enter plan mode, then:
...
3. **Determine the e2e test recipe.** Figure out how a worker can verify its change actually works end-to-end — not just that unit tests pass. Look for:
   ...
   If you cannot find a concrete e2e path, use the \`${prompt_js_1.ASK_USER_QUESTION_TOOL_NAME}\` tool to ask the user how to verify this change end-to-end. Offer 2–3 specific options based on what you found (e.g., "Screenshot via chrome extension", "Run \`bun run dev\` and curl the endpoint", "No e2e — unit tests are sufficient"). Do not skip this — the workers cannot ask the user themselves.
...
5. Call \`${constants_js_3.EXIT_PLAN_MODE_TOOL_NAME}\` to present the plan for approval.

## Phase 2: Spawn Workers (After Plan Approval)

Once the plan is approved, spawn one background agent per work unit using the \`${constants_js_1.AGENT_TOOL_NAME}\` tool. **All agents must use \`isolation: "worktree"\` and \`run_in_background: true\`.** Launch them all in a single message block so they run in parallel.
