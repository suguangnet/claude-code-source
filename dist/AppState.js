...
const applySettingsChange_js_1 = require("../utils/settings/applySettingsChange.js");
const store_js_1 = require("./store.js");
// DCE: voice context is ant-only. External builds get a passthrough.
/* eslint-disable @typescript-eslint/no-require-imports */
const VoiceProvider = (0, bun_bundle_1.feature)('VOICE_MODE') ? require('../context/voice.js').VoiceProvider : ({ children }) => children;
/* eslint-enable @typescript-eslint/no-require-imports */
const AppStateStore_js_1 = require("./AppStateStore.js");
...
const HasAppStateContext = react_1.default.createContext(false);
function AppStateProvider(t0) {
    const $ = (0, compiler_runtime_1.c)(13);
    const { children, initialState, onChangeAppState } = t0;
    const hasAppStateContext = (0, react_1.useContext)(HasAppStateContext);
    if (hasAppStateContext) {
        throw new Error("AppStateProvider can not be nested within another AppStateProvider");
    }
    let t1;
    if ($[0] !== initialState || $[1] !== onChangeAppState) {
        t1 = () => (0, store_js_1.createStore)(initialState ?? (0, AppStateStore_js_1.getDefaultAppState)(), onChangeAppState);
        $[0] = initialState;
        $[1] = onChangeAppState;
        $[2] = t1;
    }
    ...
    const [store] = (0, react_1.useState)(t1);
    let t2;
    if ($[3] !== store) {
        t2 = () => {
    ...
    let t4;
    if ($[6] !== store.setState) {
        t4 = source => (0, applySettingsChange_js_1.applySettingsChange)(source, store.setState);
        $[6] = store.setState;
        $[7] = t4;
    }
    else {
    ...
    const onSettingsChange = (0, react_1.useEffectEvent)(t4);
    (0, useSettingsChange_js_1.useSettingsChange)(onSettingsChange);
    let t5;
    if ($[8] !== children) {
        t5 = react_1.default.createElement(mailbox_js_1.MailboxProvider, null,
            react_1.default.createElement(VoiceProvider, null, children));
        $[8] = children;
        $[9] = t5;
    }
    else {
    ...
    let t6;
    if ($[10] !== store || $[11] !== t5) {
        t6 = react_1.default.createElement(HasAppStateContext.Provider, { value: true },
            react_1.default.createElement(exports.AppStoreContext.Provider, { value: store }, t5));
        $[10] = store;
        $[11] = t5;
        $[12] = t6;
    }
...
function useAppStore() {
    ...
    const store = (0, react_1.useContext)(exports.AppStoreContext);
    if (!store) {
