...
async function main() {
    ...
    const { profileCheckpoint } = await Promise.resolve().then(() => __importStar(require('../utils/startupProfiler.js')));
    profileCheckpoint('cli_entry');
    // Fast-path for --dump-system-prompt: output the rendered system prompt and exit.
    // Used by prompt sensitivity evals to extract the system prompt at a specific commit.
    // Ant-only: eliminated from external builds via feature flag.
    if ((0, bun_bundle_1.feature)('DUMP_SYSTEM_PROMPT') && args[0] === '--dump-system-prompt') {
        profileCheckpoint('cli_dump_system_prompt_path');
        const { enableConfigs } = await Promise.resolve().then(() => __importStar(require('../utils/config.js')));
        enableConfigs();
        const { getMainLoopModel } = await Promise.resolve().then(() => __importStar(require('../utils/model/model.js')));
        const modelIdx = args.indexOf('--model');
        const model = modelIdx !== -1 && args[modelIdx + 1] || getMainLoopModel();
        const { getSystemPrompt } = await Promise.resolve().then(() => __importStar(require('../constants/prompts.js')));
    ...
    if (process.argv[2] === '--claude-in-chrome-mcp') {
        ...
        const { runClaudeInChromeMcpServer } = await Promise.resolve().then(() => __importStar(require('../utils/claudeInChrome/mcpServer.js')));
        await runClaudeInChromeMcpServer();
        return;
    }
    else if (process.argv[2] === '--chrome-native-host') {
        profileCheckpoint('cli_chrome_native_host_path');
        const { runChromeNativeHost } = await Promise.resolve().then(() => __importStar(require('../utils/claudeInChrome/chromeNativeHost.js')));
        await runChromeNativeHost();
        return;
    }
    else if ((0, bun_bundle_1.feature)('CHICAGO_MCP') && process.argv[2] === '--computer-use-mcp') {
        profileCheckpoint('cli_computer_use_mcp_path');
        const { runComputerUseMcpServer } = await Promise.resolve().then(() => __importStar(require('../utils/computerUse/mcpServer.js')));
    ...
    if ((0, bun_bundle_1.feature)('DAEMON') && args[0] === '--daemon-worker') {
        const { runDaemonWorker } = await Promise.resolve().then(() => __importStar(require('../daemon/workerRegistry.js')));
    ...
    if ((0, bun_bundle_1.feature)('BRIDGE_MODE') && (args[0] === 'remote-control' || args[0] === 'rc' || args[0] === 'remote' || args[0] === 'sync' || args[0] === 'bridge')) {
        profileCheckpoint('cli_bridge_path');
        const { enableConfigs } = await Promise.resolve().then(() => __importStar(require('../utils/config.js')));
        enableConfigs();
        const { getBridgeDisabledReason, checkBridgeMinVersion } = await Promise.resolve().then(() => __importStar(require('../bridge/bridgeEnabled.js')));
        const { BRIDGE_LOGIN_ERROR } = await Promise.resolve().then(() => __importStar(require('../bridge/types.js')));
        const { bridgeMain } = await Promise.resolve().then(() => __importStar(require('../bridge/bridgeMain.js')));
        const { exitWithError } = await Promise.resolve().then(() => __importStar(require('../utils/process.js')));
        ...
        const { waitForPolicyLimitsToLoad, isPolicyAllowed } = await Promise.resolve().then(() => __importStar(require('../services/policyLimits/index.js')));
    ...
    if ((0, bun_bundle_1.feature)('DAEMON') && args[0] === 'daemon') {
        profileCheckpoint('cli_daemon_path');
        const { enableConfigs } = await Promise.resolve().then(() => __importStar(require('../utils/config.js')));
        enableConfigs();
        const { initSinks } = await Promise.resolve().then(() => __importStar(require('../utils/sinks.js')));
        initSinks();
        const { daemonMain } = await Promise.resolve().then(() => __importStar(require('../daemon/main.js')));
    ...
    if ((0, bun_bundle_1.feature)('BG_SESSIONS') && (args[0] === 'ps' || args[0] === 'logs' || args[0] === 'attach' || args[0] === 'kill' || args.includes('--bg') || args.includes('--background'))) {
        profileCheckpoint('cli_bg_path');
        const { enableConfigs } = await Promise.resolve().then(() => __importStar(require('../utils/config.js')));
        enableConfigs();
        const bg = await Promise.resolve().then(() => __importStar(require('../cli/bg.js')));
        switch (args[0]) {
            case 'ps':
    ...
    if ((0, bun_bundle_1.feature)('TEMPLATES') && (args[0] === 'new' || args[0] === 'list' || args[0] === 'reply')) {
        profileCheckpoint('cli_templates_path');
        const { templatesMain } = await Promise.resolve().then(() => __importStar(require('../cli/handlers/templateJobs.js')));
    ...
    // feature() must stay inline for build-time dead code elimination.
    if ((0, bun_bundle_1.feature)('BYOC_ENVIRONMENT_RUNNER') && args[0] === 'environment-runner') {
        profileCheckpoint('cli_environment_runner_path');
        const { environmentRunnerMain } = await Promise.resolve().then(() => __importStar(require('../environment-runner/main.js')));
    ...
    if ((0, bun_bundle_1.feature)('SELF_HOSTED_RUNNER') && args[0] === 'self-hosted-runner') {
        profileCheckpoint('cli_self_hosted_runner_path');
        const { selfHostedRunnerMain } = await Promise.resolve().then(() => __importStar(require('../self-hosted-runner/main.js')));
    ...
    if (hasTmuxFlag && (args.includes('-w') || args.includes('--worktree') || args.some(a => a.startsWith('--worktree=')))) {
        ...
        const { enableConfigs } = await Promise.resolve().then(() => __importStar(require('../utils/config.js')));
        enableConfigs();
        const { isWorktreeModeEnabled } = await Promise.resolve().then(() => __importStar(require('../utils/worktreeModeEnabled.js')));
        if (isWorktreeModeEnabled()) {
            const { execIntoTmuxWorktree } = await Promise.resolve().then(() => __importStar(require('../utils/worktree.js')));
    ...
    const { startCapturingEarlyInput } = await Promise.resolve().then(() => __importStar(require('../utils/earlyInput.js')));
    startCapturingEarlyInput();
    profileCheckpoint('cli_before_main_import');
    const { main: cliMain } = await Promise.resolve().then(() => __importStar(require('../main.js')));
