...
const teammate_js_1 = require("src/utils/teammate.js");
...
function getTeammateThemeColor() {
    ...
    const colorName = (0, teammate_js_1.getTeammateColor)();
    if (!colorName) {
        return undefined;
    }
...
function PromptChar(t0) {
    ...
    if ($[0] !== color || $[1] !== isLoading) {
        t1 = React.createElement(ink_js_1.Text, { color: color, dimColor: isLoading },
...
function PromptInputModeIndicator(t0) {
    ...
    const viewedTeammateThemeColor = viewingAgentColor ? agentColorManager_js_1.AGENT_COLOR_TO_THEME_COLOR[viewingAgentColor] : undefined;
    let t2;
    if ($[1] !== isLoading || $[2] !== mode || $[3] !== viewedTeammateThemeColor || $[4] !== viewingAgentName) {
        t2 = React.createElement(ink_js_1.Box, { alignItems: "flex-start", alignSelf: "flex-start", flexWrap: "nowrap", justifyContent: "flex-start" }, viewingAgentName ? React.createElement(PromptChar, { isLoading: isLoading, themeColor: viewedTeammateThemeColor }) : mode === "bash" ? React.createElement(ink_js_1.Text, { color: "bashBorder", dimColor: isLoading }, "!\u00A0") : React.createElement(PromptChar, { isLoading: isLoading, themeColor: (0, agentSwarmsEnabled_js_1.isAgentSwarmsEnabled)() ? teammateColor : undefined }));
        $[1] = isLoading;
        $[2] = mode;
