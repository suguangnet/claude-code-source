...
function DevChannelsDialog(t0) {
    ...
    if ($[2] === Symbol.for("react.memo_cache_sentinel")) {
        t2 = react_1.default.createElement(ink_js_1.Text, null, "--dangerously-load-development-channels is for local channel development only. Do not use this option to run channels you have downloaded off the internet.");
        t3 = react_1.default.createElement(ink_js_1.Text, null, "Please use --channels to run a list of approved channels.");
        $[2] = t2;
        $[3] = t3;
    }
    else {
    ...
    let t5;
    if ($[6] !== t4) {
        t5 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", gap: 1 },
            t2,
            t3,
    ...
    let t7;
    if ($[9] !== onChange) {
        t7 = react_1.default.createElement(index_js_1.Select, { options: t6, onChange: value_0 => onChange(value_0) });
        $[9] = onChange;
        $[10] = t7;
    }
    else {
    ...
    let t8;
    if ($[11] !== t5 || $[12] !== t7) {
        t8 = react_1.default.createElement(Dialog_js_1.Dialog, { title: "WARNING: Loading development channels", color: "error", onCancel: handleEscape },
            t5,
            t7);
        $[11] = t5;
        $[12] = t7;
