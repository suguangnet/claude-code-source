...
function isIdleNotification(value) {
    try {
        const parsed = (0, slowOperations_js_1.jsonParse)(value);
...
function createOverflowNotificationMessage(count) {
    return `<${xml_js_1.TASK_NOTIFICATION_TAG}>
<${xml_js_1.SUMMARY_TAG}>+${count} more tasks completed</${xml_js_1.SUMMARY_TAG}>
...
function PromptInputQueuedCommandsImpl() {
    ...
    const viewingAgent = (0, AppState_js_1.useAppState)(s => !!s.viewingAgentTaskId);
    ...
    const useBriefLayout = (0, bun_bundle_1.feature)('KAIROS') || (0, bun_bundle_1.feature)('KAIROS_BRIEF') ?
        // biome-ignore lint/correctness/useHookAtTopLevel: feature() is a compile-time constant
        (0, AppState_js_1.useAppState)(s_0 => s_0.isBriefOnly) : false;
    ...
    const messages = (0, react_1.useMemo)(() => {
        ...
        const visibleCommands = queuedCommands.filter(messageQueueManager_js_1.isQueuedCommandVisible);
        if (visibleCommands.length === 0)
            return null;
        const processedCommands = processQueuedCommands(visibleCommands);
        return (0, messages_js_1.normalizeMessages)(processedCommands.map(cmd => {
    ...
    return React.createElement(ink_js_1.Box, { marginTop: 1, flexDirection: "column" }, messages.map((message, i) => React.createElement(QueuedMessageContext_js_1.QueuedMessageProvider, { key: i, isFirst: i === 0, useBriefLayout: useBriefLayout },
        React.createElement(Message_js_1.Message, { message: message, lookups: messages_js_1.EMPTY_LOOKUPS, addMargin: false, tools: [], commands: [], verbose: false, inProgressToolUseIDs: EMPTY_SET, progressMessagesForMessage: [], shouldAnimate: false, shouldShowDot: false, isTranscriptMode: false, isStatic: true }))));
}
