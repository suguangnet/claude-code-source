...
const api_js_1 = require("src/utils/api.js");
...
6 **Example agent descriptions**:
  ...
  - examples should be of the form:
    - <example>
      ...
      <commentary>
      Since a significant piece of code was written, use the ${constants_js_1.AGENT_TOOL_NAME} tool to launch the test-runner agent to run the tests.
      </commentary>
    ...
    - <example>
      ...
      user: "Hello"
      assistant: "I'm going to use the ${constants_js_1.AGENT_TOOL_NAME} tool to launch the greeting-responder agent to respond with a friendly joke"
      <commentary>
...
async function generateAgent(userPrompt, model, existingIdentifiers, abortSignal) {
  ...
  Return ONLY the JSON object, no other text.`;
    const userMessage = (0, messages_js_1.createUserMessage)({ content: prompt });
    // Fetch user and system contexts
    const userContext = await (0, context_js_1.getUserContext)();
    // Prepend user context to messages and append system context to system prompt
    const messagesWithContext = (0, api_js_1.prependUserContext)([userMessage], userContext);
    // Include memory instructions when the feature is enabled
    const systemPrompt = (0, paths_js_1.isAutoMemoryEnabled)()
