...
const doctorDiagnostic_js_1 = require("../utils/doctorDiagnostic.js");
const AutoUpdater_js_1 = require("./AutoUpdater.js");
const NativeAutoUpdater_js_1 = require("./NativeAutoUpdater.js");
const PackageManagerAutoUpdater_js_1 = require("./PackageManagerAutoUpdater.js");
function AutoUpdaterWrapper(t0) {
    ...
    if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
        t1 = () => {
            const checkInstallation = async function checkInstallation() {
                ...
                (0, debug_js_1.logForDebugging)(`AutoUpdaterWrapper: Installation type: ${installationType}`);
    ...
    if (isPackageManager) {
        ...
        if ($[2] !== autoUpdaterResult || $[3] !== isUpdating || $[4] !== onAutoUpdaterResult || $[5] !== onChangeIsUpdating || $[6] !== showSuccessMessage || $[7] !== verbose) {
            t3 = React.createElement(PackageManagerAutoUpdater_js_1.PackageManagerAutoUpdater, { verbose: verbose, onAutoUpdaterResult: onAutoUpdaterResult, autoUpdaterResult: autoUpdaterResult, isUpdating: isUpdating, onChangeIsUpdating: onChangeIsUpdating, showSuccessMessage: showSuccessMessage });
            $[2] = autoUpdaterResult;
            $[3] = isUpdating;
    ...
    const Updater = useNativeInstaller ? NativeAutoUpdater_js_1.NativeAutoUpdater : AutoUpdater_js_1.AutoUpdater;
    let t3;
