...
function OutputStylePicker(t0) {
    ...
    const [styleOptions, setStyleOptions] = (0, react_1.useState)(t1);
    const [isLoading, setIsLoading] = (0, react_1.useState)(true);
    let t2;
    let t3;
    if ($[1] === Symbol.for("react.memo_cache_sentinel")) {
        t2 = () => {
            (0, outputStyles_js_1.getAllOutputStyles)((0, cwd_js_1.getCwd)()).then(allStyles => {
                const options = mapConfigsToOptions(allStyles);
                setStyleOptions(options);
                setIsLoading(false);
            }).catch(() => {
                const builtInOptions = mapConfigsToOptions(outputStyles_js_1.OUTPUT_STYLE_CONFIG);
    ...
    if ($[5] === Symbol.for("react.memo_cache_sentinel")) {
        t7 = React.createElement(ink_js_1.Box, { marginTop: 1 },
            React.createElement(ink_js_1.Text, { dimColor: true }, "This changes how Claude Code communicates with you"));
        $[5] = t7;
    }
    else {
        t7 = $[5];
    ...
    if ($[6] !== handleStyleSelect || $[7] !== initialStyle || $[8] !== isLoading || $[9] !== styleOptions) {
        t8 = React.createElement(ink_js_1.Box, { flexDirection: "column", gap: 1 },
            t7,
            isLoading ? React.createElement(ink_js_1.Text, { dimColor: true }, "Loading output styles\u2026") : React.createElement(select_js_1.Select, { options: styleOptions, onChange: handleStyleSelect, visibleOptionCount: 10, defaultValue: initialStyle }));
        $[6] = handleStyleSelect;
        $[7] = initialStyle;
        $[8] = isLoading;
        $[9] = styleOptions;
    ...
    if ($[11] !== onCancel || $[12] !== t5 || $[13] !== t6 || $[14] !== t8) {
        t9 = React.createElement(Dialog_js_1.Dialog, { title: "Preferred output style", onCancel: onCancel, hideInputGuide: t5, hideBorder: t6 }, t8);
        $[11] = onCancel;
        $[12] = t5;
        $[13] = t6;
        $[14] = t8;
