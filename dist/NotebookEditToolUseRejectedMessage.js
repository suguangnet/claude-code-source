...
function NotebookEditToolUseRejectedMessage(t0) {
    ...
    let t2;
    if ($[0] !== operation) {
        t2 = React.createElement(ink_js_1.Text, { color: "subtle" },
            "User rejected ",
            operation,
            " ");
        $[0] = operation;
    ...
    let t4;
    if ($[5] !== t3) {
        t4 = React.createElement(ink_js_1.Text, { bold: true, color: "subtle" }, t3);
        $[5] = t3;
        $[6] = t4;
    }
    else {
    ...
    if ($[13] !== cell_type || $[14] !== edit_mode || $[15] !== new_source) {
        t7 = edit_mode !== "delete" && React.createElement(ink_js_1.Box, { marginTop: 1, flexDirection: "column" },
            React.createElement(HighlightedCode_js_1.HighlightedCode, { code: new_source, filePath: cell_type === "markdown" ? "file.md" : "file.py", dim: true }));
        $[13] = cell_type;
        $[14] = edit_mode;
        $[15] = new_source;
        $[16] = t7;
