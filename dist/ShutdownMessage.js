"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.ShutdownRequestDisplay = ShutdownRequestDisplay;
exports.ShutdownRejectedDisplay = ShutdownRejectedDisplay;
exports.tryRenderShutdownMessage = tryRenderShutdownMessage;
exports.getShutdownMessageSummary = getShutdownMessageSummary;
const compiler_runtime_1 = require("react/compiler-runtime");
const React = __importStar(require("react"));
const ink_js_1 = require("../../ink.js");
const teammateMailbox_js_1 = require("../../utils/teammateMailbox.js");
/**
 * Renders a shutdown request with a warning-colored border.
 */
function ShutdownRequestDisplay(t0) {
    const $ = (0, compiler_runtime_1.c)(7);
    const { request } = t0;
    let t1;
    if ($[0] !== request.from) {
        t1 = React.createElement(ink_js_1.Box, { marginBottom: 1 },
            React.createElement(ink_js_1.Text, { color: "warning", bold: true },
                "Shutdown request from ",
                request.from));
        $[0] = request.from;
        $[1] = t1;
    }
    else {
        t1 = $[1];
    }
    let t2;
    if ($[2] !== request.reason) {
        t2 = request.reason && React.createElement(ink_js_1.Box, null,
            React.createElement(ink_js_1.Text, null,
                "Reason: ",
                request.reason));
        $[2] = request.reason;
        $[3] = t2;
    }
    else {
        t2 = $[3];
    }
    let t3;
    if ($[4] !== t1 || $[5] !== t2) {
        t3 = React.createElement(ink_js_1.Box, { flexDirection: "column", marginY: 1 },
            React.createElement(ink_js_1.Box, { borderStyle: "round", borderColor: "warning", flexDirection: "column", paddingX: 1, paddingY: 1 },
                t1,
                t2));
        $[4] = t1;
        $[5] = t2;
        $[6] = t3;
    }
    else {
        t3 = $[6];
    }
    return t3;
}
/**
 * Renders a shutdown rejected message with a subtle (grey) border.
 */
function ShutdownRejectedDisplay(t0) {
    const $ = (0, compiler_runtime_1.c)(8);
    const { response } = t0;
    let t1;
    if ($[0] !== response.from) {
        t1 = React.createElement(ink_js_1.Text, { color: "subtle", bold: true },
            "Shutdown rejected by ",
            response.from);
        $[0] = response.from;
        $[1] = t1;
    }
    else {
        t1 = $[1];
    }
    let t2;
    if ($[2] !== response.reason) {
        t2 = React.createElement(ink_js_1.Box, { marginTop: 1, borderStyle: "dashed", borderColor: "subtle", borderLeft: false, borderRight: false, paddingX: 1 },
            React.createElement(ink_js_1.Text, null,
                "Reason: ",
                response.reason));
        $[2] = response.reason;
        $[3] = t2;
    }
    else {
        t2 = $[3];
    }
    let t3;
    if ($[4] === Symbol.for("react.memo_cache_sentinel")) {
        t3 = React.createElement(ink_js_1.Box, { marginTop: 1 },
            React.createElement(ink_js_1.Text, { dimColor: true }, "Teammate is continuing to work. You may request shutdown again later."));
        $[4] = t3;
    }
    else {
        t3 = $[4];
    }
    let t4;
    if ($[5] !== t1 || $[6] !== t2) {
        t4 = React.createElement(ink_js_1.Box, { flexDirection: "column", marginY: 1 },
            React.createElement(ink_js_1.Box, { borderStyle: "round", borderColor: "subtle", flexDirection: "column", paddingX: 1, paddingY: 1 },
                t1,
                t2,
                t3));
        $[5] = t1;
        $[6] = t2;
        $[7] = t4;
    }
    else {
        t4 = $[7];
    }
    return t4;
}
/**
 * Try to parse and render a shutdown message from raw content.
 * Returns the rendered component if it's a shutdown message, null otherwise.
 */
function tryRenderShutdownMessage(content) {
    const request = (0, teammateMailbox_js_1.isShutdownRequest)(content);
    if (request) {
        return React.createElement(ShutdownRequestDisplay, { request: request });
    }
    // Shutdown approved is handled inline by the caller — skip it here
    if ((0, teammateMailbox_js_1.isShutdownApproved)(content)) {
        return null;
    }
    const rejected = (0, teammateMailbox_js_1.isShutdownRejected)(content);
    if (rejected) {
        return React.createElement(ShutdownRejectedDisplay, { response: rejected });
    }
    return null;
}
/**
 * Get a brief summary text for a shutdown message.
 * Used in places like the inbox queue where we want a short description.
 * Returns null if the content is not a shutdown message.
 */
function getShutdownMessageSummary(content) {
    const request = (0, teammateMailbox_js_1.isShutdownRequest)(content);
    if (request) {
        return `[Shutdown Request from ${request.from}]${request.reason ? ` ${request.reason}` : ''}`;
    }
    const approved = (0, teammateMailbox_js_1.isShutdownApproved)(content);
    if (approved) {
        return `[Shutdown Approved] ${approved.from} is now exiting`;
    }
    const rejected = (0, teammateMailbox_js_1.isShutdownRejected)(content);
    if (rejected) {
        return `[Shutdown Rejected] ${rejected.from}: ${rejected.reason}`;
    }
    return null;
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
