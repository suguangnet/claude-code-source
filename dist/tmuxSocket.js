...
async function execTmux(args, opts) {
    if ((0, platform_js_1.getPlatform)() === 'windows') {
        ...
        const result = await (0, execFileNoThrow_js_1.execFileNoThrow)('wsl', ['-e', TMUX_COMMAND, ...args], {
    ...
    const result = await (0, execFileNoThrow_js_1.execFileNoThrow)(TMUX_COMMAND, args, opts);
    return {
        stdout: result.stdout || '',
        stderr: result.stderr || '',
        code: result.code || 0,
...
async function checkTmuxAvailable() {
    if (!tmuxAvailabilityChecked) {
        const result = (0, platform_js_1.getPlatform)() === 'windows'
            ? await (0, execFileNoThrow_js_1.execFileNoThrow)('wsl', ['-e', TMUX_COMMAND, '-V'], {
                env: { ...process.env, WSL_UTF8: '1' },
                useCwd: false,
            })
            : await (0, execFileNoThrow_js_1.execFileNoThrow)('which', [TMUX_COMMAND], {
                useCwd: false,
            });
...
async function ensureSocketInitialized() {
    ...
    try {
        ...
        (0, debug_js_1.logForDebugging)(`[Socket] Failed to initialize tmux socket: ${err.message}. Tmux isolation will be disabled.`);
    }
    finally {
...
async function killTmuxServer() {
    ...
    (0, debug_js_1.logForDebugging)(`[Socket] Killing tmux server for socket: ${socket}`);
    ...
    if (result.code === 0) {
        ...
        (0, debug_js_1.logForDebugging)(`[Socket] Failed to kill tmux server (exit ${result.code}): ${result.stderr}`);
    }
}
async function doInitialize() {
    ...
    if (infoResult.code === 0) {
        ...
        (0, debug_js_1.logForDebugging)(`[Socket] Failed to parse socket info from tmux output: "${infoResult.stdout.trim()}". Using fallback path.`);
    }
    else {
        // Command failed - log and fall through to fallback
        (0, debug_js_1.logForDebugging)(`[Socket] Failed to get socket info via display-message (exit ${infoResult.code}): ${infoResult.stderr}. Using fallback path.`);
    }
    ...
    const fallbackPath = path_1.posix.join(baseTmpDir, `tmux-${uid}`, socket);
    ...
    if (pidResult.code === 0) {
        ...
        if (!isNaN(pid)) {
            (0, debug_js_1.logForDebugging)(`[Socket] Using fallback socket path: ${fallbackPath} (server PID: ${pid})`);
            setClaudeSocketInfo(fallbackPath, pid);
            return;
        }
        // PID parsing failed
        (0, debug_js_1.logForDebugging)(`[Socket] Failed to parse server PID from tmux output: "${pidResult.stdout.trim()}"`);
    }
    else {
        (0, debug_js_1.logForDebugging)(`[Socket] Failed to get server PID (exit ${pidResult.code}): ${pidResult.stderr}`);
    }
