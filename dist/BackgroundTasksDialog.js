...
const coordinatorMode_js_1 = require("src/coordinator/coordinatorMode.js");
const useTerminalSize_js_1 = require("src/hooks/useTerminalSize.js");
const AppState_js_1 = require("src/state/AppState.js");
const teammateViewHelpers_js_1 = require("src/state/teammateViewHelpers.js");
const DreamTask_js_1 = require("src/tasks/DreamTask/DreamTask.js");
const InProcessTeammateTask_js_1 = require("src/tasks/InProcessTeammateTask/InProcessTeammateTask.js");
const LocalAgentTask_js_1 = require("src/tasks/LocalAgentTask/LocalAgentTask.js");
const LocalShellTask_js_1 = require("src/tasks/LocalShellTask/LocalShellTask.js");
const RemoteAgentTask_js_1 = require("src/tasks/RemoteAgentTask/RemoteAgentTask.js");
const types_js_1 = require("src/tasks/types.js");
const array_js_1 = require("src/utils/array.js");
const constants_js_1 = require("src/utils/swarm/constants.js");
const ultraplan_js_1 = require("../../commands/ultraplan.js");
...
const array_js_2 = require("../../utils/array.js");
...
const AsyncAgentDetailDialog_js_1 = require("./AsyncAgentDetailDialog.js");
const BackgroundTask_js_1 = require("./BackgroundTask.js");
const DreamDetailDialog_js_1 = require("./DreamDetailDialog.js");
const InProcessTeammateDetailDialog_js_1 = require("./InProcessTeammateDetailDialog.js");
const RemoteSessionDetailDialog_js_1 = require("./RemoteSessionDetailDialog.js");
const ShellDetailDialog_js_1 = require("./ShellDetailDialog.js");
// WORKFLOW_SCRIPTS is ant-only (build_flags.yaml). Static imports would leak
// ~1.3K lines into external builds. Gate with feature() + require so the
// bundler can dead-code-eliminate the branch.
/* eslint-disable @typescript-eslint/no-require-imports */
const WorkflowDetailDialog = (0, bun_bundle_1.feature)('WORKFLOW_SCRIPTS') ? require('./WorkflowDetailDialog.js').WorkflowDetailDialog : null;
const workflowTaskModule = (0, bun_bundle_1.feature)('WORKFLOW_SCRIPTS') ? require('src/tasks/LocalWorkflowTask/LocalWorkflowTask.js') : null;
...
const monitorMcpModule = (0, bun_bundle_1.feature)('MONITOR_TOOL') ? require('../../tasks/MonitorMcpTask/MonitorMcpTask.js') : null;
const killMonitorMcp = monitorMcpModule?.killMonitorMcp ?? null;
const MonitorMcpDetailDialog = (0, bun_bundle_1.feature)('MONITOR_TOOL') ? require('./MonitorMcpDetailDialog.js').MonitorMcpDetailDialog : null;
...
function getSelectableBackgroundTasks(tasks, foregroundedTaskId) {
    const backgroundTasks = Object.values(tasks ?? {}).filter(types_js_1.isBackgroundTask);
...
function BackgroundTasksDialog({ onDone, toolUseContext, initialDetailTaskId }) {
    ...
    const foregroundedTaskId = (0, AppState_js_1.useAppState)(s_0 => s_0.foregroundedTaskId);
    const showSpinnerTree = (0, AppState_js_1.useAppState)(s_1 => s_1.expandedView) === 'teammates';
    const setAppState = (0, AppState_js_1.useSetAppState)();
    const killAgentsShortcut = (0, useShortcutDisplay_js_1.useShortcutDisplay)('chat:killAgents', 'Chat', 'ctrl+x ctrl+k');
    const typedTasks = tasks;
    // Track if we skipped list view on mount (for back button behavior)
    const skippedListOnMount = (0, react_1.useRef)(false);
    ...
    const { bashTasks, remoteSessions, agentTasks, teammateTasks, workflowTasks, mcpMonitors, dreamTasks: dreamTasks_0, allSelectableItems } = (0, react_1.useMemo)(() => {
        // Filter to only show running/pending background tasks, matching the status bar count
        const backgroundTasks = Object.values(typedTasks ?? {}).filter(types_js_1.isBackgroundTask);
        ...
        const leaderItem = teammates.length > 0 ? [{
                id: '__leader__',
                type: 'leader',
                label: `@${constants_js_1.TEAM_LEAD_NAME}`,
                status: 'running'
            }] : [];
        return {
            bashTasks: bash,
    ...
    async function killShellTask(taskId) {
        await LocalShellTask_js_1.LocalShellTask.kill(taskId, setAppState);
    }
    async function killAgentTask(taskId_0) {
        await LocalAgentTask_js_1.LocalAgentTask.kill(taskId_0, setAppState);
    }
    async function killTeammateTask(taskId_1) {
        await InProcessTeammateTask_js_1.InProcessTeammateTask.kill(taskId_1, setAppState);
    }
    async function killDreamTask(taskId_2) {
        await DreamTask_js_1.DreamTask.kill(taskId_2, setAppState);
    }
    async function killRemoteAgentTask(taskId_3) {
        await RemoteAgentTask_js_1.RemoteAgentTask.kill(taskId_3, setAppState);
    }
    ...
    const onDoneEvent = (0, react_1.useEffectEvent)(onDone);
    (0, react_1.useEffect)(() => {
        if (viewState.mode !== 'list') {
    ...
    if (viewState.mode !== 'list' && typedTasks) {
        ...
        switch (task_0.type) {
            case 'local_bash':
                return react_1.default.createElement(ShellDetailDialog_js_1.ShellDetailDialog, { shell: task_0, onDone: onDone, onKillShell: () => void killShellTask(task_0.id), onBack: goBackToList, key: `shell-${task_0.id}` });
            case 'local_agent':
                return react_1.default.createElement(AsyncAgentDetailDialog_js_1.AsyncAgentDetailDialog, { agent: task_0, onDone: onDone, onKillAgent: () => void killAgentTask(task_0.id), onBack: goBackToList, key: `agent-${task_0.id}` });
            case 'remote_agent':
                return react_1.default.createElement(RemoteSessionDetailDialog_js_1.RemoteSessionDetailDialog, { session: task_0, onDone: onDone, toolUseContext: toolUseContext, onBack: goBackToList, onKill: task_0.status !== 'running' ? undefined : task_0.isUltraplan ? () => void (0, ultraplan_js_1.stopUltraplan)(task_0.id, task_0.sessionId, setAppState) : () => void killRemoteAgentTask(task_0.id), key: `session-${task_0.id}` });
            case 'in_process_teammate':
                return react_1.default.createElement(InProcessTeammateDetailDialog_js_1.InProcessTeammateDetailDialog, { teammate: task_0, onDone: onDone, onKill: task_0.status === 'running' ? () => void killTeammateTask(task_0.id) : undefined, onBack: goBackToList, onForeground: task_0.status === 'running' ? () => {
                        (0, teammateViewHelpers_js_1.enterTeammateView)(task_0.id, setAppState);
                        onDone('Viewing teammate', {
                            display: 'system'
                        });
            ...
            case 'local_workflow':
                if (!WorkflowDetailDialog)
                    return null;
                return react_1.default.createElement(WorkflowDetailDialog, { workflow: task_0, onDone: onDone, onKill: task_0.status === 'running' && killWorkflowTask ? () => killWorkflowTask(task_0.id, setAppState) : undefined, onSkipAgent: task_0.status === 'running' && skipWorkflowAgent ? agentId => skipWorkflowAgent(task_0.id, agentId, setAppState) : undefined, onRetryAgent: task_0.status === 'running' && retryWorkflowAgent ? agentId_0 => retryWorkflowAgent(task_0.id, agentId_0, setAppState) : undefined, onBack: goBackToList, key: `workflow-${task_0.id}` });
            case 'monitor_mcp':
                if (!MonitorMcpDetailDialog)
                    return null;
                return react_1.default.createElement(MonitorMcpDetailDialog, { task: task_0, onKill: task_0.status === 'running' && killMonitorMcp ? () => killMonitorMcp(task_0.id, setAppState) : undefined, onBack: goBackToList, key: `monitor-mcp-${task_0.id}` });
            case 'dream':
                return react_1.default.createElement(DreamDetailDialog_js_1.DreamDetailDialog, { task: task_0, onDone: () => onDone('Background tasks dialog dismissed', {
                        display: 'system'
                    }), onBack: goBackToList, onKill: task_0.status === 'running' ? () => void killDreamTask(task_0.id) : undefined, key: `dream-${task_0.id}` });
        }
    }
    const runningBashCount = (0, array_js_2.count)(bashTasks, _ => _.status === 'running');
    const runningAgentCount = (0, array_js_2.count)(remoteSessions, __0 => __0.status === 'running' || __0.status === 'pending') + (0, array_js_2.count)(agentTasks, __1 => __1.status === 'running');
    const runningTeammateCount = (0, array_js_2.count)(teammateTasks, __2 => __2.status === 'running');
    const subtitle = (0, array_js_1.intersperse)([...(runningTeammateCount > 0 ? [react_1.default.createElement(ink_js_1.Text, { key: "teammates" },
                runningTeammateCount,
                ' ',
                runningTeammateCount !== 1 ? 'agents' : 'agent')] : []), ...(runningBashCount > 0 ? [react_1.default.createElement(ink_js_1.Text, { key: "shells" },
                runningBashCount,
                ' ',
                runningBashCount !== 1 ? 'active shells' : 'active shell')] : []), ...(runningAgentCount > 0 ? [react_1.default.createElement(ink_js_1.Text, { key: "agents" },
                runningAgentCount,
                ' ',
                runningAgentCount !== 1 ? 'active agents' : 'active agent')] : [])], index => react_1.default.createElement(ink_js_1.Text, { key: `separator-${index}` }, " \u00B7 "));
    const actions = [react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { key: "upDown", shortcut: "\u2191/\u2193", action: "select" }), react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { key: "enter", shortcut: "Enter", action: "view" }), ...(currentSelection?.type === 'in_process_teammate' && currentSelection.status === 'running' ? [react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { key: "foreground", shortcut: "f", action: "foreground" })] : []), ...((currentSelection?.type === 'local_bash' || currentSelection?.type === 'local_agent' || currentSelection?.type === 'in_process_teammate' || currentSelection?.type === 'local_workflow' || currentSelection?.type === 'monitor_mcp' || currentSelection?.type === 'dream' || currentSelection?.type === 'remote_agent') && currentSelection.status === 'running' ? [react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { key: "kill", shortcut: "x", action: "stop" })] : []), ...(agentTasks.some(t => t.status === 'running') ? [react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { key: "kill-all", shortcut: killAgentsShortcut, action: "stop all agents" })] : []), react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { key: "esc", shortcut: "\u2190/Esc", action: "close" })];
    ...
    function renderInputGuide(exitState) {
        ...
        return react_1.default.createElement(Byline_js_1.Byline, null, actions);
    }
    return react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", tabIndex: 0, autoFocus: true, onKeyDown: handleKeyDown },
        react_1.default.createElement(Dialog_js_1.Dialog, { title: "Background tasks", subtitle: react_1.default.createElement(react_1.default.Fragment, null, subtitle), onCancel: handleCancel, color: "background", inputGuide: renderInputGuide }, allSelectableItems.length === 0 ? react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "No tasks currently running") : react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
            teammateTasks.length > 0 && react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
                (bashTasks.length > 0 || remoteSessions.length > 0 || agentTasks.length > 0) && react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                    react_1.default.createElement(ink_js_1.Text, { bold: true },
                        '  ',
                        "Agents"),
                    " (",
                    (0, array_js_2.count)(teammateTasks, i => i.type !== 'leader'),
                    ")"),
                react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
                    react_1.default.createElement(TeammateTaskGroups, { teammateTasks: teammateTasks, currentSelectionId: currentSelection?.id }))),
            bashTasks.length > 0 && react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: teammateTasks.length > 0 ? 1 : 0 },
                (teammateTasks.length > 0 || remoteSessions.length > 0 || agentTasks.length > 0) && react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                    react_1.default.createElement(ink_js_1.Text, { bold: true },
                        '  ',
                        "Shells"),
                    " (",
                    bashTasks.length,
                    ")"),
                react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" }, bashTasks.map(item_6 => react_1.default.createElement(Item, { key: item_6.id, item: item_6, isSelected: item_6.id === currentSelection?.id })))),
            mcpMonitors.length > 0 && react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: teammateTasks.length > 0 || bashTasks.length > 0 ? 1 : 0 },
                react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                    ...
                    mcpMonitors.length,
                    ")"),
                react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" }, mcpMonitors.map(item_7 => react_1.default.createElement(Item, { key: item_7.id, item: item_7, isSelected: item_7.id === currentSelection?.id })))),
            remoteSessions.length > 0 && react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: teammateTasks.length > 0 || bashTasks.length > 0 || mcpMonitors.length > 0 ? 1 : 0 },
                react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                    react_1.default.createElement(ink_js_1.Text, { bold: true },
                        '  ',
                        "Remote agents"),
                    ...
                    ")"),
                react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" }, remoteSessions.map(item_8 => react_1.default.createElement(Item, { key: item_8.id, item: item_8, isSelected: item_8.id === currentSelection?.id })))),
            agentTasks.length > 0 && react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: teammateTasks.length > 0 || bashTasks.length > 0 || mcpMonitors.length > 0 || remoteSessions.length > 0 ? 1 : 0 },
                react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                    react_1.default.createElement(ink_js_1.Text, { bold: true },
                        '  ',
                        "Local agents"),
                    ...
                    agentTasks.length,
                    ")"),
                react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" }, agentTasks.map(item_9 => react_1.default.createElement(Item, { key: item_9.id, item: item_9, isSelected: item_9.id === currentSelection?.id })))),
            workflowTasks.length > 0 && react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: teammateTasks.length > 0 || bashTasks.length > 0 || mcpMonitors.length > 0 || remoteSessions.length > 0 || agentTasks.length > 0 ? 1 : 0 },
                react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                    react_1.default.createElement(ink_js_1.Text, { bold: true },
                        '  ',
                        "Workflows"),
                    ...
                    ")"),
                react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" }, workflowTasks.map(item_10 => react_1.default.createElement(Item, { key: item_10.id, item: item_10, isSelected: item_10.id === currentSelection?.id })))),
            dreamTasks_0.length > 0 && react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: teammateTasks.length > 0 || bashTasks.length > 0 || mcpMonitors.length > 0 || remoteSessions.length > 0 || agentTasks.length > 0 || workflowTasks.length > 0 ? 1 : 0 },
                react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" }, dreamTasks_0.map(item_11 => react_1.default.createElement(Item, { key: item_11.id, item: item_11, isSelected: item_11.id === currentSelection?.id })))))));
}
...
function Item(t0) {
    const $ = (0, compiler_runtime_1.c)(14);
    ...
    const t3 = isSelected ? figures_1.default.pointer + " " : "  ";
    let t4;
    if ($[1] !== t2 || $[2] !== t3) {
        t4 = react_1.default.createElement(ink_js_1.Text, { dimColor: t2 }, t3);
        $[1] = t2;
        $[2] = t3;
        $[3] = t4;
    }
    ...
    if ($[4] !== item.task || $[5] !== item.type || $[6] !== maxActivityWidth) {
        t6 = item.type === "leader" ? react_1.default.createElement(ink_js_1.Text, null,
            "@",
            constants_js_1.TEAM_LEAD_NAME) : react_1.default.createElement(BackgroundTask_js_1.BackgroundTask, { task: item.task, maxActivityWidth: maxActivityWidth });
        $[4] = item.task;
        $[5] = item.type;
        $[6] = maxActivityWidth;
        $[7] = t6;
    ...
    let t7;
    if ($[8] !== t5 || $[9] !== t6) {
        t7 = react_1.default.createElement(ink_js_1.Text, { color: t5 }, t6);
        $[8] = t5;
        $[9] = t6;
        $[10] = t7;
    }
    ...
    let t8;
    if ($[11] !== t4 || $[12] !== t7) {
        t8 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "row" },
            t4,
            t7);
        $[11] = t4;
        $[12] = t7;
...
function TeammateTaskGroups(t0) {
    ...
    if ($[0] !== currentSelectionId || $[1] !== teammateTasks) {
        ...
        t1 = react_1.default.createElement(react_1.default.Fragment, null, teamEntries.map(t2 => {
            const [teamName_0, items] = t2;
            const memberCount = items.length + leaderItems.length;
            return react_1.default.createElement(ink_js_1.Box, { key: teamName_0, flexDirection: "column" },
                react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                    "  ",
                    "Team: ",
                    teamName_0,
                    ...
                    memberCount,
                    ")"),
                leaderItems.map(item_0 => react_1.default.createElement(Item, { key: `${item_0.id}-${teamName_0}`, item: item_0, isSelected: item_0.id === currentSelectionId })),
                items.map(item_1 => react_1.default.createElement(Item, { key: item_1.id, item: item_1, isSelected: item_1.id === currentSelectionId })));
        }));
