...
const hyperlink_js_1 = require("../../utils/hyperlink.js");
...
const ExpandShellOutputContext_js_1 = require("./ExpandShellOutputContext.js");
function tryFormatJson(line) {
    try {
        const parsed = (0, slowOperations_js_1.jsonParse)(line);
        const stringified = (0, slowOperations_js_1.jsonStringify)(parsed);
...
function linkifyUrlsInText(content) {
    return content.replace(URL_IN_JSON, url => (0, hyperlink_js_1.createHyperlink)(url));
}
function OutputLine(t0) {
    ...
    const expandShellOutput = (0, ExpandShellOutputContext_js_1.useExpandShellOutput)();
    const inVirtualList = React.useContext(messageActions_js_1.InVirtualListContext);
    ...
    if ($[6] !== formattedContent) {
        t2 = React.createElement(ink_js_1.Ansi, null, formattedContent);
        $[6] = formattedContent;
        $[7] = t2;
    }
    else {
    ...
    let t3;
    if ($[8] !== color || $[9] !== t2) {
        t3 = React.createElement(MessageResponse_js_1.MessageResponse, null,
            React.createElement(ink_js_1.Text, { color: color }, t2));
        $[8] = color;
        $[9] = t2;
        $[10] = t3;
    }
