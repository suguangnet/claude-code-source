...
const ModelSelector_js_1 = require("./ModelSelector.js");
const ToolSelector_js_1 = require("./ToolSelector.js");
const utils_js_1 = require("./utils.js");
function AgentEditor({ agent, tools, onSaved, onBack }) {
    const setAppState = (0, AppState_js_1.useSetAppState)();
    const [editMode, setEditMode] = (0, react_1.useState)('menu');
    const [selectedMenuIndex, setSelectedMenuIndex] = (0, react_1.useState)(0);
    const [error, setError] = (0, react_1.useState)(null);
    const [selectedColor, setSelectedColor] = (0, react_1.useState)(agent.color);
    const handleOpenInEditor = (0, react_1.useCallback)(async () => {
        const filePath = (0, agentFileUtils_js_1.getActualAgentFilePath)(agent);
        const result = await (0, promptEditor_js_1.editFileInEditor)(filePath);
        if (result.error) {
    ...
    const handleSave = (0, react_1.useCallback)(async (changes = {}) => {
        ...
        try {
            ...
            await (0, agentFileUtils_js_1.updateAgentFile)(agent, agent.whenToUse, newTools ?? agent.tools, agent.getSystemPrompt(), finalColor, newModel ?? agent.model);
            ...
            onSaved(`Updated agent: ${chalk_1.default.bold(agent.agentType)}`);
            return true;
        }
        catch (err) {
    ...
    const handleMenuKeyDown = (0, react_1.useCallback)((e) => {
        if (e.key === 'up') {
            e.preventDefault();
    ...
    const renderMenu = () => React.createElement(ink_js_1.Box, { flexDirection: "column", tabIndex: 0, autoFocus: true, onKeyDown: handleMenuKeyDown },
        React.createElement(ink_js_1.Text, { dimColor: true },
            "Source: ",
            (0, utils_js_1.getAgentSourceDisplayName)(agent.source)),
        React.createElement(ink_js_1.Box, { marginTop: 1, flexDirection: "column" }, menuItems.map((item, index_1) => React.createElement(ink_js_1.Text, { key: item.label, color: index_1 === selectedMenuIndex ? 'suggestion' : undefined },
            index_1 === selectedMenuIndex ? `${figures_1.default.pointer} ` : '  ',
            item.label))),
    ...
    switch (editMode) {
        case 'menu':
            return renderMenu();
        case 'edit-tools':
            return React.createElement(ToolSelector_js_1.ToolSelector, { tools: tools, initialTools: agent.tools, onComplete: async (finalTools) => {
                    setEditMode('menu');
                    await handleSave({
                        tools: finalTools
                    });
        ...
        case 'edit-color':
            return React.createElement(ColorPicker_js_1.ColorPicker, { agentName: agent.agentType, currentColor: selectedColor || agent.color || 'automatic', onConfirm: async (color) => {
        ...
        case 'edit-model':
            return React.createElement(ModelSelector_js_1.ModelSelector, { initialModel: agent.model, onComplete: async (model) => {
                    setEditMode('menu');
                    await handleSave({
                        model
                    });
