...
const H = figures_1.default.heart;
...
function SpeechBubble(t0) {
    ...
    if ($[0] !== color || $[1] !== fading || $[2] !== text) {
        ...
        let t7;
        if ($[11] !== fading) {
            t7 = (l, i) => react_1.default.createElement(ink_js_1.Text, { key: i, italic: true, dimColor: !fading, color: fading ? "inactive" : undefined }, l);
            $[11] = fading;
            $[12] = t7;
        }
        else {
    ...
    if ($[13] !== T0 || $[14] !== t1 || $[15] !== t2 || $[16] !== t3 || $[17] !== t4 || $[18] !== t5 || $[19] !== t6) {
        t7 = react_1.default.createElement(T0, { flexDirection: t1, borderStyle: t2, borderColor: t3, paddingX: t4, width: t5 }, t6);
        $[13] = T0;
        $[14] = t1;
        $[15] = t2;
        $[16] = t3;
    ...
    if (tail === "right") {
        let t8;
        if ($[21] !== borderColor) {
            t8 = react_1.default.createElement(ink_js_1.Text, { color: borderColor }, "\u2500");
            $[21] = borderColor;
            $[22] = t8;
        }
        else {
        ...
        let t9;
        if ($[23] !== bubble || $[24] !== t8) {
            t9 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "row", alignItems: "center" },
                bubble,
                t8);
            $[23] = bubble;
            $[24] = t8;
    ...
    let t8;
    if ($[26] !== borderColor) {
        t8 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", alignItems: "flex-end", paddingRight: 6 },
            react_1.default.createElement(ink_js_1.Text, { color: borderColor }, "\u2572 "),
            react_1.default.createElement(ink_js_1.Text, { color: borderColor }, "\u2572"));
        $[26] = borderColor;
        $[27] = t8;
    }
    ...
    let t9;
    if ($[28] !== bubble || $[29] !== t8) {
        t9 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", alignItems: "flex-end", marginRight: 1 },
            bubble,
            t8);
        $[28] = bubble;
        $[29] = t8;
...
function companionReservedColumns(terminalColumns, speaking) {
    if (!(0, bun_bundle_1.feature)('BUDDY'))
        return 0;
    const companion = (0, companion_js_1.getCompanion)();
    if (!companion || (0, config_js_1.getGlobalConfig)().companionMuted)
        return 0;
    if (terminalColumns < exports.MIN_COLS_FOR_FULL_SPRITE)
        return 0;
    const nameWidth = (0, stringWidth_js_1.stringWidth)(companion.name);
    const bubble = speaking && !(0, fullscreen_js_1.isFullscreenActive)() ? BUBBLE_WIDTH : 0;
    return spriteColWidth(nameWidth) + SPRITE_PADDING_X + bubble;
}
function CompanionSprite() {
    const reaction = (0, AppState_js_1.useAppState)(s => s.companionReaction);
    const petAt = (0, AppState_js_1.useAppState)(s => s.companionPetAt);
    const focused = (0, AppState_js_1.useAppState)(s => s.footerSelection === 'companion');
    ...
    const lastSpokeTick = (0, react_1.useRef)(0);
    ...
    const [{ petStartTick, forPetAt }, setPetStart] = (0, react_1.useState)({
        petStartTick: 0,
        forPetAt: petAt
    });
    if (petAt !== forPetAt) {
    ...
    if (!companion || (0, config_js_1.getGlobalConfig)().companionMuted)
        return null;
    const color = types_js_1.RARITY_COLORS[companion.rarity];
    const colWidth = spriteColWidth((0, stringWidth_js_1.stringWidth)(companion.name));
    ...
    if (columns < exports.MIN_COLS_FOR_FULL_SPRITE) {
        ...
        return react_1.default.createElement(ink_js_1.Box, { paddingX: 1, alignSelf: "flex-end" },
            react_1.default.createElement(ink_js_1.Text, null,
                petting && react_1.default.createElement(ink_js_1.Text, { color: "autoAccept" },
                    figures_1.default.heart,
                    " "),
                react_1.default.createElement(ink_js_1.Text, { bold: true, color: color }, (0, sprites_js_1.renderFace)(companion)),
                ' ',
                react_1.default.createElement(ink_js_1.Text, { italic: true, dimColor: !focused && !reaction, bold: focused, inverse: focused && !reaction, color: reaction ? fading ? 'inactive' : color : focused ? color : undefined }, label)));
    }
    const frameCount = (0, sprites_js_1.spriteFrameCount)(companion.species);
    ...
    const body = (0, sprites_js_1.renderSprite)(companion, spriteFrame).map(line => blink ? line.replaceAll(companion.eye, '-') : line);
    ...
    const spriteColumn = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", flexShrink: 0, alignItems: "center", width: colWidth },
        sprite.map((line, i) => react_1.default.createElement(ink_js_1.Text, { key: i, color: i === 0 && heartFrame ? 'autoAccept' : color }, line)),
        react_1.default.createElement(ink_js_1.Text, { italic: true, bold: focused, dimColor: !focused, color: focused ? color : undefined, inverse: focused }, focused ? ` ${companion.name} ` : companion.name));
    if (!reaction) {
        return react_1.default.createElement(ink_js_1.Box, { paddingX: 1 }, spriteColumn);
    }
    ...
    return react_1.default.createElement(ink_js_1.Box, { flexDirection: "row", alignItems: "flex-end", paddingX: 1, flexShrink: 0 },
        react_1.default.createElement(SpeechBubble, { text: reaction, color: color, fading: fading, tail: "right" }),
        spriteColumn);
}
...
function CompanionFloatingBubble() {
    ...
    const reaction = (0, AppState_js_1.useAppState)(_temp);
    let t0;
    if ($[0] !== reaction) {
        t0 = {
            tick: 0,
    ...
    const [t1, setTick] = (0, react_1.useState)(t0);
    ...
    let t5;
    if ($[5] !== reaction || $[6] !== t4) {
        t5 = react_1.default.createElement(SpeechBubble, { text: reaction, color: types_js_1.RARITY_COLORS[companion.rarity], fading: t4, tail: "down" });
        $[5] = reaction;
        $[6] = t4;
        $[7] = t5;
    }
