...
const orphanedPluginFilter_js_1 = require("../../utils/plugins/orphanedPluginFilter.js");
...
exports.GrepTool = (0, Tool_js_1.buildTool)({
    ...
    strict: true,
    async description() {
        return (0, prompt_js_1.getDescription)();
    },
    userFacingName() {
        return 'Search';
    },
        ...
        if (path) {
            ...
            try {
                ...
                if ((0, errors_js_1.isENOENT)(e)) {
                    ...
                    let message = `Path does not exist: ${path}. ${file_js_1.FILE_NOT_FOUND_CWD_NOTE} ${(0, cwd_js_1.getCwd)()}.`;
                    if (cwdSuggestion) {
    ...
    async checkPermissions(input, context) {
        ...
        return (0, filesystem_js_1.checkReadPermissionForTool)(exports.GrepTool, input, appState.toolPermissionContext);
    },
    async prompt() {
        return (0, prompt_js_1.getDescription)();
    },
    ...
    mapToolResultToToolResultBlockParam({ mode = 'files_with_matches', numFiles, filenames, content, numLines: _numLines, numMatches, appliedLimit, appliedOffset, }, toolUseID) {
        ...
        const result = `Found ${numFiles} ${(0, stringUtils_js_1.plural)(numFiles, 'file')}${limitInfo ? ` ${limitInfo}` : ''}\n${filenames.join('\n')}`;
        return {
            tool_use_id: toolUseID,
            type: 'tool_result',
            content: result,
    ...
    async call({ pattern, path, glob, type, output_mode = 'files_with_matches', '-B': context_before, '-A': context_after, '-C': context_c, context, '-n': show_line_numbers = true, '-i': case_insensitive = false, head_limit, offset = 0, multiline = false, }, { abortController, getAppState }) {
        const absolutePath = path ? (0, path_js_1.expandPath)(path) : (0, cwd_js_1.getCwd)();
        const args = ['--hidden'];
        ...
        const ignorePatterns = (0, filesystem_js_1.normalizePatternsToPath)((0, filesystem_js_1.getFileReadIgnorePatterns)(appState.toolPermissionContext), (0, cwd_js_1.getCwd)());
        ...
        for (const exclusion of await (0, orphanedPluginFilter_js_1.getGlobExclusionsForPluginCache)(absolutePath)) {
        ...
        const results = await (0, ripgrep_js_1.ripGrep)(args, absolutePath, abortController.signal);
        if (output_mode === 'content') {
        ...
        const stats = await Promise.allSettled(results.map(_ => (0, fsOperations_js_1.getFsImplementation)().stat(_)));
        ...
        // Convert absolute paths to relative paths to save tokens
        const relativeMatches = finalMatches.map(path_js_1.toRelativePath);
        const output = {
