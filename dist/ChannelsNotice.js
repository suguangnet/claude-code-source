...
const channelAllowlist_js_1 = require("../../services/mcp/channelAllowlist.js");
...
function ChannelsNotice() {
    const $ = (0, compiler_runtime_1.c)(32);
    const [t0] = (0, react_1.useState)(_temp);
    const { channels, disabled, noAuth, policyBlocked, list, unmatched } = t0;
    if (channels.length === 0) {
        return null;
    }
    const hasNonDev = channels.some(_temp2);
    const flag = (0, state_js_1.getHasDevChannels)() && hasNonDev ? "Channels" : (0, state_js_1.getHasDevChannels)() ? "--dangerously-load-development-channels" : "--channels";
    if (disabled) {
        let t1;
        if ($[0] !== flag || $[1] !== list) {
            t1 = React.createElement(ink_js_1.Text, { color: "error" },
                flag,
                " ignored (",
                list,
                ")");
        ...
        if ($[3] === Symbol.for("react.memo_cache_sentinel")) {
            t2 = React.createElement(ink_js_1.Text, { dimColor: true }, "Channels are not currently available");
            $[3] = t2;
        }
        else {
            t2 = $[3];
        ...
        let t3;
        if ($[4] !== t1) {
            t3 = React.createElement(ink_js_1.Box, { paddingLeft: 2, flexDirection: "column" },
                t1,
                t2);
            $[4] = t1;
            $[5] = t3;
    ...
    if (noAuth) {
        ...
        if ($[9] === Symbol.for("react.memo_cache_sentinel")) {
            t2 = React.createElement(ink_js_1.Text, { dimColor: true }, "Channels require claude.ai authentication \u00B7 run /login, then restart");
            $[9] = t2;
        }
        else {
            t2 = $[9];
    ...
    if (policyBlocked) {
        ...
        if ($[15] === Symbol.for("react.memo_cache_sentinel")) {
            t2 = React.createElement(ink_js_1.Text, { dimColor: true }, "Inbound messages will be silently dropped");
            t3 = React.createElement(ink_js_1.Text, { dimColor: true }, "Have an administrator set channelsEnabled: true in managed settings to enable");
            $[15] = t2;
            $[16] = t3;
        }
        else {
        ...
        let t5;
        if ($[19] !== t1 || $[20] !== t4) {
            t5 = React.createElement(ink_js_1.Box, { paddingLeft: 2, flexDirection: "column" },
                t1,
                t2,
                t3,
                t4);
    ...
    if ($[28] !== t1 || $[29] !== t2 || $[30] !== t3) {
        t4 = React.createElement(ink_js_1.Box, { paddingLeft: 2, flexDirection: "column" },
            t1,
            t2,
            t3);
        $[28] = t1;
...
function _temp4(u_0) {
    return React.createElement(ink_js_1.Text, { key: `${formatEntry(u_0.entry)}:${u_0.why}`, color: "warning" },
        formatEntry(u_0.entry),
        " \u00B7 ",
        u_0.why);
}
function _temp3(u) {
    return React.createElement(ink_js_1.Text, { key: `${formatEntry(u.entry)}:${u.why}`, color: "warning" },
        formatEntry(u.entry),
        " \u00B7 ",
        u.why);
}
...
function _temp() {
    const ch = (0, state_js_1.getAllowedChannels)();
    if (ch.length === 0) {
        return {
            channels: ch,
            disabled: false,
    ...
    const allowlist = (0, channelNotification_js_1.getEffectiveChannelAllowlist)(sub, policy?.allowedChannelPlugins);
    return {
        channels: ch,
        disabled: !(0, channelAllowlist_js_1.isChannelsEnabled)(),
        noAuth: !(0, auth_js_1.getClaudeAIOAuthTokens)()?.accessToken,
...
function findUnmatched(entries, allowlist) {
    ...
    for (const scope of scopes) {
        for (const name of Object.keys((0, config_js_1.getMcpConfigsByScope)(scope).servers)) {
            configured.add(name);
        }
    }
    // Plugin-kind installed check: installed_plugins.json keys are
    // `name@marketplace`. loadInstalledPluginsV2 is cached.
    const installedPluginIds = new Set(Object.keys((0, installedPluginsManager_js_1.loadInstalledPluginsV2)().plugins));
