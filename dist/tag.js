...
function ConfirmRemoveTag(t0) {
    ...
    if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
        t2 = React.createElement(ink_js_1.Text, null, "This will remove the tag from the current session.");
        $[0] = t2;
    }
    else {
        t2 = $[0];
    ...
    let t5;
    if ($[5] !== t3) {
        t5 = React.createElement(ink_js_1.Box, { flexDirection: "column", gap: 1 },
            t2,
            React.createElement(select_js_1.Select, { onChange: t3, options: t4 }));
        $[5] = t3;
        $[6] = t5;
    }
    else {
    ...
    if ($[7] !== onCancel || $[8] !== t1 || $[9] !== t5) {
        t6 = React.createElement(Dialog_js_1.Dialog, { title: "Remove tag?", subtitle: t1, onCancel: onCancel, color: "warning" }, t5);
        $[7] = onCancel;
        $[8] = t1;
        $[9] = t5;
        $[10] = t6;
...
function ToggleTagAndClose(t0) {
    ...
    if ($[2] !== normalizedTag || $[3] !== onDone) {
        t2 = () => {
            const id = (0, state_js_1.getSessionId)();
            if (!id) {
            ...
            setSessionId(id);
            const currentTag = (0, sessionStorage_js_1.getCurrentSessionTag)(id);
            if (currentTag === normalizedTag) {
                ...
                (async () => {
                    ...
                    await (0, sessionStorage_js_1.saveTag)(id, normalizedTag, fullPath);
                    onDone(`Tagged session with ${chalk_1.default.cyan(`#${normalizedTag}`)}`, {
                        display: "system"
                    });
                })();
            }
    ...
    if (showConfirm && sessionId) {
        ...
        if ($[6] !== normalizedTag || $[7] !== onDone || $[8] !== sessionId) {
            t4 = async () => {
                ...
                const fullPath_0 = (0, sessionStorage_js_1.getTranscriptPath)();
                await (0, sessionStorage_js_1.saveTag)(sessionId, "", fullPath_0);
                onDone(`Removed tag ${chalk_1.default.cyan(`#${normalizedTag}`)}`, {
                    display: "system"
                });
            };
            $[6] = normalizedTag;
        ...
        if ($[10] !== normalizedTag || $[11] !== onDone) {
            t5 = () => {
                ...
                onDone(`Kept tag ${chalk_1.default.cyan(`#${normalizedTag}`)}`, {
                    display: "system"
                });
            };
            $[10] = normalizedTag;
