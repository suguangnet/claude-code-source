"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PermissionDecisionDebugInfo = PermissionDecisionDebugInfo;
const compiler_runtime_1 = require("react/compiler-runtime");
const bun_bundle_1 = require("bun:bundle");
const chalk_1 = __importDefault(require("chalk"));
const figures_1 = __importDefault(require("figures"));
const react_1 = __importDefault(require("react"));
const ink_js_1 = require("../../ink.js");
const AppState_js_1 = require("../../state/AppState.js");
const PermissionMode_js_1 = require("../../utils/permissions/PermissionMode.js");
const PermissionUpdate_js_1 = require("../../utils/permissions/PermissionUpdate.js");
const permissionRuleParser_js_1 = require("../../utils/permissions/permissionRuleParser.js");
const shadowedRuleDetection_js_1 = require("../../utils/permissions/shadowedRuleDetection.js");
const sandbox_adapter_js_1 = require("../../utils/sandbox/sandbox-adapter.js");
const constants_js_1 = require("../../utils/settings/constants.js");
function decisionReasonDisplayString(decisionReason) {
    if (((0, bun_bundle_1.feature)('BASH_CLASSIFIER') || (0, bun_bundle_1.feature)('TRANSCRIPT_CLASSIFIER')) && decisionReason.type === 'classifier') {
        return `${chalk_1.default.bold(decisionReason.classifier)} classifier: ${decisionReason.reason}`;
    }
    switch (decisionReason.type) {
        case 'rule':
            return `${chalk_1.default.bold((0, permissionRuleParser_js_1.permissionRuleValueToString)(decisionReason.rule.ruleValue))} rule from ${(0, constants_js_1.getSettingSourceDisplayNameLowercase)(decisionReason.rule.source)}`;
        case 'mode':
            return `${(0, PermissionMode_js_1.permissionModeTitle)(decisionReason.mode)} mode`;
        case 'sandboxOverride':
            return 'Requires permission to bypass sandbox';
        case 'workingDir':
            return decisionReason.reason;
        case 'safetyCheck':
        case 'other':
            return decisionReason.reason;
        case 'permissionPromptTool':
            return `${chalk_1.default.bold(decisionReason.permissionPromptToolName)} permission prompt tool`;
        case 'hook':
            return decisionReason.reason ? `${chalk_1.default.bold(decisionReason.hookName)} hook: ${decisionReason.reason}` : `${chalk_1.default.bold(decisionReason.hookName)} hook`;
        case 'asyncAgent':
            return decisionReason.reason;
        default:
            return '';
    }
}
function PermissionDecisionInfoItem(t0) {
    const $ = (0, compiler_runtime_1.c)(10);
    const { title, decisionReason } = t0;
    const [theme] = (0, ink_js_1.useTheme)();
    let t1;
    if ($[0] !== decisionReason || $[1] !== theme) {
        t1 = function formatDecisionReason() {
            switch (decisionReason.type) {
                case "subcommandResults":
                    {
                        return react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" }, Array.from(decisionReason.reasons.entries()).map(t2 => {
                            const [subcommand, result] = t2;
                            const icon = result.behavior === "allow" ? (0, ink_js_1.color)("success", theme)(figures_1.default.tick) : (0, ink_js_1.color)("error", theme)(figures_1.default.cross);
                            return react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", key: subcommand },
                                react_1.default.createElement(ink_js_1.Text, null,
                                    icon,
                                    " ",
                                    subcommand),
                                result.decisionReason !== undefined && result.decisionReason.type !== "subcommandResults" && react_1.default.createElement(ink_js_1.Text, null,
                                    react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                                        "  ",
                                        "\u23BF",
                                        "  "),
                                    react_1.default.createElement(ink_js_1.Ansi, null, decisionReasonDisplayString(result.decisionReason))),
                                result.behavior === "ask" && react_1.default.createElement(SuggestedRules, { suggestions: result.suggestions }));
                        }));
                    }
                default:
                    {
                        return react_1.default.createElement(ink_js_1.Text, null,
                            react_1.default.createElement(ink_js_1.Ansi, null, decisionReasonDisplayString(decisionReason)));
                    }
            }
        };
        $[0] = decisionReason;
        $[1] = theme;
        $[2] = t1;
    }
    else {
        t1 = $[2];
    }
    const formatDecisionReason = t1;
    let t2;
    if ($[3] !== title) {
        t2 = title && react_1.default.createElement(ink_js_1.Text, null, title);
        $[3] = title;
        $[4] = t2;
    }
    else {
        t2 = $[4];
    }
    let t3;
    if ($[5] !== formatDecisionReason) {
        t3 = formatDecisionReason();
        $[5] = formatDecisionReason;
        $[6] = t3;
    }
    else {
        t3 = $[6];
    }
    let t4;
    if ($[7] !== t2 || $[8] !== t3) {
        t4 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
            t2,
            t3);
        $[7] = t2;
        $[8] = t3;
        $[9] = t4;
    }
    else {
        t4 = $[9];
    }
    return t4;
}
function SuggestedRules(t0) {
    const $ = (0, compiler_runtime_1.c)(18);
    const { suggestions } = t0;
    let T0;
    let T1;
    let t1;
    let t2;
    let t3;
    let t4;
    let t5;
    if ($[0] !== suggestions) {
        t5 = Symbol.for("react.early_return_sentinel");
        bb0: {
            const rules = (0, PermissionUpdate_js_1.extractRules)(suggestions);
            if (rules.length === 0) {
                t5 = null;
                break bb0;
            }
            T1 = ink_js_1.Text;
            if ($[8] === Symbol.for("react.memo_cache_sentinel")) {
                t2 = react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                    "  ",
                    "\u23BF",
                    "  ");
                $[8] = t2;
            }
            else {
                t2 = $[8];
            }
            t3 = "Suggested rules:";
            t4 = " ";
            T0 = ink_js_1.Ansi;
            t1 = rules.map(_temp).join(", ");
        }
        $[0] = suggestions;
        $[1] = T0;
        $[2] = T1;
        $[3] = t1;
        $[4] = t2;
        $[5] = t3;
        $[6] = t4;
        $[7] = t5;
    }
    else {
        T0 = $[1];
        T1 = $[2];
        t1 = $[3];
        t2 = $[4];
        t3 = $[5];
        t4 = $[6];
        t5 = $[7];
    }
    if (t5 !== Symbol.for("react.early_return_sentinel")) {
        return t5;
    }
    let t6;
    if ($[9] !== T0 || $[10] !== t1) {
        t6 = react_1.default.createElement(T0, null, t1);
        $[9] = T0;
        $[10] = t1;
        $[11] = t6;
    }
    else {
        t6 = $[11];
    }
    let t7;
    if ($[12] !== T1 || $[13] !== t2 || $[14] !== t3 || $[15] !== t4 || $[16] !== t6) {
        t7 = react_1.default.createElement(T1, null,
            t2,
            t3,
            t4,
            t6);
        $[12] = T1;
        $[13] = t2;
        $[14] = t3;
        $[15] = t4;
        $[16] = t6;
        $[17] = t7;
    }
    else {
        t7 = $[17];
    }
    return t7;
}
function _temp(rule) {
    return chalk_1.default.bold((0, permissionRuleParser_js_1.permissionRuleValueToString)(rule));
}
// Helper function to extract directories from permission updates
function extractDirectories(updates) {
    if (!updates)
        return [];
    return updates.flatMap(update => {
        switch (update.type) {
            case 'addDirectories':
                return update.directories;
            default:
                return [];
        }
    });
}
// Helper function to extract mode from permission updates
function extractMode(updates) {
    if (!updates)
        return undefined;
    const update = updates.findLast(u => u.type === 'setMode');
    return update?.type === 'setMode' ? update.mode : undefined;
}
function SuggestionDisplay(t0) {
    const $ = (0, compiler_runtime_1.c)(22);
    const { suggestions, width } = t0;
    if (!suggestions || suggestions.length === 0) {
        let t1;
        if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
            t1 = react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "Suggestions ");
            $[0] = t1;
        }
        else {
            t1 = $[0];
        }
        let t2;
        if ($[1] !== width) {
            t2 = react_1.default.createElement(ink_js_1.Box, { justifyContent: "flex-end", minWidth: width }, t1);
            $[1] = width;
            $[2] = t2;
        }
        else {
            t2 = $[2];
        }
        let t3;
        if ($[3] === Symbol.for("react.memo_cache_sentinel")) {
            t3 = react_1.default.createElement(ink_js_1.Text, null, "None");
            $[3] = t3;
        }
        else {
            t3 = $[3];
        }
        let t4;
        if ($[4] !== t2) {
            t4 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "row" },
                t2,
                t3);
            $[4] = t2;
            $[5] = t4;
        }
        else {
            t4 = $[5];
        }
        return t4;
    }
    let t1;
    let t2;
    if ($[6] !== suggestions || $[7] !== width) {
        t2 = Symbol.for("react.early_return_sentinel");
        bb0: {
            const rules = (0, PermissionUpdate_js_1.extractRules)(suggestions);
            const directories = extractDirectories(suggestions);
            const mode = extractMode(suggestions);
            if (rules.length === 0 && directories.length === 0 && !mode) {
                let t3;
                if ($[10] === Symbol.for("react.memo_cache_sentinel")) {
                    t3 = react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "Suggestion ");
                    $[10] = t3;
                }
                else {
                    t3 = $[10];
                }
                let t4;
                if ($[11] !== width) {
                    t4 = react_1.default.createElement(ink_js_1.Box, { justifyContent: "flex-end", minWidth: width }, t3);
                    $[11] = width;
                    $[12] = t4;
                }
                else {
                    t4 = $[12];
                }
                let t5;
                if ($[13] === Symbol.for("react.memo_cache_sentinel")) {
                    t5 = react_1.default.createElement(ink_js_1.Text, null, "None");
                    $[13] = t5;
                }
                else {
                    t5 = $[13];
                }
                let t6;
                if ($[14] !== t4) {
                    t6 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "row" },
                        t4,
                        t5);
                    $[14] = t4;
                    $[15] = t6;
                }
                else {
                    t6 = $[15];
                }
                t2 = t6;
                break bb0;
            }
            let t3;
            if ($[16] === Symbol.for("react.memo_cache_sentinel")) {
                t3 = react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "Suggestions ");
                $[16] = t3;
            }
            else {
                t3 = $[16];
            }
            let t4;
            if ($[17] !== width) {
                t4 = react_1.default.createElement(ink_js_1.Box, { justifyContent: "flex-end", minWidth: width }, t3);
                $[17] = width;
                $[18] = t4;
            }
            else {
                t4 = $[18];
            }
            let t5;
            if ($[19] === Symbol.for("react.memo_cache_sentinel")) {
                t5 = react_1.default.createElement(ink_js_1.Text, null, " ");
                $[19] = t5;
            }
            else {
                t5 = $[19];
            }
            let t6;
            if ($[20] !== t4) {
                t6 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "row" },
                    t4,
                    t5);
                $[20] = t4;
                $[21] = t6;
            }
            else {
                t6 = $[21];
            }
            t1 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
                t6,
                rules.length > 0 && react_1.default.createElement(ink_js_1.Box, { flexDirection: "row" },
                    react_1.default.createElement(ink_js_1.Box, { justifyContent: "flex-end", minWidth: width },
                        react_1.default.createElement(ink_js_1.Text, { dimColor: true }, " Rules ")),
                    react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" }, rules.map(_temp2))),
                directories.length > 0 && react_1.default.createElement(ink_js_1.Box, { flexDirection: "row" },
                    react_1.default.createElement(ink_js_1.Box, { justifyContent: "flex-end", minWidth: width },
                        react_1.default.createElement(ink_js_1.Text, { dimColor: true }, " Directories ")),
                    react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" }, directories.map(_temp3))),
                mode && react_1.default.createElement(ink_js_1.Box, { flexDirection: "row" },
                    react_1.default.createElement(ink_js_1.Box, { justifyContent: "flex-end", minWidth: width },
                        react_1.default.createElement(ink_js_1.Text, { dimColor: true }, " Mode ")),
                    react_1.default.createElement(ink_js_1.Text, null, (0, PermissionMode_js_1.permissionModeTitle)(mode))));
        }
        $[6] = suggestions;
        $[7] = width;
        $[8] = t1;
        $[9] = t2;
    }
    else {
        t1 = $[8];
        t2 = $[9];
    }
    if (t2 !== Symbol.for("react.early_return_sentinel")) {
        return t2;
    }
    return t1;
}
function _temp3(dir, index_0) {
    return react_1.default.createElement(ink_js_1.Text, { key: index_0 },
        figures_1.default.bullet,
        " ",
        dir);
}
function _temp2(rule, index) {
    return react_1.default.createElement(ink_js_1.Text, { key: index },
        figures_1.default.bullet,
        " ",
        (0, permissionRuleParser_js_1.permissionRuleValueToString)(rule));
}
function PermissionDecisionDebugInfo(t0) {
    const $ = (0, compiler_runtime_1.c)(25);
    const { permissionResult, toolName } = t0;
    const toolPermissionContext = (0, AppState_js_1.useAppState)(_temp4);
    const decisionReason = permissionResult.decisionReason;
    const suggestions = "suggestions" in permissionResult ? permissionResult.suggestions : undefined;
    let t1;
    if ($[0] !== suggestions || $[1] !== toolName || $[2] !== toolPermissionContext) {
        bb0: {
            const sandboxAutoAllowEnabled = sandbox_adapter_js_1.SandboxManager.isSandboxingEnabled() && sandbox_adapter_js_1.SandboxManager.isAutoAllowBashIfSandboxedEnabled();
            const all = (0, shadowedRuleDetection_js_1.detectUnreachableRules)(toolPermissionContext, {
                sandboxAutoAllowEnabled
            });
            const suggestedRules = (0, PermissionUpdate_js_1.extractRules)(suggestions);
            if (suggestedRules.length > 0) {
                t1 = all.filter(u => suggestedRules.some(suggested => suggested.toolName === u.rule.ruleValue.toolName && suggested.ruleContent === u.rule.ruleValue.ruleContent));
                break bb0;
            }
            if (toolName) {
                let t2;
                if ($[4] !== toolName) {
                    t2 = u_0 => u_0.rule.ruleValue.toolName === toolName;
                    $[4] = toolName;
                    $[5] = t2;
                }
                else {
                    t2 = $[5];
                }
                t1 = all.filter(t2);
                break bb0;
            }
            t1 = all;
        }
        $[0] = suggestions;
        $[1] = toolName;
        $[2] = toolPermissionContext;
        $[3] = t1;
    }
    else {
        t1 = $[3];
    }
    const unreachableRules = t1;
    let t2;
    if ($[6] === Symbol.for("react.memo_cache_sentinel")) {
        t2 = react_1.default.createElement(ink_js_1.Box, { justifyContent: "flex-end", minWidth: 10 },
            react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "Behavior "));
        $[6] = t2;
    }
    else {
        t2 = $[6];
    }
    let t3;
    if ($[7] !== permissionResult.behavior) {
        t3 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "row" },
            t2,
            react_1.default.createElement(ink_js_1.Text, null, permissionResult.behavior));
        $[7] = permissionResult.behavior;
        $[8] = t3;
    }
    else {
        t3 = $[8];
    }
    let t4;
    if ($[9] !== permissionResult.behavior || $[10] !== permissionResult.message) {
        t4 = permissionResult.behavior !== "allow" && react_1.default.createElement(ink_js_1.Box, { flexDirection: "row" },
            react_1.default.createElement(ink_js_1.Box, { justifyContent: "flex-end", minWidth: 10 },
                react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "Message ")),
            react_1.default.createElement(ink_js_1.Text, null, permissionResult.message));
        $[9] = permissionResult.behavior;
        $[10] = permissionResult.message;
        $[11] = t4;
    }
    else {
        t4 = $[11];
    }
    let t5;
    if ($[12] === Symbol.for("react.memo_cache_sentinel")) {
        t5 = react_1.default.createElement(ink_js_1.Box, { justifyContent: "flex-end", minWidth: 10 },
            react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "Reason "));
        $[12] = t5;
    }
    else {
        t5 = $[12];
    }
    let t6;
    if ($[13] !== decisionReason) {
        t6 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "row" },
            t5,
            decisionReason === undefined ? react_1.default.createElement(ink_js_1.Text, null, "undefined") : react_1.default.createElement(PermissionDecisionInfoItem, { decisionReason: decisionReason }));
        $[13] = decisionReason;
        $[14] = t6;
    }
    else {
        t6 = $[14];
    }
    let t7;
    if ($[15] !== suggestions) {
        t7 = react_1.default.createElement(SuggestionDisplay, { suggestions: suggestions, width: 10 });
        $[15] = suggestions;
        $[16] = t7;
    }
    else {
        t7 = $[16];
    }
    let t8;
    if ($[17] !== unreachableRules) {
        t8 = unreachableRules.length > 0 && react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: 1 },
            react_1.default.createElement(ink_js_1.Text, { color: "warning" },
                figures_1.default.warning,
                " Unreachable Rules (",
                unreachableRules.length,
                ")"),
            unreachableRules.map(_temp5));
        $[17] = unreachableRules;
        $[18] = t8;
    }
    else {
        t8 = $[18];
    }
    let t9;
    if ($[19] !== t3 || $[20] !== t4 || $[21] !== t6 || $[22] !== t7 || $[23] !== t8) {
        t9 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
            t3,
            t4,
            t6,
            t7,
            t8);
        $[19] = t3;
        $[20] = t4;
        $[21] = t6;
        $[22] = t7;
        $[23] = t8;
        $[24] = t9;
    }
    else {
        t9 = $[24];
    }
    return t9;
}
function _temp5(u_1, i) {
    return react_1.default.createElement(ink_js_1.Box, { key: i, flexDirection: "column", marginLeft: 2 },
        react_1.default.createElement(ink_js_1.Text, { color: "warning" }, (0, permissionRuleParser_js_1.permissionRuleValueToString)(u_1.rule.ruleValue)),
        react_1.default.createElement(ink_js_1.Text, { dimColor: true },
            "  ",
            u_1.reason),
        react_1.default.createElement(ink_js_1.Text, { dimColor: true },
            "  ",
            "Fix: ",
            u_1.fix));
}
function _temp4(s) {
    return s.toolPermissionContext;
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
