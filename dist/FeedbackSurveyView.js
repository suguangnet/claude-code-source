"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.isValidResponseInput = void 0;
exports.FeedbackSurveyView = FeedbackSurveyView;
const compiler_runtime_1 = require("react/compiler-runtime");
const react_1 = __importDefault(require("react"));
const ink_js_1 = require("../../ink.js");
const useDebouncedDigitInput_js_1 = require("./useDebouncedDigitInput.js");
const RESPONSE_INPUTS = ['0', '1', '2', '3'];
const inputToResponse = {
    '0': 'dismissed',
    '1': 'bad',
    '2': 'fine',
    '3': 'good'
};
const isValidResponseInput = (input) => RESPONSE_INPUTS.includes(input);
exports.isValidResponseInput = isValidResponseInput;
const DEFAULT_MESSAGE = 'How is Claude doing this session? (optional)';
function FeedbackSurveyView(t0) {
    const $ = (0, compiler_runtime_1.c)(15);
    const { onSelect, inputValue, setInputValue, message: t1 } = t0;
    const message = t1 === undefined ? DEFAULT_MESSAGE : t1;
    let t2;
    if ($[0] !== onSelect) {
        t2 = digit => onSelect(inputToResponse[digit]);
        $[0] = onSelect;
        $[1] = t2;
    }
    else {
        t2 = $[1];
    }
    let t3;
    if ($[2] !== inputValue || $[3] !== setInputValue || $[4] !== t2) {
        t3 = {
            inputValue,
            setInputValue,
            isValidDigit: exports.isValidResponseInput,
            onDigit: t2
        };
        $[2] = inputValue;
        $[3] = setInputValue;
        $[4] = t2;
        $[5] = t3;
    }
    else {
        t3 = $[5];
    }
    (0, useDebouncedDigitInput_js_1.useDebouncedDigitInput)(t3);
    let t4;
    if ($[6] === Symbol.for("react.memo_cache_sentinel")) {
        t4 = react_1.default.createElement(ink_js_1.Text, { color: "ansi:cyan" }, "\u25CF ");
        $[6] = t4;
    }
    else {
        t4 = $[6];
    }
    let t5;
    if ($[7] !== message) {
        t5 = react_1.default.createElement(ink_js_1.Box, null,
            t4,
            react_1.default.createElement(ink_js_1.Text, { bold: true }, message));
        $[7] = message;
        $[8] = t5;
    }
    else {
        t5 = $[8];
    }
    let t6;
    if ($[9] === Symbol.for("react.memo_cache_sentinel")) {
        t6 = react_1.default.createElement(ink_js_1.Box, { width: 10 },
            react_1.default.createElement(ink_js_1.Text, null,
                react_1.default.createElement(ink_js_1.Text, { color: "ansi:cyan" }, "1"),
                ": Bad"));
        $[9] = t6;
    }
    else {
        t6 = $[9];
    }
    let t7;
    if ($[10] === Symbol.for("react.memo_cache_sentinel")) {
        t7 = react_1.default.createElement(ink_js_1.Box, { width: 10 },
            react_1.default.createElement(ink_js_1.Text, null,
                react_1.default.createElement(ink_js_1.Text, { color: "ansi:cyan" }, "2"),
                ": Fine"));
        $[10] = t7;
    }
    else {
        t7 = $[10];
    }
    let t8;
    if ($[11] === Symbol.for("react.memo_cache_sentinel")) {
        t8 = react_1.default.createElement(ink_js_1.Box, { width: 10 },
            react_1.default.createElement(ink_js_1.Text, null,
                react_1.default.createElement(ink_js_1.Text, { color: "ansi:cyan" }, "3"),
                ": Good"));
        $[11] = t8;
    }
    else {
        t8 = $[11];
    }
    let t9;
    if ($[12] === Symbol.for("react.memo_cache_sentinel")) {
        t9 = react_1.default.createElement(ink_js_1.Box, { marginLeft: 2 },
            t6,
            t7,
            t8,
            react_1.default.createElement(ink_js_1.Box, null,
                react_1.default.createElement(ink_js_1.Text, null,
                    react_1.default.createElement(ink_js_1.Text, { color: "ansi:cyan" }, "0"),
                    ": Dismiss")));
        $[12] = t9;
    }
    else {
        t9 = $[12];
    }
    let t10;
    if ($[13] !== t5) {
        t10 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: 1 },
            t5,
            t9);
        $[13] = t5;
        $[14] = t10;
    }
    else {
        t10 = $[14];
    }
    return t10;
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
