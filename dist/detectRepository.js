...
async function detectCurrentRepositoryWithHost() {
    ...
    try {
        ...
        (0, debug_js_1.logForDebugging)(`Git remote URL: ${remoteUrl}`);
        if (!remoteUrl) {
        ...
        (0, debug_js_1.logForDebugging)(`Parsed repository: ${parsed ? `${parsed.host}/${parsed.owner}/${parsed.name}` : null} from URL: ${remoteUrl}`);
        repositoryWithHostCache.set(cwd, parsed);
        return parsed;
    }
    catch (error) {
        (0, debug_js_1.logForDebugging)(`Error detecting repository: ${error}`);
        repositoryWithHostCache.set(cwd, null);
        return null;
    }
}
...
function getCachedRepository() {
    const parsed = repositoryWithHostCache.get((0, cwd_js_1.getCwd)());
...
function parseGitHubRepository(input) {
    ...
    (0, debug_js_1.logForDebugging)(`Could not parse repository from: ${trimmed}`);
    return null;
}
/**
