...
function stringsForDecisionReason(reason, toolType) {
    ...
    if (((0, bun_bundle_1.feature)('BASH_CLASSIFIER') || (0, bun_bundle_1.feature)('TRANSCRIPT_CLASSIFIER')) && reason.type === 'classifier') {
        ...
        return {
            reasonString: `Classifier ${chalk_1.default.bold(reason.classifier)} requires confirmation for this ${toolType}.\n${reason.reason}`,
            configString: undefined
        };
    }
    switch (reason.type) {
        case 'rule':
            return {
                reasonString: `Permission rule ${chalk_1.default.bold((0, permissionRuleParser_js_1.permissionRuleValueToString)(reason.rule.ruleValue))} requires confirmation for this ${toolType}.`,
                configString: reason.rule.source === 'policySettings' ? undefined : '/permissions to update rules'
            };
        case 'hook':
            {
                const hookReasonString = reason.reason ? `:\n${reason.reason}` : '.';
                const sourceLabel = reason.hookSource ? ` ${chalk_1.default.dim(`[${reason.hookSource}]`)}` : '';
                return {
                    reasonString: `Hook ${chalk_1.default.bold(reason.hookName)} requires confirmation for this ${toolType}${hookReasonString}${sourceLabel}`,
...
function PermissionRuleExplanation(t0) {
    ...
    const permissionMode = (0, AppState_js_1.useAppState)(_temp);
    ...
    if ($[3] !== strings.reasonString || $[4] !== themeColor) {
        t3 = themeColor ? react_1.default.createElement(ThemedText_js_1.default, { color: themeColor }, strings.reasonString) : react_1.default.createElement(ink_js_1.Text, null,
            react_1.default.createElement(ink_js_1.Ansi, null, strings.reasonString));
    ...
    if ($[6] !== strings.configString) {
        t4 = strings.configString && react_1.default.createElement(ink_js_1.Text, { dimColor: true }, strings.configString);
    ...
    let t5;
    if ($[8] !== t3 || $[9] !== t4) {
        t5 = react_1.default.createElement(ink_js_1.Box, { marginBottom: 1, flexDirection: "column" },
            t3,
            t4);
        $[8] = t3;
        $[9] = t4;
