...
function processQueueIfReady({ executeInput, }) {
    ...
    const next = (0, messageQueueManager_js_1.peek)(isMainThread);
    if (!next) {
    ...
    if (isSlashCommand(next) || next.mode === 'bash') {
        const cmd = (0, messageQueueManager_js_1.dequeue)(isMainThread);
        void executeInput([cmd]);
        return { processed: true };
    }
    // Drain all non-slash-command items with the same mode at once.
    const targetMode = next.mode;
    const commands = (0, messageQueueManager_js_1.dequeueAllMatching)(cmd => isMainThread(cmd) && !isSlashCommand(cmd) && cmd.mode === targetMode);
