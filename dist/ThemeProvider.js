...
const use_stdin_js_1 = __importDefault(require("../../ink/hooks/use-stdin.js"));
...
function ThemeProvider({ children, initialState, onThemeSave = defaultSaveTheme }) {
    const [themeSetting, setThemeSetting] = (0, react_1.useState)(initialState ?? defaultInitialTheme);
    const [previewTheme, setPreviewTheme] = (0, react_1.useState)(null);
    ...
    const [systemTheme, setSystemTheme] = (0, react_1.useState)(() => (initialState ?? themeSetting) === 'auto' ? (0, systemTheme_js_1.getSystemThemeName)() : 'dark');
    // The setting currently in effect (preview wins while picker is open)
    const activeSetting = previewTheme ?? themeSetting;
    const { internal_querier } = (0, use_stdin_js_1.default)();
    ...
    (0, react_1.useEffect)(() => {
        if ((0, bun_bundle_1.feature)('AUTO_THEME')) {
            ...
            let cleanup;
            let cancelled = false;
            void Promise.resolve().then(() => __importStar(require('../../utils/systemThemeWatcher.js'))).then(({ watchSystemTheme }) => {
                if (cancelled)
                    return;
...
function useTheme() {
    ...
    const { currentTheme, setThemeSetting } = (0, react_1.useContext)(ThemeContext);
    let t0;
...
function usePreviewTheme() {
    ...
    const { setPreviewTheme, savePreview, cancelPreview } = (0, react_1.useContext)(ThemeContext);
    let t0;
