...
class ITermBackend {
     ...
     */
    async isAvailable() {
        const inITerm2 = (0, detection_js_1.isInITerm2)();
        (0, debug_js_1.logForDebugging)(`[ITermBackend] isAvailable check: inITerm2=${inITerm2}`);
        if (!inITerm2) {
            (0, debug_js_1.logForDebugging)('[ITermBackend] isAvailable: false (not in iTerm2)');
            return false;
        }
        const it2Available = await (0, detection_js_1.isIt2CliAvailable)();
        (0, debug_js_1.logForDebugging)(`[ITermBackend] isAvailable: ${it2Available} (it2 CLI ${it2Available ? 'found' : 'not found'})`);
        return it2Available;
    }
    /**
    ...
    async isRunningInside() {
        const result = (0, detection_js_1.isInITerm2)();
        (0, debug_js_1.logForDebugging)(`[ITermBackend] isRunningInside: ${result}`);
        return result;
    }
    /**
    ...
    async createTeammatePaneInSwarmView(name, color) {
        (0, debug_js_1.logForDebugging)(`[ITermBackend] createTeammatePaneInSwarmView called for ${name} with color ${color}`);
        ...
        try {
            ...
            while (true) {
                ...
                (0, debug_js_1.logForDebugging)(`[ITermBackend] Creating pane: isFirstTeammate=${isFirstTeammate}, existingPanes=${teammateSessionIds.length}`);
                let splitArgs;
                ...
                if (isFirstTeammate) {
                    ...
                    if (leaderSessionId) {
                        ...
                        (0, debug_js_1.logForDebugging)(`[ITermBackend] First split from leader session: ${leaderSessionId}`);
                    }
                    else {
                    ...
                    if (targetedTeammateId) {
                        ...
                        (0, debug_js_1.logForDebugging)(`[ITermBackend] Subsequent split from teammate session: ${targetedTeammateId}`);
                    }
                    else {
                ...
                if (splitResult.code !== 0) {
                    ...
                    if (targetedTeammateId) {
                        ...
                        if (listResult.code === 0 &&
                            ...
                            (0, debug_js_1.logForDebugging)(`[ITermBackend] Split failed targeting dead session ${targetedTeammateId}, pruning and retrying: ${splitResult.stderr}`);
                ...
                (0, debug_js_1.logForDebugging)(`[ITermBackend] Created teammate pane for ${name}: ${paneId}`);
