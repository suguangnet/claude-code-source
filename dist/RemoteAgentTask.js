...
const remoteSession_js_1 = require("../../utils/background/remote/remoteSession.js");
...
async function persistRemoteAgentMetadata(meta) {
    try {
        await (0, sessionStorage_js_1.writeRemoteAgentMetadata)(meta.taskId, meta);
    }
    catch (e) {
        (0, debug_js_1.logForDebugging)(`persistRemoteAgentMetadata failed: ${String(e)}`);
    }
}
/**
...
async function removeRemoteAgentMetadata(taskId) {
    try {
        await (0, sessionStorage_js_1.deleteRemoteAgentMetadata)(taskId);
    }
    catch (e) {
        (0, debug_js_1.logForDebugging)(`removeRemoteAgentMetadata failed: ${String(e)}`);
    }
}
/**
 * Check eligibility for creating a remote agent session.
 */
async function checkRemoteAgentEligibility({ skipBundle = false } = {}) {
    const errors = await (0, remoteSession_js_1.checkBackgroundRemoteSessionEligibility)({
        skipBundle
    });
    if (errors.length > 0) {
        return {
...
<${xml_js_1.TASK_TYPE_TAG}>remote_agent</${xml_js_1.TASK_TYPE_TAG}>
<${xml_js_1.OUTPUT_FILE_TAG}>${outputPath}</${xml_js_1.OUTPUT_FILE_TAG}>
<${xml_js_1.STATUS_TAG}>${status}</${xml_js_1.STATUS_TAG}>
<${xml_js_1.SUMMARY_TAG}>Remote task "${title}" ${statusText}</${xml_js_1.SUMMARY_TAG}>
...
function extractPlanFromLog(log) {
    ...
    for (let i = log.length - 1; i >= 0; i--) {
        ...
        if (msg?.type !== 'assistant')
            continue;
        const fullText = (0, messages_js_1.extractTextContent)(msg.message.content, '\n');
        const plan = (0, messages_js_1.extractTag)(fullText, xml_js_1.ULTRAPLAN_TAG);
        if (plan?.trim())
            return plan.trim();
    }
    return null;
...
function enqueueUltraplanFailureNotification(taskId, sessionId, reason, setAppState) {
    ...
    const message = `<${xml_js_1.TASK_NOTIFICATION_TAG}>
<${xml_js_1.TASK_ID_TAG}>${taskId}</${xml_js_1.TASK_ID_TAG}>
<${xml_js_1.TASK_TYPE_TAG}>remote_agent</${xml_js_1.TASK_TYPE_TAG}>
<${xml_js_1.STATUS_TAG}>failed</${xml_js_1.STATUS_TAG}>
<${xml_js_1.SUMMARY_TAG}>Ultraplan failed: ${reason}</${xml_js_1.SUMMARY_TAG}>
</${xml_js_1.TASK_NOTIFICATION_TAG}>
...
function extractReviewFromLog(log) {
    for (let i = log.length - 1; i >= 0; i--) {
        ...
        if (msg?.type === 'system' && (msg.subtype === 'hook_progress' || msg.subtype === 'hook_response')) {
            const tagged = (0, messages_js_1.extractTag)(msg.stdout, xml_js_1.REMOTE_REVIEW_TAG);
            if (tagged?.trim())
                return tagged.trim();
        }
    }
    for (let i = log.length - 1; i >= 0; i--) {
        ...
        if (msg?.type !== 'assistant')
            continue;
        const fullText = (0, messages_js_1.extractTextContent)(msg.message.content, '\n');
        const tagged = (0, messages_js_1.extractTag)(fullText, xml_js_1.REMOTE_REVIEW_TAG);
        if (tagged?.trim())
            return tagged.trim();
    }
    ...
    const hookTagged = (0, messages_js_1.extractTag)(hookStdout, xml_js_1.REMOTE_REVIEW_TAG);
    if (hookTagged?.trim())
        return hookTagged.trim();
    // Fallback: concatenate all assistant text in chronological order.
    const allText = log.filter((msg) => msg.type === 'assistant').map(msg => (0, messages_js_1.extractTextContent)(msg.message.content, '\n')).join('\n').trim();
    return allText || null;
}
/**
...
<${xml_js_1.TASK_ID_TAG}>${taskId}</${xml_js_1.TASK_ID_TAG}>
<${xml_js_1.TASK_TYPE_TAG}>remote_agent</${xml_js_1.TASK_TYPE_TAG}>
<${xml_js_1.STATUS_TAG}>completed</${xml_js_1.STATUS_TAG}>
<${xml_js_1.SUMMARY_TAG}>Remote review completed</${xml_js_1.SUMMARY_TAG}>
</${xml_js_1.TASK_NOTIFICATION_TAG}>
...
<${xml_js_1.TASK_TYPE_TAG}>remote_agent</${xml_js_1.TASK_TYPE_TAG}>
<${xml_js_1.STATUS_TAG}>failed</${xml_js_1.STATUS_TAG}>
<${xml_js_1.SUMMARY_TAG}>Remote review failed: ${reason}</${xml_js_1.SUMMARY_TAG}>
</${xml_js_1.TASK_NOTIFICATION_TAG}>
...
function extractTodoListFromLog(log) {
    const todoListMessage = log.findLast((msg) => msg.type === 'assistant' && msg.message.content.some(block => block.type === 'tool_use' && block.name === TodoWriteTool_js_1.TodoWriteTool.name));
    if (!todoListMessage) {
        return [];
    }
    const input = todoListMessage.message.content.find((block) => block.type === 'tool_use' && block.name === TodoWriteTool_js_1.TodoWriteTool.name)?.input;
    if (!input) {
        return [];
    }
    const parsedInput = TodoWriteTool_js_1.TodoWriteTool.inputSchema.safeParse(input);
...
function registerRemoteAgentTask(options) {
    ...
    const taskId = (0, Task_js_1.generateTaskId)('remote_agent');
    ...
    const taskState = {
        ...(0, Task_js_1.createTaskStateBase)(taskId, 'remote_agent', session.title, toolUseId),
        type: 'remote_agent',
        remoteTaskType,
        status: 'running',
...
async function restoreRemoteAgentTasks(context) {
    try {
        ...
        (0, debug_js_1.logForDebugging)(`restoreRemoteAgentTasks failed: ${String(e)}`);
    }
}
async function restoreRemoteAgentTasksImpl(context) {
    const persisted = await (0, sessionStorage_js_1.listRemoteAgentMetadata)();
    if (persisted.length === 0)
        return;
    for (const meta of persisted) {
        let remoteStatus;
        try {
            const session = await (0, api_js_1.fetchSession)(meta.sessionId);
            ...
            if (e instanceof Error && e.message.startsWith('Session not found:')) {
                (0, debug_js_1.logForDebugging)(`restoreRemoteAgentTasks: dropping ${meta.taskId} (404: ${String(e)})`);
                void removeRemoteAgentMetadata(meta.taskId);
            }
            else {
                (0, debug_js_1.logForDebugging)(`restoreRemoteAgentTasks: skipping ${meta.taskId} (recoverable: ${String(e)})`);
            }
            continue;
        }
        ...
        const taskState = {
            ...(0, Task_js_1.createTaskStateBase)(meta.taskId, 'remote_agent', meta.title, meta.toolUseId),
            type: 'remote_agent',
...
function startRemoteSessionPolling(taskId, context) {
    ...
    const poll = async () => {
        ...
        try {
            ...
            const response = await (0, teleport_js_1.pollRemoteSessionEvents)(task.sessionId, lastEventId);
            ...
            if (response.sessionStatus === 'archived') {
                (0, framework_js_1.updateTaskState)(taskId, context.setAppState, t => t.status === 'running' ? {
                    ...t,
                    status: 'completed',
                    endTime: Date.now()
                } : t);
            ...
            if (task.isRemoteReview && logGrew) {
                const open = `<${xml_js_1.REMOTE_REVIEW_PROGRESS_TAG}>`;
                const close = `</${xml_js_1.REMOTE_REVIEW_PROGRESS_TAG}>`;
            ...
            (0, framework_js_1.updateTaskState)(taskId, context.setAppState, prevTask => {
            ...
            if (result || sessionDone || reviewTimedOut) {
                ...
                if (task.isRemoteReview) {
                    ...
                    (0, framework_js_1.updateTaskState)(taskId, context.setAppState, t => ({
                        ...t,
                        status: 'failed'
                    }));
...
exports.RemoteAgentTask = {
    ...
    async kill(taskId, setAppState) {
        ...
        if (killed) {
            ...
            if (sessionId) {
                void (0, teleport_js_1.archiveRemoteSession)(sessionId).catch(e => (0, debug_js_1.logForDebugging)(`RemoteAgentTask archive failed: ${String(e)}`));
            }
        }
        void (0, diskOutput_js_1.evictTaskOutput)(taskId);
        void removeRemoteAgentMetadata(taskId);
        (0, debug_js_1.logForDebugging)(`RemoteAgentTask ${taskId} killed, archiving session ${sessionId ?? 'unknown'}`);
    }
};
/**
