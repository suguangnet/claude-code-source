...
async function openForScan(path) {
    try {
        return await (0, promises_1.open)(path, 'r');
    }
    catch (e) {
...
async function scanForContext(handle, needle, contextLines) {
    ...
    const buf = Buffer.allocUnsafe(exports.CHUNK_SIZE + overlap);
    let pos = 0;
    let linesBeforePos = 0;
    let prevTail = 0;
    while (pos < exports.MAX_SCAN_BYTES) {
        const { bytesRead } = await handle.read(buf, prevTail, exports.CHUNK_SIZE, pos);
        if (bytesRead === 0)
            break;
        ...
        if (matchAt === -1 && nlCount > 0) {
            needleCRLF ?? (needleCRLF = Buffer.from(needle.replaceAll('\n', '\r\n'), 'utf8'));
    ...
    return { content: '', lineOffset: 1, truncated: pos >= exports.MAX_SCAN_BYTES };
}
/**
...
async function readCapped(handle) {
    let buf = Buffer.allocUnsafe(exports.CHUNK_SIZE);
    let total = 0;
    for (;;) {
        if (total === buf.length) {
            const grown = Buffer.allocUnsafe(Math.min(buf.length * 2, exports.MAX_SCAN_BYTES + exports.CHUNK_SIZE));
...
async function sliceContext(handle, scratch, matchStart, matchLen, contextLines, linesBeforeMatch) {
    ...
    const backChunk = Math.min(matchStart, exports.CHUNK_SIZE);
    ...
    const { bytesRead: fwdRead } = await handle.read(scratch, 0, exports.CHUNK_SIZE, matchEnd);
    let ctxEnd = matchEnd;
    nlSeen = 0;
