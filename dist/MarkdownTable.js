...
function wrapText(text, width, options) {
    ...
    const wrapped = (0, wrapAnsi_js_1.wrapAnsi)(trimmedText, width, {
 ...
 */
function MarkdownTable({ token, highlight, forceWidth }) {
    const [theme] = (0, ink_js_1.useTheme)();
    const { columns: actualTerminalWidth } = (0, useTerminalSize_js_1.useTerminalSize)();
    const terminalWidth = forceWidth ?? actualTerminalWidth;
    // Format cell content to ANSI string
    function formatCell(tokens) {
        return tokens?.map(_ => (0, markdown_js_1.formatToken)(_, theme, 0, null, null, highlight)).join('') ?? '';
    }
    ...
    function getMinWidth(tokens_1) {
        ...
        return Math.max(...words.map(w_0 => (0, stringWidth_js_1.stringWidth)(w_0)), MIN_COLUMN_WIDTH);
    }
    ...
    function renderVerticalFormat() {
        ...
        token.rows.forEach((row_2, rowIndex) => {
            ...
            row_2.forEach((cell_0, colIndex_4) => {
                ...
                const firstLineWidth = terminalWidth - (0, stringWidth_js_1.stringWidth)(label) - 3;
    ...
    if (useVerticalFormat) {
        return react_1.default.createElement(ink_js_1.Ansi, null, renderVerticalFormat());
    }
    ...
    const maxLineWidth = Math.max(...tableLines.map(line_2 => (0, stringWidth_js_1.stringWidth)((0, strip_ansi_1.default)(line_2))));
    ...
    return react_1.default.createElement(ink_js_1.Ansi, null, tableLines.join('\n'));
}
