...
function TaskListV2({ tasks, isStandalone = false }) {
    const teamContext = (0, AppState_js_1.useAppState)(s => s.teamContext);
    const appStateTasks = (0, AppState_js_1.useAppState)(s_0 => s_0.tasks);
    const [, forceUpdate] = React.useState(0);
    const { rows, columns } = (0, useTerminalSize_js_1.useTerminalSize)();
    ...
    if ((0, agentSwarmsEnabled_js_1.isAgentSwarmsEnabled)() && teamContext?.teammates) {
        for (const teammate of Object.values(teamContext.teammates)) {
            if (teammate.color) {
                const themeColor = agentColorManager_js_1.AGENT_COLOR_TO_THEME_COLOR[teammate.color];
                if (themeColor) {
    ...
    if ((0, agentSwarmsEnabled_js_1.isAgentSwarmsEnabled)()) {
        for (const bgTask of Object.values(appStateTasks)) {
            if ((0, types_js_1.isInProcessTeammateTask)(bgTask) && bgTask.status === 'running') {
                ...
                const desc = (activities && (0, collapseReadSearch_js_1.summarizeRecentActivities)(activities)) ?? bgTask.progress?.lastActivity?.activityDescription;
                if (desc) {
    ...
    const completedCount = (0, array_js_1.count)(tasks, t_3 => t_3.status === 'completed');
    const pendingCount = (0, array_js_1.count)(tasks, t_4 => t_4.status === 'pending');
    ...
    if (hiddenTasks.length > 0) {
        const parts = [];
        const hiddenPending = (0, array_js_1.count)(hiddenTasks, t_10 => t_10.status === 'pending');
        const hiddenInProgress = (0, array_js_1.count)(hiddenTasks, t_11 => t_11.status === 'in_progress');
        const hiddenCompleted = (0, array_js_1.count)(hiddenTasks, t_12 => t_12.status === 'completed');
    ...
    const content = React.createElement(React.Fragment, null,
        ...
        maxDisplay > 0 && hiddenSummary && React.createElement(ink_js_1.Text, { dimColor: true }, hiddenSummary));
    if (isStandalone) {
        return React.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: 1, marginLeft: 2 },
            React.createElement(ink_js_1.Box, null,
                React.createElement(ink_js_1.Text, { dimColor: true },
                    React.createElement(ink_js_1.Text, { bold: true }, tasks.length),
                    ' tasks (',
                    React.createElement(ink_js_1.Text, { bold: true }, completedCount),
                    ' done, ',
                    inProgressCount > 0 && React.createElement(React.Fragment, null,
                        React.createElement(ink_js_1.Text, { bold: true }, inProgressCount),
                        ' in progress, '),
                    React.createElement(ink_js_1.Text, { bold: true }, pendingCount),
                    ' open)')),
            content);
    }
    return React.createElement(ink_js_1.Box, { flexDirection: "column" }, content);
}
...
function TaskItem(t0) {
    const $ = (0, compiler_runtime_1.c)(37);
    ...
    if ($[2] !== showOwner || $[3] !== task.owner) {
        t2 = showOwner ? (0, stringWidth_js_1.stringWidth)(` (@${task.owner})`) : 0;
        $[2] = showOwner;
        $[3] = task.owner;
        $[4] = t2;
    }
    ...
    let t5;
    if ($[11] !== color || $[12] !== icon) {
        t5 = React.createElement(ink_js_1.Text, { color: color },
            icon,
            " ");
        $[11] = color;
        $[12] = icon;
    ...
    if ($[14] !== displaySubject || $[15] !== isCompleted || $[16] !== isInProgress || $[17] !== t6) {
        t7 = React.createElement(ink_js_1.Text, { bold: isInProgress, strikethrough: isCompleted, dimColor: t6 }, displaySubject);
        $[14] = displaySubject;
        $[15] = isCompleted;
        $[16] = isInProgress;
        $[17] = t6;
    ...
    if ($[19] !== ownerColor || $[20] !== showOwner || $[21] !== task.owner) {
        t8 = showOwner && React.createElement(ink_js_1.Text, { dimColor: true },
            " (",
            ownerColor ? React.createElement(ThemedText_js_1.default, { color: ownerColor },
                "@",
    ...
    if ($[23] !== isBlocked || $[24] !== openBlockers) {
        t9 = isBlocked && React.createElement(ink_js_1.Text, { dimColor: true },
            " ",
    ...
    if ($[26] !== t5 || $[27] !== t7 || $[28] !== t8 || $[29] !== t9) {
        t10 = React.createElement(ink_js_1.Box, null,
            t5,
            t7,
            t8,
            t9);
    ...
    if ($[31] !== displayActivity || $[32] !== showActivity) {
        t11 = showActivity && displayActivity && React.createElement(ink_js_1.Box, null,
            React.createElement(ink_js_1.Text, { dimColor: true },
                "  ",
                displayActivity,
                figures_1.default.ellipsis));
        $[31] = displayActivity;
        $[32] = showActivity;
        $[33] = t11;
    }
    ...
    let t12;
    if ($[34] !== t10 || $[35] !== t11) {
        t12 = React.createElement(ink_js_1.Box, { flexDirection: "column" },
            t10,
            t11);
        $[34] = t10;
        $[35] = t11;
