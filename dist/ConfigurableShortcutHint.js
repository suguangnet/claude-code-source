...
function ConfigurableShortcutHint(t0) {
    ...
    const shortcut = (0, useShortcutDisplay_js_1.useShortcutDisplay)(action, context, fallback);
    let t1;
    if ($[0] !== bold || $[1] !== description || $[2] !== parens || $[3] !== shortcut) {
        t1 = React.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: shortcut, action: description, parens: parens, bold: bold });
        $[0] = bold;
        $[1] = description;
        $[2] = parens;
        $[3] = shortcut;
