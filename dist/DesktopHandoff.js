...
const desktopDeepLink_js_1 = require("../utils/desktopDeepLink.js");
...
function DesktopHandoff(t0) {
    ...
    const { onDone } = t0;
    const [state, setState] = (0, react_1.useState)("checking");
    const [error, setError] = (0, react_1.useState)(null);
    const [downloadMessage, setDownloadMessage] = (0, react_1.useState)("");
    let t1;
    ...
    if ($[4] !== onDone) {
        t2 = () => {
            const performHandoff = async function performHandoff() {
                setState("checking");
                const installStatus = await (0, desktopDeepLink_js_1.getDesktopInstallStatus)();
                ...
                setState("opening");
                const result = await (0, desktopDeepLink_js_1.openCurrentSessionInDesktop)();
                if (!result.success) {
    ...
    if (state === "error") {
        let t4;
        if ($[7] !== error) {
            t4 = react_1.default.createElement(ink_js_1.Text, { color: "error" },
                "Error: ",
                error);
            $[7] = error;
            $[8] = t4;
        ...
        if ($[9] === Symbol.for("react.memo_cache_sentinel")) {
            t5 = react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "Press any key to continue\u2026");
            $[9] = t5;
        }
        else {
            t5 = $[9];
        ...
        let t6;
        if ($[10] !== t4) {
            t6 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", paddingX: 2 },
                t4,
                t5);
            $[10] = t4;
            $[11] = t6;
    ...
    if (state === "prompt-download") {
        ...
        if ($[12] !== downloadMessage) {
            t4 = react_1.default.createElement(ink_js_1.Text, null, downloadMessage);
            $[12] = downloadMessage;
            $[13] = t4;
        }
        else {
        ...
        if ($[14] === Symbol.for("react.memo_cache_sentinel")) {
            t5 = react_1.default.createElement(ink_js_1.Text, null, "Download now? (y/n)");
            $[14] = t5;
        }
        else {
            t5 = $[14];
    ...
    let t6;
    if ($[18] !== t5) {
        t6 = react_1.default.createElement(LoadingState_js_1.LoadingState, { message: t5 });
        $[18] = t5;
        $[19] = t6;
    }
    else {
