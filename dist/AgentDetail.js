"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AgentDetail = AgentDetail;
const compiler_runtime_1 = require("react/compiler-runtime");
const figures_1 = __importDefault(require("figures"));
const React = __importStar(require("react"));
const ink_js_1 = require("../../ink.js");
const useKeybinding_js_1 = require("../../keybindings/useKeybinding.js");
const agentColorManager_js_1 = require("../../tools/AgentTool/agentColorManager.js");
const agentMemory_js_1 = require("../../tools/AgentTool/agentMemory.js");
const agentToolUtils_js_1 = require("../../tools/AgentTool/agentToolUtils.js");
const loadAgentsDir_js_1 = require("../../tools/AgentTool/loadAgentsDir.js");
const agent_js_1 = require("../../utils/model/agent.js");
const Markdown_js_1 = require("../Markdown.js");
const agentFileUtils_js_1 = require("./agentFileUtils.js");
function AgentDetail(t0) {
    const $ = (0, compiler_runtime_1.c)(48);
    const { agent, tools, onBack } = t0;
    const resolvedTools = (0, agentToolUtils_js_1.resolveAgentTools)(agent, tools, false);
    let t1;
    if ($[0] !== agent) {
        t1 = (0, agentFileUtils_js_1.getActualRelativeAgentFilePath)(agent);
        $[0] = agent;
        $[1] = t1;
    }
    else {
        t1 = $[1];
    }
    const filePath = t1;
    let t2;
    if ($[2] !== agent.agentType) {
        t2 = (0, agentColorManager_js_1.getAgentColor)(agent.agentType);
        $[2] = agent.agentType;
        $[3] = t2;
    }
    else {
        t2 = $[3];
    }
    const backgroundColor = t2;
    let t3;
    if ($[4] === Symbol.for("react.memo_cache_sentinel")) {
        t3 = {
            context: "Confirmation"
        };
        $[4] = t3;
    }
    else {
        t3 = $[4];
    }
    (0, useKeybinding_js_1.useKeybinding)("confirm:no", onBack, t3);
    let t4;
    if ($[5] !== onBack) {
        t4 = e => {
            if (e.key === "return") {
                e.preventDefault();
                onBack();
            }
        };
        $[5] = onBack;
        $[6] = t4;
    }
    else {
        t4 = $[6];
    }
    const handleKeyDown = t4;
    const renderToolsList = function renderToolsList() {
        if (resolvedTools.hasWildcard) {
            return React.createElement(ink_js_1.Text, null, "All tools");
        }
        if (!agent.tools || agent.tools.length === 0) {
            return React.createElement(ink_js_1.Text, null, "None");
        }
        return React.createElement(React.Fragment, null,
            resolvedTools.validTools.length > 0 && React.createElement(ink_js_1.Text, null, resolvedTools.validTools.join(", ")),
            resolvedTools.invalidTools.length > 0 && React.createElement(ink_js_1.Text, { color: "warning" },
                figures_1.default.warning,
                " Unrecognized:",
                " ",
                resolvedTools.invalidTools.join(", ")));
    };
    const T0 = ink_js_1.Box;
    const t5 = "column";
    const t6 = 1;
    const t7 = 0;
    const t8 = true;
    let t9;
    if ($[7] !== filePath) {
        t9 = React.createElement(ink_js_1.Text, { dimColor: true }, filePath);
        $[7] = filePath;
        $[8] = t9;
    }
    else {
        t9 = $[8];
    }
    let t10;
    if ($[9] === Symbol.for("react.memo_cache_sentinel")) {
        t10 = React.createElement(ink_js_1.Text, null,
            React.createElement(ink_js_1.Text, { bold: true }, "Description"),
            " (tells Claude when to use this agent):");
        $[9] = t10;
    }
    else {
        t10 = $[9];
    }
    let t11;
    if ($[10] !== agent.whenToUse) {
        t11 = React.createElement(ink_js_1.Box, { flexDirection: "column" },
            t10,
            React.createElement(ink_js_1.Box, { marginLeft: 2 },
                React.createElement(ink_js_1.Text, null, agent.whenToUse)));
        $[10] = agent.whenToUse;
        $[11] = t11;
    }
    else {
        t11 = $[11];
    }
    const T1 = ink_js_1.Box;
    let t12;
    if ($[12] === Symbol.for("react.memo_cache_sentinel")) {
        t12 = React.createElement(ink_js_1.Text, null,
            React.createElement(ink_js_1.Text, { bold: true }, "Tools"),
            ":",
            " ");
        $[12] = t12;
    }
    else {
        t12 = $[12];
    }
    const t13 = renderToolsList();
    let t14;
    if ($[13] !== T1 || $[14] !== t12 || $[15] !== t13) {
        t14 = React.createElement(T1, null,
            t12,
            t13);
        $[13] = T1;
        $[14] = t12;
        $[15] = t13;
        $[16] = t14;
    }
    else {
        t14 = $[16];
    }
    let t15;
    if ($[17] === Symbol.for("react.memo_cache_sentinel")) {
        t15 = React.createElement(ink_js_1.Text, { bold: true }, "Model");
        $[17] = t15;
    }
    else {
        t15 = $[17];
    }
    let t16;
    if ($[18] !== agent.model) {
        t16 = (0, agent_js_1.getAgentModelDisplay)(agent.model);
        $[18] = agent.model;
        $[19] = t16;
    }
    else {
        t16 = $[19];
    }
    let t17;
    if ($[20] !== t16) {
        t17 = React.createElement(ink_js_1.Text, null,
            t15,
            ": ",
            t16);
        $[20] = t16;
        $[21] = t17;
    }
    else {
        t17 = $[21];
    }
    let t18;
    if ($[22] !== agent.permissionMode) {
        t18 = agent.permissionMode && React.createElement(ink_js_1.Text, null,
            React.createElement(ink_js_1.Text, { bold: true }, "Permission mode"),
            ": ",
            agent.permissionMode);
        $[22] = agent.permissionMode;
        $[23] = t18;
    }
    else {
        t18 = $[23];
    }
    let t19;
    if ($[24] !== agent.memory) {
        t19 = agent.memory && React.createElement(ink_js_1.Text, null,
            React.createElement(ink_js_1.Text, { bold: true }, "Memory"),
            ": ",
            (0, agentMemory_js_1.getMemoryScopeDisplay)(agent.memory));
        $[24] = agent.memory;
        $[25] = t19;
    }
    else {
        t19 = $[25];
    }
    let t20;
    if ($[26] !== agent.hooks) {
        t20 = agent.hooks && Object.keys(agent.hooks).length > 0 && React.createElement(ink_js_1.Text, null,
            React.createElement(ink_js_1.Text, { bold: true }, "Hooks"),
            ": ",
            Object.keys(agent.hooks).join(", "));
        $[26] = agent.hooks;
        $[27] = t20;
    }
    else {
        t20 = $[27];
    }
    let t21;
    if ($[28] !== agent.skills) {
        t21 = agent.skills && agent.skills.length > 0 && React.createElement(ink_js_1.Text, null,
            React.createElement(ink_js_1.Text, { bold: true }, "Skills"),
            ":",
            " ",
            agent.skills.length > 10 ? `${agent.skills.length} skills` : agent.skills.join(", "));
        $[28] = agent.skills;
        $[29] = t21;
    }
    else {
        t21 = $[29];
    }
    let t22;
    if ($[30] !== agent.agentType || $[31] !== backgroundColor) {
        t22 = backgroundColor && React.createElement(ink_js_1.Box, null,
            React.createElement(ink_js_1.Text, null,
                React.createElement(ink_js_1.Text, { bold: true }, "Color"),
                ":",
                " ",
                React.createElement(ink_js_1.Text, { backgroundColor: backgroundColor, color: "inverseText" },
                    " ",
                    agent.agentType,
                    " ")));
        $[30] = agent.agentType;
        $[31] = backgroundColor;
        $[32] = t22;
    }
    else {
        t22 = $[32];
    }
    let t23;
    if ($[33] !== agent) {
        t23 = !(0, loadAgentsDir_js_1.isBuiltInAgent)(agent) && React.createElement(React.Fragment, null,
            React.createElement(ink_js_1.Box, null,
                React.createElement(ink_js_1.Text, null,
                    React.createElement(ink_js_1.Text, { bold: true }, "System prompt"),
                    ":")),
            React.createElement(ink_js_1.Box, { marginLeft: 2, marginRight: 2 },
                React.createElement(Markdown_js_1.Markdown, null, agent.getSystemPrompt())));
        $[33] = agent;
        $[34] = t23;
    }
    else {
        t23 = $[34];
    }
    let t24;
    if ($[35] !== T0 || $[36] !== handleKeyDown || $[37] !== t11 || $[38] !== t14 || $[39] !== t17 || $[40] !== t18 || $[41] !== t19 || $[42] !== t20 || $[43] !== t21 || $[44] !== t22 || $[45] !== t23 || $[46] !== t9) {
        t24 = React.createElement(T0, { flexDirection: t5, gap: t6, tabIndex: t7, autoFocus: t8, onKeyDown: handleKeyDown },
            t9,
            t11,
            t14,
            t17,
            t18,
            t19,
            t20,
            t21,
            t22,
            t23);
        $[35] = T0;
        $[36] = handleKeyDown;
        $[37] = t11;
        $[38] = t14;
        $[39] = t17;
        $[40] = t18;
        $[41] = t19;
        $[42] = t20;
        $[43] = t21;
        $[44] = t22;
        $[45] = t23;
        $[46] = t9;
        $[47] = t24;
    }
    else {
        t24 = $[47];
    }
    return t24;
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
