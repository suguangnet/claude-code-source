"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
const compiler_runtime_1 = require("react/compiler-runtime");
const bun_bundle_1 = require("bun:bundle");
const sdk_1 = require("@anthropic-ai/sdk");
const React = __importStar(require("react"));
const ink_js_1 = require("../ink.js");
const bashPermissions_js_1 = require("../tools/BashTool/bashPermissions.js");
const toolName_js_1 = require("../tools/BashTool/toolName.js");
const autoModeDenials_js_1 = require("../utils/autoModeDenials.js");
const classifierApprovals_js_1 = require("../utils/classifierApprovals.js");
const debug_js_1 = require("../utils/debug.js");
const errors_js_1 = require("../utils/errors.js");
const log_js_1 = require("../utils/log.js");
const permissions_js_1 = require("../utils/permissions/permissions.js");
const coordinatorHandler_js_1 = require("./toolPermission/handlers/coordinatorHandler.js");
const interactiveHandler_js_1 = require("./toolPermission/handlers/interactiveHandler.js");
const swarmWorkerHandler_js_1 = require("./toolPermission/handlers/swarmWorkerHandler.js");
const PermissionContext_js_1 = require("./toolPermission/PermissionContext.js");
const permissionLogging_js_1 = require("./toolPermission/permissionLogging.js");
function useCanUseTool(setToolUseConfirmQueue, setToolPermissionContext) {
    const $ = (0, compiler_runtime_1.c)(3);
    let t0;
    if ($[0] !== setToolPermissionContext || $[1] !== setToolUseConfirmQueue) {
        t0 = async (tool, input, toolUseContext, assistantMessage, toolUseID, forceDecision) => new Promise(resolve => {
            const ctx = (0, PermissionContext_js_1.createPermissionContext)(tool, input, toolUseContext, assistantMessage, toolUseID, setToolPermissionContext, (0, PermissionContext_js_1.createPermissionQueueOps)(setToolUseConfirmQueue));
            if (ctx.resolveIfAborted(resolve)) {
                return;
            }
            const decisionPromise = forceDecision !== undefined ? Promise.resolve(forceDecision) : (0, permissions_js_1.hasPermissionsToUseTool)(tool, input, toolUseContext, assistantMessage, toolUseID);
            return decisionPromise.then(async (result) => {
                if (result.behavior === "allow") {
                    if (ctx.resolveIfAborted(resolve)) {
                        return;
                    }
                    if ((0, bun_bundle_1.feature)("TRANSCRIPT_CLASSIFIER") && result.decisionReason?.type === "classifier" && result.decisionReason.classifier === "auto-mode") {
                        (0, classifierApprovals_js_1.setYoloClassifierApproval)(toolUseID, result.decisionReason.reason);
                    }
                    ctx.logDecision({
                        decision: "accept",
                        source: "config"
                    });
                    resolve(ctx.buildAllow(result.updatedInput ?? input, {
                        decisionReason: result.decisionReason
                    }));
                    return;
                }
                const appState = toolUseContext.getAppState();
                const description = await tool.description(input, {
                    isNonInteractiveSession: toolUseContext.options.isNonInteractiveSession,
                    toolPermissionContext: appState.toolPermissionContext,
                    tools: toolUseContext.options.tools
                });
                if (ctx.resolveIfAborted(resolve)) {
                    return;
                }
                switch (result.behavior) {
                    case "deny":
                        {
                            (0, permissionLogging_js_1.logPermissionDecision)({
                                tool,
                                input,
                                toolUseContext,
                                messageId: ctx.messageId,
                                toolUseID
                            }, {
                                decision: "reject",
                                source: "config"
                            });
                            if ((0, bun_bundle_1.feature)("TRANSCRIPT_CLASSIFIER") && result.decisionReason?.type === "classifier" && result.decisionReason.classifier === "auto-mode") {
                                (0, autoModeDenials_js_1.recordAutoModeDenial)({
                                    toolName: tool.name,
                                    display: description,
                                    reason: result.decisionReason.reason ?? "",
                                    timestamp: Date.now()
                                });
                                toolUseContext.addNotification?.({
                                    key: "auto-mode-denied",
                                    priority: "immediate",
                                    jsx: React.createElement(React.Fragment, null,
                                        React.createElement(ink_js_1.Text, { color: "error" },
                                            tool.userFacingName(input).toLowerCase(),
                                            " denied by auto mode"),
                                        React.createElement(ink_js_1.Text, { dimColor: true }, " \u00B7 /permissions"))
                                });
                            }
                            resolve(result);
                            return;
                        }
                    case "ask":
                        {
                            if (appState.toolPermissionContext.awaitAutomatedChecksBeforeDialog) {
                                const coordinatorDecision = await (0, coordinatorHandler_js_1.handleCoordinatorPermission)({
                                    ctx,
                                    ...((0, bun_bundle_1.feature)("BASH_CLASSIFIER") ? {
                                        pendingClassifierCheck: result.pendingClassifierCheck
                                    } : {}),
                                    updatedInput: result.updatedInput,
                                    suggestions: result.suggestions,
                                    permissionMode: appState.toolPermissionContext.mode
                                });
                                if (coordinatorDecision) {
                                    resolve(coordinatorDecision);
                                    return;
                                }
                            }
                            if (ctx.resolveIfAborted(resolve)) {
                                return;
                            }
                            const swarmDecision = await (0, swarmWorkerHandler_js_1.handleSwarmWorkerPermission)({
                                ctx,
                                description,
                                ...((0, bun_bundle_1.feature)("BASH_CLASSIFIER") ? {
                                    pendingClassifierCheck: result.pendingClassifierCheck
                                } : {}),
                                updatedInput: result.updatedInput,
                                suggestions: result.suggestions
                            });
                            if (swarmDecision) {
                                resolve(swarmDecision);
                                return;
                            }
                            if ((0, bun_bundle_1.feature)("BASH_CLASSIFIER") && result.pendingClassifierCheck && tool.name === toolName_js_1.BASH_TOOL_NAME && !appState.toolPermissionContext.awaitAutomatedChecksBeforeDialog) {
                                const speculativePromise = (0, bashPermissions_js_1.peekSpeculativeClassifierCheck)(input.command);
                                if (speculativePromise) {
                                    const raceResult = await Promise.race([speculativePromise.then(_temp), new Promise(_temp2)]);
                                    if (ctx.resolveIfAborted(resolve)) {
                                        return;
                                    }
                                    if (raceResult.type === "result" && raceResult.result.matches && raceResult.result.confidence === "high" && (0, bun_bundle_1.feature)("BASH_CLASSIFIER")) {
                                        (0, bashPermissions_js_1.consumeSpeculativeClassifierCheck)(input.command);
                                        const matchedRule = raceResult.result.matchedDescription ?? undefined;
                                        if (matchedRule) {
                                            (0, classifierApprovals_js_1.setClassifierApproval)(toolUseID, matchedRule);
                                        }
                                        ctx.logDecision({
                                            decision: "accept",
                                            source: {
                                                type: "classifier"
                                            }
                                        });
                                        resolve(ctx.buildAllow(result.updatedInput ?? input, {
                                            decisionReason: {
                                                type: "classifier",
                                                classifier: "bash_allow",
                                                reason: `Allowed by prompt rule: "${raceResult.result.matchedDescription}"`
                                            }
                                        }));
                                        return;
                                    }
                                }
                            }
                            (0, interactiveHandler_js_1.handleInteractivePermission)({
                                ctx,
                                description,
                                result,
                                awaitAutomatedChecksBeforeDialog: appState.toolPermissionContext.awaitAutomatedChecksBeforeDialog,
                                bridgeCallbacks: (0, bun_bundle_1.feature)("BRIDGE_MODE") ? appState.replBridgePermissionCallbacks : undefined,
                                channelCallbacks: (0, bun_bundle_1.feature)("KAIROS") || (0, bun_bundle_1.feature)("KAIROS_CHANNELS") ? appState.channelPermissionCallbacks : undefined
                            }, resolve);
                            return;
                        }
                }
            }).catch(error => {
                if (error instanceof errors_js_1.AbortError || error instanceof sdk_1.APIUserAbortError) {
                    (0, debug_js_1.logForDebugging)(`Permission check threw ${error.constructor.name} for tool=${tool.name}: ${error.message}`);
                    ctx.logCancelled();
                    resolve(ctx.cancelAndAbort(undefined, true));
                }
                else {
                    (0, log_js_1.logError)(error);
                    resolve(ctx.cancelAndAbort(undefined, true));
                }
            }).finally(() => {
                (0, classifierApprovals_js_1.clearClassifierChecking)(toolUseID);
            });
        });
        $[0] = setToolPermissionContext;
        $[1] = setToolUseConfirmQueue;
        $[2] = t0;
    }
    else {
        t0 = $[2];
    }
    return t0;
}
function _temp2(res) {
    return setTimeout(res, 2000, {
        type: "timeout"
    });
}
function _temp(r) {
    return {
        type: "result",
        result: r
    };
}
exports.default = useCanUseTool;
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
