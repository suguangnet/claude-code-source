...
const errorUtils_js_1 = require("src/services/api/errorUtils.js");
...
function SystemAPIErrorMessage(t0) {
    ...
    const [countdownMs, setCountdownMs] = (0, react_1.useState)(0);
    ...
    if ($[4] !== error || $[5] !== verbose) {
        const formatted = (0, errorUtils_js_1.formatAPIError)(error);
    ...
    let t8;
    if ($[17] !== truncated) {
        t8 = truncated && React.createElement(CtrlOToExpand_js_1.CtrlOToExpand, null);
        $[17] = truncated;
        $[18] = t8;
    }
    else {
