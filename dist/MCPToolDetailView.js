"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.MCPToolDetailView = MCPToolDetailView;
const compiler_runtime_1 = require("react/compiler-runtime");
const react_1 = __importDefault(require("react"));
const ink_js_1 = require("../../ink.js");
const mcpStringUtils_js_1 = require("../../services/mcp/mcpStringUtils.js");
const ConfigurableShortcutHint_js_1 = require("../ConfigurableShortcutHint.js");
const Dialog_js_1 = require("../design-system/Dialog.js");
function MCPToolDetailView(t0) {
    const $ = (0, compiler_runtime_1.c)(44);
    const { tool, server, onBack } = t0;
    const [toolDescription, setToolDescription] = react_1.default.useState("");
    let t1;
    let toolName;
    if ($[0] !== server.name || $[1] !== tool) {
        toolName = (0, mcpStringUtils_js_1.getMcpDisplayName)(tool.name, server.name);
        const fullDisplayName = tool.userFacingName ? tool.userFacingName({}) : toolName;
        t1 = (0, mcpStringUtils_js_1.extractMcpToolDisplayName)(fullDisplayName);
        $[0] = server.name;
        $[1] = tool;
        $[2] = t1;
        $[3] = toolName;
    }
    else {
        t1 = $[2];
        toolName = $[3];
    }
    const displayName = t1;
    let t2;
    if ($[4] !== tool) {
        t2 = tool.isReadOnly?.({}) ?? false;
        $[4] = tool;
        $[5] = t2;
    }
    else {
        t2 = $[5];
    }
    const isReadOnly = t2;
    let t3;
    if ($[6] !== tool) {
        t3 = tool.isDestructive?.({}) ?? false;
        $[6] = tool;
        $[7] = t3;
    }
    else {
        t3 = $[7];
    }
    const isDestructive = t3;
    let t4;
    if ($[8] !== tool) {
        t4 = tool.isOpenWorld?.({}) ?? false;
        $[8] = tool;
        $[9] = t4;
    }
    else {
        t4 = $[9];
    }
    const isOpenWorld = t4;
    let t5;
    let t6;
    if ($[10] !== tool) {
        t5 = () => {
            const loadDescription = async function loadDescription() {
                try {
                    const desc = await tool.description({}, {
                        isNonInteractiveSession: false,
                        toolPermissionContext: {
                            mode: "default",
                            additionalWorkingDirectories: new Map(),
                            alwaysAllowRules: {},
                            alwaysDenyRules: {},
                            alwaysAskRules: {},
                            isBypassPermissionsModeAvailable: false
                        },
                        tools: []
                    });
                    setToolDescription(desc);
                }
                catch {
                    setToolDescription("Failed to load description");
                }
            };
            loadDescription();
        };
        t6 = [tool];
        $[10] = tool;
        $[11] = t5;
        $[12] = t6;
    }
    else {
        t5 = $[11];
        t6 = $[12];
    }
    react_1.default.useEffect(t5, t6);
    let t7;
    if ($[13] !== isReadOnly) {
        t7 = isReadOnly && react_1.default.createElement(ink_js_1.Text, { color: "success" }, " [read-only]");
        $[13] = isReadOnly;
        $[14] = t7;
    }
    else {
        t7 = $[14];
    }
    let t8;
    if ($[15] !== isDestructive) {
        t8 = isDestructive && react_1.default.createElement(ink_js_1.Text, { color: "error" }, " [destructive]");
        $[15] = isDestructive;
        $[16] = t8;
    }
    else {
        t8 = $[16];
    }
    let t9;
    if ($[17] !== isOpenWorld) {
        t9 = isOpenWorld && react_1.default.createElement(ink_js_1.Text, { dimColor: true }, " [open-world]");
        $[17] = isOpenWorld;
        $[18] = t9;
    }
    else {
        t9 = $[18];
    }
    let t10;
    if ($[19] !== displayName || $[20] !== t7 || $[21] !== t8 || $[22] !== t9) {
        t10 = react_1.default.createElement(react_1.default.Fragment, null,
            displayName,
            t7,
            t8,
            t9);
        $[19] = displayName;
        $[20] = t7;
        $[21] = t8;
        $[22] = t9;
        $[23] = t10;
    }
    else {
        t10 = $[23];
    }
    const titleContent = t10;
    let t11;
    if ($[24] === Symbol.for("react.memo_cache_sentinel")) {
        t11 = react_1.default.createElement(ink_js_1.Text, { bold: true }, "Tool name: ");
        $[24] = t11;
    }
    else {
        t11 = $[24];
    }
    let t12;
    if ($[25] !== toolName) {
        t12 = react_1.default.createElement(ink_js_1.Box, null,
            t11,
            react_1.default.createElement(ink_js_1.Text, { dimColor: true }, toolName));
        $[25] = toolName;
        $[26] = t12;
    }
    else {
        t12 = $[26];
    }
    let t13;
    if ($[27] === Symbol.for("react.memo_cache_sentinel")) {
        t13 = react_1.default.createElement(ink_js_1.Text, { bold: true }, "Full name: ");
        $[27] = t13;
    }
    else {
        t13 = $[27];
    }
    let t14;
    if ($[28] !== tool.name) {
        t14 = react_1.default.createElement(ink_js_1.Box, null,
            t13,
            react_1.default.createElement(ink_js_1.Text, { dimColor: true }, tool.name));
        $[28] = tool.name;
        $[29] = t14;
    }
    else {
        t14 = $[29];
    }
    let t15;
    if ($[30] !== toolDescription) {
        t15 = toolDescription && react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: 1 },
            react_1.default.createElement(ink_js_1.Text, { bold: true }, "Description:"),
            react_1.default.createElement(ink_js_1.Text, { wrap: "wrap" }, toolDescription));
        $[30] = toolDescription;
        $[31] = t15;
    }
    else {
        t15 = $[31];
    }
    let t16;
    if ($[32] !== tool.inputJSONSchema) {
        t16 = tool.inputJSONSchema && tool.inputJSONSchema.properties && Object.keys(tool.inputJSONSchema.properties).length > 0 && react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: 1 },
            react_1.default.createElement(ink_js_1.Text, { bold: true }, "Parameters:"),
            react_1.default.createElement(ink_js_1.Box, { marginLeft: 2, flexDirection: "column" }, Object.entries(tool.inputJSONSchema.properties).map(t17 => {
                const [key, value] = t17;
                const required = tool.inputJSONSchema?.required;
                const isRequired = required?.includes(key);
                return react_1.default.createElement(ink_js_1.Text, { key: key },
                    "\u2022 ",
                    key,
                    isRequired && react_1.default.createElement(ink_js_1.Text, { dimColor: true }, " (required)"),
                    ":",
                    " ",
                    react_1.default.createElement(ink_js_1.Text, { dimColor: true }, typeof value === "object" && value && "type" in value ? String(value.type) : "unknown"),
                    typeof value === "object" && value && "description" in value && react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                        " - ",
                        String(value.description)));
            })));
        $[32] = tool.inputJSONSchema;
        $[33] = t16;
    }
    else {
        t16 = $[33];
    }
    let t17;
    if ($[34] !== t12 || $[35] !== t14 || $[36] !== t15 || $[37] !== t16) {
        t17 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
            t12,
            t14,
            t15,
            t16);
        $[34] = t12;
        $[35] = t14;
        $[36] = t15;
        $[37] = t16;
        $[38] = t17;
    }
    else {
        t17 = $[38];
    }
    let t18;
    if ($[39] !== onBack || $[40] !== server.name || $[41] !== t17 || $[42] !== titleContent) {
        t18 = react_1.default.createElement(Dialog_js_1.Dialog, { title: titleContent, subtitle: server.name, onCancel: onBack, inputGuide: _temp }, t17);
        $[39] = onBack;
        $[40] = server.name;
        $[41] = t17;
        $[42] = titleContent;
        $[43] = t18;
    }
    else {
        t18 = $[43];
    }
    return t18;
}
function _temp(exitState) {
    return exitState.pending ? react_1.default.createElement(ink_js_1.Text, null,
        "Press ",
        exitState.keyName,
        " again to exit") : react_1.default.createElement(ConfigurableShortcutHint_js_1.ConfigurableShortcutHint, { action: "confirm:no", context: "Confirmation", fallback: "Esc", description: "go back" });
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
