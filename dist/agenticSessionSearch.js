...
function logContainsQuery(log, queryLower) {
    // Check title
    const title = (0, log_js_1.getLogDisplayTitle)(log).toLowerCase();
...
async function agenticSessionSearch(query, logs, signal) {
    ...
    (0, debug_js_1.logForDebugging)(`Agentic search: ${logsToSearch.length}/${logs.length} logs, query="${query}", ` +
        `matching: ${matchingLogs.length}, with messages: ${(0, array_js_1.count)(logsToSearch, l => l.messages?.length > 0)}`);
    ...
    const logsWithTranscriptsPromises = logsToSearch.map(async (log) => {
        if ((0, sessionStorage_js_1.isLiteLog)(log)) {
            try {
                return await (0, sessionStorage_js_1.loadFullLog)(log);
            }
            catch (error) {
    ...
    (0, debug_js_1.logForDebugging)(`Agentic search: loaded ${(0, array_js_1.count)(logsWithTranscripts, l => l.messages?.length > 0)}/${logsToSearch.length} logs with transcripts`);
    ...
    const sessionList = logsWithTranscripts
        ...
        const displayTitle = (0, log_js_1.getLogDisplayTitle)(log);
...
Find the sessions that are most relevant to this query.`;
    ...
    (0, debug_js_1.logForDebugging)(`Agentic search prompt (first 500 chars): ${userMessage.slice(0, 500)}...`);
    try {
        const model = (0, model_js_1.getSmallFastModel)();
        (0, debug_js_1.logForDebugging)(`Agentic search using model: ${model}`);
        ...
        (0, debug_js_1.logForDebugging)(`Agentic search response: ${textContent.text}`);
        ...
        const result = (0, slowOperations_js_1.jsonParse)(jsonMatch[0]);
        ...
        (0, debug_js_1.logForDebugging)(`Agentic search found ${relevantLogs.length} relevant sessions`);
        return relevantLogs;
    }
    catch (error) {
        (0, log_js_1.logError)(error);
        (0, debug_js_1.logForDebugging)(`Agentic search error: ${error}`);
        return [];
    }
}
