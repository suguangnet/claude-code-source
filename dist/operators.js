...
const textObjects_js_1 = require("./textObjects.js");
/**
 * Execute an operator with a simple motion.
 */
function executeOperatorMotion(op, motion, count, ctx) {
    const target = (0, motions_js_1.resolveMotion)(motion, ctx.cursor, count);
...
function executeOperatorFind(op, findType, char, count, ctx) {
    ...
    if (targetOffset === null)
        return;
    const target = new Cursor_js_1.Cursor(ctx.cursor.measuredText, targetOffset);
...
function executeOperatorTextObj(op, scope, objType, count, ctx) {
    const range = (0, textObjects_js_1.findTextObject)(ctx.text, ctx.cursor.offset, objType, scope === 'inner');
    if (!range)
        return;
...
function executeLineOp(op, count, ctx) {
    ...
    const currentLine = (0, stringUtils_js_1.countCharInString)(text.slice(0, ctx.cursor.offset), '\n');
    ...
    if (op === 'yank') {
        ...
        ctx.setText(newText || '');
        const maxOff = Math.max(0, newText.length - ((0, intl_js_1.lastGrapheme)(newText).length || 1));
...
function executeReplace(char, count, ctx) {
    ...
    for (let i = 0; i < count && offset < newText.length; i++) {
        const graphemeLen = (0, intl_js_1.firstGrapheme)(newText.slice(offset)).length || 1;
        newText =
...
function executeToggleCase(count, ctx) {
    ...
    while (offset < newText.length && toggled < count) {
        const grapheme = (0, intl_js_1.firstGrapheme)(newText.slice(offset));
...
function executePaste(after, count, ctx) {
    ...
    if (isLinewise) {
        ...
        const lastGr = (0, intl_js_1.lastGrapheme)(textToInsert);
