...
async function detectMacosTerminal() {
    ...
    const stored = (0, config_js_1.getGlobalConfig)().deepLinkTerminal;
    if (stored) {
    ...
    for (const terminal of MACOS_TERMINALS) {
        const { code, stdout } = await (0, execFileNoThrow_js_1.execFileNoThrow)('mdfind', [`kMDItemCFBundleIdentifier == "${terminal.bundleId}"`], { timeout: 5000, useCwd: false });
    ...
    for (const terminal of MACOS_TERMINALS) {
        const { code: lsCode } = await (0, execFileNoThrow_js_1.execFileNoThrow)('ls', [`/Applications/${terminal.app}.app`], { timeout: 1000, useCwd: false });
        if (lsCode === 0) {
            return { name: terminal.name, command: terminal.app };
        }
    }
...
async function detectLinuxTerminal() {
    ...
    if (termEnv) {
        const resolved = await (0, which_js_1.which)(termEnv);
        if (resolved) {
            return { name: (0, path_1.basename)(termEnv), command: resolved };
        }
    }
    // Check x-terminal-emulator (Debian/Ubuntu alternative)
    const xte = await (0, which_js_1.which)('x-terminal-emulator');
    if (xte) {
        return { name: 'x-terminal-emulator', command: xte };
    }
    // Walk the priority list
    for (const terminal of LINUX_TERMINALS) {
        const resolved = await (0, which_js_1.which)(terminal);
        if (resolved) {
...
async function detectWindowsTerminal() {
    ...
    const wt = await (0, which_js_1.which)('wt.exe');
    if (wt) {
        return { name: 'Windows Terminal', command: wt };
    }
    // PowerShell 7+ (separate install)
    const pwsh = await (0, which_js_1.which)('pwsh.exe');
    if (pwsh) {
        return { name: 'PowerShell', command: pwsh };
    }
    // Windows PowerShell 5.1 (built into Windows)
    const powershell = await (0, which_js_1.which)('powershell.exe');
    if (powershell) {
...
async function launchInTerminal(claudePath, action) {
    ...
    (0, debug_js_1.logForDebugging)(`Launching in terminal: ${terminal.name} (${terminal.command})`);
...
async function launchMacosTerminal(terminal, claudePath, claudeArgs, cwd) {
  ...
  tell current session of current window
    write text ${appleScriptQuote(shCmd)}
            ...
            const { code } = await (0, execFileNoThrow_js_1.execFileNoThrow)('osascript', ['-e', script], {
                useCwd: false,
            });
            if (code === 0)
                return true;
  ...
  activate
        ...
        case 'Ghostty': {
            ...
            const { code } = await (0, execFileNoThrow_js_1.execFileNoThrow)('open', args, { useCwd: false });
            if (code === 0)
                return true;
            break;
        }
    ...
    (0, debug_js_1.logForDebugging)(`Failed to launch ${terminal.name}, falling back to Terminal.app`);
...
function spawnDetached(command, args, opts = {}) {
    return new Promise(resolve => {
        ...
        child.once('error', err => {
            (0, debug_js_1.logForDebugging)(`Failed to spawn ${command}: ${err.message}`, {
                level: 'error',
            });
            void resolve(false);
        });
