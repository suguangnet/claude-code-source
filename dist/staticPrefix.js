...
const registry_js_1 = require("../bash/registry.js");
const specPrefix_js_1 = require("../shell/specPrefix.js");
const stringUtils_js_1 = require("../stringUtils.js");
const dangerousCmdlets_js_1 = require("./dangerousCmdlets.js");
...
async function extractPrefixFromElement(cmd) {
    ...
    const spec = await (0, registry_js_1.getCommandSpec)(nameLower);
    const prefix = await (0, specPrefix_js_1.buildPrefix)(name, cmd.args, spec);
    ...
    if (!prefix.includes(' ') &&
        (spec?.subcommands?.length || specPrefix_js_1.DEPTH_RULES[nameLower])) {
        return null;
    }
    return prefix;
}
...
async function getCommandPrefixStatic(command) {
    ...
    const firstCommand = (0, parser_js_1.getAllCommands)(parsed).find(cmd => cmd.elementType === 'CommandAst');
    if (!firstCommand) {
...
async function getCompoundCommandPrefixesStatic(command, excludeSubcommand) {
    ...
    const commands = (0, parser_js_1.getAllCommands)(parsed).filter(cmd => cmd.elementType === 'CommandAst');
    ...
    for (const [rootLower, group] of groups) {
        ...
        const lcpWordCount = lcp === '' ? 0 : (0, stringUtils_js_1.countCharInString)(lcp, ' ') + 1;
        if (lcpWordCount <= 1) {
            ...
            const rootSpec = await (0, registry_js_1.getCommandSpec)(rootLower);
            if (rootSpec?.subcommands?.length || specPrefix_js_1.DEPTH_RULES[rootLower]) {
                continue;
            }
        }
        collapsed.push(lcp);
