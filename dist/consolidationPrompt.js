...
${memdir_js_1.DIR_EXISTS_GUIDANCE}
...
- Read \`${memdir_js_1.ENTRYPOINT_NAME}\` to understand the current index
...
Update \`${memdir_js_1.ENTRYPOINT_NAME}\` so it stays under ${memdir_js_1.MAX_ENTRYPOINT_LINES} lines AND under ~25KB. It's an **index**, not a dump — each entry should be one line under ~150 characters: \`- [Title](file.md) — one-line hook\`. Never write memory content directly into it.
