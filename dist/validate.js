...
function validateKeystroke(keystroke) {
    ...
    const parsed = (0, parser_js_1.parseKeystroke)(keystroke);
    if (!parsed.key &&
        !parsed.ctrl &&
        !parsed.alt &&
        !parsed.shift &&
...
function validateBlock(block, blockIndex) {
    ...
    for (const [key, action] of Object.entries(bindings)) {
        ...
        if (action !== null && typeof action !== 'string') {
            ...
            const ks = (0, parser_js_1.parseChord)(key)[0];
            if (ks &&
                !ks.ctrl &&
                !ks.alt &&
                !ks.shift &&
...
function checkDuplicates(blocks) {
    ...
    for (const block of blocks) {
        ...
        for (const [key, action] of Object.entries(block.bindings)) {
            const normalizedKey = (0, reservedShortcuts_js_1.normalizeKeyForComparison)(key);
...
function checkReservedShortcuts(bindings) {
    const warnings = [];
    const reserved = (0, reservedShortcuts_js_1.getReservedShortcuts)();
    for (const binding of bindings) {
        const keyDisplay = (0, parser_js_1.chordToString)(binding.chord);
        const normalizedKey = (0, reservedShortcuts_js_1.normalizeKeyForComparison)(keyDisplay);
...
function getUserBindingsForValidation(userBlocks) {
    ...
    for (const block of userBlocks) {
        for (const [key, action] of Object.entries(block.bindings)) {
            const chord = key.split(' ').map(k => (0, parser_js_1.parseKeystroke)(k));
            bindings.push({
                chord,
                action,
                context: block.context,
...
function formatWarnings(warnings) {
    ...
    const lines = [];
    if (errors.length > 0) {
        lines.push(`Found ${errors.length} keybinding ${(0, stringUtils_js_1.plural)(errors.length, 'error')}:`);
    ...
    if (warns.length > 0) {
        if (lines.length > 0)
            lines.push('');
        lines.push(`Found ${warns.length} keybinding ${(0, stringUtils_js_1.plural)(warns.length, 'warning')}:`);
