"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.BashModeProgress = BashModeProgress;
const compiler_runtime_1 = require("react/compiler-runtime");
const react_1 = __importDefault(require("react"));
const ink_js_1 = require("../ink.js");
const BashTool_js_1 = require("../tools/BashTool/BashTool.js");
const UserBashInputMessage_js_1 = require("./messages/UserBashInputMessage.js");
const ShellProgressMessage_js_1 = require("./shell/ShellProgressMessage.js");
function BashModeProgress(t0) {
    const $ = (0, compiler_runtime_1.c)(8);
    const { input, progress, verbose } = t0;
    const t1 = `<bash-input>${input}</bash-input>`;
    let t2;
    if ($[0] !== t1) {
        t2 = react_1.default.createElement(UserBashInputMessage_js_1.UserBashInputMessage, { addMargin: false, param: {
                text: t1,
                type: "text"
            } });
        $[0] = t1;
        $[1] = t2;
    }
    else {
        t2 = $[1];
    }
    let t3;
    if ($[2] !== progress || $[3] !== verbose) {
        t3 = progress ? react_1.default.createElement(ShellProgressMessage_js_1.ShellProgressMessage, { fullOutput: progress.fullOutput, output: progress.output, elapsedTimeSeconds: progress.elapsedTimeSeconds, totalLines: progress.totalLines, verbose: verbose }) : BashTool_js_1.BashTool.renderToolUseProgressMessage?.([], {
            verbose,
            tools: [],
            terminalSize: undefined
        });
        $[2] = progress;
        $[3] = verbose;
        $[4] = t3;
    }
    else {
        t3 = $[4];
    }
    let t4;
    if ($[5] !== t2 || $[6] !== t3) {
        t4 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: 1 },
            t2,
            t3);
        $[5] = t2;
        $[6] = t3;
        $[7] = t4;
    }
    else {
        t4 = $[7];
    }
    return t4;
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
