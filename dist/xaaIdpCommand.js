...
function registerMcpXaaIdpCommand(mcp) {
    ...
    xaaIdp
        ...
        try {
            ...
            return (0, exit_js_1.cliError)(`Error: --issuer must be a valid URL (got "${options.issuer}")`);
        }
        ...
        if (issuerUrl.protocol !== 'https:' &&
            ...
            return (0, exit_js_1.cliError)(`Error: --issuer must use https:// (got "${issuerUrl.protocol}//${issuerUrl.host}")`);
        }
        ...
        // the new one.
        const old = (0, xaaIdpLogin_js_1.getXaaIdpSettings)();
        ...
        if (error) {
            return (0, exit_js_1.cliError)(`Error writing settings: ${error.message}`);
        }
        ...
        if (secret) {
            const { success, warning } = (0, xaaIdpLogin_js_1.saveIdpClientSecret)(options.issuer, secret);
            if (!success) {
                return (0, exit_js_1.cliError)(`Error: settings written but keychain save failed${warning ? ` — ${warning}` : ''}. ` +
                    `Re-run with --client-secret once keychain is available.`);
            }
        }
        (0, exit_js_1.cliOk)(`XAA IdP connection configured for ${options.issuer}`);
    });
    xaaIdp
        .command('login')
        ...
        const idp = (0, xaaIdpLogin_js_1.getXaaIdpSettings)();
        if (!idp) {
            return (0, exit_js_1.cliError)("Error: no XAA IdP connection. Run 'claude mcp xaa setup' first.");
        }
        ...
        if (options.idToken) {
            const expiresAt = (0, xaaIdpLogin_js_1.saveIdpIdTokenFromJwt)(idp.issuer, options.idToken);
            return (0, exit_js_1.cliOk)(`id_token cached for ${idp.issuer} (expires ${new Date(expiresAt).toISOString()})`);
        }
        if (options.force) {
            (0, xaaIdpLogin_js_1.clearIdpIdToken)(idp.issuer);
        }
        const wasCached = (0, xaaIdpLogin_js_1.getCachedIdpIdToken)(idp.issuer) !== undefined;
        if (wasCached) {
            return (0, exit_js_1.cliOk)(`Already logged in to ${idp.issuer} (cached id_token still valid). Use --force to re-login.`);
        }
        process.stdout.write(`Opening browser for IdP login at ${idp.issuer}…\n`);
        try {
            await (0, xaaIdpLogin_js_1.acquireIdpIdToken)({
            ...
            (0, exit_js_1.cliError)(`IdP login failed: ${(0, errors_js_1.errorMessage)(e)}`);
        }
    });
    xaaIdp
        .command('show')
        ...
        const hasSecret = (0, xaaIdpLogin_js_1.getIdpClientSecret)(idp.issuer) !== undefined;
        const hasIdToken = (0, xaaIdpLogin_js_1.getCachedIdpIdToken)(idp.issuer) !== undefined;
