...
function usePluginInstallationStatus() {
    ...
    const installationStatus = (0, AppState_js_1.useAppState)(_temp);
    let t0;
    bb0: {
    ...
    if ($[9] !== addNotification || $[10] !== failedMarketplacesCount || $[11] !== failedPluginsCount || $[12] !== installationStatus || $[13] !== totalFailed) {
        t1 = () => {
            ...
            (0, debug_js_1.logForDebugging)(`Plugin installation status: ${failedMarketplacesCount} failed marketplaces, ${failedPluginsCount} failed plugins`);
            if (totalFailed === 0) {
                return;
            }
            (0, debug_js_1.logForDebugging)(`Adding notification for ${totalFailed} failed installations`);
            addNotification({
                ...
                jsx: React.createElement(React.Fragment, null,
                    React.createElement(ink_js_1.Text, { color: "error" },
                        totalFailed,
                        " ",
                        (0, stringUtils_js_1.plural)(totalFailed, "plugin"),
                        " failed to install"),
                    React.createElement(ink_js_1.Text, { dimColor: true }, " \u00B7 /plugin for details")),
                priority: "medium"
            });
        };
        $[9] = addNotification;
