"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.UserTextMessage = UserTextMessage;
const compiler_runtime_1 = require("react/compiler-runtime");
const bun_bundle_1 = require("bun:bundle");
const React = __importStar(require("react"));
const messages_js_1 = require("../../constants/messages.js");
const xml_js_1 = require("../../constants/xml.js");
const agentSwarmsEnabled_js_1 = require("../../utils/agentSwarmsEnabled.js");
const messages_js_2 = require("../../utils/messages.js");
const InterruptedByUser_js_1 = require("../InterruptedByUser.js");
const MessageResponse_js_1 = require("../MessageResponse.js");
const UserAgentNotificationMessage_js_1 = require("./UserAgentNotificationMessage.js");
const UserBashInputMessage_js_1 = require("./UserBashInputMessage.js");
const UserBashOutputMessage_js_1 = require("./UserBashOutputMessage.js");
const UserCommandMessage_js_1 = require("./UserCommandMessage.js");
const UserLocalCommandOutputMessage_js_1 = require("./UserLocalCommandOutputMessage.js");
const UserMemoryInputMessage_js_1 = require("./UserMemoryInputMessage.js");
const UserPlanMessage_js_1 = require("./UserPlanMessage.js");
const UserPromptMessage_js_1 = require("./UserPromptMessage.js");
const UserResourceUpdateMessage_js_1 = require("./UserResourceUpdateMessage.js");
const UserTeammateMessage_js_1 = require("./UserTeammateMessage.js");
function UserTextMessage(t0) {
    const $ = (0, compiler_runtime_1.c)(49);
    const { addMargin, param, verbose, planContent, isTranscriptMode, timestamp } = t0;
    if (param.text.trim() === messages_js_1.NO_CONTENT_MESSAGE) {
        return null;
    }
    if (planContent) {
        let t1;
        if ($[0] !== addMargin || $[1] !== planContent) {
            t1 = React.createElement(UserPlanMessage_js_1.UserPlanMessage, { addMargin: addMargin, planContent: planContent });
            $[0] = addMargin;
            $[1] = planContent;
            $[2] = t1;
        }
        else {
            t1 = $[2];
        }
        return t1;
    }
    if ((0, messages_js_2.extractTag)(param.text, xml_js_1.TICK_TAG)) {
        return null;
    }
    if (param.text.includes(`<${xml_js_1.LOCAL_COMMAND_CAVEAT_TAG}>`)) {
        return null;
    }
    if (param.text.startsWith("<bash-stdout") || param.text.startsWith("<bash-stderr")) {
        let t1;
        if ($[3] !== param.text || $[4] !== verbose) {
            t1 = React.createElement(UserBashOutputMessage_js_1.UserBashOutputMessage, { content: param.text, verbose: verbose });
            $[3] = param.text;
            $[4] = verbose;
            $[5] = t1;
        }
        else {
            t1 = $[5];
        }
        return t1;
    }
    if (param.text.startsWith("<local-command-stdout") || param.text.startsWith("<local-command-stderr")) {
        let t1;
        if ($[6] !== param.text) {
            t1 = React.createElement(UserLocalCommandOutputMessage_js_1.UserLocalCommandOutputMessage, { content: param.text });
            $[6] = param.text;
            $[7] = t1;
        }
        else {
            t1 = $[7];
        }
        return t1;
    }
    if (param.text === messages_js_2.INTERRUPT_MESSAGE || param.text === messages_js_2.INTERRUPT_MESSAGE_FOR_TOOL_USE) {
        let t1;
        if ($[8] === Symbol.for("react.memo_cache_sentinel")) {
            t1 = React.createElement(MessageResponse_js_1.MessageResponse, { height: 1 },
                React.createElement(InterruptedByUser_js_1.InterruptedByUser, null));
            $[8] = t1;
        }
        else {
            t1 = $[8];
        }
        return t1;
    }
    if ((0, bun_bundle_1.feature)("KAIROS_GITHUB_WEBHOOKS")) {
        if (param.text.startsWith("<github-webhook-activity>")) {
            let t1;
            if ($[9] === Symbol.for("react.memo_cache_sentinel")) {
                t1 = require("./UserGitHubWebhookMessage.js");
                $[9] = t1;
            }
            else {
                t1 = $[9];
            }
            const { UserGitHubWebhookMessage } = t1;
            let t2;
            if ($[10] !== addMargin || $[11] !== param) {
                t2 = React.createElement(UserGitHubWebhookMessage, { addMargin: addMargin, param: param });
                $[10] = addMargin;
                $[11] = param;
                $[12] = t2;
            }
            else {
                t2 = $[12];
            }
            return t2;
        }
    }
    if (param.text.includes("<bash-input>")) {
        let t1;
        if ($[13] !== addMargin || $[14] !== param) {
            t1 = React.createElement(UserBashInputMessage_js_1.UserBashInputMessage, { addMargin: addMargin, param: param });
            $[13] = addMargin;
            $[14] = param;
            $[15] = t1;
        }
        else {
            t1 = $[15];
        }
        return t1;
    }
    if (param.text.includes(`<${xml_js_1.COMMAND_MESSAGE_TAG}>`)) {
        let t1;
        if ($[16] !== addMargin || $[17] !== param) {
            t1 = React.createElement(UserCommandMessage_js_1.UserCommandMessage, { addMargin: addMargin, param: param });
            $[16] = addMargin;
            $[17] = param;
            $[18] = t1;
        }
        else {
            t1 = $[18];
        }
        return t1;
    }
    if (param.text.includes("<user-memory-input>")) {
        let t1;
        if ($[19] !== addMargin || $[20] !== param.text) {
            t1 = React.createElement(UserMemoryInputMessage_js_1.UserMemoryInputMessage, { addMargin: addMargin, text: param.text });
            $[19] = addMargin;
            $[20] = param.text;
            $[21] = t1;
        }
        else {
            t1 = $[21];
        }
        return t1;
    }
    if ((0, agentSwarmsEnabled_js_1.isAgentSwarmsEnabled)() && param.text.includes(`<${xml_js_1.TEAMMATE_MESSAGE_TAG}`)) {
        let t1;
        if ($[22] !== addMargin || $[23] !== isTranscriptMode || $[24] !== param) {
            t1 = React.createElement(UserTeammateMessage_js_1.UserTeammateMessage, { addMargin: addMargin, param: param, isTranscriptMode: isTranscriptMode });
            $[22] = addMargin;
            $[23] = isTranscriptMode;
            $[24] = param;
            $[25] = t1;
        }
        else {
            t1 = $[25];
        }
        return t1;
    }
    if (param.text.includes(`<${xml_js_1.TASK_NOTIFICATION_TAG}`)) {
        let t1;
        if ($[26] !== addMargin || $[27] !== param) {
            t1 = React.createElement(UserAgentNotificationMessage_js_1.UserAgentNotificationMessage, { addMargin: addMargin, param: param });
            $[26] = addMargin;
            $[27] = param;
            $[28] = t1;
        }
        else {
            t1 = $[28];
        }
        return t1;
    }
    if (param.text.includes("<mcp-resource-update") || param.text.includes("<mcp-polling-update")) {
        let t1;
        if ($[29] !== addMargin || $[30] !== param) {
            t1 = React.createElement(UserResourceUpdateMessage_js_1.UserResourceUpdateMessage, { addMargin: addMargin, param: param });
            $[29] = addMargin;
            $[30] = param;
            $[31] = t1;
        }
        else {
            t1 = $[31];
        }
        return t1;
    }
    if ((0, bun_bundle_1.feature)("FORK_SUBAGENT")) {
        if (param.text.includes("<fork-boilerplate>")) {
            let t1;
            if ($[32] === Symbol.for("react.memo_cache_sentinel")) {
                t1 = require("./UserForkBoilerplateMessage.js");
                $[32] = t1;
            }
            else {
                t1 = $[32];
            }
            const { UserForkBoilerplateMessage } = t1;
            let t2;
            if ($[33] !== addMargin || $[34] !== param) {
                t2 = React.createElement(UserForkBoilerplateMessage, { addMargin: addMargin, param: param });
                $[33] = addMargin;
                $[34] = param;
                $[35] = t2;
            }
            else {
                t2 = $[35];
            }
            return t2;
        }
    }
    if ((0, bun_bundle_1.feature)("UDS_INBOX")) {
        if (param.text.includes("<cross-session-message")) {
            let t1;
            if ($[36] === Symbol.for("react.memo_cache_sentinel")) {
                t1 = require("./UserCrossSessionMessage.js");
                $[36] = t1;
            }
            else {
                t1 = $[36];
            }
            const { UserCrossSessionMessage } = t1;
            let t2;
            if ($[37] !== addMargin || $[38] !== param) {
                t2 = React.createElement(UserCrossSessionMessage, { addMargin: addMargin, param: param });
                $[37] = addMargin;
                $[38] = param;
                $[39] = t2;
            }
            else {
                t2 = $[39];
            }
            return t2;
        }
    }
    if ((0, bun_bundle_1.feature)("KAIROS") || (0, bun_bundle_1.feature)("KAIROS_CHANNELS")) {
        if (param.text.includes("<channel source=\"")) {
            let t1;
            if ($[40] === Symbol.for("react.memo_cache_sentinel")) {
                t1 = require("./UserChannelMessage.js");
                $[40] = t1;
            }
            else {
                t1 = $[40];
            }
            const { UserChannelMessage } = t1;
            let t2;
            if ($[41] !== addMargin || $[42] !== param) {
                t2 = React.createElement(UserChannelMessage, { addMargin: addMargin, param: param });
                $[41] = addMargin;
                $[42] = param;
                $[43] = t2;
            }
            else {
                t2 = $[43];
            }
            return t2;
        }
    }
    let t1;
    if ($[44] !== addMargin || $[45] !== isTranscriptMode || $[46] !== param || $[47] !== timestamp) {
        t1 = React.createElement(UserPromptMessage_js_1.UserPromptMessage, { addMargin: addMargin, param: param, isTranscriptMode: isTranscriptMode, timestamp: timestamp });
        $[44] = addMargin;
        $[45] = isTranscriptMode;
        $[46] = param;
        $[47] = timestamp;
        $[48] = t1;
    }
    else {
        t1 = $[48];
    }
    return t1;
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
