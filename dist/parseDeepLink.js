...
function parseDeepLink(uri) {
    ...
    const normalized = uri.startsWith(`${exports.DEEP_LINK_PROTOCOL}://`)
        ? uri
        : uri.startsWith(`${exports.DEEP_LINK_PROTOCOL}:`)
            ? uri.replace(`${exports.DEEP_LINK_PROTOCOL}:`, `${exports.DEEP_LINK_PROTOCOL}://`)
            : null;
    if (!normalized) {
        throw new Error(`Invalid deep link: expected ${exports.DEEP_LINK_PROTOCOL}:// scheme, got "${uri}"`);
    }
    let url;
    try {
...
function buildDeepLink(action) {
    const url = new URL(`${exports.DEEP_LINK_PROTOCOL}://open`);
    if (action.query) {
