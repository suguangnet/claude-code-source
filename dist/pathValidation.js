...
const pathValidation_js_1 = require("../../utils/permissions/pathValidation.js");
const platform_js_1 = require("../../utils/platform.js");
const parser_js_1 = require("../../utils/powershell/parser.js");
const commonParameters_js_1 = require("./commonParameters.js");
...
function isPathAllowed(resolvedPath, context, operationType, precomputedPathsToCheck) {
    ...
    const denyRule = (0, filesystem_js_1.matchingRuleForInput)(resolvedPath, context, permissionType, 'deny');
    if (denyRule !== null) {
        return {
            allowed: false,
    ...
    if (operationType !== 'read') {
        const internalEditResult = (0, filesystem_js_1.checkEditableInternalPath)(resolvedPath, {});
    ...
    if (operationType !== 'read') {
        const safetyCheck = (0, filesystem_js_1.checkPathSafetyForAutoEdit)(resolvedPath, precomputedPathsToCheck);
        if (!safetyCheck.safe) {
            return {
                allowed: false,
                decisionReason: {
    ...
    const isInWorkingDir = (0, filesystem_js_1.pathInAllowedWorkingPath)(resolvedPath, context, precomputedPathsToCheck);
    if (isInWorkingDir) {
    ...
    if (operationType === 'read') {
        const internalReadResult = (0, filesystem_js_1.checkReadableInternalPath)(resolvedPath, {});
    ...
    // 4. Check allow rules
    const allowRule = (0, filesystem_js_1.matchingRuleForInput)(resolvedPath, context, permissionType, 'allow');
    if (allowRule !== null) {
        return {
            allowed: true,
...
function checkDenyRuleForGuessedPath(strippedPath, cwd, toolPermissionContext, operationType) {
    ...
    const abs = (0, path_1.isAbsolute)(tildeExpanded)
        ? tildeExpanded
        : (0, path_1.resolve)(cwd, tildeExpanded);
    const { resolvedPath } = (0, fsOperations_js_1.safeResolvePath)((0, fsOperations_js_1.getFsImplementation)(), abs);
    const permissionType = operationType === 'read' ? 'read' : 'edit';
    const denyRule = (0, filesystem_js_1.matchingRuleForInput)(resolvedPath, toolPermissionContext, permissionType, 'deny');
...
function validatePath(filePath, cwd, toolPermissionContext, operationType) {
    ...
    const providerPathRegex = (0, platform_js_1.getPlatform)() === 'windows' ? /^[a-z0-9]{2,}:/i : /^[a-z0-9]+:/i;
    ...
    if (GLOB_PATTERN_REGEX.test(normalizedPath)) {
        ...
        if ((0, path_js_1.containsPathTraversal)(normalizedPath)) {
            const absolutePath = (0, path_1.isAbsolute)(normalizedPath)
                ? normalizedPath
                : (0, path_1.resolve)(cwd, normalizedPath);
            const { resolvedPath, isCanonical } = (0, fsOperations_js_1.safeResolvePath)((0, fsOperations_js_1.getFsImplementation)(), absolutePath);
        ...
        const absoluteBasePath = (0, path_1.isAbsolute)(basePath)
            ? basePath
            : (0, path_1.resolve)(cwd, basePath);
        const { resolvedPath } = (0, fsOperations_js_1.safeResolvePath)((0, fsOperations_js_1.getFsImplementation)(), absoluteBasePath);
        const permissionType = operationType === 'read' ? 'read' : 'edit';
        const denyRule = (0, filesystem_js_1.matchingRuleForInput)(resolvedPath, toolPermissionContext, permissionType, 'deny');
        if (denyRule !== null) {
            return {
...
function extractPathsFromCommand(cmd) {
    const canonical = (0, readOnlyValidation_js_1.resolveToCanonical)(cmd.name);
    ...
    const switchParams = [...config.knownSwitches, ...commonParameters_js_1.COMMON_SWITCHES];
    const valueParams = [...config.knownValueParams, ...commonParameters_js_1.COMMON_VALUE_PARAMS];
    const paths = [];
    const args = cmd.args;
...
function checkPathConstraintsForStatement(statement, toolPermissionContext, compoundCommandHasCd = false) {
    ...
    for (const cmd of statement.commands) {
        ...
        const isRemoval = (0, readOnlyValidation_js_1.resolveToCanonical)(cmd.name) === 'remove-item';
        for (const filePath of paths) {
            ...
            if (!allowed) {
                const canonical = (0, readOnlyValidation_js_1.resolveToCanonical)(cmd.name);
                const workingDirs = Array.from((0, filesystem_js_1.allWorkingDirectories)(toolPermissionContext));
                ...
                if (resolvedPath) {
                    if (operationType === 'read') {
                        const suggestion = (0, PermissionUpdate_js_1.createReadRuleSuggestion)((0, path_js_1.getDirectoryForPath)(resolvedPath), 'session');
                        if (suggestion) {
    ...
    if (statement.nestedCommands) {
        for (const cmd of statement.nestedCommands) {
            ...
            if (hasExpressionPipelineSource) {
                firstAsk ?? (firstAsk = {
                    behavior: 'ask',
                    message: `${(0, readOnlyValidation_js_1.resolveToCanonical)(cmd.name)} appears inside a control-flow or chain statement where piped expression sources cannot be statically validated and requires manual approval`,
                });
            }
        }
    }
