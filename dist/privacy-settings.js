...
const Grove_js_1 = require("../../components/grove/Grove.js");
...
async function call(onDone) {
    const qualified = await (0, grove_js_1.isQualifiedForGrove)();
    if (!qualified) {
        onDone(FALLBACK_MESSAGE);
        return null;
    }
    const [settingsResult, configResult] = await Promise.all([(0, grove_js_1.getGroveSettings)(), (0, grove_js_1.getGroveNoticeConfig)()]);
    ...
    async function onDoneWithSettingsCheck() {
        const updatedSettingsResult = await (0, grove_js_1.getGroveSettings)();
    ...
    if (settings.grove_enabled !== null) {
        return React.createElement(Grove_js_1.PrivacySettingsDialog, { settings: settings, domainExcluded: config?.domain_excluded, onDone: onDoneWithSettingsCheck });
    }
    // Show the GroveDialog for users who haven't accepted terms yet
    return React.createElement(Grove_js_1.GroveDialog, { showIfAlreadyViewed: true, onDone: onDoneWithDecision, location: 'settings' });
}
