"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.TeleportRepoMismatchDialog = TeleportRepoMismatchDialog;
const compiler_runtime_1 = require("react/compiler-runtime");
const react_1 = __importStar(require("react"));
const ink_js_1 = require("../ink.js");
const file_js_1 = require("../utils/file.js");
const githubRepoPathMapping_js_1 = require("../utils/githubRepoPathMapping.js");
const index_js_1 = require("./CustomSelect/index.js");
const Dialog_js_1 = require("./design-system/Dialog.js");
const Spinner_js_1 = require("./Spinner.js");
function TeleportRepoMismatchDialog(t0) {
    const $ = (0, compiler_runtime_1.c)(18);
    const { targetRepo, initialPaths, onSelectPath, onCancel } = t0;
    const [availablePaths, setAvailablePaths] = (0, react_1.useState)(initialPaths);
    const [errorMessage, setErrorMessage] = (0, react_1.useState)(null);
    const [validating, setValidating] = (0, react_1.useState)(false);
    let t1;
    if ($[0] !== availablePaths || $[1] !== onCancel || $[2] !== onSelectPath || $[3] !== targetRepo) {
        t1 = async (value) => {
            if (value === "cancel") {
                onCancel();
                return;
            }
            setValidating(true);
            setErrorMessage(null);
            const isValid = await (0, githubRepoPathMapping_js_1.validateRepoAtPath)(value, targetRepo);
            if (isValid) {
                onSelectPath(value);
                return;
            }
            (0, githubRepoPathMapping_js_1.removePathFromRepo)(targetRepo, value);
            const updatedPaths = availablePaths.filter(p => p !== value);
            setAvailablePaths(updatedPaths);
            setValidating(false);
            setErrorMessage(`${(0, file_js_1.getDisplayPath)(value)} no longer contains the correct repository. Select another path.`);
        };
        $[0] = availablePaths;
        $[1] = onCancel;
        $[2] = onSelectPath;
        $[3] = targetRepo;
        $[4] = t1;
    }
    else {
        t1 = $[4];
    }
    const handleChange = t1;
    let t2;
    if ($[5] !== availablePaths) {
        let t3;
        if ($[7] === Symbol.for("react.memo_cache_sentinel")) {
            t3 = {
                label: "Cancel",
                value: "cancel"
            };
            $[7] = t3;
        }
        else {
            t3 = $[7];
        }
        t2 = [...availablePaths.map(_temp), t3];
        $[5] = availablePaths;
        $[6] = t2;
    }
    else {
        t2 = $[6];
    }
    const options = t2;
    let t3;
    if ($[8] !== availablePaths.length || $[9] !== errorMessage || $[10] !== handleChange || $[11] !== options || $[12] !== targetRepo || $[13] !== validating) {
        t3 = availablePaths.length > 0 ? react_1.default.createElement(react_1.default.Fragment, null,
            react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", gap: 1 },
                errorMessage && react_1.default.createElement(ink_js_1.Text, { color: "error" }, errorMessage),
                react_1.default.createElement(ink_js_1.Text, null,
                    "Open Claude Code in ",
                    react_1.default.createElement(ink_js_1.Text, { bold: true }, targetRepo),
                    ":")),
            validating ? react_1.default.createElement(ink_js_1.Box, null,
                react_1.default.createElement(Spinner_js_1.Spinner, null),
                react_1.default.createElement(ink_js_1.Text, null, " Validating repository\u2026")) : react_1.default.createElement(index_js_1.Select, { options: options, onChange: value_0 => void handleChange(value_0) })) : react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", gap: 1 },
            errorMessage && react_1.default.createElement(ink_js_1.Text, { color: "error" }, errorMessage),
            react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                "Run claude --teleport from a checkout of ",
                targetRepo));
        $[8] = availablePaths.length;
        $[9] = errorMessage;
        $[10] = handleChange;
        $[11] = options;
        $[12] = targetRepo;
        $[13] = validating;
        $[14] = t3;
    }
    else {
        t3 = $[14];
    }
    let t4;
    if ($[15] !== onCancel || $[16] !== t3) {
        t4 = react_1.default.createElement(Dialog_js_1.Dialog, { title: "Teleport to Repo", onCancel: onCancel, color: "background" }, t3);
        $[15] = onCancel;
        $[16] = t3;
        $[17] = t4;
    }
    else {
        t4 = $[17];
    }
    return t4;
}
function _temp(path) {
    return {
        label: react_1.default.createElement(ink_js_1.Text, null,
            "Use ",
            react_1.default.createElement(ink_js_1.Text, { bold: true }, (0, file_js_1.getDisplayPath)(path))),
        value: path
    };
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
