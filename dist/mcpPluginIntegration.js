...
const mcpbHandler_js_1 = require("./mcpbHandler.js");
...
async function loadMcpServersFromMcpb(plugin, mcpbPath, errors) {
    try {
        (0, debug_js_1.logForDebugging)(`Loading MCP servers from MCPB: ${mcpbPath}`);
        // Use plugin.repository directly - it's already in "plugin@marketplace" format
        const pluginId = plugin.repository;
        const result = await (0, mcpbHandler_js_1.loadMcpbFile)(mcpbPath, plugin.path, pluginId, status => {
            (0, debug_js_1.logForDebugging)(`MCPB [${plugin.name}]: ${status}`);
        });
        ...
        if ('status' in result && result.status === 'needs-config') {
            ...
            (0, debug_js_1.logForDebugging)(`MCPB ${mcpbPath} requires user configuration. ` +
        ...
        (0, debug_js_1.logForDebugging)(`Loaded MCP server "${serverName}" from MCPB (extracted to ${successResult.extractedPath})`);
        return { [serverName]: successResult.mcpConfig };
    }
    catch (error) {
        const errorMsg = (0, errors_js_1.errorMessage)(error);
        (0, debug_js_1.logForDebugging)(`Failed to load MCPB ${mcpbPath}: ${errorMsg}`, {
            level: 'error',
        });
...
async function loadPluginMcpServers(plugin, errors = []) {
    ...
    if (plugin.manifest.mcpServers) {
        ...
        if (typeof mcpServersSpec === 'string') {
            ...
            if ((0, mcpbHandler_js_1.isMcpbSource)(mcpServersSpec)) {
            ...
            const results = await Promise.all(mcpServersSpec.map(async (spec) => {
                try {
                    if (typeof spec === 'string') {
                        // Check if it's an MCPB file
                        if ((0, mcpbHandler_js_1.isMcpbSource)(spec)) {
                    ...
                    (0, debug_js_1.logForDebugging)(`Failed to load MCP servers from spec for plugin ${plugin.name}: ${e}`, { level: 'error' });
                    return null;
                }
            }));
...
async function loadMcpServersFromFile(pluginPath, relativePath) {
    ...
    const filePath = (0, path_1.join)(pluginPath, relativePath);
    let content;
    try {
        ...
        (0, debug_js_1.logForDebugging)(`Failed to load MCP servers from ${filePath}: ${e}`, {
            level: 'error',
        });
        return null;
    }
    try {
        ...
        for (const [name, config] of Object.entries(mcpServers)) {
            const result = (0, types_js_1.McpServerConfigSchema)().safeParse(config);
            if (result.success) {
                validatedServers[name] = result.data;
            }
            else {
                (0, debug_js_1.logForDebugging)(`Invalid MCP server config for ${name} in ${filePath}: ${result.error.message}`, { level: 'error' });
            }
        }
        return validatedServers;
    }
    catch (error) {
        (0, debug_js_1.logForDebugging)(`Failed to load MCP servers from ${filePath}: ${error}`, {
            level: 'error',
        });
        return null;
    }
...
function getUnconfiguredChannels(plugin) {
    ...
    for (const channel of channels) {
        ...
        const saved = (0, mcpbHandler_js_1.loadMcpServerUserConfig)(pluginId, channel.server) ?? {};
        const validation = (0, mcpbHandler_js_1.validateUserConfig)(saved, channel.userConfig);
        if (!validation.valid) {
            unconfigured.push({
...
function loadChannelUserConfig(plugin, serverName) {
    ...
    return (0, mcpbHandler_js_1.loadMcpServerUserConfig)(plugin.repository, serverName) ?? undefined;
}
/**
...
async function extractMcpServersFromPlugins(plugins, errors = []) {
    ...
    const scopedResults = await Promise.all(plugins.map(async (plugin) => {
        ...
        (0, debug_js_1.logForDebugging)(`Loaded ${Object.keys(servers).length} MCP servers from plugin ${plugin.name}`);
...
function resolvePluginMcpEnvironment(config, plugin, userConfig, errors, pluginName, serverName) {
    ...
    if (errors && allMissingVars.length > 0) {
        ...
        (0, debug_js_1.logForDebugging)(`Missing environment variables in plugin MCP config: ${varList}`, { level: 'warn' });
