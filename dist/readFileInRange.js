...
class FileTooLargeError extends Error {
    constructor(sizeInBytes, maxSizeBytes) {
        super(`File content (${(0, format_js_1.formatFileSize)(sizeInBytes)}) exceeds maximum allowed size (${(0, format_js_1.formatFileSize)(maxSizeBytes)}). Use offset and limit parameters to read specific portions of the file, or search for specific content instead of reading the whole file.`);
...
async function readFileInRange(filePath, offset = 0, maxLines, maxBytes, signal, options) {
    ...
    if (stats.isFile() && stats.size < FAST_PATH_MAX_SIZE) {
        ...
        const text = await (0, promises_1.readFile)(filePath, { encoding: 'utf8', signal });
