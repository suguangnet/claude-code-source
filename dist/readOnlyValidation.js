...
function resolveToCanonical(name) {
    ...
    const alias = parser_js_1.COMMON_ALIASES[lower];
    if (alias) {
...
function lookupAllowlist(name) {
    ...
    // Direct lookup first
    const direct = exports.CMDLET_ALLOWLIST[lower];
    if (direct) {
        return direct;
    }
...
function isReadOnlyCommand(command, parsed) {
    ...
    const security = (0, parser_js_1.deriveSecurityFlags)(parsed);
    ...
    const segments = (0, parser_js_1.getPipelineSegments)(parsed);
    ...
    for (const pipeline of segments) {
        ...
        if (pipeline.redirections.length > 0) {
            const hasFileRedirection = pipeline.redirections.some(r => !r.isMerging && !(0, parser_js_1.isNullRedirectionTarget)(r.target));
...
function isAllowlistedCommand(cmd, originalCommand) {
    ...
    if (!config.safeFlags || config.safeFlags.length === 0) {
        ...
        const hasFlags = cmd.args.some((arg, i) => {
            if (isCmdlet) {
                return (0, parser_js_1.isPowerShellParameter)(arg, cmd.elementTypes?.[i + 1]);
            }
    ...
    for (let i = 0; i < cmd.args.length; i++) {
        ...
        const isFlag = isCmdlet
            ? (0, parser_js_1.isPowerShellParameter)(arg, cmd.elementTypes?.[i + 1])
            : arg.startsWith('-') ||
                (process.platform === 'win32' && arg.startsWith('/'));
        if (isFlag) {
            ...
            if (isCmdlet && commonParameters_js_1.COMMON_PARAMETERS.has(paramLower)) {
                continue;
            }
...
function isGitSafe(args) {
    ...
    let config = readOnlyCommandValidation_js_1.GIT_READ_ONLY_COMMANDS[twoWordKey];
...
function isDockerSafe(args) {
    ...
    const config = readOnlyCommandValidation_js_1.DOCKER_READ_ONLY_COMMANDS[oneWordKey];
    if (!config) {
        return false;
    }
