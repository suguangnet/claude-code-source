...
function Tabs(t0) {
    ...
    const [internalSelectedTab, setInternalSelectedTab] = (0, react_1.useState)(defaultTabIndex !== -1 ? defaultTabIndex : 0);
    ...
    const modalScrollRef = (0, modalContext_js_1.useModalScrollRef)();
    const [headerFocused, setHeaderFocused] = (0, react_1.useState)(initialHeaderFocused);
    let t3;
    ...
    const blurHeader = t4;
    const [optInCount, setOptInCount] = (0, react_1.useState)(0);
    let t5;
    ...
    const titleWidth = title ? (0, stringWidth_js_1.stringWidth)(title) + 1 : 0;
    ...
    const t15 = !hidden && react_1.default.createElement(ink_js_1.Box, { flexDirection: "row", gap: 1, flexShrink: modalScrollRef ? 0 : undefined },
        title !== undefined && react_1.default.createElement(ink_js_1.Text, { bold: true, color: color }, title),
        tabs.map((t16, i) => {
            const [id, title_0] = t16;
            const isCurrent = selectedTabIndex === i;
            const hasColorCursor = color && isCurrent && headerFocused;
            return react_1.default.createElement(ink_js_1.Text, { key: id, backgroundColor: hasColorCursor ? color : undefined, color: hasColorCursor ? "inverseText" : undefined, inverse: isCurrent && !hasColorCursor, bold: isCurrent },
                " ",
                title_0,
                " ");
        }),
        spacerWidth > 0 && react_1.default.createElement(ink_js_1.Text, null, " ".repeat(spacerWidth)));
    let t17;
    if ($[11] !== children || $[12] !== contentHeight || $[13] !== contentWidth || $[14] !== hidden || $[15] !== modalScrollRef || $[16] !== selectedTabIndex) {
        t17 = modalScrollRef ? react_1.default.createElement(ink_js_1.Box, { width: contentWidth, marginTop: hidden ? 0 : 1, flexShrink: 0 },
            react_1.default.createElement(ScrollBox_js_1.default, { key: selectedTabIndex, ref: modalScrollRef, flexDirection: "column", flexShrink: 0 }, children)) : react_1.default.createElement(ink_js_1.Box, { width: contentWidth, marginTop: hidden ? 0 : 1, height: contentHeight, overflowY: contentHeight !== undefined ? "hidden" : undefined }, children);
        $[11] = children;
        $[12] = contentHeight;
        $[13] = contentWidth;
        $[14] = hidden;
    ...
    if ($[18] !== T0 || $[19] !== banner || $[20] !== handleKeyDown || $[21] !== t14 || $[22] !== t15 || $[23] !== t17) {
        t18 = react_1.default.createElement(T0, { flexDirection: t11, tabIndex: t12, autoFocus: t13, onKeyDown: handleKeyDown, flexShrink: t14 },
            t15,
            banner,
            t17);
        $[18] = T0;
    ...
    return react_1.default.createElement(TabsContext.Provider, { value: {
...
function Tab(t0) {
    ...
    const { selectedTab, width } = (0, react_1.useContext)(TabsContext);
    ...
    if ($[0] !== children || $[1] !== t1 || $[2] !== width) {
        t2 = react_1.default.createElement(ink_js_1.Box, { width: width, flexShrink: t1 }, children);
        $[0] = children;
        $[1] = t1;
        $[2] = width;
        $[3] = t2;
...
function useTabsWidth() {
    const { width } = (0, react_1.useContext)(TabsContext);
    return width;
}
/**
...
function useTabHeaderFocus() {
    ...
    const { headerFocused, focusHeader, blurHeader, registerOptIn } = (0, react_1.useContext)(TabsContext);
    let t0;
    if ($[0] !== registerOptIn) {
        t0 = [registerOptIn];
        $[0] = registerOptIn;
