"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.UserToolErrorMessage = UserToolErrorMessage;
const compiler_runtime_1 = require("react/compiler-runtime");
const bun_bundle_1 = require("bun:bundle");
const React = __importStar(require("react"));
const figures_js_1 = require("../../../constants/figures.js");
const ink_js_1 = require("../../../ink.js");
const Tool_js_1 = require("../../../Tool.js");
const messages_js_1 = require("../../../utils/messages.js");
const FallbackToolUseErrorMessage_js_1 = require("../../FallbackToolUseErrorMessage.js");
const InterruptedByUser_js_1 = require("../../InterruptedByUser.js");
const MessageResponse_js_1 = require("../../MessageResponse.js");
const RejectedPlanMessage_js_1 = require("./RejectedPlanMessage.js");
const RejectedToolUseMessage_js_1 = require("./RejectedToolUseMessage.js");
function UserToolErrorMessage(t0) {
    const $ = (0, compiler_runtime_1.c)(14);
    const { progressMessagesForMessage, tool, tools, param, verbose, isTranscriptMode } = t0;
    if (typeof param.content === "string" && param.content.includes(messages_js_1.INTERRUPT_MESSAGE_FOR_TOOL_USE)) {
        let t1;
        if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
            t1 = React.createElement(MessageResponse_js_1.MessageResponse, { height: 1 },
                React.createElement(InterruptedByUser_js_1.InterruptedByUser, null));
            $[0] = t1;
        }
        else {
            t1 = $[0];
        }
        return t1;
    }
    if (typeof param.content === "string" && param.content.startsWith(messages_js_1.PLAN_REJECTION_PREFIX)) {
        let t1;
        if ($[1] !== param.content) {
            t1 = param.content.substring(messages_js_1.PLAN_REJECTION_PREFIX.length);
            $[1] = param.content;
            $[2] = t1;
        }
        else {
            t1 = $[2];
        }
        const planContent = t1;
        let t2;
        if ($[3] !== planContent) {
            t2 = React.createElement(RejectedPlanMessage_js_1.RejectedPlanMessage, { plan: planContent });
            $[3] = planContent;
            $[4] = t2;
        }
        else {
            t2 = $[4];
        }
        return t2;
    }
    if (typeof param.content === "string" && param.content.startsWith(messages_js_1.REJECT_MESSAGE_WITH_REASON_PREFIX)) {
        let t1;
        if ($[5] === Symbol.for("react.memo_cache_sentinel")) {
            t1 = React.createElement(RejectedToolUseMessage_js_1.RejectedToolUseMessage, null);
            $[5] = t1;
        }
        else {
            t1 = $[5];
        }
        return t1;
    }
    if ((0, bun_bundle_1.feature)("TRANSCRIPT_CLASSIFIER") && typeof param.content === "string" && (0, messages_js_1.isClassifierDenial)(param.content)) {
        let t1;
        if ($[6] === Symbol.for("react.memo_cache_sentinel")) {
            t1 = React.createElement(MessageResponse_js_1.MessageResponse, { height: 1 },
                React.createElement(ink_js_1.Text, { dimColor: true },
                    "Denied by auto mode classifier ",
                    figures_js_1.BULLET_OPERATOR,
                    " /feedback if incorrect"));
            $[6] = t1;
        }
        else {
            t1 = $[6];
        }
        return t1;
    }
    let t1;
    if ($[7] !== isTranscriptMode || $[8] !== param.content || $[9] !== progressMessagesForMessage || $[10] !== tool || $[11] !== tools || $[12] !== verbose) {
        t1 = tool?.renderToolUseErrorMessage?.(param.content, {
            progressMessagesForMessage: (0, Tool_js_1.filterToolProgressMessages)(progressMessagesForMessage),
            tools,
            verbose,
            isTranscriptMode
        }) ?? React.createElement(FallbackToolUseErrorMessage_js_1.FallbackToolUseErrorMessage, { result: param.content, verbose: verbose });
        $[7] = isTranscriptMode;
        $[8] = param.content;
        $[9] = progressMessagesForMessage;
        $[10] = tool;
        $[11] = tools;
        $[12] = verbose;
        $[13] = t1;
    }
    else {
        t1 = $[13];
    }
    return t1;
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
