...
const COPY_DIR = (0, path_1.join)((0, os_1.tmpdir)(), 'claude');
const RESPONSE_FILENAME = 'response.md';
const MAX_LOOKBACK = 20;
function extractCodeBlocks(markdown) {
    const tokens = marked_1.marked.lexer((0, messages_js_1.stripPromptXMLTags)(markdown));
    const blocks = [];
...
function collectRecentAssistantTexts(messages) {
    ...
    for (let i = messages.length - 1; i >= 0 && texts.length < MAX_LOOKBACK; i--) {
        ...
        if (!Array.isArray(content))
            continue;
        const text = (0, messages_js_1.extractTextContent)(content, '\n\n');
        if (text)
            texts.push(text);
    }
    return texts;
...
async function writeToFile(text, filename) {
    const filePath = (0, path_1.join)(COPY_DIR, filename);
    await (0, promises_1.mkdir)(COPY_DIR, {
        recursive: true
    });
    await (0, promises_1.writeFile)(filePath, text, 'utf-8');
    return filePath;
}
async function copyOrWriteToFile(text, filename) {
    const raw = await (0, osc_js_1.setClipboard)(text);
    if (raw)
        process.stdout.write(raw);
    const lineCount = (0, stringUtils_js_1.countCharInString)(text, '\n') + 1;
...
function truncateLine(text, maxLen) {
    ...
    for (const char of firstLine) {
        const charWidth = (0, stringWidth_js_1.stringWidth)(char);
...
function CopyPicker(t0) {
    const $ = (0, compiler_runtime_1.c)(33);
    const { fullText, codeBlocks, messageAge, onDone } = t0;
    const focusedRef = (0, react_1.useRef)("full");
    const t1 = `${fullText.length} chars, ${(0, stringUtils_js_1.countCharInString)(fullText, "\n") + 1} lines`;
    let t2;
    if ($[0] !== t1) {
        t2 = {
            label: "Full response",
    ...
    if ($[19] === Symbol.for("react.memo_cache_sentinel")) {
        t7 = react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "Select content to copy:");
        $[19] = t7;
    }
    else {
        t7 = $[19];
    ...
    if ($[25] !== options || $[26] !== t10 || $[27] !== t9) {
        t11 = react_1.default.createElement(select_js_1.Select, { options: options, hideIndexes: false, onFocus: t8, onChange: t9, onCancel: t10 });
        $[25] = options;
        $[26] = t10;
        $[27] = t9;
        $[28] = t11;
    ...
    if ($[29] === Symbol.for("react.memo_cache_sentinel")) {
        t12 = react_1.default.createElement(ink_js_1.Text, { dimColor: true },
            react_1.default.createElement(Byline_js_1.Byline, null,
                react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "enter", action: "copy" }),
                react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "w", action: "write to file" }),
                react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "esc", action: "cancel" })));
        $[29] = t12;
    }
    else {
        t12 = $[29];
    ...
    if ($[30] !== handleKeyDown || $[31] !== t11) {
        t13 = react_1.default.createElement(Pane_js_1.Pane, null,
            react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", gap: 1, tabIndex: 0, autoFocus: true, onKeyDown: handleKeyDown },
                t7,
                t11,
                t12));
        $[30] = handleKeyDown;
...
function _temp(block, index) {
    const blockLines = (0, stringUtils_js_1.countCharInString)(block.code, "\n") + 1;
    return {
...
const call = async (onDone, context, args) => {
    ...
    return react_1.default.createElement(CopyPicker, { fullText: text, codeBlocks: codeBlocks, messageAge: age, onDone: onDone });
};
exports.call = call;
