...
const Notifications_js_1 = require("../components/PromptInput/Notifications.js");
...
function useArrowKeyHistory(onSetInput, currentInput, pastedContents, setCursorOffset, currentMode) {
    ...
    const [lastShownHistoryEntry, setLastShownHistoryEntry] = (0, react_1.useState)(undefined);
    const hasShownSearchHintRef = (0, react_1.useRef)(false);
    const { addNotification, removeNotification } = (0, notifications_js_1.useNotifications)();
    // Cache loaded history entries
    const historyCache = (0, react_1.useRef)([]);
    // Track which mode filter the cache was loaded with
    const historyCacheModeFilter = (0, react_1.useRef)(undefined);
    ...
    const initialModeFilterRef = (0, react_1.useRef)(undefined);
    ...
    const currentInputRef = (0, react_1.useRef)(currentInput);
    const pastedContentsRef = (0, react_1.useRef)(pastedContents);
    const currentModeRef = (0, react_1.useRef)(currentMode);
    // Keep refs in sync with props (synchronous update on each render)
    currentInputRef.current = currentInput;
    pastedContentsRef.current = pastedContents;
    currentModeRef.current = currentMode;
    const setInputWithCursor = (0, react_1.useCallback)((value, mode, contents, cursorToStart = false) => {
        onSetInput(value, mode, contents);
        setCursorOffset?.(cursorToStart ? 0 : value.length);
    }, [onSetInput, setCursorOffset]);
    const updateInput = (0, react_1.useCallback)((input, cursorToStart_0 = false) => {
        if (!input || !input.display)
            return;
        const mode_0 = (0, inputModes_js_1.getModeFromInput)(input.display);
    ...
    const showSearchHint = (0, react_1.useCallback)(() => {
        addNotification({
            key: 'search-history-hint',
            jsx: react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                react_1.default.createElement(ConfigurableShortcutHint_js_1.ConfigurableShortcutHint, { action: "history:search", context: "Global", fallback: "ctrl+r", description: "search history" })),
            priority: 'immediate',
            timeoutMs: Notifications_js_1.FOOTER_TEMPORARY_STATUS_TIMEOUT
        });
    }, [addNotification]);
    const onHistoryUp = (0, react_1.useCallback)(() => {
    ...
    const onHistoryDown = (0, react_1.useCallback)(() => {
    ...
    const resetHistory = (0, react_1.useCallback)(() => {
    ...
    const dismissSearchHint = (0, react_1.useCallback)(() => {
        removeNotification('search-history-hint');
    }, [removeNotification]);
    return {
        historyIndex,
