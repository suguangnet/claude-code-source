...
const TEAMMATE_MSG_REGEX = new RegExp(`<${xml_js_1.TEAMMATE_MESSAGE_TAG}\\s+teammate_id="([^"]+)"(?:\\s+color="([^"]+)")?(?:\\s+summary="([^"]+)")?>\\n?([\\s\\S]*?)\\n?<\\/${xml_js_1.TEAMMATE_MESSAGE_TAG}>`, 'g');
/**
...
function UserTeammateMessage({ addMargin, param: { text }, isTranscriptMode }) {
    const messages = parseTeammateMessages(text).filter(msg => {
        ...
        try {
            const parsed = (0, slowOperations_js_1.jsonParse)(msg.content);
    ...
    return React.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: addMargin ? 1 : 0, width: "100%" }, messages.map((msg_0, index) => {
        ...
        const planApprovalElement = (0, PlanApprovalMessage_js_1.tryRenderPlanApprovalMessage)(msg_0.content, displayName);
        ...
        const shutdownElement = (0, ShutdownMessage_js_1.tryRenderShutdownMessage)(msg_0.content);
        if (shutdownElement) {
            return React.createElement(React.Fragment, { key: index }, shutdownElement);
        }
        // Try to render as task assignment message
        const taskAssignmentElement = (0, TaskAssignmentMessage_js_1.tryRenderTaskAssignmentMessage)(msg_0.content);
        ...
        if (parsedIdleNotification?.type === 'task_completed') {
            ...
            return React.createElement(ink_js_1.Box, { key: index, flexDirection: "column", marginTop: 1 },
                React.createElement(ink_js_1.Text, { color: inkColor }, `@${displayName}${figures_1.default.pointer}`),
                React.createElement(MessageResponse_js_1.MessageResponse, null,
                    React.createElement(ink_js_1.Text, { color: "success" }, "\u2713"),
                    React.createElement(ink_js_1.Text, null,
                        ' ',
                        "Completed task #",
                        taskCompleted.taskId,
                        taskCompleted.taskSubject && React.createElement(ink_js_1.Text, { dimColor: true },
                            " (",
...
function TeammateMessageContent(t0) {
    ...
    const t1 = `@${displayName}${figures_1.default.pointer}`;
    let t2;
    if ($[0] !== inkColor || $[1] !== t1) {
        t2 = React.createElement(ink_js_1.Text, { color: inkColor }, t1);
        $[0] = inkColor;
        $[1] = t1;
        $[2] = t2;
    }
    ...
    let t3;
    if ($[3] !== summary) {
        t3 = summary && React.createElement(ink_js_1.Text, null,
            " ",
            summary);
        $[3] = summary;
        $[4] = t3;
    ...
    let t4;
    if ($[5] !== t2 || $[6] !== t3) {
        t4 = React.createElement(ink_js_1.Box, null,
            t2,
            t3);
        $[5] = t2;
        $[6] = t3;
    ...
    if ($[8] !== content || $[9] !== isTranscriptMode) {
        t5 = isTranscriptMode && React.createElement(ink_js_1.Box, { paddingLeft: 2 },
            React.createElement(ink_js_1.Text, null,
                React.createElement(ink_js_1.Ansi, null, content)));
        $[8] = content;
        $[9] = isTranscriptMode;
        $[10] = t5;
    }
    ...
    let t6;
    if ($[11] !== t4 || $[12] !== t5) {
        t6 = React.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: 1 },
            t4,
            t5);
        $[11] = t4;
        $[12] = t5;
