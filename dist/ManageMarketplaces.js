...
const KeyboardShortcutHint_js_1 = require("../../components/design-system/KeyboardShortcutHint.js");
...
const pluginAutoupdate_js_1 = require("../../utils/plugins/pluginAutoupdate.js");
const pluginLoader_js_1 = require("../../utils/plugins/pluginLoader.js");
const schemas_js_1 = require("../../utils/plugins/schemas.js");
const settings_js_1 = require("../../utils/settings/settings.js");
const stringUtils_js_1 = require("../../utils/stringUtils.js");
function ManageMarketplaces({ setViewState, error, setError, setResult, exitState, onManageComplete, targetMarketplace, action }) {
    const [marketplaceStates, setMarketplaceStates] = (0, react_1.useState)([]);
    ...
    const [progressMessage, setProgressMessage] = (0, react_1.useState)(null);
    const [internalView, setInternalView] = (0, react_1.useState)('list');
    ...
    const hasAttemptedAutoAction = (0, react_1.useRef)(false);
            ...
            try {
                ...
                for (const { name, config: entry, data: marketplace } of marketplaces) {
                    ...
                    states.push({
                        ...
                        pendingUpdate: false,
                        pendingRemove: false,
                        autoUpdate: (0, schemas_js_1.isMarketplaceAutoUpdate)(name, entry)
                    });
                }
    ...
    const getPendingCounts = () => {
        const updateCount = (0, array_js_1.count)(marketplaceStates, s => s.pendingUpdate);
        const removeCount = (0, array_js_1.count)(marketplaceStates, s => s.pendingRemove);
        return {
            updateCount,
            removeCount
        };
        ...
        try {
            ...
            for (const state of statesToProcess) {
                ...
                if (state.pendingRemove) {
                    ...
                    if (state.installedPlugins && state.installedPlugins.length > 0) {
                        ...
                        for (const plugin of state.installedPlugins) {
                            const pluginId = (0, marketplaceHelpers_js_1.createPluginId)(plugin.name, state.name);
                    ...
                    await (0, marketplaceManager_js_1.removeMarketplaceSource)(state.name);
                    removedCount++;
                ...
                if (state.pendingUpdate) {
                    ...
                    await (0, marketplaceManager_js_1.refreshMarketplace)(state.name, (message) => {
            ...
            if (refreshedMarketplaces.size > 0) {
                const updatedPluginIds = await (0, pluginAutoupdate_js_1.updatePluginsForMarketplaces)(refreshedMarketplaces);
            ...
            const actions = [];
            if (updatedCount > 0) {
                const pluginPart = updatedPluginCount > 0 ? ` (${updatedPluginCount} ${(0, stringUtils_js_1.plural)(updatedPluginCount, 'plugin')} bumped)` : '';
                actions.push(`Updated ${updatedCount} ${(0, stringUtils_js_1.plural)(updatedCount, 'marketplace')}${pluginPart}`);
            }
            if (removedCount > 0) {
                actions.push(`Removed ${removedCount} ${(0, stringUtils_js_1.plural)(removedCount, 'marketplace')}`);
            }
            if (actions.length > 0) {
                const successMsg = `${figures_1.default.tick} ${actions.join(', ')}`;
            ...
            const errorMsg = (0, errors_js_1.errorMessage)(err);
    ...
    const handleToggleAutoUpdate = async (marketplace) => {
        ...
        try {
            await (0, marketplaceManager_js_1.setMarketplaceAutoUpdate)(marketplace.name, newAutoUpdate);
            // Update local state
    ...
    if (loading) {
        return React.createElement(ink_js_1.Text, null, "Loading marketplaces\u2026");
    }
    if (marketplaceStates.length === 0) {
        return React.createElement(ink_js_1.Box, { flexDirection: "column" },
            React.createElement(ink_js_1.Box, { marginBottom: 1 },
                React.createElement(ink_js_1.Text, { bold: true }, "Manage marketplaces")),
            React.createElement(ink_js_1.Box, { flexDirection: "row", gap: 1 },
                React.createElement(ink_js_1.Text, { color: "suggestion" },
                    figures_1.default.pointer,
                    " +"),
                React.createElement(ink_js_1.Text, { bold: true, color: "suggestion" }, "Add Marketplace")),
            React.createElement(ink_js_1.Box, { marginLeft: 3 },
                React.createElement(ink_js_1.Text, { dimColor: true, italic: true }, exitState.pending ? React.createElement(React.Fragment, null,
                    "Press ",
                    exitState.keyName,
                    " again to go back") : React.createElement(Byline_js_1.Byline, null,
    ...
    if (internalView === 'confirm-remove' && selectedMarketplace) {
        ...
        return React.createElement(ink_js_1.Box, { flexDirection: "column" },
            React.createElement(ink_js_1.Text, { bold: true, color: "warning" },
                "Remove marketplace ",
                React.createElement(ink_js_1.Text, { italic: true }, selectedMarketplace.name),
                "?"),
            React.createElement(ink_js_1.Box, { flexDirection: "column" },
                pluginCount > 0 && React.createElement(ink_js_1.Box, { marginTop: 1 },
                    React.createElement(ink_js_1.Text, { color: "warning" },
                        "This will also uninstall ",
                        pluginCount,
                        ' ',
                        (0, stringUtils_js_1.plural)(pluginCount, 'plugin'),
                        " from this marketplace:")),
                selectedMarketplace.installedPlugins && selectedMarketplace.installedPlugins.length > 0 && React.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: 1, marginLeft: 2 }, selectedMarketplace.installedPlugins.map(plugin => React.createElement(ink_js_1.Text, { key: plugin.name, dimColor: true },
                    "\u2022 ",
                    plugin.name))),
    ...
    if (internalView === 'details' && selectedMarketplace) {
        ...
        const menuOptions = buildDetailsMenuOptions(selectedMarketplace);
        return React.createElement(ink_js_1.Box, { flexDirection: "column" },
            React.createElement(ink_js_1.Text, { bold: true }, selectedMarketplace.name),
            React.createElement(ink_js_1.Text, { dimColor: true }, selectedMarketplace.source),
            React.createElement(ink_js_1.Box, { marginTop: 1 },
                React.createElement(ink_js_1.Text, null,
            ...
            selectedMarketplace.installedPlugins && selectedMarketplace.installedPlugins.length > 0 && React.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: 1 },
                React.createElement(ink_js_1.Text, { bold: true },
                    "Installed plugins (",
                    selectedMarketplace.installedPlugins.length,
                    "):"),
                React.createElement(ink_js_1.Box, { flexDirection: "column", marginLeft: 1 }, selectedMarketplace.installedPlugins.map(plugin => React.createElement(ink_js_1.Box, { key: plugin.name, flexDirection: "row", gap: 1 },
                    React.createElement(ink_js_1.Text, null, figures_1.default.bullet),
                    React.createElement(ink_js_1.Box, { flexDirection: "column" },
                        React.createElement(ink_js_1.Text, null, plugin.name),
                        React.createElement(ink_js_1.Text, { dimColor: true }, plugin.manifest.description)))))),
            isUpdating && React.createElement(ink_js_1.Box, { marginTop: 1, flexDirection: "column" },
                React.createElement(ink_js_1.Text, { color: "claude" }, "Updating marketplace\u2026"),
                progressMessage && React.createElement(ink_js_1.Text, { dimColor: true }, progressMessage)),
            !isUpdating && successMessage && React.createElement(ink_js_1.Box, { marginTop: 1 },
                React.createElement(ink_js_1.Text, { color: "claude" }, successMessage)),
            !isUpdating && processError && React.createElement(ink_js_1.Box, { marginTop: 1 },
                React.createElement(ink_js_1.Text, { color: "error" }, processError)),
            !isUpdating && React.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: 1 }, menuOptions.map((option, idx) => {
                if (!option)
                    return null;
                const isSelected = idx === detailsMenuIndex;
                return React.createElement(ink_js_1.Box, { key: option.value },
                    React.createElement(ink_js_1.Text, { color: isSelected ? 'suggestion' : undefined },
                        isSelected ? figures_1.default.pointer : ' ',
                        " ",
                        option.label),
                    option.secondaryLabel && React.createElement(ink_js_1.Text, { dimColor: true },
                        " ",
                        option.secondaryLabel));
            })),
            !isUpdating && !(0, config_js_1.shouldSkipPluginAutoupdate)() && selectedMarketplace.autoUpdate && React.createElement(ink_js_1.Box, { marginTop: 1 },
                React.createElement(ink_js_1.Text, { dimColor: true }, "Auto-update enabled. Claude Code will automatically update this marketplace and its installed plugins.")),
            React.createElement(ink_js_1.Box, { marginLeft: 3 },
                React.createElement(ink_js_1.Text, { dimColor: true, italic: true }, isUpdating ? React.createElement(React.Fragment, null, "Please wait\u2026") : React.createElement(Byline_js_1.Byline, null,
                    React.createElement(ConfigurableShortcutHint_js_1.ConfigurableShortcutHint, { action: "select:accept", context: "Select", fallback: "Enter", description: "select" }),
                    React.createElement(ConfigurableShortcutHint_js_1.ConfigurableShortcutHint, { action: "confirm:no", context: "Confirmation", fallback: "Esc", description: "go back" })))));
    }
    ...
    return React.createElement(ink_js_1.Box, { flexDirection: "column" },
        React.createElement(ink_js_1.Box, { marginBottom: 1 },
            React.createElement(ink_js_1.Text, { bold: true }, "Manage marketplaces")),
        React.createElement(ink_js_1.Box, { flexDirection: "row", gap: 1, marginBottom: 1 },
            React.createElement(ink_js_1.Text, { color: selectedIndex === 0 ? 'suggestion' : undefined },
                selectedIndex === 0 ? figures_1.default.pointer : ' ',
                " +"),
            React.createElement(ink_js_1.Text, { bold: true, color: selectedIndex === 0 ? 'suggestion' : undefined }, "Add Marketplace")),
        React.createElement(ink_js_1.Box, { flexDirection: "column" }, marketplaceStates.map((state, idx) => {
            ...
            return React.createElement(ink_js_1.Box, { key: state.name, flexDirection: "row", gap: 1, marginBottom: 1 },
                React.createElement(ink_js_1.Text, { color: isSelected ? 'suggestion' : undefined },
                    isSelected ? figures_1.default.pointer : ' ',
                    ' ',
                    state.pendingRemove ? figures_1.default.cross : figures_1.default.bullet),
                React.createElement(ink_js_1.Box, { flexDirection: "column", flexGrow: 1 },
                    React.createElement(ink_js_1.Box, { flexDirection: "row", gap: 1 },
                        React.createElement(ink_js_1.Text, { bold: true, strikethrough: state.pendingRemove, dimColor: state.pendingRemove },
                            state.name === 'claude-plugins-official' && React.createElement(ink_js_1.Text, { color: "claude" }, "\u273B "),
                            state.name,
                            state.name === 'claude-plugins-official' && React.createElement(ink_js_1.Text, { color: "claude" }, " \u273B")),
                        indicators.length > 0 && React.createElement(ink_js_1.Text, { color: "warning" },
                            "[",
                            indicators.join(', '),
                            "]")),
                    React.createElement(ink_js_1.Text, { dimColor: true }, state.source),
        ...
        hasPendingChanges() && React.createElement(ink_js_1.Box, { marginTop: 1, flexDirection: "column" },
            React.createElement(ink_js_1.Text, null,
                React.createElement(ink_js_1.Text, { bold: true }, "Pending changes:"),
                ' ',
                React.createElement(ink_js_1.Text, { dimColor: true }, "Enter to apply")),
            updateCount > 0 && React.createElement(ink_js_1.Text, null,
                "\u2022 Update ",
                updateCount,
                " ",
                (0, stringUtils_js_1.plural)(updateCount, 'marketplace')),
            removeCount > 0 && React.createElement(ink_js_1.Text, { color: "warning" },
                "\u2022 Remove ",
                removeCount,
                " ",
                (0, stringUtils_js_1.plural)(removeCount, 'marketplace'))),
        isProcessing && React.createElement(ink_js_1.Box, { marginTop: 1 },
            React.createElement(ink_js_1.Text, { color: "claude" }, "Processing changes\u2026")),
...
function ManageMarketplacesKeyHints(t0) {
    const $ = (0, compiler_runtime_1.c)(18);
    const { exitState, hasPendingActions } = t0;
    if (exitState.pending) {
        let t1;
        if ($[0] !== exitState.keyName) {
            t1 = React.createElement(ink_js_1.Box, { marginTop: 1 },
    ...
    if ($[2] !== hasPendingActions) {
        t1 = hasPendingActions && React.createElement(ConfigurableShortcutHint_js_1.ConfigurableShortcutHint, { action: "select:accept", context: "Select", fallback: "Enter", description: "apply changes" });
        $[2] = hasPendingActions;
        $[3] = t1;
    }
    else {
    ...
    if ($[4] !== hasPendingActions) {
        t2 = !hasPendingActions && React.createElement(ConfigurableShortcutHint_js_1.ConfigurableShortcutHint, { action: "select:accept", context: "Select", fallback: "Enter", description: "select" });
        $[4] = hasPendingActions;
        $[5] = t2;
    }
    else {
    ...
    if ($[6] !== hasPendingActions) {
        t3 = !hasPendingActions && React.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "u", action: "update" });
        $[6] = hasPendingActions;
        $[7] = t3;
    }
    else {
    ...
    if ($[8] !== hasPendingActions) {
        t4 = !hasPendingActions && React.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "r", action: "remove" });
        $[8] = hasPendingActions;
        $[9] = t4;
    }
    else {
    ...
    let t6;
    if ($[10] !== t5) {
        t6 = React.createElement(ConfigurableShortcutHint_js_1.ConfigurableShortcutHint, { action: "confirm:no", context: "Confirmation", fallback: "Esc", description: t5 });
        $[10] = t5;
        $[11] = t6;
    }
    else {
