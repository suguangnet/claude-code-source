...
function classifyImageError(error) {
    ...
    const message = (0, errors_js_1.errorMessage)(error);
...
async function maybeResizeAndDownsampleImageBuffer(imageBuffer, originalSize, ext) {
    ...
    try {
        ...
        const needsDimensionResize = width > apiLimits_js_1.IMAGE_MAX_WIDTH || height > apiLimits_js_1.IMAGE_MAX_HEIGHT;
        ...
        if (!needsDimensionResize && originalSize > apiLimits_js_1.IMAGE_TARGET_RAW_SIZE) {
        ...
        (0, debug_js_1.logForDebugging)(`Resizing to ${width}x${height}`);
        ...
        if (resizedImageBuffer.length > apiLimits_js_1.IMAGE_TARGET_RAW_SIZE) {
            // For PNGs, try PNG compression first to preserve transparency
            if (isPng) {
                const pngCompressed = await sharp(imageBuffer)
                    .resize(width, height, {
            ...
            const compressedBuffer = await sharp(imageBuffer)
                ...
                .jpeg({ quality: 20 })
                .toBuffer();
            (0, debug_js_1.logForDebugging)(`JPEG compressed buffer size: ${compressedBuffer.length}`);
            return {
                buffer: compressedBuffer,
                mediaType: 'jpeg',
                dimensions: {
        ...
        throw new ImageResizeError(overDim
            ? `Unable to resize image — dimensions exceed the ${apiLimits_js_1.IMAGE_MAX_WIDTH}x${apiLimits_js_1.IMAGE_MAX_HEIGHT}px limit and image processing failed. ` +
                `Please resize the image to reduce its pixel dimensions.`
            : `Unable to resize image (${(0, format_js_1.formatFileSize)(originalSize)} raw, ${(0, format_js_1.formatFileSize)(base64Size)} base64). ` +
 ...
 */
async function compressImageBuffer(imageBuffer, maxBytes = apiLimits_js_1.IMAGE_TARGET_RAW_SIZE, originalMediaType) {
    ...
    try {
        ...
        throw new ImageResizeError(`Unable to compress image (${(0, format_js_1.formatFileSize)(imageBuffer.length)}) to fit within ${(0, format_js_1.formatFileSize)(maxBytes)}. ` +
 ...
 */
async function compressImageBlock(imageBlock, maxBytes = apiLimits_js_1.IMAGE_TARGET_RAW_SIZE) {
    // Only process base64 images
    if (imageBlock.source.type !== 'base64') {
        return imageBlock;
    }
