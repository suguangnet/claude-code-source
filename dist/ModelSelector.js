...
function ModelSelector(t0) {
    ...
    if ($[0] !== initialModel) {
        bb0: {
            const base = (0, agent_js_1.getAgentModelOptions)();
    ...
    if ($[2] === Symbol.for("react.memo_cache_sentinel")) {
        t2 = React.createElement(ink_js_1.Box, { marginBottom: 1 },
            React.createElement(ink_js_1.Text, { dimColor: true }, "Model determines the agent's reasoning capabilities and speed."));
        $[2] = t2;
    }
    else {
        t2 = $[2];
    ...
    if ($[6] !== defaultModel || $[7] !== modelOptions || $[8] !== onComplete || $[9] !== t3) {
        t4 = React.createElement(ink_js_1.Box, { flexDirection: "column" },
            t2,
            React.createElement(select_js_1.Select, { options: modelOptions, defaultValue: defaultModel, onChange: onComplete, onCancel: t3 }));
        $[6] = defaultModel;
        $[7] = modelOptions;
        $[8] = onComplete;
        $[9] = t3;
