...
const commentLabel_js_1 = require("../tools/BashTool/commentLabel.js");
...
const teamMemOps = (0, bun_bundle_1.feature)('TEAMMEM')
    ? require('./teamMemoryOps.js')
    : null;
const SNIP_TOOL_NAME = (0, bun_bundle_1.feature)('HISTORY_SNIP')
...
function getToolSearchOrReadInfo(toolName, toolInput, tools) {
    ...
    const tool = (0, Tool_js_1.findToolByName)(tools, toolName) ??
        (0, Tool_js_1.findToolByName)((0, primitiveTools_js_1.getReplPrimitiveTools)(), toolName);
    ...
    return {
        isCollapsible: isCollapsible ||
            ((0, fullscreen_js_1.isFullscreenEnvEnabled)() ? toolName === toolName_js_1.BASH_TOOL_NAME : false),
        ...
        isBash: (0, fullscreen_js_1.isFullscreenEnvEnabled)()
...
function scanBashResultForGitOps(msg, group) {
    ...
    for (const c of msg.message.content) {
        ...
        if (!command)
            continue;
        const { commit, push, branch, pr } = (0, gitOperationTracking_js_1.detectGitOperation)(command, combined);
        if (commit)
...
function createCollapsedGroup(group) {
    ...
    const teamMemReadPaths = (0, bun_bundle_1.feature)('TEAMMEM')
    ...
    const teamMemSearchCount = (0, bun_bundle_1.feature)('TEAMMEM')
        ? (group.teamMemorySearchCount ?? 0)
        : 0;
    const teamMemReadCount = (0, bun_bundle_1.feature)('TEAMMEM')
        ? (group.teamMemoryReadFilePaths?.size ?? 0)
        : 0;
    const teamMemWriteCount = (0, bun_bundle_1.feature)('TEAMMEM')
        ? (group.teamMemoryWriteCount ?? 0)
        : 0;
    const result = {
...
function collapseReadSearchGroups(messages, tools) {
    ...
    for (const msg of messages) {
        if (isCollapsibleToolUse(msg, tools)) {
            ...
            if (toolInfo.isMemoryWrite) {
                ...
                if (input?.command) {
                    ...
                    currentGroup.latestDisplayHint =
                        (0, commentLabel_js_1.extractBashCommentLabel)(input.command) ??
                ...
                for (const filePath of filePaths) {
                    ...
                    if ((0, bun_bundle_1.feature)('TEAMMEM') && teamMemOps?.isTeamMemFile(filePath)) {
            ...
            if ((0, fullscreen_js_1.isFullscreenEnvEnabled)() && currentGroup.bashCommands?.size) {
            ...
            currentGroup.relevantMemories ?? (currentGroup.relevantMemories = []);
