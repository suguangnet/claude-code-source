...
function qualifyDependency(dep, declaringPluginId) {
    if ((0, pluginIdentifier_js_1.parsePluginIdentifier)(dep).marketplace)
        return dep;
    const mkt = (0, pluginIdentifier_js_1.parsePluginIdentifier)(declaringPluginId).marketplace;
...
async function resolveDependencyClosure(rootId, lookup, alreadyEnabled, allowedCrossMarketplaces = new Set()) {
    const rootMarketplace = (0, pluginIdentifier_js_1.parsePluginIdentifier)(rootId).marketplace;
    const closure = [];
    ...
    async function walk(id, requiredBy) {
        ...
        const idMarketplace = (0, pluginIdentifier_js_1.parsePluginIdentifier)(id).marketplace;
...
function verifyAndDemote(plugins) {
    ...
    const knownByName = new Set(plugins.map(p => (0, pluginIdentifier_js_1.parsePluginIdentifier)(p.source).name));
    const enabledByName = new Map();
    for (const id of enabled) {
        const n = (0, pluginIdentifier_js_1.parsePluginIdentifier)(id).name;
    ...
    while (changed) {
        ...
        for (const p of plugins) {
            ...
            for (const rawDep of p.manifest.dependencies ?? []) {
                ...
                const isBare = !(0, pluginIdentifier_js_1.parsePluginIdentifier)(dep).marketplace;
...
function findReverseDependents(pluginId, plugins) {
    const { name: targetName } = (0, pluginIdentifier_js_1.parsePluginIdentifier)(pluginId);
    return plugins
        .filter(p => p.enabled &&
        p.source !== pluginId &&
...
function getEnabledPluginIdsForScope(settingSource) {
    return new Set(Object.entries((0, settings_js_1.getSettingsForSource)(settingSource)?.enabledPlugins ?? {})
