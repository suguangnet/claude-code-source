...
const CRON_FILE_REL = (0, path_1.join)('.claude', 'scheduled_tasks.json');
/**
...
function getCronFilePath(dir) {
    return (0, path_1.join)(dir ?? (0, state_js_1.getProjectRoot)(), CRON_FILE_REL);
}
/**
...
async function readCronTasks(dir) {
    ...
    const parsed = (0, json_js_1.safeParseJSON)(raw, false);
    ...
    for (const t of file.tasks) {
        if (!t ||
            ...
            (0, debug_js_1.logForDebugging)(`[ScheduledTasks] skipping malformed task: ${(0, slowOperations_js_1.jsonStringify)(t)}`);
            continue;
        }
        if (!(0, cron_js_1.parseCronExpression)(t.cron)) {
            (0, debug_js_1.logForDebugging)(`[ScheduledTasks] skipping task ${t.id} with invalid cron '${t.cron}'`);
            continue;
        }
        out.push({
            id: t.id,
...
async function writeCronTasks(tasks, dir) {
    const root = dir ?? (0, state_js_1.getProjectRoot)();
    await (0, promises_1.mkdir)((0, path_1.join)(root, '.claude'), { recursive: true });
    ...
    await (0, promises_1.writeFile)(getCronFilePath(root), (0, slowOperations_js_1.jsonStringify)(body, null, 2) + '\n', 'utf-8');
}
/**
...
async function addCronTask(cron, prompt, recurring, durable, agentId) {
    ...
    if (!durable) {
        (0, state_js_1.addSessionCronTask)({ ...task, ...(agentId ? { agentId } : {}) });
        return id;
    }
...
async function listAllCronTasks(dir) {
    ...
    if (dir !== undefined)
        return fileTasks;
    const sessionTasks = (0, state_js_1.getSessionCronTasks)().map(t => ({
        ...t,
        durable: false,
    }));
...
function nextCronRunMs(cron, fromMs) {
    const fields = (0, cron_js_1.parseCronExpression)(cron);
    if (!fields)
        return null;
    const next = (0, cron_js_1.computeNextCronRun)(fields, new Date(fromMs));
 ...
 */
function jitteredNextCronRunMs(cron, fromMs, taskId, cfg = exports.DEFAULT_CRON_JITTER_CONFIG) {
 ...
 */
function oneShotJitteredNextCronRunMs(cron, fromMs, taskId, cfg = exports.DEFAULT_CRON_JITTER_CONFIG) {
    const t1 = nextCronRunMs(cron, fromMs);
    if (t1 === null)
        return null;
