...
const readEditContext_js_1 = require("../utils/readEditContext.js");
const stringUtils_js_1 = require("../utils/stringUtils.js");
const StructuredDiffList_js_1 = require("./StructuredDiffList.js");
function FileEditToolDiff(props) {
    ...
    const [dataPromise] = (0, react_1.useState)(t0);
    let t1;
    ...
    if ($[4] !== dataPromise || $[5] !== props.file_path) {
        t2 = React.createElement(react_1.Suspense, { fallback: t1 },
...
function DiffBody(t0) {
    ...
    const { patch, firstLine, fileContent } = (0, react_1.use)(promise);
    ...
    if ($[0] !== columns || $[1] !== fileContent || $[2] !== file_path || $[3] !== firstLine || $[4] !== patch) {
        t1 = React.createElement(DiffFrame, null,
            React.createElement(StructuredDiffList_js_1.StructuredDiffList, { hunks: patch, dim: false, width: columns, filePath: file_path, firstLine: firstLine, fileContent: fileContent }));
        $[0] = columns;
        $[1] = fileContent;
        $[2] = file_path;
        $[3] = firstLine;
...
function DiffFrame(t0) {
    ...
    if ($[0] !== children || $[1] !== placeholder) {
        t1 = placeholder ? React.createElement(ink_js_1.Text, { dimColor: true }, "\u2026") : children;
        $[0] = children;
        $[1] = placeholder;
        $[2] = t1;
    }
    ...
    let t2;
    if ($[3] !== t1) {
        t2 = React.createElement(ink_js_1.Box, { flexDirection: "column" },
            React.createElement(ink_js_1.Box, { borderColor: "subtle", borderStyle: "dashed", flexDirection: "column", borderLeft: false, borderRight: false }, t1));
        $[3] = t1;
        $[4] = t2;
    }
    else {
...
async function loadDiffData(file_path, edits) {
    ...
    try {
        const handle = await (0, readEditContext_js_1.openForScan)(file_path);
        if (handle === null)
        ...
        try {
            ...
            if (!single || single.old_string === '') {
                const file = await (0, readEditContext_js_1.readCapped)(handle);
                if (file === null)
                    return diffToolInputsOnly(file_path, valid);
                ...
                return {
                    ...
                    firstLine: (0, stringUtils_js_1.firstLineOf)(file),
                    fileContent: file
                };
            }
            const ctx = await (0, readEditContext_js_1.scanForContext)(handle, single.old_string, diff_js_1.CONTEXT_LINES);
            if (ctx.truncated || ctx.content === '') {
                return diffToolInputsOnly(file_path, [single]);
            }
            const normalized = normalizeEdit(ctx.content, single);
            const hunks = (0, diff_js_1.getPatchForDisplay)({
                filePath: file_path,
...
function diffToolInputsOnly(filePath, edits) {
    return {
        patch: edits.flatMap(e => (0, diff_js_1.getPatchForDisplay)({
            filePath,
...
function normalizeEdit(fileContent, edit) {
    const actualOld = (0, utils_js_1.findActualString)(fileContent, edit.old_string) || edit.old_string;
    const actualNew = (0, utils_js_1.preserveQuoteStyle)(edit.old_string, actualOld, edit.new_string);
    return {
        ...edit,
