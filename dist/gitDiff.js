...
async function fetchGitDiff() {
    ...
    const { stdout: shortstatOut, code: shortstatCode } = await (0, execFileNoThrow_js_1.execFileNoThrow)((0, git_js_1.gitExe)(), ['--no-optional-locks', 'diff', 'HEAD', '--shortstat'], { timeout: GIT_TIMEOUT_MS, preserveOutputOnError: false });
    if (shortstatCode === 0) {
    ...
    const { stdout: numstatOut, code: numstatCode } = await (0, execFileNoThrow_js_1.execFileNoThrow)((0, git_js_1.gitExe)(), ['--no-optional-locks', 'diff', 'HEAD', '--numstat'], { timeout: GIT_TIMEOUT_MS, preserveOutputOnError: false });
    if (numstatCode !== 0)
        return null;
...
async function fetchGitDiffHunks() {
    ...
    const { stdout: diffOut, code: diffCode } = await (0, execFileNoThrow_js_1.execFileNoThrow)((0, git_js_1.gitExe)(), ['--no-optional-locks', 'diff', 'HEAD'], { timeout: GIT_TIMEOUT_MS, preserveOutputOnError: false });
    if (diffCode !== 0) {
        return new Map();
    }
...
async function isInTransientGitState() {
    const gitDir = await (0, git_js_1.getGitDir)((0, cwd_js_1.getCwd)());
    if (!gitDir)
        return false;
    ...
    const results = await Promise.all(transientFiles.map(file => (0, promises_1.access)((0, path_1.join)(gitDir, file))
        .then(() => true)
        .catch(() => false)));
...
async function fetchUntrackedFiles(maxFiles) {
    ...
    const { stdout, code } = await (0, execFileNoThrow_js_1.execFileNoThrow)((0, git_js_1.gitExe)(), ['--no-optional-locks', 'ls-files', '--others', '--exclude-standard'], { timeout: GIT_TIMEOUT_MS, preserveOutputOnError: false });
...
async function fetchSingleFileGitDiff(absoluteFilePath) {
    const gitRoot = (0, git_js_1.findGitRoot)((0, path_1.dirname)(absoluteFilePath));
    if (!gitRoot)
        return null;
    const gitPath = (0, path_1.relative)(gitRoot, absoluteFilePath).split(path_1.sep).join('/');
    const repository = (0, detectRepository_js_1.getCachedRepository)();
    // Check if the file is tracked by git
    const { code: lsFilesCode } = await (0, execFileNoThrow_js_1.execFileNoThrowWithCwd)((0, git_js_1.gitExe)(), ['--no-optional-locks', 'ls-files', '--error-unmatch', gitPath], { cwd: gitRoot, timeout: SINGLE_FILE_DIFF_TIMEOUT_MS });
    if (lsFilesCode === 0) {
        // File is tracked - diff against merge base for PR-like view
        const diffRef = await getDiffRef(gitRoot);
        const { stdout, code } = await (0, execFileNoThrow_js_1.execFileNoThrowWithCwd)((0, git_js_1.gitExe)(), ['--no-optional-locks', 'diff', diffRef, '--', gitPath], { cwd: gitRoot, timeout: SINGLE_FILE_DIFF_TIMEOUT_MS });
        if (code !== 0)
            return null;
        if (!stdout)
            return null;
...
async function getDiffRef(gitRoot) {
    const baseBranch = process.env.CLAUDE_CODE_BASE_REF || (await (0, git_js_1.getDefaultBranch)());
    const { stdout, code } = await (0, execFileNoThrow_js_1.execFileNoThrowWithCwd)((0, git_js_1.gitExe)(), ['--no-optional-locks', 'merge-base', 'HEAD', baseBranch], { cwd: gitRoot, timeout: SINGLE_FILE_DIFF_TIMEOUT_MS });
    if (code === 0 && stdout.trim()) {
        return stdout.trim();
    }
    return 'HEAD';
...
async function generateSyntheticDiff(gitPath, absoluteFilePath) {
    try {
        ...
        const content = await (0, promises_1.readFile)(absoluteFilePath, 'utf-8');
