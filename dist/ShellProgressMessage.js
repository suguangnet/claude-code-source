"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ShellProgressMessage = ShellProgressMessage;
const compiler_runtime_1 = require("react/compiler-runtime");
const react_1 = __importDefault(require("react"));
const strip_ansi_1 = __importDefault(require("strip-ansi"));
const ink_js_1 = require("../../ink.js");
const format_js_1 = require("../../utils/format.js");
const MessageResponse_js_1 = require("../MessageResponse.js");
const OffscreenFreeze_js_1 = require("../OffscreenFreeze.js");
const ShellTimeDisplay_js_1 = require("./ShellTimeDisplay.js");
function ShellProgressMessage(t0) {
    const $ = (0, compiler_runtime_1.c)(30);
    const { output, fullOutput, elapsedTimeSeconds, totalLines, totalBytes, timeoutMs, verbose } = t0;
    let t1;
    if ($[0] !== fullOutput) {
        t1 = (0, strip_ansi_1.default)(fullOutput.trim());
        $[0] = fullOutput;
        $[1] = t1;
    }
    else {
        t1 = $[1];
    }
    const strippedFullOutput = t1;
    let lines;
    let t2;
    if ($[2] !== output || $[3] !== strippedFullOutput || $[4] !== verbose) {
        const strippedOutput = (0, strip_ansi_1.default)(output.trim());
        lines = strippedOutput.split("\n").filter(_temp);
        t2 = verbose ? strippedFullOutput : lines.slice(-5).join("\n");
        $[2] = output;
        $[3] = strippedFullOutput;
        $[4] = verbose;
        $[5] = lines;
        $[6] = t2;
    }
    else {
        lines = $[5];
        t2 = $[6];
    }
    const displayLines = t2;
    if (!lines.length) {
        let t3;
        if ($[7] === Symbol.for("react.memo_cache_sentinel")) {
            t3 = react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "Running\u2026 ");
            $[7] = t3;
        }
        else {
            t3 = $[7];
        }
        let t4;
        if ($[8] !== elapsedTimeSeconds || $[9] !== timeoutMs) {
            t4 = react_1.default.createElement(MessageResponse_js_1.MessageResponse, null,
                react_1.default.createElement(OffscreenFreeze_js_1.OffscreenFreeze, null,
                    t3,
                    react_1.default.createElement(ShellTimeDisplay_js_1.ShellTimeDisplay, { elapsedTimeSeconds: elapsedTimeSeconds, timeoutMs: timeoutMs })));
            $[8] = elapsedTimeSeconds;
            $[9] = timeoutMs;
            $[10] = t4;
        }
        else {
            t4 = $[10];
        }
        return t4;
    }
    const extraLines = totalLines ? Math.max(0, totalLines - 5) : 0;
    let lineStatus = "";
    if (!verbose && totalBytes && totalLines) {
        lineStatus = `~${totalLines} lines`;
    }
    else {
        if (!verbose && extraLines > 0) {
            lineStatus = `+${extraLines} lines`;
        }
    }
    const t3 = verbose ? undefined : Math.min(5, lines.length);
    let t4;
    if ($[11] !== displayLines) {
        t4 = react_1.default.createElement(ink_js_1.Text, { dimColor: true }, displayLines);
        $[11] = displayLines;
        $[12] = t4;
    }
    else {
        t4 = $[12];
    }
    let t5;
    if ($[13] !== t3 || $[14] !== t4) {
        t5 = react_1.default.createElement(ink_js_1.Box, { height: t3, flexDirection: "column", overflow: "hidden" }, t4);
        $[13] = t3;
        $[14] = t4;
        $[15] = t5;
    }
    else {
        t5 = $[15];
    }
    let t6;
    if ($[16] !== lineStatus) {
        t6 = lineStatus ? react_1.default.createElement(ink_js_1.Text, { dimColor: true }, lineStatus) : null;
        $[16] = lineStatus;
        $[17] = t6;
    }
    else {
        t6 = $[17];
    }
    let t7;
    if ($[18] !== elapsedTimeSeconds || $[19] !== timeoutMs) {
        t7 = react_1.default.createElement(ShellTimeDisplay_js_1.ShellTimeDisplay, { elapsedTimeSeconds: elapsedTimeSeconds, timeoutMs: timeoutMs });
        $[18] = elapsedTimeSeconds;
        $[19] = timeoutMs;
        $[20] = t7;
    }
    else {
        t7 = $[20];
    }
    let t8;
    if ($[21] !== totalBytes) {
        t8 = totalBytes ? react_1.default.createElement(ink_js_1.Text, { dimColor: true }, (0, format_js_1.formatFileSize)(totalBytes)) : null;
        $[21] = totalBytes;
        $[22] = t8;
    }
    else {
        t8 = $[22];
    }
    let t9;
    if ($[23] !== t6 || $[24] !== t7 || $[25] !== t8) {
        t9 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "row", gap: 1 },
            t6,
            t7,
            t8);
        $[23] = t6;
        $[24] = t7;
        $[25] = t8;
        $[26] = t9;
    }
    else {
        t9 = $[26];
    }
    let t10;
    if ($[27] !== t5 || $[28] !== t9) {
        t10 = react_1.default.createElement(MessageResponse_js_1.MessageResponse, null,
            react_1.default.createElement(OffscreenFreeze_js_1.OffscreenFreeze, null,
                react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
                    t5,
                    t9)));
        $[27] = t5;
        $[28] = t9;
        $[29] = t10;
    }
    else {
        t10 = $[29];
    }
    return t10;
}
function _temp(line) {
    return line;
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
