...
const useVirtualScroll_js_1 = require("../hooks/useVirtualScroll.js");
const ink_js_1 = require("../ink.js");
const ThemedText_js_1 = require("./design-system/ThemedText.js");
const FullscreenLayout_js_1 = require("./FullscreenLayout.js");
// Rows of breathing room above the target when we scrollTo.
const HEADROOM = 3;
const debug_js_1 = require("../utils/debug.js");
const sleep_js_1 = require("../utils/sleep.js");
const transcriptSearch_js_1 = require("../utils/transcriptSearch.js");
...
function defaultExtractSearchText(msg) {
    ...
    if (cached !== undefined)
        return cached;
    const lowered = (0, transcriptSearch_js_1.renderableSearchText)(msg);
...
function computeStickyPromptText(msg) {
    ...
    if (raw === null)
        return null;
    const t = (0, messageActions_js_1.stripSystemReminders)(raw);
    if (t.startsWith('<') || t === '')
        return null;
    return t;
}
...
function VirtualItem(t0) {
    const $ = (0, compiler_runtime_1.c)(30);
    ...
    let t9;
    if ($[19] !== t7 || $[20] !== t8) {
        t9 = React.createElement(ThemedText_js_1.TextHoverColorContext.Provider, { value: t7 }, t8);
        $[19] = t7;
        $[20] = t8;
        $[21] = t9;
    }
    ...
    if ($[22] !== t1 || $[23] !== t2 || $[24] !== t3 || $[25] !== t4 || $[26] !== t5 || $[27] !== t6 || $[28] !== t9) {
        t10 = React.createElement(ink_js_1.Box, { ref: t1, flexDirection: "column", backgroundColor: t2, paddingBottom: t3, onClick: t4, onMouseEnter: t5, onMouseLeave: t6 }, t9);
        $[22] = t1;
        $[23] = t2;
        $[24] = t3;
        $[25] = t4;
...
function VirtualMessageList({ messages, scrollRef, columns, itemKey, renderItem, onItemClick, isItemClickable, isItemExpanded, extractSearchText = defaultExtractSearchText, trackStickyPrompt, selectedIndex, cursorNavRef, setCursor, jumpRef, onSearchMatchesChange, scanElement, setPositions }) {
    ...
    const keysRef = (0, react_1.useRef)([]);
    const prevMessagesRef = (0, react_1.useRef)(messages);
    const prevItemKeyRef = (0, react_1.useRef)(itemKey);
    ...
    const { range, topSpacer, bottomSpacer, measureRef, spacerRef, offsets, getItemTop, getItemElement, getItemHeight, scrollToIndex } = (0, useVirtualScroll_js_1.useVirtualScroll)(scrollRef, keys, columns);
    const [start, end] = range;
    // Unmeasured (undefined height) falls through — assume visible.
    const isVisible = (0, react_1.useCallback)((i) => {
        const h = getItemHeight(i);
        if (h === 0)
            return false;
        return (0, messageActions_js_1.isNavigableMessage)(messages[i]);
    }, [getItemHeight, messages]);
    (0, react_1.useImperativeHandle)(cursorNavRef, () => {
        const select = (m) => setCursor?.({
            uuid: m.uuid,
            msgType: m.type,
            expanded: false,
            toolName: (0, messageActions_js_1.toolCallOf)(m)?.name
        });
    ...
    const jumpState = (0, react_1.useRef)({
        offsets,
        start,
        getItemElement,
        getItemTop,
    ...
    const scanRequestRef = (0, react_1.useRef)(null);
    ...
    const elementPositions = (0, react_1.useRef)({
        msgIdx: -1,
        positions: []
    });
    // Wraparound guard. Auto-advance stops if ptr wraps back to here.
    const startPtrRef = (0, react_1.useRef)(-1);
    // Phantom-burst cap. Resets on scan success.
    const phantomBurstRef = (0, react_1.useRef)(0);
    ...
    const pendingStepRef = (0, react_1.useRef)(0);
    ...
    const highlightRef = (0, react_1.useRef)(() => { });
    const searchState = (0, react_1.useRef)({
        matches: [],
    ...
    const searchAnchor = (0, react_1.useRef)(-1);
    const indexWarmed = (0, react_1.useRef)(false);
    ...
    function highlight(ord) {
        ...
        (0, debug_js_1.logForDebugging)(`highlight(i=${msgIdx}, ord=${idx}/${positions.length}): ` + `pos={row:${p.row},col:${p.col}} lo=${lo} screenRow=${screenRow} ` + `badge=${current}/${total}`);
    }
    ...
    const [seekGen, setSeekGen] = (0, react_1.useState)(0);
    const bumpSeek = (0, react_1.useCallback)(() => setSeekGen(g => g + 1), []);
    (0, react_1.useEffect)(() => {
        ...
        if (!el || h === 0) {
            ...
            if (tries > 1) {
                ...
                (0, debug_js_1.logForDebugging)(`seek(i=${idx}): no mount after scrollToIndex, skip`);
        ...
        (0, debug_js_1.logForDebugging)(`seek(i=${idx} t=${tries}): ${positions.length} positions`);
    ...
    function step(delta) {
        ...
        if (ptr === startPtrRef.current) {
            setPositions?.(null);
            startPtrRef.current = -1;
            (0, debug_js_1.logForDebugging)(`step: wraparound at ptr=${ptr}, all ${matches.length} msgs phantoms`);
            return;
        }
        st.ptr = ptr;
    ...
    (0, react_1.useImperativeHandle)(jumpRef, () => ({
        ...
        setSearchQuery: (q) => {
            ...
            if (matches.length > 0 && s) {
                ...
                (0, debug_js_1.logForDebugging)(`setSearchQuery('${q}'): ${matches.length} msgs · ptr=${ptr} ` + `msgIdx=${matches[ptr]} curTop=${curTop} origin=${origin}`);
            }
            searchState.current = {
                matches,
                ptr,
        ...
        warmSearchIndex: async () => {
            ...
            (0, debug_js_1.logForDebugging)(`warmSearchIndex: ${msgs.length} msgs · work=${Math.round(workMs)}ms wall=${wallMs}ms chunks=${Math.ceil(msgs.length / CHUNK)}`);
    ...
    const [hoveredKey, setHoveredKey] = (0, react_1.useState)(null);
    ...
    const onClickK = (0, react_1.useCallback)((msg, cellIsBlank) => {
        const h = handlersRef.current;
        if (!cellIsBlank && h.onItemClick)
            h.onItemClick(msg);
    }, []);
    const onEnterK = (0, react_1.useCallback)((k) => {
        handlersRef.current.setHoveredKey(k);
    }, []);
    const onLeaveK = (0, react_1.useCallback)((k) => {
        handlersRef.current.setHoveredKey(prev => prev === k ? null : prev);
    }, []);
    return React.createElement(React.Fragment, null,
        React.createElement(ink_js_1.Box, { ref: spacerRef, height: topSpacer, flexShrink: 0 }),
        ...
        bottomSpacer > 0 && React.createElement(ink_js_1.Box, { height: bottomSpacer, flexShrink: 0 }),
...
function StickyTracker({ messages, start, end, offsets, getItemTop, getItemElement, scrollRef }) {
    const { setStickyPrompt } = (0, react_1.useContext)(FullscreenLayout_js_1.ScrollChromeContext);
    ...
    const subscribe = (0, react_1.useCallback)((listener) => scrollRef.current?.subscribe(listener) ?? NOOP_UNSUB, [scrollRef]);
    (0, react_1.useSyncExternalStore)(subscribe, () => {
        const s = scrollRef.current;
        if (!s)
            return NaN;
    ...
    const suppress = (0, react_1.useRef)('none');
    ...
    const lastIdx = (0, react_1.useRef)(-1);
