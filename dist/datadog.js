...
async function flushLogs() {
    ...
    logBatch = [];
    try {
        await axios_1.default.post(DATADOG_LOGS_ENDPOINT, logsToSend, {
            headers: {
...
exports.initializeDatadog = (0, memoize_js_1.default)(async () => {
...
async function trackDatadogEvent(eventName, properties) {
    ...
    if (initialized === null) {
        initialized = await (0, exports.initializeDatadog)();
    }
    if (!initialized || !DATADOG_ALLOWED_EVENTS.has(eventName)) {
        return;
    }
    try {
        const metadata = await (0, metadata_js_1.getEventMetadata)({
        ...
        if (process.env.USER_TYPE !== 'ant' && typeof allData.model === 'string') {
            const shortName = (0, model_js_1.getCanonicalName)(allData.model.replace(/\[1m]$/i, ''));
 ...
 */
const getUserBucket = (0, memoize_js_1.default)(() => {
    const userId = (0, config_js_1.getOrCreateUserID)();
    const hash = (0, crypto_1.createHash)('sha256').update(userId).digest('hex');
