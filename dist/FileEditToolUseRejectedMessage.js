...
function FileEditToolUseRejectedMessage(t0) {
    const $ = (0, compiler_runtime_1.c)(38);
    ...
    let t1;
    if ($[0] !== operation) {
        t1 = React.createElement(ink_js_1.Text, { color: "subtle" },
            "User rejected ",
            operation,
            " to ");
        $[0] = operation;
    ...
    let t3;
    if ($[5] !== t2) {
        t3 = React.createElement(ink_js_1.Text, { bold: true, color: "subtle" }, t2);
        $[5] = t2;
        $[6] = t3;
    }
    else {
    ...
    if (style === "condensed" && !verbose) {
        let t5;
        if ($[10] !== text) {
            t5 = React.createElement(MessageResponse_js_1.MessageResponse, null, text);
            $[10] = text;
            $[11] = t5;
        }
        else {
    ...
    if (operation === "write" && content !== undefined) {
        ...
        if ($[16] !== file_path || $[17] !== t6 || $[18] !== t7) {
            t8 = React.createElement(HighlightedCode_js_1.HighlightedCode, { code: t6, filePath: file_path, width: t7, dim: true });
            $[16] = file_path;
            $[17] = t6;
            $[18] = t7;
            $[19] = t8;
        ...
        if ($[20] !== plusLines || $[21] !== verbose) {
            t9 = !verbose && plusLines > 0 && React.createElement(ink_js_1.Text, { dimColor: true },
                "\u2026 +",
                plusLines,
                " lines");
            $[20] = plusLines;
        ...
        if ($[23] !== t8 || $[24] !== t9 || $[25] !== text) {
            t10 = React.createElement(MessageResponse_js_1.MessageResponse, null,
    ...
    if ($[29] !== fileContent || $[30] !== file_path || $[31] !== firstLine || $[32] !== patch || $[33] !== t5) {
        t6 = React.createElement(StructuredDiffList_js_1.StructuredDiffList, { hunks: patch, dim: true, width: t5, filePath: file_path, firstLine: firstLine, fileContent: fileContent });
        $[29] = fileContent;
        $[30] = file_path;
        $[31] = firstLine;
        $[32] = patch;
    ...
    let t7;
    if ($[35] !== t6 || $[36] !== text) {
        t7 = React.createElement(MessageResponse_js_1.MessageResponse, null,
            React.createElement(ink_js_1.Box, { flexDirection: "column" },
                text,
                t6));
        $[35] = t6;
