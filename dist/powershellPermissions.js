...
const parser_js_1 = require("../../utils/powershell/parser.js");
const readOnlyCommandValidation_js_1 = require("../../utils/shell/readOnlyCommandValidation.js");
const gitSafety_js_1 = require("./gitSafety.js");
const modeValidation_js_1 = require("./modeValidation.js");
const pathValidation_js_1 = require("./pathValidation.js");
const powershellSecurity_js_1 = require("./powershellSecurity.js");
...
async function extractCommandName(command) {
    ...
    const parsed = await (0, parser_js_1.parsePowerShellCommand)(trimmed);
    const names = (0, parser_js_1.getAllCommandNames)(parsed);
    return names[0] ?? '';
}
/**
...
function filterRulesByContentsMatchingInput(input, rules, matchMode, behavior) {
    ...
    const inputCmdName = (0, parser_js_1.stripModulePrefix)(rawCmdName);
    const inputCanonical = (0, readOnlyValidation_js_1.resolveToCanonical)(inputCmdName);
    ...
    return Array.from(rules.entries())
        ...
        if (rule.type === 'exact') {
            ...
            const ruleCanonical = (0, readOnlyValidation_js_1.resolveToCanonical)(stripModulePrefixForRule(rawRuleCmdName));
...
function matchingRulesForInput(input, toolPermissionContext, matchMode) {
    const denyRuleByContents = (0, permissions_js_1.getRuleByContentsForToolName)(toolPermissionContext, toolName_js_1.POWERSHELL_TOOL_NAME, 'deny');
    const matchingDenyRules = filterRulesByContentsMatchingInput(input, denyRuleByContents, matchMode, 'deny');
    const askRuleByContents = (0, permissions_js_1.getRuleByContentsForToolName)(toolPermissionContext, toolName_js_1.POWERSHELL_TOOL_NAME, 'ask');
    const matchingAskRules = filterRulesByContentsMatchingInput(input, askRuleByContents, matchMode, 'ask');
    const allowRuleByContents = (0, permissions_js_1.getRuleByContentsForToolName)(toolPermissionContext, toolName_js_1.POWERSHELL_TOOL_NAME, 'allow');
...
function powershellToolCheckExactMatchPermission(input, toolPermissionContext) {
    ...
    if (matchingDenyRules[0] !== undefined) {
        return {
            behavior: 'deny',
            message: `Permission to use ${toolName_js_1.POWERSHELL_TOOL_NAME} with command ${trimmedCommand} has been denied.`,
...
function powershellToolCheckPermission(input, toolPermissionContext) {
    ...
    if (matchingDenyRules[0] !== undefined) {
        return {
            behavior: 'deny',
            message: `Permission to use ${toolName_js_1.POWERSHELL_TOOL_NAME} with command ${command} has been denied.`,
            decisionReason: {
                type: 'rule',
...
async function powershellToolHasPermission(input, context) {
    ...
    const parsed = await (0, parser_js_1.parsePowerShellCommand)(command);
    ...
    if (exactMatchResult.behavior === 'allow' &&
        ...
        (0, parser_js_1.classifyCommandName)(command.split(/\s+/)[0] ?? '') !== 'application') {
        return exactMatchResult;
    }
        ...
        for (const fragment of backtickStripped.split(/[;|\n\r{}()&]+/)) {
            ...
            if ((0, readOnlyValidation_js_1.resolveToCanonical)(firstTok) === 'remove-item') {
                for (const arg of normalized.split(/\s+/).slice(1)) {
                    if (parser_js_1.PS_TOKENIZER_DASH_CHARS.has(arg[0] ?? ''))
                        continue;
                    if ((0, pathValidation_js_1.isDangerousRemovalRawPath)(arg)) {
                        return (0, pathValidation_js_1.dangerousRemovalDeny)(arg);
                    }
                }
            }
    ...
    const safetyResult = (0, powershellSecurity_js_1.powershellCommandIsSafe)(command, parsed);
    ...
    const hasCdSubCommand = allSubCommands.length > 1 &&
        allSubCommands.some(({ element }) => (0, readOnlyValidation_js_1.isCwdChangingCmdlet)(element.name));
    ...
    const hasSymlinkCreate = allSubCommands.length > 1 &&
        allSubCommands.some(({ element }) => (0, modeValidation_js_1.isSymlinkCreatingCommand)(element));
    const hasGitSubCommand = allSubCommands.some(({ element }) => (0, readOnlyValidation_js_1.resolveToCanonical)(element.name) === 'git');
    ...
    if (hasGitSubCommand) {
        const writesToGitInternal = allSubCommands.some(({ element, statement }) => {
            ...
            const canonical = (0, readOnlyValidation_js_1.resolveToCanonical)(element.name);
            ...
            if (element.args
                .flatMap(a => a.split(','))
                .some(a => (0, gitSafety_js_1.isGitInternalPathPS)(a))) {
                return true;
            }
        ...
        const redirWritesToGitInternal = (0, parser_js_1.getFileRedirections)(parsed).some(r => (0, gitSafety_js_1.isGitInternalPathPS)(r.target));
    ...
    // is a common project dirname), `.git/` is unambiguous.
    {
        const found = allSubCommands.some(({ element }) => {
            ...
            if (!GIT_SAFETY_WRITE_CMDLETS.has(canonical))
                return false;
            return element.args.flatMap(a => a.split(',')).some(gitSafety_js_1.isDotGitPathPS);
        }) || (0, parser_js_1.getFileRedirections)(parsed).some(r => (0, gitSafety_js_1.isDotGitPathPS)(r.target));
        if (found) {
            decisions.push({
                behavior: 'ask',
    ...
    const pathResult = (0, pathValidation_js_1.checkPathConstraints)(input, parsed, toolPermissionContext, hasCdSubCommand);
    ...
    const fileRedirections = (0, parser_js_1.getFileRedirections)(parsed);
    ...
    const modeResult = (0, modeValidation_js_1.checkPermissionMode)(input, parsed, toolPermissionContext);
    ...
    const subCommands = allSubCommands.filter(({ element, isSafeOutput }) => {
        ...
        if (canonical === 'set-location' && element.args.length > 0) {
            ...
            const target = element.args.find(a => a.length === 0 || !parser_js_1.PS_TOKENIZER_DASH_CHARS.has(a[0]));
    ...
    for (const { text: subCmd, element, statement } of subCommands) {
        ...
        if (statement !== null && !hasCdSubCommand && !hasSymlinkCreate) {
            const subModeResult = (0, modeValidation_js_1.checkPermissionMode)({ command: subCmd }, {
                valid: true,
                errors: [],
