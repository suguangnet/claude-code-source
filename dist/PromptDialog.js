...
function PromptDialog(t0) {
    ...
    if ($[3] !== toolInputSummary) {
        t3 = toolInputSummary ? React.createElement(ink_js_1.Text, { dimColor: true }, toolInputSummary) : undefined;
        $[3] = toolInputSummary;
        $[4] = t3;
    }
    else {
    ...
    let t5;
    if ($[7] !== options || $[8] !== t4) {
        t5 = React.createElement(ink_js_1.Box, { flexDirection: "column", paddingY: 1 },
            React.createElement(select_js_1.Select, { options: options, onChange: t4 }));
        $[7] = options;
        $[8] = t4;
        $[9] = t5;
    }
    ...
    if ($[10] !== request.message || $[11] !== t3 || $[12] !== t5 || $[13] !== title) {
        t6 = React.createElement(PermissionDialog_js_1.PermissionDialog, { title: title, subtitle: request.message, titleRight: t3 }, t5);
        $[10] = request.message;
        $[11] = t3;
        $[12] = t5;
        $[13] = title;
