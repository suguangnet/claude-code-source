...
const FallbackToolUseErrorMessage_js_1 = require("../../components/FallbackToolUseErrorMessage.js");
const FallbackToolUseRejectedMessage_js_1 = require("../../components/FallbackToolUseRejectedMessage.js");
const MessageResponse_js_1 = require("../../components/MessageResponse.js");
...
const outputFormatting_js_1 = require("../../utils/task/outputFormatting.js");
const UI_js_1 = require("../AgentTool/UI.js");
const BashToolResultMessage_js_1 = __importDefault(require("../BashTool/BashToolResultMessage.js"));
...
async function getTaskOutputData(task) {
    ...
    if (task.type === 'local_bash') {
        ...
        if (taskOutputObj) {
            ...
            output = await (0, diskOutput_js_1.getTaskOutput)(task.id);
        }
    }
    else {
        output = await (0, diskOutput_js_1.getTaskOutput)(task.id);
    ...
    if (task.type === 'local_agent') {
        ...
        const cleanResult = agentTask.result ? (0, messages_js_1.extractTextContent)(agentTask.result.content, '\n') : undefined;
        return {
            ...baseOutput,
...
- Works with all task types: background shells, async agents, and remote sessions`;
    ...
    async call(input, toolUseContext, _canUseTool, _parentMessage, onProgress) {
        ...
        if (!block) {
            ...
            if (task.status !== 'running' && task.status !== 'pending') {
                // Mark as notified
                (0, framework_js_1.updateTaskState)(task_id, toolUseContext.setAppState, t => ({
                    ...t,
                    notified: true
                }));
                return {
    ...
    mapToolResultToToolResultBlockParam(data, toolUseID) {
        ...
        if (data.task) {
            ...
            if (data.task.output?.trim()) {
                const { content } = (0, outputFormatting_js_1.formatTaskOutput)(data.task.output, data.task.task_id);
                parts.push(`<output>\n${content.trimEnd()}\n</output>`);
            }
            if (data.task.error) {
    ...
    renderToolUseTag(input) {
        ...
        return react_1.default.createElement(ink_js_1.Text, { dimColor: true },
            " ",
            input.task_id);
    },
    renderToolUseProgressMessage(progressMessages) {
        ...
        return react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
            progressData?.taskDescription && react_1.default.createElement(ink_js_1.Text, null,
                "\u00A0\u00A0",
                progressData.taskDescription),
            react_1.default.createElement(ink_js_1.Text, null,
                "\u00A0\u00A0\u00A0\u00A0\u00A0Waiting for task",
                ' ',
                react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "(esc to give additional instructions)")));
    },
    renderToolResultMessage(content, _, { verbose, theme }) {
        return react_1.default.createElement(TaskOutputResultDisplay, { content: content, verbose: verbose, theme: theme });
    },
    renderToolUseRejectedMessage() {
        return react_1.default.createElement(FallbackToolUseRejectedMessage_js_1.FallbackToolUseRejectedMessage, null);
    },
    renderToolUseErrorMessage(result, { verbose }) {
        return react_1.default.createElement(FallbackToolUseErrorMessage_js_1.FallbackToolUseErrorMessage, { result: result, verbose: verbose });
    }
});
function TaskOutputResultDisplay(t0) {
    const $ = (0, compiler_runtime_1.c)(54);
    ...
    if (!result.task) {
        ...
        if ($[2] === Symbol.for("react.memo_cache_sentinel")) {
            t3 = react_1.default.createElement(MessageResponse_js_1.MessageResponse, null,
                react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "No task output available"));
            $[2] = t3;
        }
        else {
            t3 = $[2];
    ...
    if (task.task_type === "local_bash") {
        ...
        if ($[6] !== bashOut || $[7] !== verbose) {
            t4 = react_1.default.createElement(BashToolResultMessage_js_1.default, { content: bashOut, verbose: verbose });
            $[6] = bashOut;
            $[7] = verbose;
            $[8] = t4;
        }
    ...
    if (task.task_type === "local_agent") {
        const lineCount = task.result ? (0, stringUtils_js_1.countCharInString)(task.result, "\n") + 1 : 0;
        if (result.retrieval_status === "success") {
            if (verbose) {
                let t3;
                if ($[9] !== lineCount || $[10] !== task.description) {
                    t3 = react_1.default.createElement(ink_js_1.Text, null,
                        task.description,
                        " (",
                        lineCount,
                        " lines)");
                ...
                if ($[12] !== task.prompt || $[13] !== theme) {
                    t4 = task.prompt && react_1.default.createElement(UI_js_1.AgentPromptDisplay, { prompt: task.prompt, theme: theme, dim: true });
                    $[12] = task.prompt;
                    $[13] = theme;
                    $[14] = t4;
                }
                ...
                if ($[15] !== task.result || $[16] !== theme) {
                    t5 = task.result && react_1.default.createElement(ink_js_1.Box, { marginTop: 1 },
                        react_1.default.createElement(UI_js_1.AgentResponseDisplay, { content: [{
                                    type: "text",
                                    text: task.result
                                }], theme: theme }));
                    $[15] = task.result;
                ...
                let t6;
                if ($[18] !== task.error) {
                    t6 = task.error && react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: 1 },
                        react_1.default.createElement(ink_js_1.Text, { color: "error", bold: true }, "Error:"),
                        react_1.default.createElement(ink_js_1.Box, { paddingLeft: 2 },
                            react_1.default.createElement(ink_js_1.Text, { color: "error" }, task.error)));
                    $[18] = task.error;
                    $[19] = t6;
                }
                else {
                ...
                if ($[20] !== t4 || $[21] !== t5 || $[22] !== t6) {
                    t7 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", paddingLeft: 2, marginTop: 1 },
                        t4,
                        t5,
                        t6);
                    $[20] = t4;
                ...
                let t8;
                if ($[24] !== t3 || $[25] !== t7) {
                    t8 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
                        t3,
                        t7);
                    $[24] = t3;
                    $[25] = t7;
        ...
        if (result.retrieval_status === "timeout" || task.status === "running") {
            ...
            if ($[29] === Symbol.for("react.memo_cache_sentinel")) {
                t3 = react_1.default.createElement(MessageResponse_js_1.MessageResponse, null,
                    react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "Task is still running\u2026"));
                $[29] = t3;
            }
            else {
                t3 = $[29];
        ...
        if ($[31] === Symbol.for("react.memo_cache_sentinel")) {
            t3 = react_1.default.createElement(MessageResponse_js_1.MessageResponse, null,
                react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "Task not ready"));
            $[31] = t3;
        }
        else {
            t3 = $[31];
    ...
    if (task.task_type === "remote_agent") {
        ...
        if ($[35] !== task.output || $[36] !== verbose) {
            t4 = task.output && verbose && react_1.default.createElement(ink_js_1.Box, { paddingLeft: 4, marginTop: 1 },
                react_1.default.createElement(ink_js_1.Text, null, task.output));
            $[35] = task.output;
            $[36] = verbose;
            $[37] = t4;
        }
        ...
        if ($[38] !== expandShortcut || $[39] !== task.output || $[40] !== verbose) {
            t5 = !verbose && task.output && react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                "     ",
                "(",
                expandShortcut,
                " to expand)");
        ...
        if ($[42] !== t3 || $[43] !== t4 || $[44] !== t5) {
            t6 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
                t3,
                t4,
                t5);
            $[42] = t3;
    ...
    let t4;
    if ($[49] !== task.output) {
        t4 = task.output && react_1.default.createElement(ink_js_1.Box, { paddingLeft: 4 },
            react_1.default.createElement(ink_js_1.Text, null, task.output.slice(0, 500)));
        $[49] = task.output;
        $[50] = t4;
    }
    else {
