"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.PermissionRequest = PermissionRequest;
const compiler_runtime_1 = require("react/compiler-runtime");
const bun_bundle_1 = require("bun:bundle");
const React = __importStar(require("react"));
const EnterPlanModeTool_js_1 = require("src/tools/EnterPlanModeTool/EnterPlanModeTool.js");
const ExitPlanModeV2Tool_js_1 = require("src/tools/ExitPlanModeTool/ExitPlanModeV2Tool.js");
const useNotifyAfterTimeout_js_1 = require("../../hooks/useNotifyAfterTimeout.js");
const useKeybinding_js_1 = require("../../keybindings/useKeybinding.js");
const AskUserQuestionTool_js_1 = require("../../tools/AskUserQuestionTool/AskUserQuestionTool.js");
const BashTool_js_1 = require("../../tools/BashTool/BashTool.js");
const FileEditTool_js_1 = require("../../tools/FileEditTool/FileEditTool.js");
const FileReadTool_js_1 = require("../../tools/FileReadTool/FileReadTool.js");
const FileWriteTool_js_1 = require("../../tools/FileWriteTool/FileWriteTool.js");
const GlobTool_js_1 = require("../../tools/GlobTool/GlobTool.js");
const GrepTool_js_1 = require("../../tools/GrepTool/GrepTool.js");
const NotebookEditTool_js_1 = require("../../tools/NotebookEditTool/NotebookEditTool.js");
const PowerShellTool_js_1 = require("../../tools/PowerShellTool/PowerShellTool.js");
const SkillTool_js_1 = require("../../tools/SkillTool/SkillTool.js");
const WebFetchTool_js_1 = require("../../tools/WebFetchTool/WebFetchTool.js");
const AskUserQuestionPermissionRequest_js_1 = require("./AskUserQuestionPermissionRequest/AskUserQuestionPermissionRequest.js");
const BashPermissionRequest_js_1 = require("./BashPermissionRequest/BashPermissionRequest.js");
const EnterPlanModePermissionRequest_js_1 = require("./EnterPlanModePermissionRequest/EnterPlanModePermissionRequest.js");
const ExitPlanModePermissionRequest_js_1 = require("./ExitPlanModePermissionRequest/ExitPlanModePermissionRequest.js");
const FallbackPermissionRequest_js_1 = require("./FallbackPermissionRequest.js");
const FileEditPermissionRequest_js_1 = require("./FileEditPermissionRequest/FileEditPermissionRequest.js");
const FilesystemPermissionRequest_js_1 = require("./FilesystemPermissionRequest/FilesystemPermissionRequest.js");
const FileWritePermissionRequest_js_1 = require("./FileWritePermissionRequest/FileWritePermissionRequest.js");
const NotebookEditPermissionRequest_js_1 = require("./NotebookEditPermissionRequest/NotebookEditPermissionRequest.js");
const PowerShellPermissionRequest_js_1 = require("./PowerShellPermissionRequest/PowerShellPermissionRequest.js");
const SkillPermissionRequest_js_1 = require("./SkillPermissionRequest/SkillPermissionRequest.js");
const WebFetchPermissionRequest_js_1 = require("./WebFetchPermissionRequest/WebFetchPermissionRequest.js");
/* eslint-disable @typescript-eslint/no-require-imports */
const ReviewArtifactTool = (0, bun_bundle_1.feature)('REVIEW_ARTIFACT') ? require('../../tools/ReviewArtifactTool/ReviewArtifactTool.js').ReviewArtifactTool : null;
const ReviewArtifactPermissionRequest = (0, bun_bundle_1.feature)('REVIEW_ARTIFACT') ? require('./ReviewArtifactPermissionRequest/ReviewArtifactPermissionRequest.js').ReviewArtifactPermissionRequest : null;
const WorkflowTool = (0, bun_bundle_1.feature)('WORKFLOW_SCRIPTS') ? require('../../tools/WorkflowTool/WorkflowTool.js').WorkflowTool : null;
const WorkflowPermissionRequest = (0, bun_bundle_1.feature)('WORKFLOW_SCRIPTS') ? require('../../tools/WorkflowTool/WorkflowPermissionRequest.js').WorkflowPermissionRequest : null;
const MonitorTool = (0, bun_bundle_1.feature)('MONITOR_TOOL') ? require('../../tools/MonitorTool/MonitorTool.js').MonitorTool : null;
const MonitorPermissionRequest = (0, bun_bundle_1.feature)('MONITOR_TOOL') ? require('./MonitorPermissionRequest/MonitorPermissionRequest.js').MonitorPermissionRequest : null;
function permissionComponentForTool(tool) {
    switch (tool) {
        case FileEditTool_js_1.FileEditTool:
            return FileEditPermissionRequest_js_1.FileEditPermissionRequest;
        case FileWriteTool_js_1.FileWriteTool:
            return FileWritePermissionRequest_js_1.FileWritePermissionRequest;
        case BashTool_js_1.BashTool:
            return BashPermissionRequest_js_1.BashPermissionRequest;
        case PowerShellTool_js_1.PowerShellTool:
            return PowerShellPermissionRequest_js_1.PowerShellPermissionRequest;
        case ReviewArtifactTool:
            return ReviewArtifactPermissionRequest ?? FallbackPermissionRequest_js_1.FallbackPermissionRequest;
        case WebFetchTool_js_1.WebFetchTool:
            return WebFetchPermissionRequest_js_1.WebFetchPermissionRequest;
        case NotebookEditTool_js_1.NotebookEditTool:
            return NotebookEditPermissionRequest_js_1.NotebookEditPermissionRequest;
        case ExitPlanModeV2Tool_js_1.ExitPlanModeV2Tool:
            return ExitPlanModePermissionRequest_js_1.ExitPlanModePermissionRequest;
        case EnterPlanModeTool_js_1.EnterPlanModeTool:
            return EnterPlanModePermissionRequest_js_1.EnterPlanModePermissionRequest;
        case SkillTool_js_1.SkillTool:
            return SkillPermissionRequest_js_1.SkillPermissionRequest;
        case AskUserQuestionTool_js_1.AskUserQuestionTool:
            return AskUserQuestionPermissionRequest_js_1.AskUserQuestionPermissionRequest;
        case WorkflowTool:
            return WorkflowPermissionRequest ?? FallbackPermissionRequest_js_1.FallbackPermissionRequest;
        case MonitorTool:
            return MonitorPermissionRequest ?? FallbackPermissionRequest_js_1.FallbackPermissionRequest;
        case GlobTool_js_1.GlobTool:
        case GrepTool_js_1.GrepTool:
        case FileReadTool_js_1.FileReadTool:
            return FilesystemPermissionRequest_js_1.FilesystemPermissionRequest;
        default:
            return FallbackPermissionRequest_js_1.FallbackPermissionRequest;
    }
}
function getNotificationMessage(toolUseConfirm) {
    const toolName = toolUseConfirm.tool.userFacingName(toolUseConfirm.input);
    if (toolUseConfirm.tool === ExitPlanModeV2Tool_js_1.ExitPlanModeV2Tool) {
        return 'Claude Code needs your approval for the plan';
    }
    if (toolUseConfirm.tool === EnterPlanModeTool_js_1.EnterPlanModeTool) {
        return 'Claude Code wants to enter plan mode';
    }
    if ((0, bun_bundle_1.feature)('REVIEW_ARTIFACT') && toolUseConfirm.tool === ReviewArtifactTool) {
        return 'Claude needs your approval for a review artifact';
    }
    if (!toolName || toolName.trim() === '') {
        return 'Claude Code needs your attention';
    }
    return `Claude needs your permission to use ${toolName}`;
}
// TODO: Move this to Tool.renderPermissionRequest
function PermissionRequest(t0) {
    const $ = (0, compiler_runtime_1.c)(18);
    const { toolUseConfirm, toolUseContext, onDone, onReject, verbose, workerBadge, setStickyFooter } = t0;
    let t1;
    if ($[0] !== onDone || $[1] !== onReject || $[2] !== toolUseConfirm) {
        t1 = () => {
            onDone();
            onReject();
            toolUseConfirm.onReject();
        };
        $[0] = onDone;
        $[1] = onReject;
        $[2] = toolUseConfirm;
        $[3] = t1;
    }
    else {
        t1 = $[3];
    }
    let t2;
    if ($[4] === Symbol.for("react.memo_cache_sentinel")) {
        t2 = {
            context: "Confirmation"
        };
        $[4] = t2;
    }
    else {
        t2 = $[4];
    }
    (0, useKeybinding_js_1.useKeybinding)("app:interrupt", t1, t2);
    let t3;
    if ($[5] !== toolUseConfirm) {
        t3 = getNotificationMessage(toolUseConfirm);
        $[5] = toolUseConfirm;
        $[6] = t3;
    }
    else {
        t3 = $[6];
    }
    const notificationMessage = t3;
    (0, useNotifyAfterTimeout_js_1.useNotifyAfterTimeout)(notificationMessage, "permission_prompt");
    let t4;
    if ($[7] !== toolUseConfirm.tool) {
        t4 = permissionComponentForTool(toolUseConfirm.tool);
        $[7] = toolUseConfirm.tool;
        $[8] = t4;
    }
    else {
        t4 = $[8];
    }
    const PermissionComponent = t4;
    let t5;
    if ($[9] !== PermissionComponent || $[10] !== onDone || $[11] !== onReject || $[12] !== setStickyFooter || $[13] !== toolUseConfirm || $[14] !== toolUseContext || $[15] !== verbose || $[16] !== workerBadge) {
        t5 = React.createElement(PermissionComponent, { toolUseContext: toolUseContext, toolUseConfirm: toolUseConfirm, onDone: onDone, onReject: onReject, verbose: verbose, workerBadge: workerBadge, setStickyFooter: setStickyFooter });
        $[9] = PermissionComponent;
        $[10] = onDone;
        $[11] = onReject;
        $[12] = setStickyFooter;
        $[13] = toolUseConfirm;
        $[14] = toolUseContext;
        $[15] = verbose;
        $[16] = workerBadge;
        $[17] = t5;
    }
    else {
        t5 = $[17];
    }
    return t5;
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
