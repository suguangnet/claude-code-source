...
const wrapAnsi_js_1 = require("../ink/wrapAnsi.js");
...
const isVimWordChar = (ch) => exports.VIM_WORD_CHAR_REGEX.test(ch);
exports.isVimWordChar = isVimWordChar;
const isVimWhitespace = (ch) => exports.WHITESPACE_REGEX.test(ch);
exports.isVimWhitespace = isVimWhitespace;
const isVimPunctuation = (ch) => ch.length > 0 && !(0, exports.isVimWhitespace)(ch) && !(0, exports.isVimWordChar)(ch);
...
class Cursor {
    ...
    render(cursorChar, mask, invert, ghostText, maxVisibleLines) {
        ...
        return allLines
            ...
            if (mask) {
                const graphemes = Array.from((0, intl_js_1.getGraphemeSegmenter)().segment(text));
            ...
            for (const { segment } of (0, intl_js_1.getGraphemeSegmenter)().segment(displayText)) {
                if (cursorFound) {
                    afterCursor += segment;
                    continue;
                }
                const nextWidth = currentWidth + (0, stringWidth_js_1.stringWidth)(segment);
                if (nextWidth > column) {
                    atCursor = segment;
                    cursorFound = true;
                }
            ...
            if (ghostText &&
                ...
                const firstGhostChar = (0, intl_js_1.firstGrapheme)(ghostText.text) || ghostText.text[0];
    ...
    up() {
        ...
        const prevLineDisplayWidth = (0, stringWidth_js_1.stringWidth)(prevLine);
    ...
    down() {
        ...
        const nextLineDisplayWidth = (0, stringWidth_js_1.stringWidth)(nextLine);
...
class MeasuredText {
    ...
    getGraphemeBoundaries() {
        if (!this.graphemeBoundaries) {
            ...
            for (const { index } of (0, intl_js_1.getGraphemeSegmenter)().segment(this.text)) {
    ...
    getWordBoundaries() {
        if (!this.wordBoundariesCache) {
            ...
            for (const segment of (0, intl_js_1.getWordSegmenter)().segment(this.text)) {
    ...
    displayWidthToStringIndex(text, targetWidth) {
        ...
        for (const { segment, index } of (0, intl_js_1.getGraphemeSegmenter)().segment(text)) {
            const segmentWidth = (0, stringWidth_js_1.stringWidth)(segment);
    ...
    measureWrappedText() {
        const wrappedText = (0, wrapAnsi_js_1.wrapAnsi)(this.text, this.columns, {
            hard: true,
            trim: false,
        });
        const wrappedLines = [];
    ...
    getOffsetFromPosition(position) {
        ...
        const lineDisplayWidth = (0, stringWidth_js_1.stringWidth)(wrappedLine.text);
