...
function stripBashRedirections(command) {
    const { commandWithoutRedirections, redirections } = (0, commands_js_1.extractOutputRedirections)(command);
...
function bashToolUseOptions({ suggestions = [], decisionReason, onRejectFeedbackChange, onAcceptFeedbackChange, onClassifierDescriptionChange, classifierDescription, initialClassifierDescriptionEmpty = false, existingAllowDescriptions = [], yesInputMode = false, noInputMode = false, editablePrefix, onEditablePrefixChange }) {
    const options = [];
    if (yesInputMode) {
        options.push({
            type: 'input',
    ...
    if ((0, permissionsLoader_js_1.shouldShowAlwaysAllowOptions)()) {
        ...
        const hasNonBashSuggestions = suggestions.some(s => s.type === 'addDirectories' || s.type === 'addRules' && s.rules?.some(r => r.toolName !== toolName_js_1.BASH_TOOL_NAME));
        if (editablePrefix !== undefined && onEditablePrefixChange && !hasNonBashSuggestions && suggestions.length > 0) {
            ...
            const label = (0, shellPermissionHelpers_js_1.generateShellSuggestionsLabel)(suggestions, toolName_js_1.BASH_TOOL_NAME, stripBashRedirections);
            if (label) {
                options.push({
                    label,
        ...
        if ("external" === 'ant' && !editablePrefixShown && (0, bashClassifier_js_1.isClassifierPermissionsEnabled)() && onClassifierDescriptionChange && !initialClassifierDescriptionEmpty && !descriptionAlreadyExists(classifierDescription ?? '', existingAllowDescriptions) && decisionReason?.type !== 'classifier') {
            options.push({
                type: 'input',
                label: 'Yes, and don\u2019t ask again for',
