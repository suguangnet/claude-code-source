...
function detectEncodingForResolvedPath(resolvedPath) {
    const { buffer, bytesRead } = (0, fsOperations_js_1.getFsImplementation)().readSync(resolvedPath, {
        length: 4096,
    });
...
function readFileSyncWithMetadata(filePath) {
    ...
    const { resolvedPath, isSymlink } = (0, fsOperations_js_1.safeResolvePath)(fs, filePath);
    if (isSymlink) {
        (0, debug_js_1.logForDebugging)(`Reading through symlink: ${filePath} -> ${resolvedPath}`);
    }
