"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Dialog = Dialog;
const compiler_runtime_1 = require("react/compiler-runtime");
const react_1 = __importDefault(require("react"));
const useExitOnCtrlCDWithKeybindings_js_1 = require("../../hooks/useExitOnCtrlCDWithKeybindings.js");
const ink_js_1 = require("../../ink.js");
const useKeybinding_js_1 = require("../../keybindings/useKeybinding.js");
const ConfigurableShortcutHint_js_1 = require("../ConfigurableShortcutHint.js");
const Byline_js_1 = require("./Byline.js");
const KeyboardShortcutHint_js_1 = require("./KeyboardShortcutHint.js");
const Pane_js_1 = require("./Pane.js");
function Dialog(t0) {
    const $ = (0, compiler_runtime_1.c)(27);
    const { title, subtitle, children, onCancel, color: t1, hideInputGuide, hideBorder, inputGuide, isCancelActive: t2 } = t0;
    const color = t1 === undefined ? "permission" : t1;
    const isCancelActive = t2 === undefined ? true : t2;
    const exitState = (0, useExitOnCtrlCDWithKeybindings_js_1.useExitOnCtrlCDWithKeybindings)(undefined, undefined, isCancelActive);
    let t3;
    if ($[0] !== isCancelActive) {
        t3 = {
            context: "Confirmation",
            isActive: isCancelActive
        };
        $[0] = isCancelActive;
        $[1] = t3;
    }
    else {
        t3 = $[1];
    }
    (0, useKeybinding_js_1.useKeybinding)("confirm:no", onCancel, t3);
    let t4;
    if ($[2] !== exitState.keyName || $[3] !== exitState.pending) {
        t4 = exitState.pending ? react_1.default.createElement(ink_js_1.Text, null,
            "Press ",
            exitState.keyName,
            " again to exit") : react_1.default.createElement(Byline_js_1.Byline, null,
            react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "Enter", action: "confirm" }),
            react_1.default.createElement(ConfigurableShortcutHint_js_1.ConfigurableShortcutHint, { action: "confirm:no", context: "Confirmation", fallback: "Esc", description: "cancel" }));
        $[2] = exitState.keyName;
        $[3] = exitState.pending;
        $[4] = t4;
    }
    else {
        t4 = $[4];
    }
    const defaultInputGuide = t4;
    let t5;
    if ($[5] !== color || $[6] !== title) {
        t5 = react_1.default.createElement(ink_js_1.Text, { bold: true, color: color }, title);
        $[5] = color;
        $[6] = title;
        $[7] = t5;
    }
    else {
        t5 = $[7];
    }
    let t6;
    if ($[8] !== subtitle) {
        t6 = subtitle && react_1.default.createElement(ink_js_1.Text, { dimColor: true }, subtitle);
        $[8] = subtitle;
        $[9] = t6;
    }
    else {
        t6 = $[9];
    }
    let t7;
    if ($[10] !== t5 || $[11] !== t6) {
        t7 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
            t5,
            t6);
        $[10] = t5;
        $[11] = t6;
        $[12] = t7;
    }
    else {
        t7 = $[12];
    }
    let t8;
    if ($[13] !== children || $[14] !== t7) {
        t8 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", gap: 1 },
            t7,
            children);
        $[13] = children;
        $[14] = t7;
        $[15] = t8;
    }
    else {
        t8 = $[15];
    }
    let t9;
    if ($[16] !== defaultInputGuide || $[17] !== exitState || $[18] !== hideInputGuide || $[19] !== inputGuide) {
        t9 = !hideInputGuide && react_1.default.createElement(ink_js_1.Box, { marginTop: 1 },
            react_1.default.createElement(ink_js_1.Text, { dimColor: true, italic: true }, inputGuide ? inputGuide(exitState) : defaultInputGuide));
        $[16] = defaultInputGuide;
        $[17] = exitState;
        $[18] = hideInputGuide;
        $[19] = inputGuide;
        $[20] = t9;
    }
    else {
        t9 = $[20];
    }
    let t10;
    if ($[21] !== t8 || $[22] !== t9) {
        t10 = react_1.default.createElement(react_1.default.Fragment, null,
            t8,
            t9);
        $[21] = t8;
        $[22] = t9;
        $[23] = t10;
    }
    else {
        t10 = $[23];
    }
    const content = t10;
    if (hideBorder) {
        return content;
    }
    let t11;
    if ($[24] !== color || $[25] !== content) {
        t11 = react_1.default.createElement(Pane_js_1.Pane, { color: color }, content);
        $[24] = color;
        $[25] = content;
        $[26] = t11;
    }
    else {
        t11 = $[26];
    }
    return t11;
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
