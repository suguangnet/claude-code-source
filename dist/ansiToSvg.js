...
function parseAnsi(text) {
    ...
    for (const line of rawLines) {
        const spans = [];
        let currentColor = exports.DEFAULT_FG;
        let bold = false;
        let i = 0;
        while (i < line.length) {
...
function ansiToSvg(ansiText, options = {}) {
    const { fontFamily = 'Menlo, Monaco, monospace', fontSize = 14, lineHeight = 22, paddingX = 24, paddingY = 24, backgroundColor = `rgb(${exports.DEFAULT_BG.r}, ${exports.DEFAULT_BG.g}, ${exports.DEFAULT_BG.b})`, borderRadius = 8, } = options;
    ...
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
        ...
        for (const span of spans) {
            ...
            svg += `<tspan fill="${colorStr}"${boldClass}>${(0, xml_js_1.escapeXml)(span.text)}</tspan>`;
        }
        svg += `</text>\n`;
    }
    svg += `</svg>`;
