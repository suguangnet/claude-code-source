...
function formatUri(uri, cwd) {
    ...
    try {
        ...
        (0, debug_js_1.logForDebugging)(`Failed to decode LSP URI '${uri}': ${errorMsg}. Using un-decoded path: ${filePath}`, { level: 'warn' });
    ...
    if (cwd) {
        ...
        const relativePath = (0, path_1.relative)(cwd, filePath).replaceAll('\\', '/');
...
function formatGoToDefinitionResult(result, cwd) {
    ...
    if (Array.isArray(result)) {
        ...
        if (invalidLocations.length > 0) {
            (0, debug_js_1.logForDebugging)(`formatGoToDefinitionResult: Filtering out ${invalidLocations.length} invalid location(s) - this should have been caught earlier`, { level: 'warn' });
        }
...
function formatFindReferencesResult(result, cwd) {
    ...
    if (invalidLocations.length > 0) {
        (0, debug_js_1.logForDebugging)(`formatFindReferencesResult: Filtering out ${invalidLocations.length} invalid location(s) - this should have been caught earlier`, { level: 'warn' });
    }
...
function formatWorkspaceSymbolResult(result, cwd) {
    ...
    if (invalidSymbols.length > 0) {
        (0, debug_js_1.logForDebugging)(`formatWorkspaceSymbolResult: Filtering out ${invalidSymbols.length} invalid symbol(s) - this should have been caught earlier`, { level: 'warn' });
    }
    ...
    const lines = [
        `Found ${validSymbols.length} ${(0, stringUtils_js_1.plural)(validSymbols.length, 'symbol')} in workspace:`,
    ];
    // Group by file
...
function formatIncomingCallsResult(result, cwd) {
    ...
    const lines = [
        `Found ${result.length} incoming ${(0, stringUtils_js_1.plural)(result.length, 'call')}:`,
    ];
    // Group by file
    const byFile = new Map();
...
function formatOutgoingCallsResult(result, cwd) {
    ...
    const lines = [
        `Found ${result.length} outgoing ${(0, stringUtils_js_1.plural)(result.length, 'call')}:`,
    ];
    // Group by file
    const byFile = new Map();
    for (const call of result) {
