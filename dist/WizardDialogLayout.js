"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.WizardDialogLayout = WizardDialogLayout;
const compiler_runtime_1 = require("react/compiler-runtime");
const react_1 = __importDefault(require("react"));
const Dialog_js_1 = require("../design-system/Dialog.js");
const useWizard_js_1 = require("./useWizard.js");
const WizardNavigationFooter_js_1 = require("./WizardNavigationFooter.js");
function WizardDialogLayout(t0) {
    const $ = (0, compiler_runtime_1.c)(11);
    const { title: titleOverride, color: t1, children, subtitle, footerText } = t0;
    const color = t1 === undefined ? "suggestion" : t1;
    const { currentStepIndex, totalSteps, title: providerTitle, showStepCounter, goBack } = (0, useWizard_js_1.useWizard)();
    const title = titleOverride || providerTitle || "Wizard";
    const stepSuffix = showStepCounter !== false ? ` (${currentStepIndex + 1}/${totalSteps})` : "";
    const t2 = `${title}${stepSuffix}`;
    let t3;
    if ($[0] !== children || $[1] !== color || $[2] !== goBack || $[3] !== subtitle || $[4] !== t2) {
        t3 = react_1.default.createElement(Dialog_js_1.Dialog, { title: t2, subtitle: subtitle, onCancel: goBack, color: color, hideInputGuide: true, isCancelActive: false }, children);
        $[0] = children;
        $[1] = color;
        $[2] = goBack;
        $[3] = subtitle;
        $[4] = t2;
        $[5] = t3;
    }
    else {
        t3 = $[5];
    }
    let t4;
    if ($[6] !== footerText) {
        t4 = react_1.default.createElement(WizardNavigationFooter_js_1.WizardNavigationFooter, { instructions: footerText });
        $[6] = footerText;
        $[7] = t4;
    }
    else {
        t4 = $[7];
    }
    let t5;
    if ($[8] !== t3 || $[9] !== t4) {
        t5 = react_1.default.createElement(react_1.default.Fragment, null,
            t3,
            t4);
        $[8] = t3;
        $[9] = t4;
        $[10] = t5;
    }
    else {
        t5 = $[10];
    }
    return t5;
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
