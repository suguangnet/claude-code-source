...
function AnimatedClawd() {
    ...
    let t0;
    if ($[0] !== pose) {
        t0 = React.createElement(Clawd_js_1.Clawd, { pose: pose });
        $[0] = pose;
        $[1] = t0;
    }
    else {
    ...
    if ($[2] !== bounceOffset || $[3] !== t0) {
        t1 = React.createElement(ink_js_1.Box, { marginTop: bounceOffset, flexShrink: 0 }, t0);
        $[2] = bounceOffset;
        $[3] = t0;
        $[4] = t1;
    }
    ...
    let t2;
    if ($[5] !== onClick || $[6] !== t1) {
        t2 = React.createElement(ink_js_1.Box, { height: CLAWD_HEIGHT, flexDirection: "column", onClick: onClick }, t1);
        $[5] = onClick;
        $[6] = t1;
        $[7] = t2;
    }
...
function useClawdAnimation() {
    ...
    const [frameIndex, setFrameIndex] = (0, react_1.useState)(-1);
    const sequenceRef = (0, react_1.useRef)(JUMP_WAVE);
    const onClick = () => {
