...
const statsCache_js_1 = require("./statsCache.js");
/**
...
async function processSessionFiles(sessionFiles, options = {}) {
    ...
    const modelUsageAgg = {};
    const shotDistributionMap = (0, bun_bundle_1.feature)('SHOT_STATS')
        ? new Map()
        : undefined;
    ...
    for (let i = 0; i < sessionFiles.length; i += BATCH_SIZE) {
            ...
            try {
                ...
                if (fromDate) {
                    ...
                    try {
                        ...
                        const fileModifiedDate = (0, statsCache_js_1.toDateString)(fileStat.mtime);
                ...
                const entries = await (0, json_js_1.readJSONLFile)(sessionFile);
        ...
        for (const { sessionFile, entries, error, skipped } of results) {
            if (skipped)
                continue;
            if (error || !entries) {
                (0, debug_js_1.logForDebugging)(`Failed to read session file ${sessionFile}: ${(0, errors_js_1.errorMessage)(error)}`);
                continue;
            }
            const sessionId = (0, path_1.basename)(sessionFile, '.jsonl');
            const messages = [];
            ...
            const isSubagentFile = sessionFile.includes(`${path_1.sep}subagents${path_1.sep}`);
            ...
            if (isNaN(firstTimestamp.getTime()) || isNaN(lastTimestamp.getTime())) {
                (0, debug_js_1.logForDebugging)(`Skipping session with invalid timestamp: ${sessionFile}`);
                continue;
            }
            const dateKey = (0, statsCache_js_1.toDateString)(firstTimestamp);
            // Apply date filters
    ...
    return {
        ...
        ...((0, bun_bundle_1.feature)('SHOT_STATS') && shotDistributionMap
    ...
    const projectResults = await Promise.all(projectDirs.map(async (projectDir) => {
        try {
            ...
            const mainFiles = entries
                ...
                .map(dirent => (0, path_1.join)(projectDir, dirent.name));
            ...
            const subagentResults = await Promise.all(sessionDirs.map(async (sessionDir) => {
                const subagentsDir = (0, path_1.join)(projectDir, sessionDir.name, 'subagents');
                try {
                    ...
                    return subagentEntries
                        ...
                        .map(dirent => (0, path_1.join)(subagentsDir, dirent.name));
                }
                catch {
            ...
            (0, debug_js_1.logForDebugging)(`Failed to read project directory ${projectDir}: ${(0, errors_js_1.errorMessage)(error)}`);
            return [];
        }
    }));
...
async function aggregateClaudeCodeStats() {
    ...
    const updatedCache = await (0, statsCache_js_1.withStatsCacheLock)(async () => {
        // Load the cache
        const cache = await (0, statsCache_js_1.loadStatsCache)();
        const yesterday = (0, statsCache_js_1.getYesterdayDateString)();
        ...
        if (!cache.lastComputedDate) {
            ...
            if (historicalStats.sessionStats.length > 0 ||
                ...
                await (0, statsCache_js_1.saveStatsCache)(result);
            }
        }
        else if ((0, statsCache_js_1.isDateBefore)(cache.lastComputedDate, yesterday)) {
            ...
            (0, debug_js_1.logForDebugging)(`Stats cache stale (${cache.lastComputedDate}), processing ${nextDay} to ${yesterday}`);
    ...
    const today = (0, statsCache_js_1.getTodayDateString)();
...
async function aggregateClaudeCodeStatsForRange(range) {
    ...
    const fromDateStr = (0, statsCache_js_1.toDateString)(fromDate);
...
function calculateStreaks(dailyActivity) {
    ...
    while (true) {
        const dateStr = (0, statsCache_js_1.toDateString)(checkDate);
