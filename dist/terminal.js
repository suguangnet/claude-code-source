...
const CtrlOToExpand_js_1 = require("../components/CtrlOToExpand.js");
const stringWidth_js_1 = require("../ink/stringWidth.js");
const sliceAnsi_js_1 = __importDefault(require("./sliceAnsi.js"));
...
function wrapText(text, wrapWidth) {
    ...
    for (const line of lines) {
        const visibleWidth = (0, stringWidth_js_1.stringWidth)(line);
        if (visibleWidth <= wrapWidth) {
            ...
            while (position < visibleWidth) {
                const chunk = (0, sliceAnsi_js_1.default)(line, position, position + wrapWidth);
...
function renderTruncatedContent(content, terminalWidth, suppressExpandHint = false) {
    ...
    return [
        ...
        estimatedRemaining > 0
            ? chalk_1.default.dim(`… +${estimatedRemaining} lines${suppressExpandHint ? '' : ` ${(0, CtrlOToExpand_js_1.ctrlOToExpand)()}`}`)
            : '',
    ]
        .filter(Boolean)
        .join('\n');
