...
function TeleportProgress(t0) {
    ...
    const [ref, time] = (0, ink_js_1.useAnimationFrame)(100);
    ...
    let t3;
    if ($[2] !== t2) {
        t3 = React.createElement(ink_js_1.Box, { marginBottom: 1 },
            React.createElement(ink_js_1.Text, { bold: true, color: "claude" },
                t2,
                " Teleporting session\u2026"));
        $[2] = t2;
        $[3] = t3;
    ...
    let t4;
    if ($[4] !== sessionId) {
        t4 = sessionId && React.createElement(ink_js_1.Box, { marginBottom: 1 },
            React.createElement(ink_js_1.Text, { dimColor: true }, sessionId));
        $[4] = sessionId;
        $[5] = t4;
    }
    else {
    ...
    if ($[6] !== currentStepIndex || $[7] !== frame) {
        t5 = STEPS.map((step, index) => {
            ...
            return React.createElement(ink_js_1.Box, { key: step.key, flexDirection: "row" },
                React.createElement(ink_js_1.Box, { width: 2 },
                    React.createElement(ink_js_1.Text, { color: color, dimColor: isPending }, icon)),
                React.createElement(ink_js_1.Text, { dimColor: isPending, bold: isCurrent }, step.label));
        });
        $[6] = currentStepIndex;
        $[7] = frame;
        $[8] = t5;
    ...
    let t6;
    if ($[9] !== t5) {
        t6 = React.createElement(ink_js_1.Box, { flexDirection: "column", marginLeft: 2 }, t5);
        $[9] = t5;
        $[10] = t6;
    }
    else {
    ...
    if ($[11] !== ref || $[12] !== t3 || $[13] !== t4 || $[14] !== t6) {
        t7 = React.createElement(ink_js_1.Box, { ref: ref, flexDirection: "column", paddingX: 1, paddingY: 1 },
            t3,
            t4,
            t6);
        $[11] = ref;
...
async function teleportWithProgress(root, sessionId) {
    ...
    function TeleportProgressWrapper() {
        const [step, _setStep] = (0, react_1.useState)('validating');
        setStep = _setStep;
        return React.createElement(TeleportProgress, { currentStep: step, sessionId: sessionId });
    }
    root.render(React.createElement(AppState_js_1.AppStateProvider, null,
        React.createElement(TeleportProgressWrapper, null)));
    const result = await (0, teleport_js_1.teleportResumeCodeSession)(sessionId, setStep);
    setStep('checking_out');
    const { branchName, branchError } = await (0, teleport_js_1.checkOutTeleportedSessionBranch)(result.branch);
    return {
