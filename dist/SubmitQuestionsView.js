"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SubmitQuestionsView = SubmitQuestionsView;
const compiler_runtime_1 = require("react/compiler-runtime");
const figures_1 = __importDefault(require("figures"));
const react_1 = __importDefault(require("react"));
const ink_js_1 = require("../../../ink.js");
const index_js_1 = require("../../CustomSelect/index.js");
const Divider_js_1 = require("../../design-system/Divider.js");
const PermissionRequestTitle_js_1 = require("../PermissionRequestTitle.js");
const PermissionRuleExplanation_js_1 = require("../PermissionRuleExplanation.js");
const QuestionNavigationBar_js_1 = require("./QuestionNavigationBar.js");
function SubmitQuestionsView(t0) {
    const $ = (0, compiler_runtime_1.c)(27);
    const { questions, currentQuestionIndex, answers, allQuestionsAnswered, permissionResult, minContentHeight, onFinalResponse } = t0;
    let t1;
    if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
        t1 = react_1.default.createElement(Divider_js_1.Divider, { color: "inactive" });
        $[0] = t1;
    }
    else {
        t1 = $[0];
    }
    let t2;
    if ($[1] !== answers || $[2] !== currentQuestionIndex || $[3] !== questions) {
        t2 = react_1.default.createElement(QuestionNavigationBar_js_1.QuestionNavigationBar, { questions: questions, currentQuestionIndex: currentQuestionIndex, answers: answers });
        $[1] = answers;
        $[2] = currentQuestionIndex;
        $[3] = questions;
        $[4] = t2;
    }
    else {
        t2 = $[4];
    }
    let t3;
    if ($[5] === Symbol.for("react.memo_cache_sentinel")) {
        t3 = react_1.default.createElement(PermissionRequestTitle_js_1.PermissionRequestTitle, { title: "Review your answers", color: "text" });
        $[5] = t3;
    }
    else {
        t3 = $[5];
    }
    let t4;
    if ($[6] !== allQuestionsAnswered) {
        t4 = !allQuestionsAnswered && react_1.default.createElement(ink_js_1.Box, { marginBottom: 1 },
            react_1.default.createElement(ink_js_1.Text, { color: "warning" },
                figures_1.default.warning,
                " You have not answered all questions"));
        $[6] = allQuestionsAnswered;
        $[7] = t4;
    }
    else {
        t4 = $[7];
    }
    let t5;
    if ($[8] !== answers || $[9] !== questions) {
        t5 = Object.keys(answers).length > 0 && react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", marginBottom: 1 }, questions.filter(q => q?.question && answers[q.question]).map(q_0 => {
            const answer = answers[q_0?.question];
            return react_1.default.createElement(ink_js_1.Box, { key: q_0?.question || "answer", flexDirection: "column", marginLeft: 1 },
                react_1.default.createElement(ink_js_1.Text, null,
                    figures_1.default.bullet,
                    " ",
                    q_0?.question || "Question"),
                react_1.default.createElement(ink_js_1.Box, { marginLeft: 2 },
                    react_1.default.createElement(ink_js_1.Text, { color: "success" },
                        figures_1.default.arrowRight,
                        " ",
                        answer)));
        }));
        $[8] = answers;
        $[9] = questions;
        $[10] = t5;
    }
    else {
        t5 = $[10];
    }
    let t6;
    if ($[11] !== permissionResult) {
        t6 = react_1.default.createElement(PermissionRuleExplanation_js_1.PermissionRuleExplanation, { permissionResult: permissionResult, toolType: "tool" });
        $[11] = permissionResult;
        $[12] = t6;
    }
    else {
        t6 = $[12];
    }
    let t7;
    if ($[13] === Symbol.for("react.memo_cache_sentinel")) {
        t7 = react_1.default.createElement(ink_js_1.Text, { color: "inactive" }, "Ready to submit your answers?");
        $[13] = t7;
    }
    else {
        t7 = $[13];
    }
    let t8;
    if ($[14] === Symbol.for("react.memo_cache_sentinel")) {
        t8 = {
            type: "text",
            label: "Submit answers",
            value: "submit"
        };
        $[14] = t8;
    }
    else {
        t8 = $[14];
    }
    let t9;
    if ($[15] === Symbol.for("react.memo_cache_sentinel")) {
        t9 = [t8, {
                type: "text",
                label: "Cancel",
                value: "cancel"
            }];
        $[15] = t9;
    }
    else {
        t9 = $[15];
    }
    let t10;
    if ($[16] !== onFinalResponse) {
        t10 = react_1.default.createElement(ink_js_1.Box, { marginTop: 1 },
            react_1.default.createElement(index_js_1.Select, { options: t9, onChange: value => onFinalResponse(value), onCancel: () => onFinalResponse("cancel") }));
        $[16] = onFinalResponse;
        $[17] = t10;
    }
    else {
        t10 = $[17];
    }
    let t11;
    if ($[18] !== minContentHeight || $[19] !== t10 || $[20] !== t4 || $[21] !== t5 || $[22] !== t6) {
        t11 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: 1, minHeight: minContentHeight },
            t4,
            t5,
            t6,
            t7,
            t10);
        $[18] = minContentHeight;
        $[19] = t10;
        $[20] = t4;
        $[21] = t5;
        $[22] = t6;
        $[23] = t11;
    }
    else {
        t11 = $[23];
    }
    let t12;
    if ($[24] !== t11 || $[25] !== t2) {
        t12 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: 1 },
            t1,
            react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", borderTop: true, borderColor: "inactive", paddingTop: 0 },
                t2,
                t3,
                t11));
        $[24] = t11;
        $[25] = t2;
        $[26] = t12;
    }
    else {
        t12 = $[26];
    }
    return t12;
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
