...
function MCPServerApprovalDialog(t0) {
    ...
    let t6;
    if ($[7] !== onChange) {
        t6 = react_1.default.createElement(index_js_2.Select, { options: t5, onChange: value_0 => onChange(value_0), onCancel: () => onChange("no") });
        $[7] = onChange;
        $[8] = t6;
    }
    else {
    ...
    let t7;
    if ($[9] !== t2 || $[10] !== t3 || $[11] !== t6) {
        t7 = react_1.default.createElement(Dialog_js_1.Dialog, { title: t2, color: "warning", onCancel: t3 },
            t4,
            t6);
        $[9] = t2;
        $[10] = t3;
