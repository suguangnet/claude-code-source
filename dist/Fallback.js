...
function StructuredDiffFallback(t0) {
    ...
    let t3;
    if ($[8] !== t2) {
        t3 = React.createElement(ink_js_1.Box, { flexDirection: "column", flexGrow: 1 }, t2);
        $[8] = t2;
        $[9] = t3;
    }
    else {
...
function _temp(node, i) {
    return React.createElement(ink_js_1.Box, { key: i }, node);
}
...
function calculateWordDiffs(oldText, newText) {
    ...
    const result = (0, diff_1.diffWordsWithSpace)(oldText, newText, {
        ignoreCase: false
    });
    return result;
}
...
function generateWordDiffElements(item, width, maxWidth, dim, overrideTheme) {
    ...
    wordDiffs.forEach((part, partIndex) => {
        ...
        const partWrapped = (0, ink_js_1.wrapText)(part.value, availableContentWidth, 'wrap');
        ...
        partLines.forEach((partLine, lineIdx) => {
            ...
            currentLine.push(React.createElement(ink_js_1.Text, { key: `part-${partIndex}-${lineIdx}`, backgroundColor: partBgColor }, partLine));
    ...
    return wrappedLines.map(({ content, contentWidth }, lineIndex) => {
        ...
        return React.createElement(ink_js_1.Box, { key: key, flexDirection: "row" },
            React.createElement(ink_js_1.NoSelect, { fromLeftEdge: true },
                React.createElement(ink_js_1.Text, { color: overrideTheme ? 'text' : undefined, backgroundColor: lineBgColor, dimColor: dim },
                    lineNumStr,
                    diffPrefix)),
            React.createElement(ink_js_1.Text, { color: overrideTheme ? 'text' : undefined, backgroundColor: lineBgColor, dimColor: dim },
                content,
...
function formatDiff(lines, startingLineNumber, width, dim, overrideTheme) {
    ...
    return ls.flatMap((item) => {
        ...
        const wrappedText = (0, ink_js_1.wrapText)(code, availableContentWidth, 'wrap');
        ...
        return wrappedLines.map((line, lineIndex) => {
            ...
            // Calculate padding to fill the entire terminal width
            const contentWidth = lineNumStr.length + 1 + (0, stringWidth_js_1.stringWidth)(line); // lineNum + sigil + code
            ...
            return React.createElement(ink_js_1.Box, { key: key, flexDirection: "row" },
                React.createElement(ink_js_1.NoSelect, { fromLeftEdge: true },
                    React.createElement(ink_js_1.Text, { color: overrideTheme ? 'text' : undefined, backgroundColor: bgColor, dimColor: dim || type === 'nochange' },
                        lineNumStr,
                        sigil)),
                React.createElement(ink_js_1.Text, { color: overrideTheme ? 'text' : undefined, backgroundColor: bgColor, dimColor: dim },
                    line,
                    ' '.repeat(padding)));
        });
    });
