...
function MCPToolListView(t0) {
    ...
    const mcpTools = (0, AppState_js_1.useAppState)(_temp);
    let t1;
    bb0: {
        ...
        if ($[1] !== mcpTools || $[2] !== server.name) {
            t2 = (0, utils_js_1.filterToolsByServer)(mcpTools, server.name);
            $[1] = mcpTools;
            $[2] = server.name;
            $[3] = t2;
        }
    ...
    if ($[4] !== server.name || $[5] !== serverTools) {
        ...
        if ($[7] !== server.name) {
            t3 = (tool, index) => {
                const toolName = (0, mcpStringUtils_js_1.getMcpDisplayName)(tool.name, server.name);
                const fullDisplayName = tool.userFacingName ? tool.userFacingName({}) : toolName;
                const displayName = (0, mcpStringUtils_js_1.extractMcpToolDisplayName)(fullDisplayName);
    ...
    if ($[11] !== onBack || $[12] !== onSelectTool || $[13] !== serverTools || $[14] !== toolOptions) {
        t7 = serverTools.length === 0 ? react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "No tools available") : react_1.default.createElement(index_js_1.Select, { options: toolOptions, onChange: value => {
    ...
    if ($[16] !== onBack || $[17] !== t3 || $[18] !== t6 || $[19] !== t7) {
        t8 = react_1.default.createElement(Dialog_js_1.Dialog, { title: t3, subtitle: t6, onCancel: onBack, inputGuide: _temp2 }, t7);
        $[16] = onBack;
        $[17] = t3;
        $[18] = t6;
        $[19] = t7;
