...
async function realpathDeepestExisting(absolutePath) {
    ...
    for (let parent = (0, path_1.dirname)(current); current !== parent; parent = (0, path_1.dirname)(current)) {
        try {
            ...
            return tail.length === 0
                ? realCurrent
                : (0, path_1.join)(realCurrent, ...tail.reverse());
        }
        catch (e) {
            ...
            if (code === 'ENOENT') {
                ...
                try {
                    const st = await (0, promises_1.lstat)(current);
            ...
            tail.push(current.slice(parent.length + path_1.sep.length));
            current = parent;
        }
    }
...
async function isRealPathWithinTeamDir(realCandidate) {
    ...
    try {
        ...
        realTeamDir = await (0, promises_1.realpath)(getTeamMemPath().replace(/[/\\]+$/, ''));
    }
    catch (e) {
        const code = (0, errors_js_1.getErrnoCode)(e);
    ...
    return realCandidate.startsWith(realTeamDir + path_1.sep);
}
/**
...
async function validateTeamMemKey(relativeKey) {
    ...
    const teamDir = getTeamMemPath();
    const fullPath = (0, path_1.join)(teamDir, relativeKey);
    // First pass: normalize .. segments and check string-level containment.
