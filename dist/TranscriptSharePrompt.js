"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TranscriptSharePrompt = TranscriptSharePrompt;
const compiler_runtime_1 = require("react/compiler-runtime");
const react_1 = __importDefault(require("react"));
const figures_js_1 = require("../../constants/figures.js");
const ink_js_1 = require("../../ink.js");
const useDebouncedDigitInput_js_1 = require("./useDebouncedDigitInput.js");
const RESPONSE_INPUTS = ['1', '2', '3'];
const inputToResponse = {
    '1': 'yes',
    '2': 'no',
    '3': 'dont_ask_again'
};
const isValidResponseInput = (input) => RESPONSE_INPUTS.includes(input);
function TranscriptSharePrompt(t0) {
    const $ = (0, compiler_runtime_1.c)(11);
    const { onSelect, inputValue, setInputValue } = t0;
    let t1;
    if ($[0] !== onSelect) {
        t1 = digit => onSelect(inputToResponse[digit]);
        $[0] = onSelect;
        $[1] = t1;
    }
    else {
        t1 = $[1];
    }
    let t2;
    if ($[2] !== inputValue || $[3] !== setInputValue || $[4] !== t1) {
        t2 = {
            inputValue,
            setInputValue,
            isValidDigit: isValidResponseInput,
            onDigit: t1
        };
        $[2] = inputValue;
        $[3] = setInputValue;
        $[4] = t1;
        $[5] = t2;
    }
    else {
        t2 = $[5];
    }
    (0, useDebouncedDigitInput_js_1.useDebouncedDigitInput)(t2);
    let t3;
    if ($[6] === Symbol.for("react.memo_cache_sentinel")) {
        t3 = react_1.default.createElement(ink_js_1.Box, null,
            react_1.default.createElement(ink_js_1.Text, { color: "ansi:cyan" },
                figures_js_1.BLACK_CIRCLE,
                " "),
            react_1.default.createElement(ink_js_1.Text, { bold: true }, "Can Anthropic look at your session transcript to help us improve Claude Code?"));
        $[6] = t3;
    }
    else {
        t3 = $[6];
    }
    let t4;
    if ($[7] === Symbol.for("react.memo_cache_sentinel")) {
        t4 = react_1.default.createElement(ink_js_1.Box, { marginLeft: 2 },
            react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "Learn more: https://code.claude.com/docs/en/data-usage#session-quality-surveys"));
        $[7] = t4;
    }
    else {
        t4 = $[7];
    }
    let t5;
    if ($[8] === Symbol.for("react.memo_cache_sentinel")) {
        t5 = react_1.default.createElement(ink_js_1.Box, { width: 10 },
            react_1.default.createElement(ink_js_1.Text, null,
                react_1.default.createElement(ink_js_1.Text, { color: "ansi:cyan" }, "1"),
                ": Yes"));
        $[8] = t5;
    }
    else {
        t5 = $[8];
    }
    let t6;
    if ($[9] === Symbol.for("react.memo_cache_sentinel")) {
        t6 = react_1.default.createElement(ink_js_1.Box, { width: 10 },
            react_1.default.createElement(ink_js_1.Text, null,
                react_1.default.createElement(ink_js_1.Text, { color: "ansi:cyan" }, "2"),
                ": No"));
        $[9] = t6;
    }
    else {
        t6 = $[9];
    }
    let t7;
    if ($[10] === Symbol.for("react.memo_cache_sentinel")) {
        t7 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: 1 },
            t3,
            t4,
            react_1.default.createElement(ink_js_1.Box, { marginLeft: 2 },
                t5,
                t6,
                react_1.default.createElement(ink_js_1.Box, null,
                    react_1.default.createElement(ink_js_1.Text, null,
                        react_1.default.createElement(ink_js_1.Text, { color: "ansi:cyan" }, "3"),
                        ": Don't ask again"))));
        $[10] = t7;
    }
    else {
        t7 = $[10];
    }
    return t7;
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
