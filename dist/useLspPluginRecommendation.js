...
const lspRecommendation_js_1 = require("../utils/plugins/lspRecommendation.js");
...
const usePluginRecommendationBase_js_1 = require("./usePluginRecommendationBase.js");
// Threshold for detecting timeout vs explicit dismiss (ms)
// Menu auto-dismisses at 30s, so anything over 28s is likely timeout
const TIMEOUT_THRESHOLD_MS = 28000;
function useLspPluginRecommendation() {
    const $ = (0, compiler_runtime_1.c)(12);
    const trackedFiles = (0, AppState_js_1.useAppState)(_temp);
    ...
    const { recommendation, clearRecommendation, tryResolve } = (0, usePluginRecommendationBase_js_1.usePluginRecommendationBase)();
    let t1;
    let t2;
        ...
        t1 = () => {
            tryResolve(async () => {
                ...
                for (const filePath of newFiles) {
                    ;
                    try {
                        const matches = await (0, lspRecommendation_js_1.getMatchingLspPlugins)(filePath);
                        const match = matches[0];
                        if (match) {
                            (0, debug_js_1.logForDebugging)(`[useLspPluginRecommendation] Found match: ${match.pluginName} for ${filePath}`);
    ...
    if ($[5] !== addNotification || $[6] !== clearRecommendation || $[7] !== recommendation) {
        t3 = response => {
            ...
            (0, debug_js_1.logForDebugging)(`[useLspPluginRecommendation] User response: ${response} for ${pluginName}`);
            bb60: switch (response) {
                case "yes":
                    {
                        (0, usePluginRecommendationBase_js_1.installPluginAndNotify)(pluginId, pluginName, "lsp-plugin", addNotification, async (pluginData) => {
                            (0, debug_js_1.logForDebugging)(`[useLspPluginRecommendation] Installing plugin: ${pluginId}`);
                            const localSourcePath = typeof pluginData.entry.source === "string" ? (0, path_1.join)(pluginData.marketplaceInstallLocation, pluginData.entry.source) : undefined;
                            await (0, pluginInstallationHelpers_js_1.cacheAndRegisterPlugin)(pluginId, pluginData.entry, "user", undefined, localSourcePath);
                            ...
                            (0, debug_js_1.logForDebugging)(`[useLspPluginRecommendation] Plugin installed: ${pluginId}`);
                        });
                        break bb60;
                    }
                case "no":
                    {
                        ...
                        if (elapsed >= TIMEOUT_THRESHOLD_MS) {
                            (0, debug_js_1.logForDebugging)(`[useLspPluginRecommendation] Timeout detected (${elapsed}ms), incrementing ignored count`);
