...
function MessageSelector({ messages, onPreRestore, onRestoreMessage, onRestoreCode, onSummarize, onClose, preselectedMessage }) {
    const fileHistory = (0, AppState_js_1.useAppState)(s => s.fileHistory);
    const [error, setError] = (0, react_1.useState)(undefined);
    const isFileHistoryEnabled = (0, fileHistory_js_1.fileHistoryEnabled)();
    // Add current prompt as a virtual message
    const currentUUID = (0, react_1.useMemo)(crypto_1.randomUUID, []);
    const messageOptions = (0, react_1.useMemo)(() => [...messages.filter(selectableUserMessagesFilter), {
            ...(0, messages_js_1.createUserMessage)({
                content: ''
            }),
            uuid: currentUUID
        }], [messages, currentUUID]);
    const [selectedIndex, setSelectedIndex] = (0, react_1.useState)(messageOptions.length - 1);
    ...
    const [messageToRestore, setMessageToRestore] = (0, react_1.useState)(preselectedMessage);
    const [diffStatsForRestore, setDiffStatsForRestore] = (0, react_1.useState)(undefined);
    (0, react_1.useEffect)(() => {
        if (!preselectedMessage || !isFileHistoryEnabled)
            return;
        let cancelled = false;
        void (0, fileHistory_js_1.fileHistoryGetDiffStats)(fileHistory, preselectedMessage.uuid).then(stats => {
            if (!cancelled)
    ...
    const [restoringOption, setRestoringOption] = (0, react_1.useState)(null);
    const [selectedRestoreOption, setSelectedRestoreOption] = (0, react_1.useState)('both');
    ...
    const [summarizeFromFeedback, setSummarizeFromFeedback] = (0, react_1.useState)('');
    const [summarizeUpToFeedback, setSummarizeUpToFeedback] = (0, react_1.useState)('');
    ...
    async function handleSelect(message_0) {
        ...
        const diffStats = await (0, fileHistory_js_1.fileHistoryGetDiffStats)(fileHistory, message_0.uuid);
    ...
    const moveUp = (0, react_1.useCallback)(() => setSelectedIndex(prev => Math.max(0, prev - 1)), []);
    const moveDown = (0, react_1.useCallback)(() => setSelectedIndex(prev_0 => Math.min(messageOptions.length - 1, prev_0 + 1)), [messageOptions.length]);
    const jumpToTop = (0, react_1.useCallback)(() => setSelectedIndex(0), []);
    const jumpToBottom = (0, react_1.useCallback)(() => setSelectedIndex(messageOptions.length - 1), [messageOptions.length]);
    const handleSelectCurrent = (0, react_1.useCallback)(() => {
    ...
    const [fileHistoryMetadata, setFileHistoryMetadata] = (0, react_1.useState)({});
    (0, react_1.useEffect)(() => {
        async function loadFileHistoryMetadata() {
            ...
            void Promise.all(messageOptions.map(async (userMessage, itemIndex) => {
                if (userMessage.uuid !== currentUUID) {
                    const canRestore = (0, fileHistory_js_1.fileHistoryCanRestore)(fileHistory, userMessage.uuid);
    ...
    return React.createElement(ink_js_1.Box, { flexDirection: "column", width: "100%" },
        React.createElement(Divider_js_1.Divider, { color: "suggestion" }),
        React.createElement(ink_js_1.Box, { flexDirection: "column", marginX: 1, gap: 1 },
            React.createElement(ink_js_1.Text, { bold: true, color: "suggestion" }, "Rewind"),
            ...
            !hasMessagesToSelect && React.createElement(React.Fragment, null,
                React.createElement(ink_js_1.Text, null, "Nothing to rewind to yet.")),
            !error && messageToRestore && hasMessagesToSelect && React.createElement(React.Fragment, null,
                ...
                React.createElement(ink_js_1.Box, { flexDirection: "column", paddingLeft: 1, borderStyle: "single", borderRight: false, borderTop: false, borderBottom: false, borderLeft: true, borderLeftDimColor: true },
                ...
                isRestoring && isSummarizeOption(restoringOption) ? React.createElement(ink_js_1.Box, { flexDirection: "row", gap: 1 },
                    React.createElement(Spinner_js_1.Spinner, null),
                    React.createElement(ink_js_1.Text, null, "Summarizing\u2026")) : React.createElement(select_js_1.Select, { isDisabled: isRestoring, options: getRestoreOptions(!!canRestoreCode_0), defaultFocusValue: canRestoreCode_0 ? 'both' : 'conversation', onFocus: value => setSelectedRestoreOption(value), onChange: value_0 => onSelectRestoreOption(value_0), onCancel: () => preselectedMessage ? onClose() : setMessageToRestore(undefined) }),
                canRestoreCode_0 && React.createElement(ink_js_1.Box, { marginBottom: 1 },
                    React.createElement(ink_js_1.Text, { dimColor: true },
                        figures_1.default.warning,
                        " Rewinding does not affect files edited manually or via bash."))),
            showPickList && React.createElement(React.Fragment, null,
                isFileHistoryEnabled ? React.createElement(ink_js_1.Text, null, "Restore the code and/or conversation to the point before\u2026") : React.createElement(ink_js_1.Text, null, "Restore and fork the conversation to the point before\u2026"),
                React.createElement(ink_js_1.Box, { width: "100%", flexDirection: "column" }, messageOptions.slice(firstVisibleIndex, firstVisibleIndex + MAX_VISIBLE_MESSAGES).map((msg, visibleOptionIndex) => {
                    ...
                    return React.createElement(ink_js_1.Box, { key: msg.uuid, height: isFileHistoryEnabled ? 3 : 2, overflow: "hidden", width: "100%", flexDirection: "row" },
                        React.createElement(ink_js_1.Box, { width: 2, minWidth: 2 }, isSelected ? React.createElement(ink_js_1.Text, { color: "permission", bold: true },
                            figures_1.default.pointer,
                            ' ') : React.createElement(ink_js_1.Text, null, '  ')),
                        React.createElement(ink_js_1.Box, { flexDirection: "column" },
                            React.createElement(ink_js_1.Box, { flexShrink: 1, height: 1, overflow: "hidden" },
                            ...
                            isFileHistoryEnabled && metadataLoaded && React.createElement(ink_js_1.Box, { height: 1, flexDirection: "row" }, metadata ? React.createElement(React.Fragment, null,
                                React.createElement(ink_js_1.Text, { dimColor: !isSelected, color: "inactive" }, numFilesChanged ? React.createElement(React.Fragment, null,
                                    numFilesChanged === 1 && metadata.filesChanged[0] ? `${path.basename(metadata.filesChanged[0])} ` : `${numFilesChanged} files changed `,
                                    React.createElement(DiffStatsText, { diffStats: metadata })) : React.createElement(React.Fragment, null, "No code changes"))) : React.createElement(ink_js_1.Text, { dimColor: true, color: "warning" },
                                figures_1.default.warning,
                                " No code restore"))));
                }))),
            !messageToRestore && React.createElement(ink_js_1.Text, { dimColor: true, italic: true }, exitState.pending ? React.createElement(React.Fragment, null,
                "Press ",
                exitState.keyName,
...
function RestoreOptionDescription(t0) {
    ...
    if ($[4] !== diffStatsForRestore || $[5] !== selectedRestoreOption || $[6] !== showCodeRestore) {
        t3 = !isSummarizeOption(selectedRestoreOption) && (showCodeRestore ? React.createElement(RestoreCodeConfirmation, { diffStatsForRestore: diffStatsForRestore }) : React.createElement(ink_js_1.Text, { dimColor: true }, "The code will be unchanged."));
...
function RestoreCodeConfirmation(t0) {
    ...
    if (!diffStatsForRestore.filesChanged || !diffStatsForRestore.filesChanged[0]) {
        ...
        if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
            t1 = React.createElement(ink_js_1.Text, { dimColor: true }, "The code has not changed (nothing will be restored).");
            $[0] = t1;
        }
        else {
            t1 = $[0];
...
function DiffStatsText(t0) {
    ...
    if ($[0] !== diffStats.insertions) {
        t1 = React.createElement(ink_js_1.Text, { color: "diffAddedWord" },
            "+",
    ...
    if ($[2] !== diffStats.deletions) {
        t2 = React.createElement(ink_js_1.Text, { color: "diffRemovedWord" },
            "-",
            diffStats.deletions);
...
function UserMessageOption(t0) {
    const $ = (0, compiler_runtime_1.c)(31);
    ...
    if (isCurrent) {
        ...
        if ($[0] !== color || $[1] !== dimColor) {
            t1 = React.createElement(ink_js_1.Box, { width: "100%" },
                React.createElement(ink_js_1.Text, { italic: true, color: color, dimColor: dimColor }, "(current)"));
            $[0] = color;
            $[1] = dimColor;
            $[2] = t1;
        }
    ...
    if ($[3] !== color || $[4] !== columns || $[5] !== content || $[6] !== dimColor || $[7] !== lastBlock || $[8] !== paddingRight) {
        ...
        bb0: {
            ...
            const messageText = (0, displayTags_js_1.stripDisplayTags)(rawMessageText);
            if ((0, messages_js_1.isEmptyMessageText)(messageText)) {
                let t7;
                if ($[17] !== color || $[18] !== dimColor) {
                    t7 = React.createElement(ink_js_1.Box, { flexDirection: "row", width: "100%" },
                        React.createElement(ink_js_1.Text, { italic: true, color: color, dimColor: dimColor }, "((empty message))"));
                    $[17] = color;
                    $[18] = dimColor;
                    $[19] = t7;
                }
            ...
            if (messageText.includes("<bash-input>")) {
                const input = (0, messages_js_1.extractTag)(messageText, "bash-input");
                if (input) {
                    let t7;
                    if ($[20] === Symbol.for("react.memo_cache_sentinel")) {
                        t7 = React.createElement(ink_js_1.Text, { color: "bashBorder" }, "!");
                        $[20] = t7;
                    }
                    else {
                        t7 = $[20];
                    }
                    t6 = React.createElement(ink_js_1.Box, { flexDirection: "row", width: "100%" },
                        t7,
                        React.createElement(ink_js_1.Text, { color: color, dimColor: dimColor },
                            " ",
                            input));
                    break bb0;
                }
            }
            if (messageText.includes(`<${xml_js_1.COMMAND_MESSAGE_TAG}>`)) {
                const commandMessage = (0, messages_js_1.extractTag)(messageText, xml_js_1.COMMAND_MESSAGE_TAG);
                const args = (0, messages_js_1.extractTag)(messageText, "command-args");
                const isSkillFormat = (0, messages_js_1.extractTag)(messageText, "skill-format") === "true";
                if (commandMessage) {
                    if (isSkillFormat) {
                        t6 = React.createElement(ink_js_1.Box, { flexDirection: "row", width: "100%" },
                            React.createElement(ink_js_1.Text, { color: color, dimColor: dimColor },
        ...
        try {
            if ('type' in result && result.type === 'create') {
                ...
                for (const hunk of result.structuredPatch) {
                    const additions = (0, array_js_1.count)(hunk.lines, line => line.startsWith('+'));
                    const removals = (0, array_js_1.count)(hunk.lines, line => line.startsWith('-'));
...
function selectableUserMessagesFilter(message) {
    ...
    if (messageText.indexOf(`<${xml_js_1.LOCAL_COMMAND_STDOUT_TAG}>`) !== -1 || messageText.indexOf(`<${xml_js_1.LOCAL_COMMAND_STDERR_TAG}>`) !== -1 || messageText.indexOf(`<${xml_js_1.BASH_STDOUT_TAG}>`) !== -1 || messageText.indexOf(`<${xml_js_1.BASH_STDERR_TAG}>`) !== -1 || messageText.indexOf(`<${xml_js_1.TASK_NOTIFICATION_TAG}>`) !== -1 || messageText.indexOf(`<${xml_js_1.TICK_TAG}>`) !== -1 || messageText.indexOf(`<${xml_js_1.TEAMMATE_MESSAGE_TAG}`) !== -1) {
        return false;
    }
    return true;
}
