 ...
 */
exports.loadPluginOptions = (0, memoize_js_1.default)((pluginId) => {
...
function clearPluginOptionsCache() {
    exports.loadPluginOptions.cache?.clear?.();
}
/**
...
function savePluginOptions(pluginId, values, schema) {
    ...
    if (Object.keys(sensitive).length > 0 || needSecureScrub) {
        ...
        if (result.warning) {
            (0, debug_js_1.logForDebugging)(`Plugin secrets save warning: ${result.warning}`, {
                level: 'warn',
            });
        }
    }
...
function deletePluginOptions(pluginId) {
    ...
    if (settings.pluginConfigs?.[pluginId]) {
        ...
        if (error) {
            (0, debug_js_1.logForDebugging)(`deletePluginOptions: failed to clear settings.pluginConfigs[${pluginId}]: ${error.message}`, { level: 'warn' });
        }
    }
    ...
    if (existing?.pluginSecrets) {
        ...
        if (survivingEntries.length !== Object.keys(existing.pluginSecrets).length) {
            ...
            if (!result.success) {
                (0, debug_js_1.logForDebugging)(`deletePluginOptions: failed to clear pluginSecrets for ${pluginId} from keychain`, { level: 'warn' });
            }
        }
    }
    clearPluginOptionsCache();
...
function getUnconfiguredOptions(plugin) {
    ...
    const saved = (0, exports.loadPluginOptions)(getPluginStorageId(plugin));
    const validation = (0, mcpbHandler_js_1.validateUserConfig)(saved, manifestSchema);
    if (validation.valid) {
        return {};
    }
    ...
    for (const [key, fieldSchema] of Object.entries(manifestSchema)) {
        const single = (0, mcpbHandler_js_1.validateUserConfig)({ [key]: saved[key] }, { [key]: fieldSchema });
        if (!single.valid) {
...
function substitutePluginVariables(value, plugin) {
    ...
    if (plugin.source) {
        ...
        out = out.replace(/\$\{CLAUDE_PLUGIN_DATA\}/g, () => normalize((0, pluginDirectories_js_1.getPluginDataDir)(source)));
    }
    return out;
}
/**
