"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.NotebookEditToolDiff = NotebookEditToolDiff;
const compiler_runtime_1 = require("react/compiler-runtime");
const path_1 = require("path");
const React = __importStar(require("react"));
const react_1 = require("react");
const ink_js_1 = require("../../../ink.js");
const array_js_1 = require("../../../utils/array.js");
const cwd_js_1 = require("../../../utils/cwd.js");
const diff_js_1 = require("../../../utils/diff.js");
const fsOperations_js_1 = require("../../../utils/fsOperations.js");
const json_js_1 = require("../../../utils/json.js");
const notebook_js_1 = require("../../../utils/notebook.js");
const HighlightedCode_js_1 = require("../../HighlightedCode.js");
const StructuredDiff_js_1 = require("../../StructuredDiff.js");
function NotebookEditToolDiff(props) {
    const $ = (0, compiler_runtime_1.c)(5);
    let t0;
    if ($[0] !== props.notebook_path) {
        t0 = (0, fsOperations_js_1.getFsImplementation)().readFile(props.notebook_path, {
            encoding: "utf-8"
        }).then(_temp).catch(_temp2);
        $[0] = props.notebook_path;
        $[1] = t0;
    }
    else {
        t0 = $[1];
    }
    const notebookDataPromise = t0;
    let t1;
    if ($[2] !== notebookDataPromise || $[3] !== props) {
        t1 = React.createElement(react_1.Suspense, { fallback: null },
            React.createElement(NotebookEditToolDiffInner, { ...props, promise: notebookDataPromise }));
        $[2] = notebookDataPromise;
        $[3] = props;
        $[4] = t1;
    }
    else {
        t1 = $[4];
    }
    return t1;
}
function _temp2() {
    return null;
}
function _temp(content) {
    return (0, json_js_1.safeParseJSON)(content);
}
function NotebookEditToolDiffInner(t0) {
    const $ = (0, compiler_runtime_1.c)(34);
    const { notebook_path, cell_id, new_source, cell_type, edit_mode: t1, verbose, width, promise } = t0;
    const edit_mode = t1 === undefined ? "replace" : t1;
    const notebookData = (0, react_1.use)(promise);
    let t2;
    if ($[0] !== cell_id || $[1] !== notebookData) {
        bb0: {
            if (!notebookData || !cell_id) {
                t2 = "";
                break bb0;
            }
            const cellIndex = (0, notebook_js_1.parseCellId)(cell_id);
            if (cellIndex !== undefined) {
                if (notebookData.cells[cellIndex]) {
                    const source = notebookData.cells[cellIndex].source;
                    let t3;
                    if ($[3] !== source) {
                        t3 = Array.isArray(source) ? source.join("") : source;
                        $[3] = source;
                        $[4] = t3;
                    }
                    else {
                        t3 = $[4];
                    }
                    t2 = t3;
                    break bb0;
                }
                t2 = "";
                break bb0;
            }
            let t3;
            if ($[5] !== cell_id) {
                t3 = cell => cell.id === cell_id;
                $[5] = cell_id;
                $[6] = t3;
            }
            else {
                t3 = $[6];
            }
            const cell_0 = notebookData.cells.find(t3);
            if (!cell_0) {
                t2 = "";
                break bb0;
            }
            t2 = Array.isArray(cell_0.source) ? cell_0.source.join("") : cell_0.source;
        }
        $[0] = cell_id;
        $[1] = notebookData;
        $[2] = t2;
    }
    else {
        t2 = $[2];
    }
    const oldSource = t2;
    let t3;
    bb1: {
        if (!notebookData || edit_mode === "insert" || edit_mode === "delete") {
            t3 = null;
            break bb1;
        }
        let t4;
        if ($[7] !== new_source || $[8] !== notebook_path || $[9] !== oldSource) {
            t4 = (0, diff_js_1.getPatchForDisplay)({
                filePath: notebook_path,
                fileContents: oldSource,
                edits: [{
                        old_string: oldSource,
                        new_string: new_source,
                        replace_all: false
                    }],
                ignoreWhitespace: false
            });
            $[7] = new_source;
            $[8] = notebook_path;
            $[9] = oldSource;
            $[10] = t4;
        }
        else {
            t4 = $[10];
        }
        t3 = t4;
    }
    const hunks = t3;
    let editTypeDescription;
    bb2: switch (edit_mode) {
        case "insert":
            {
                editTypeDescription = "Insert new cell";
                break bb2;
            }
        case "delete":
            {
                editTypeDescription = "Delete cell";
                break bb2;
            }
        default:
            {
                editTypeDescription = "Replace cell contents";
            }
    }
    let t4;
    if ($[11] !== notebook_path || $[12] !== verbose) {
        t4 = verbose ? notebook_path : (0, path_1.relative)((0, cwd_js_1.getCwd)(), notebook_path);
        $[11] = notebook_path;
        $[12] = verbose;
        $[13] = t4;
    }
    else {
        t4 = $[13];
    }
    let t5;
    if ($[14] !== t4) {
        t5 = React.createElement(ink_js_1.Text, { bold: true }, t4);
        $[14] = t4;
        $[15] = t5;
    }
    else {
        t5 = $[15];
    }
    const t6 = cell_type ? ` (${cell_type})` : "";
    let t7;
    if ($[16] !== cell_id || $[17] !== editTypeDescription || $[18] !== t6) {
        t7 = React.createElement(ink_js_1.Text, { dimColor: true },
            editTypeDescription,
            " for cell ",
            cell_id,
            t6);
        $[16] = cell_id;
        $[17] = editTypeDescription;
        $[18] = t6;
        $[19] = t7;
    }
    else {
        t7 = $[19];
    }
    let t8;
    if ($[20] !== t5 || $[21] !== t7) {
        t8 = React.createElement(ink_js_1.Box, { paddingBottom: 1, flexDirection: "column" },
            t5,
            t7);
        $[20] = t5;
        $[21] = t7;
        $[22] = t8;
    }
    else {
        t8 = $[22];
    }
    let t9;
    if ($[23] !== cell_type || $[24] !== edit_mode || $[25] !== hunks || $[26] !== new_source || $[27] !== notebook_path || $[28] !== oldSource || $[29] !== width) {
        t9 = edit_mode === "delete" ? React.createElement(ink_js_1.Box, { flexDirection: "column", paddingLeft: 2 },
            React.createElement(HighlightedCode_js_1.HighlightedCode, { code: oldSource, filePath: notebook_path })) : edit_mode === "insert" ? React.createElement(ink_js_1.Box, { flexDirection: "column", paddingLeft: 2 },
            React.createElement(HighlightedCode_js_1.HighlightedCode, { code: new_source, filePath: cell_type === "markdown" ? "file.md" : notebook_path })) : hunks ? (0, array_js_1.intersperse)(hunks.map(_ => React.createElement(StructuredDiff_js_1.StructuredDiff, { key: _.newStart, patch: _, dim: false, width: width, filePath: notebook_path, firstLine: new_source.split("\n")[0] ?? null, fileContent: oldSource })), _temp3) : React.createElement(HighlightedCode_js_1.HighlightedCode, { code: new_source, filePath: cell_type === "markdown" ? "file.md" : notebook_path });
        $[23] = cell_type;
        $[24] = edit_mode;
        $[25] = hunks;
        $[26] = new_source;
        $[27] = notebook_path;
        $[28] = oldSource;
        $[29] = width;
        $[30] = t9;
    }
    else {
        t9 = $[30];
    }
    let t10;
    if ($[31] !== t8 || $[32] !== t9) {
        t10 = React.createElement(ink_js_1.Box, { flexDirection: "column" },
            React.createElement(ink_js_1.Box, { borderStyle: "round", flexDirection: "column", paddingX: 1 },
                t8,
                t9));
        $[31] = t8;
        $[32] = t9;
        $[33] = t10;
    }
    else {
        t10 = $[33];
    }
    return t10;
}
function _temp3(i) {
    return React.createElement(ink_js_1.NoSelect, { fromLeftEdge: true, key: `ellipsis-${i}` },
        React.createElement(ink_js_1.Text, { dimColor: true }, "..."));
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
