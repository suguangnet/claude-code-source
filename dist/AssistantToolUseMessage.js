...
const classifierApprovalsHook_js_1 = require("../../utils/classifierApprovalsHook.js");
...
const HookProgressMessage_js_1 = require("./HookProgressMessage.js");
function AssistantToolUseMessage(t0) {
    const $ = (0, compiler_runtime_1.c)(81);
    ...
    const pendingWorkerRequest = (0, AppState_js_1.useAppStateMaybeOutsideOfProvider)(_temp);
    const isClassifierCheckingRaw = (0, classifierApprovalsHook_js_1.useIsClassifierChecking)(param.id);
    const permissionMode = (0, AppState_js_1.useAppStateMaybeOutsideOfProvider)(_temp2);
    const hasStrippedRules = (0, AppState_js_1.useAppStateMaybeOutsideOfProvider)(_temp3);
    ...
    if ($[0] !== param.input || $[1] !== param.name || $[2] !== tools) {
        bb0: {
            ...
            const tool = (0, Tool_js_1.findToolByName)(tools, param.name);
            if (!tool) {
                t1 = null;
                break bb0;
            }
    ...
    const parsed = t1;
    if (!parsed) {
        (0, log_js_1.logError)(new Error(tools ? `Tool ${param.name} not found` : `Tools array is undefined for tool ${param.name}`));
        return null;
    }
    ...
    if (isTransparentWrapper) {
        ...
        let t5;
        if ($[21] !== bg || $[22] !== t4) {
            t5 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", width: "100%", backgroundColor: bg }, t4);
            $[21] = bg;
            $[22] = t4;
            $[23] = t5;
        }
    ...
    const t6 = (0, stringWidth_js_1.stringWidth)(userFacingToolName) + (shouldShowDot ? 2 : 0);
    let t7;
    if ($[31] !== isQueued || $[32] !== isResolved || $[33] !== lookups.erroredToolUseIDs || $[34] !== param.id || $[35] !== shouldAnimate || $[36] !== shouldShowDot) {
        t7 = shouldShowDot && (isQueued ? react_1.default.createElement(ink_js_1.Box, { minWidth: 2 },
            react_1.default.createElement(ink_js_1.Text, { dimColor: isQueued }, figures_js_1.BLACK_CIRCLE)) : react_1.default.createElement(ToolUseLoader_js_1.ToolUseLoader, { shouldAnimate: shouldAnimate, isUnresolved: !isResolved, isError: lookups.erroredToolUseIDs.has(param.id) }));
        $[31] = isQueued;
        $[32] = isResolved;
    ...
    if ($[38] !== t8 || $[39] !== userFacingToolName || $[40] !== userFacingToolNameBackgroundColor) {
        t9 = react_1.default.createElement(ink_js_1.Box, { flexShrink: 0 },
            react_1.default.createElement(ink_js_1.Text, { bold: true, wrap: "truncate-end", backgroundColor: userFacingToolNameBackgroundColor, color: t8 }, userFacingToolName));
        $[38] = t8;
    ...
    if ($[42] !== renderedToolUseMessage) {
        t10 = renderedToolUseMessage !== "" && react_1.default.createElement(ink_js_1.Box, { flexWrap: "nowrap" },
    ...
    if ($[48] !== t10 || $[49] !== t11 || $[50] !== t6 || $[51] !== t7 || $[52] !== t9) {
        t12 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "row", flexWrap: "nowrap", minWidth: t6 },
            t7,
            t9,
            t10,
            t11);
    ...
    if ($[54] !== inProgressToolCallCount || $[55] !== isAutoClassifier || $[56] !== isClassifierChecking || $[57] !== isQueued || $[58] !== isResolved || $[59] !== isTranscriptMode || $[60] !== isWaitingForPermission || $[61] !== lookups || $[62] !== param.id || $[63] !== progressMessagesForMessage || $[64] !== terminalSize || $[65] !== tool_0 || $[66] !== tools || $[67] !== verbose) {
        t13 = !isResolved && !isQueued && (isClassifierChecking ? react_1.default.createElement(MessageResponse_js_1.MessageResponse, { height: 1 },
            react_1.default.createElement(ink_js_1.Text, { dimColor: true }, isAutoClassifier ? "Auto classifier checking\u2026" : "Bash classifier checking\u2026")) : isWaitingForPermission ? react_1.default.createElement(MessageResponse_js_1.MessageResponse, { height: 1 },
            react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "Waiting for permission\u2026")) : renderToolUseProgressMessage(tool_0, tools, lookups, param.id, progressMessagesForMessage, {
            verbose,
            inProgressToolCallCount,
            isTranscriptMode
        }, terminalSize));
    ...
    if ($[73] !== t12 || $[74] !== t13 || $[75] !== t14) {
        t15 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
            t12,
            t13,
            t14);
        $[73] = t12;
    ...
    if ($[77] !== bg || $[78] !== t15 || $[79] !== t5) {
        t16 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "row", justifyContent: "space-between", marginTop: t5, width: "100%", backgroundColor: bg }, t15);
        $[77] = bg;
        $[78] = t15;
        $[79] = t5;
        $[80] = t16;
...
function renderToolUseMessage(tool, input, { theme, verbose, commands }) {
    try {
        ...
        (0, log_js_1.logError)(new Error(`Error rendering tool use message for ${tool.name}: ${error}`));
        return '';
    }
}
function renderToolUseProgressMessage(tool, tools, lookups, toolUseID, progressMessagesForMessage, { verbose, inProgressToolCallCount, isTranscriptMode }, terminalSize) {
    ...
    try {
        ...
        return react_1.default.createElement(react_1.default.Fragment, null,
            react_1.default.createElement(SentryErrorBoundary_js_1.SentryErrorBoundary, null,
                react_1.default.createElement(HookProgressMessage_js_1.HookProgressMessage, { hookEvent: "PreToolUse", lookups: lookups, toolUseID: toolUseID, verbose: verbose, isTranscriptMode: isTranscriptMode })),
            toolMessages);
    }
    catch (error) {
        (0, log_js_1.logError)(new Error(`Error rendering tool use progress message for ${tool.name}: ${error}`));
        return null;
    }
}
function renderToolUseQueuedMessage(tool) {
    try {
        ...
        (0, log_js_1.logError)(new Error(`Error rendering tool use queued message for ${tool.name}: ${error}`));
        return null;
    }
}
