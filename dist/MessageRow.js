...
function MessageRowImpl(t0) {
    const $ = (0, compiler_runtime_1.c)(64);
    ...
    if ($[15] !== inProgressToolUseIDs || $[16] !== isCollapsed || $[17] !== isGrouped || $[18] !== lookups || $[19] !== msg || $[20] !== screen || $[21] !== streamingToolUseIDs) {
        const siblingToolUseIDs = isGrouped || isCollapsed ? messages_js_1.EMPTY_STRING_SET : (0, messages_js_1.getSiblingToolUseIDsFromLookup)(msg, lookups);
        ...
        if (isGrouped) {
            ...
            if (isCollapsed) {
                ...
                if ($[28] !== inProgressToolUseIDs || $[29] !== msg) {
                    t5 = (0, collapseReadSearch_js_1.hasAnyToolInProgress)(msg, inProgressToolUseIDs);
                ...
                if ($[31] !== inProgressToolUseIDs || $[32] !== msg) {
                    const toolUseID = (0, messages_js_1.getToolUseID)(msg);
    ...
    if (!hasMetadata) {
        let t9;
        if ($[55] !== messageEl) {
            t9 = React.createElement(OffscreenFreeze_js_1.OffscreenFreeze, null, messageEl);
            $[55] = messageEl;
            $[56] = t9;
        }
        else {
    ...
    if ($[57] !== displayMsg || $[58] !== isTranscriptMode) {
        t9 = React.createElement(ink_js_1.Box, { flexDirection: "row", justifyContent: "flex-end", gap: 1, marginTop: 1 },
            React.createElement(MessageTimestamp_js_1.MessageTimestamp, { message: displayMsg, isTranscriptMode: isTranscriptMode }),
            React.createElement(MessageModel_js_1.MessageModel, { message: displayMsg, isTranscriptMode: isTranscriptMode }));
        $[57] = displayMsg;
        $[58] = isTranscriptMode;
        $[59] = t9;
    }
    ...
    if ($[60] !== columns || $[61] !== messageEl || $[62] !== t9) {
        t10 = React.createElement(OffscreenFreeze_js_1.OffscreenFreeze, null,
            React.createElement(ink_js_1.Box, { width: columns, flexDirection: "column" },
                t9,
                messageEl));
        $[60] = columns;
        $[61] = messageEl;
...
function isMessageStreaming(msg, streamingToolUseIDs) {
    ...
    if (msg.type === 'collapsed_read_search') {
        const toolIds = (0, collapseReadSearch_js_1.getToolUseIdsFromCollapsedGroup)(msg);
