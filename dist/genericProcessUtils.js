...
async function getAncestorPidsAsync(pid, maxDepth = 10) {
    if (process.platform === 'win32') {
      ...
      $ancestors -join ','
    `.trim();
        const result = await (0, execFileNoThrow_js_1.execFileNoThrowWithCwd)('powershell.exe', ['-NoProfile', '-Command', script], { timeout: 3000 });
    ...
    const result = await (0, execFileNoThrow_js_1.execFileNoThrowWithCwd)('sh', ['-c', script], {
        timeout: 3000,
    });
    if (result.code !== 0 || !result.stdout?.trim()) {
        return [];
...
function getProcessCommand(pid) {
    try {
        ...
        const result = (0, execFileNoThrow_js_1.execSyncWithDefaults_DEPRECATED)(command, { timeout: 1000 });
