"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.SandboxConfigTab = SandboxConfigTab;
const compiler_runtime_1 = require("react/compiler-runtime");
const React = __importStar(require("react"));
const ink_js_1 = require("../../ink.js");
const sandbox_adapter_js_1 = require("../../utils/sandbox/sandbox-adapter.js");
function SandboxConfigTab() {
    const $ = (0, compiler_runtime_1.c)(3);
    const isEnabled = sandbox_adapter_js_1.SandboxManager.isSandboxingEnabled();
    let t0;
    if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
        const depCheck = sandbox_adapter_js_1.SandboxManager.checkDependencies();
        t0 = depCheck.warnings.length > 0 ? React.createElement(ink_js_1.Box, { marginTop: 1, flexDirection: "column" }, depCheck.warnings.map(_temp)) : null;
        $[0] = t0;
    }
    else {
        t0 = $[0];
    }
    const warningsNote = t0;
    if (!isEnabled) {
        let t1;
        if ($[1] === Symbol.for("react.memo_cache_sentinel")) {
            t1 = React.createElement(ink_js_1.Box, { flexDirection: "column", paddingY: 1 },
                React.createElement(ink_js_1.Text, { color: "subtle" }, "Sandbox is not enabled"),
                warningsNote);
            $[1] = t1;
        }
        else {
            t1 = $[1];
        }
        return t1;
    }
    let t1;
    if ($[2] === Symbol.for("react.memo_cache_sentinel")) {
        const fsReadConfig = sandbox_adapter_js_1.SandboxManager.getFsReadConfig();
        const fsWriteConfig = sandbox_adapter_js_1.SandboxManager.getFsWriteConfig();
        const networkConfig = sandbox_adapter_js_1.SandboxManager.getNetworkRestrictionConfig();
        const allowUnixSockets = sandbox_adapter_js_1.SandboxManager.getAllowUnixSockets();
        const excludedCommands = sandbox_adapter_js_1.SandboxManager.getExcludedCommands();
        const globPatternWarnings = sandbox_adapter_js_1.SandboxManager.getLinuxGlobPatternWarnings();
        t1 = React.createElement(ink_js_1.Box, { flexDirection: "column", paddingY: 1 },
            React.createElement(ink_js_1.Box, { flexDirection: "column" },
                React.createElement(ink_js_1.Text, { bold: true, color: "permission" }, "Excluded Commands:"),
                React.createElement(ink_js_1.Text, { dimColor: true }, excludedCommands.length > 0 ? excludedCommands.join(", ") : "None")),
            fsReadConfig.denyOnly.length > 0 && React.createElement(ink_js_1.Box, { marginTop: 1, flexDirection: "column" },
                React.createElement(ink_js_1.Text, { bold: true, color: "permission" }, "Filesystem Read Restrictions:"),
                React.createElement(ink_js_1.Text, { dimColor: true },
                    "Denied: ",
                    fsReadConfig.denyOnly.join(", ")),
                fsReadConfig.allowWithinDeny && fsReadConfig.allowWithinDeny.length > 0 && React.createElement(ink_js_1.Text, { dimColor: true },
                    "Allowed within denied: ",
                    fsReadConfig.allowWithinDeny.join(", "))),
            fsWriteConfig.allowOnly.length > 0 && React.createElement(ink_js_1.Box, { marginTop: 1, flexDirection: "column" },
                React.createElement(ink_js_1.Text, { bold: true, color: "permission" }, "Filesystem Write Restrictions:"),
                React.createElement(ink_js_1.Text, { dimColor: true },
                    "Allowed: ",
                    fsWriteConfig.allowOnly.join(", ")),
                fsWriteConfig.denyWithinAllow.length > 0 && React.createElement(ink_js_1.Text, { dimColor: true },
                    "Denied within allowed: ",
                    fsWriteConfig.denyWithinAllow.join(", "))),
            (networkConfig.allowedHosts && networkConfig.allowedHosts.length > 0 || networkConfig.deniedHosts && networkConfig.deniedHosts.length > 0) && React.createElement(ink_js_1.Box, { marginTop: 1, flexDirection: "column" },
                React.createElement(ink_js_1.Text, { bold: true, color: "permission" },
                    "Network Restrictions",
                    (0, sandbox_adapter_js_1.shouldAllowManagedSandboxDomainsOnly)() ? " (Managed)" : "",
                    ":"),
                networkConfig.allowedHosts && networkConfig.allowedHosts.length > 0 && React.createElement(ink_js_1.Text, { dimColor: true },
                    "Allowed: ",
                    networkConfig.allowedHosts.join(", ")),
                networkConfig.deniedHosts && networkConfig.deniedHosts.length > 0 && React.createElement(ink_js_1.Text, { dimColor: true },
                    "Denied: ",
                    networkConfig.deniedHosts.join(", "))),
            allowUnixSockets && allowUnixSockets.length > 0 && React.createElement(ink_js_1.Box, { marginTop: 1, flexDirection: "column" },
                React.createElement(ink_js_1.Text, { bold: true, color: "permission" }, "Allowed Unix Sockets:"),
                React.createElement(ink_js_1.Text, { dimColor: true }, allowUnixSockets.join(", "))),
            globPatternWarnings.length > 0 && React.createElement(ink_js_1.Box, { marginTop: 1, flexDirection: "column" },
                React.createElement(ink_js_1.Text, { bold: true, color: "warning" }, "\u26A0 Warning: Glob patterns not fully supported on Linux"),
                React.createElement(ink_js_1.Text, { dimColor: true },
                    "The following patterns will be ignored:",
                    " ",
                    globPatternWarnings.slice(0, 3).join(", "),
                    globPatternWarnings.length > 3 && ` (${globPatternWarnings.length - 3} more)`)),
            warningsNote);
        $[2] = t1;
    }
    else {
        t1 = $[2];
    }
    return t1;
}
function _temp(w, i) {
    return React.createElement(ink_js_1.Text, { key: i, dimColor: true }, w);
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
