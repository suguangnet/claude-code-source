"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AsyncAgentDetailDialog = AsyncAgentDetailDialog;
const compiler_runtime_1 = require("react/compiler-runtime");
const react_1 = __importDefault(require("react"));
const useElapsedTime_js_1 = require("../../hooks/useElapsedTime.js");
const ink_js_1 = require("../../ink.js");
const useKeybinding_js_1 = require("../../keybindings/useKeybinding.js");
const Tool_js_1 = require("../../Tool.js");
const tools_js_1 = require("../../tools.js");
const format_js_1 = require("../../utils/format.js");
const messages_js_1 = require("../../utils/messages.js");
const Byline_js_1 = require("../design-system/Byline.js");
const Dialog_js_1 = require("../design-system/Dialog.js");
const KeyboardShortcutHint_js_1 = require("../design-system/KeyboardShortcutHint.js");
const UserPlanMessage_js_1 = require("../messages/UserPlanMessage.js");
const renderToolActivity_js_1 = require("./renderToolActivity.js");
const taskStatusUtils_js_1 = require("./taskStatusUtils.js");
function AsyncAgentDetailDialog(t0) {
    const $ = (0, compiler_runtime_1.c)(54);
    const { agent, onDone, onKillAgent, onBack } = t0;
    const [theme] = (0, ink_js_1.useTheme)();
    let t1;
    if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
        t1 = (0, tools_js_1.getTools)((0, Tool_js_1.getEmptyToolPermissionContext)());
        $[0] = t1;
    }
    else {
        t1 = $[0];
    }
    const tools = t1;
    const elapsedTime = (0, useElapsedTime_js_1.useElapsedTime)(agent.startTime, agent.status === "running", 1000, agent.totalPausedMs ?? 0);
    let t2;
    if ($[1] !== onDone) {
        t2 = {
            "confirm:yes": onDone
        };
        $[1] = onDone;
        $[2] = t2;
    }
    else {
        t2 = $[2];
    }
    let t3;
    if ($[3] === Symbol.for("react.memo_cache_sentinel")) {
        t3 = {
            context: "Confirmation"
        };
        $[3] = t3;
    }
    else {
        t3 = $[3];
    }
    (0, useKeybinding_js_1.useKeybindings)(t2, t3);
    let t4;
    if ($[4] !== agent.status || $[5] !== onBack || $[6] !== onDone || $[7] !== onKillAgent) {
        t4 = e => {
            if (e.key === " ") {
                e.preventDefault();
                onDone();
            }
            else {
                if (e.key === "left" && onBack) {
                    e.preventDefault();
                    onBack();
                }
                else {
                    if (e.key === "x" && agent.status === "running" && onKillAgent) {
                        e.preventDefault();
                        onKillAgent();
                    }
                }
            }
        };
        $[4] = agent.status;
        $[5] = onBack;
        $[6] = onDone;
        $[7] = onKillAgent;
        $[8] = t4;
    }
    else {
        t4 = $[8];
    }
    const handleKeyDown = t4;
    let t5;
    if ($[9] !== agent.prompt) {
        t5 = (0, messages_js_1.extractTag)(agent.prompt, "plan");
        $[9] = agent.prompt;
        $[10] = t5;
    }
    else {
        t5 = $[10];
    }
    const planContent = t5;
    const displayPrompt = agent.prompt.length > 300 ? agent.prompt.substring(0, 297) + "\u2026" : agent.prompt;
    const tokenCount = agent.result?.totalTokens ?? agent.progress?.tokenCount;
    const toolUseCount = agent.result?.totalToolUseCount ?? agent.progress?.toolUseCount;
    const t6 = agent.selectedAgent?.agentType ?? "agent";
    const t7 = agent.description || "Async agent";
    let t8;
    if ($[11] !== t6 || $[12] !== t7) {
        t8 = react_1.default.createElement(ink_js_1.Text, null,
            t6,
            " \u203A",
            " ",
            t7);
        $[11] = t6;
        $[12] = t7;
        $[13] = t8;
    }
    else {
        t8 = $[13];
    }
    const title = t8;
    let t9;
    if ($[14] !== agent.status) {
        t9 = agent.status !== "running" && react_1.default.createElement(ink_js_1.Text, { color: (0, taskStatusUtils_js_1.getTaskStatusColor)(agent.status) },
            (0, taskStatusUtils_js_1.getTaskStatusIcon)(agent.status),
            " ",
            agent.status === "completed" ? "Completed" : agent.status === "failed" ? "Failed" : "Stopped",
            " \xB7 ");
        $[14] = agent.status;
        $[15] = t9;
    }
    else {
        t9 = $[15];
    }
    let t10;
    if ($[16] !== tokenCount) {
        t10 = tokenCount !== undefined && tokenCount > 0 && react_1.default.createElement(react_1.default.Fragment, null,
            " \u00B7 ",
            (0, format_js_1.formatNumber)(tokenCount),
            " tokens");
        $[16] = tokenCount;
        $[17] = t10;
    }
    else {
        t10 = $[17];
    }
    let t11;
    if ($[18] !== toolUseCount) {
        t11 = toolUseCount !== undefined && toolUseCount > 0 && react_1.default.createElement(react_1.default.Fragment, null,
            " ",
            "\u00B7 ",
            toolUseCount,
            " ",
            toolUseCount === 1 ? "tool" : "tools");
        $[18] = toolUseCount;
        $[19] = t11;
    }
    else {
        t11 = $[19];
    }
    let t12;
    if ($[20] !== elapsedTime || $[21] !== t10 || $[22] !== t11) {
        t12 = react_1.default.createElement(ink_js_1.Text, { dimColor: true },
            elapsedTime,
            t10,
            t11);
        $[20] = elapsedTime;
        $[21] = t10;
        $[22] = t11;
        $[23] = t12;
    }
    else {
        t12 = $[23];
    }
    let t13;
    if ($[24] !== t12 || $[25] !== t9) {
        t13 = react_1.default.createElement(ink_js_1.Text, null,
            t9,
            t12);
        $[24] = t12;
        $[25] = t9;
        $[26] = t13;
    }
    else {
        t13 = $[26];
    }
    const subtitle = t13;
    let t14;
    if ($[27] !== agent.status || $[28] !== onBack || $[29] !== onKillAgent) {
        t14 = exitState => exitState.pending ? react_1.default.createElement(ink_js_1.Text, null,
            "Press ",
            exitState.keyName,
            " again to exit") : react_1.default.createElement(Byline_js_1.Byline, null,
            onBack && react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "\u2190", action: "go back" }),
            react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "Esc/Enter/Space", action: "close" }),
            agent.status === "running" && onKillAgent && react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "x", action: "stop" }));
        $[27] = agent.status;
        $[28] = onBack;
        $[29] = onKillAgent;
        $[30] = t14;
    }
    else {
        t14 = $[30];
    }
    let t15;
    if ($[31] !== agent.progress || $[32] !== agent.status || $[33] !== theme) {
        t15 = agent.status === "running" && agent.progress?.recentActivities && agent.progress.recentActivities.length > 0 && react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
            react_1.default.createElement(ink_js_1.Text, { bold: true, dimColor: true }, "Progress"),
            agent.progress.recentActivities.map((activity, i) => react_1.default.createElement(ink_js_1.Text, { key: i, dimColor: i < agent.progress.recentActivities.length - 1, wrap: "truncate-end" },
                i === agent.progress.recentActivities.length - 1 ? "\u203A " : "  ",
                (0, renderToolActivity_js_1.renderToolActivity)(activity, tools, theme))));
        $[31] = agent.progress;
        $[32] = agent.status;
        $[33] = theme;
        $[34] = t15;
    }
    else {
        t15 = $[34];
    }
    let t16;
    if ($[35] !== displayPrompt || $[36] !== planContent) {
        t16 = planContent ? react_1.default.createElement(ink_js_1.Box, { marginTop: 1 },
            react_1.default.createElement(UserPlanMessage_js_1.UserPlanMessage, { addMargin: false, planContent: planContent })) : react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: 1 },
            react_1.default.createElement(ink_js_1.Text, { bold: true, dimColor: true }, "Prompt"),
            react_1.default.createElement(ink_js_1.Text, { wrap: "wrap" }, displayPrompt));
        $[35] = displayPrompt;
        $[36] = planContent;
        $[37] = t16;
    }
    else {
        t16 = $[37];
    }
    let t17;
    if ($[38] !== agent.error || $[39] !== agent.status) {
        t17 = agent.status === "failed" && agent.error && react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: 1 },
            react_1.default.createElement(ink_js_1.Text, { bold: true, color: "error" }, "Error"),
            react_1.default.createElement(ink_js_1.Text, { color: "error", wrap: "wrap" }, agent.error));
        $[38] = agent.error;
        $[39] = agent.status;
        $[40] = t17;
    }
    else {
        t17 = $[40];
    }
    let t18;
    if ($[41] !== t15 || $[42] !== t16 || $[43] !== t17) {
        t18 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
            t15,
            t16,
            t17);
        $[41] = t15;
        $[42] = t16;
        $[43] = t17;
        $[44] = t18;
    }
    else {
        t18 = $[44];
    }
    let t19;
    if ($[45] !== onDone || $[46] !== subtitle || $[47] !== t14 || $[48] !== t18 || $[49] !== title) {
        t19 = react_1.default.createElement(Dialog_js_1.Dialog, { title: title, subtitle: subtitle, onCancel: onDone, color: "background", inputGuide: t14 }, t18);
        $[45] = onDone;
        $[46] = subtitle;
        $[47] = t14;
        $[48] = t18;
        $[49] = title;
        $[50] = t19;
    }
    else {
        t19 = $[50];
    }
    let t20;
    if ($[51] !== handleKeyDown || $[52] !== t19) {
        t20 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", tabIndex: 0, autoFocus: true, onKeyDown: handleKeyDown }, t19);
        $[51] = handleKeyDown;
        $[52] = t19;
        $[53] = t20;
    }
    else {
        t20 = $[53];
    }
    return t20;
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
