...
const dangerousCmdlets_js_1 = require("../../utils/powershell/dangerousCmdlets.js");
const parser_js_1 = require("../../utils/powershell/parser.js");
const clmTypes_js_1 = require("./clmTypes.js");
...
function checkDynamicCommandName(parsed) {
    for (const cmd of (0, parser_js_1.getAllCommands)(parsed)) {
...
function checkDownloadCradles(parsed) {
    ...
    const all = (0, parser_js_1.getAllCommands)(parsed);
...
function checkDangerousFilePathExecution(parsed) {
    for (const cmd of (0, parser_js_1.getAllCommands)(parsed)) {
        const lower = cmd.name.toLowerCase();
        const resolved = parser_js_1.COMMON_ALIASES[lower]?.toLowerCase() ?? lower;
...
function checkScriptBlockInjection(parsed) {
    ...
    const allCommandsSafe = (0, parser_js_1.getAllCommands)(parsed).every(cmd => {
        const lower = cmd.name.toLowerCase();
...
function checkEnvVarManipulation(parsed) {
    const envVars = (0, parser_js_1.getVariablesByScope)(parsed, 'env');
    if (envVars.length === 0) {
        return { behavior: 'passthrough' };
    }
