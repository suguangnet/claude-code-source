...
const textHighlighting_js_1 = require("../../utils/textHighlighting.js");
...
function HighlightedInput(t0) {
    ...
    if ($[0] !== highlights || $[1] !== text) {
        const segments = (0, textHighlighting_js_1.segmentTextByHighlights)(text, highlights);
        lines = [[]];
        let pos = 0;
    ...
    const [ref, time] = (0, ink_js_1.useAnimationFrame)(hasShimmer_0 ? 50 : null);
    const glimmerIndex = hasShimmer_0 ? sweepStart_0 + Math.floor(time / 50) % cycleLength_0 : -100;
    let t3;
    if ($[15] !== glimmerIndex || $[16] !== lines_0) {
        let t4;
        if ($[18] !== glimmerIndex) {
            t4 = (lineParts, lineIndex) => React.createElement(ink_js_1.Box, { key: lineIndex }, lineParts.length === 0 ? React.createElement(ink_js_1.Text, null, " ") : lineParts.map((part_0, partIndex) => {
                if (part_0.highlight?.shimmerColor && part_0.highlight.color) {
                    return React.createElement(ink_js_1.Text, { key: partIndex }, part_0.text.split("").map((char, charIndex) => React.createElement(ShimmerChar_js_1.ShimmerChar, { key: charIndex, char: char, index: part_0.start + charIndex, glimmerIndex: glimmerIndex, messageColor: part_0.highlight.color, shimmerColor: part_0.highlight.shimmerColor })));
                }
                return React.createElement(ink_js_1.Text, { key: partIndex, color: part_0.highlight?.color, dimColor: part_0.highlight?.dimColor, inverse: part_0.highlight?.inverse },
                    React.createElement(ink_js_1.Ansi, null, part_0.text));
            }));
            $[18] = glimmerIndex;
            $[19] = t4;
        }
    ...
    let t4;
    if ($[20] !== ref || $[21] !== t3) {
        t4 = React.createElement(ink_js_1.Box, { ref: ref, flexDirection: "column" }, t3);
        $[20] = ref;
        $[21] = t3;
        $[22] = t4;
    }
