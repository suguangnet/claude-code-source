...
const Messages_js_1 = require("../components/Messages.js");
...
const staticRender_js_1 = require("./staticRender.js");
/**
...
function StaticKeybindingProvider({ children }) {
    const { bindings } = (0, loadUserBindings_js_1.loadKeybindingsSyncWithWarnings)();
    const pendingChordRef = (0, react_1.useRef)(null);
    const handlerRegistryRef = (0, react_1.useRef)(new Map());
    const activeContexts = (0, react_1.useRef)(new Set()).current;
    return react_1.default.createElement(KeybindingContext_js_1.KeybindingProvider, { bindings: bindings, pendingChordRef: pendingChordRef, pendingChord: null, setPendingChord: () => { }, activeContexts: activeContexts, registerActiveContext: () => { }, unregisterActiveContext: () => { }, handlerRegistryRef: handlerRegistryRef }, children);
}
 ...
 */
async function streamRenderedMessages(messages, tools, sink, { columns, verbose = false, chunkSize = 40, onProgress } = {}) {
    const renderChunk = (range) => (0, staticRender_js_1.renderToAnsiString)(react_1.default.createElement(AppState_js_1.AppStateProvider, null,
        react_1.default.createElement(StaticKeybindingProvider, null,
            react_1.default.createElement(Messages_js_1.Messages, { messages: messages, tools: tools, commands: [], verbose: verbose, toolJSX: null, toolUseConfirmQueue: [], inProgressToolUseIDs: new Set(), isMessageSelectorVisible: false, conversationId: "export", screen: "prompt", streamingToolUses: [], showAllInTranscript: true, isLoading: false, renderRange: range }))), columns);
...
async function renderMessagesToPlainText(messages, tools = [], columns) {
    const parts = [];
    await streamRenderedMessages(messages, tools, chunk => void parts.push((0, strip_ansi_1.default)(chunk)), {
        columns
    });
    return parts.join('');
}
