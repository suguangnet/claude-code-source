...
function ClickableImageRef(t0) {
    ...
    const imagePath = (0, imageStore_js_1.getStoredImagePath)(imageId);
    const displayText = `[Image #${imageId}]`;
    if (imagePath && (0, supports_hyperlinks_js_1.supportsHyperlinks)()) {
        const fileUrl = (0, url_1.pathToFileURL)(imagePath).href;
        let t2;
        let t3;
        if ($[0] !== backgroundColor || $[1] !== displayText || $[2] !== isSelected) {
            t2 = React.createElement(ink_js_1.Text, { backgroundColor: backgroundColor, inverse: isSelected }, displayText);
            t3 = React.createElement(ink_js_1.Text, { backgroundColor: backgroundColor, inverse: isSelected, bold: isSelected }, displayText);
            $[0] = backgroundColor;
            $[1] = displayText;
            $[2] = isSelected;
            $[3] = t2;
        ...
        if ($[5] !== fileUrl || $[6] !== t2 || $[7] !== t3) {
            t4 = React.createElement(Link_js_1.default, { url: fileUrl, fallback: t2 }, t3);
            $[5] = fileUrl;
            $[6] = t2;
            $[7] = t3;
            $[8] = t4;
