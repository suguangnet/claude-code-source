...
const Dialog_js_1 = require("../../components/design-system/Dialog.js");
...
async function getThinkbackSkillDir() {
    const { enabled } = await (0, pluginLoader_js_1.loadAllPlugins)();
    const thinkbackPlugin = enabled.find(p => p.name === 'thinkback' || p.source && p.source.includes(getPluginId()));
    if (!thinkbackPlugin) {
        return null;
    }
    const skillDir = (0, path_1.join)(thinkbackPlugin.path, 'skills', SKILL_NAME);
    if (await (0, file_js_1.pathExists)(skillDir)) {
        return skillDir;
    }
    return null;
}
async function playAnimation(skillDir) {
    const dataPath = (0, path_1.join)(skillDir, 'year_in_review.js');
    const playerPath = (0, path_1.join)(skillDir, 'player.js');
    ...
    try {
        await (0, promises_1.readFile)(dataPath);
    }
    catch (e) {
        ...
        return {
            success: false,
            message: `Could not access animation data: ${(0, errors_js_1.toError)(e).message}`
        };
    }
    try {
        await (0, promises_1.readFile)(playerPath);
    }
    catch (e) {
        if ((0, errors_js_1.isENOENT)(e)) {
            return {
        ...
        return {
            success: false,
            message: `Could not access player script: ${(0, errors_js_1.toError)(e).message}`
        };
    }
    ...
    try {
        await (0, execa_1.execa)('node', [playerPath], {
            stdio: 'inherit',
            cwd: skillDir,
            reject: false
        });
    ...
    const htmlPath = (0, path_1.join)(skillDir, 'year_in_review.html');
    if (await (0, file_js_1.pathExists)(htmlPath)) {
...
function ThinkbackInstaller({ onReady, onError }) {
    ...
    (0, react_1.useEffect)(() => {
        async function checkAndInstall() {
            try {
                ...
                const pluginAlreadyInstalled = (0, installedPluginsManager_js_1.isPluginInstalled)(pluginId);
                if (!marketplaceInstalled) {
                    // Install the marketplace
                    setState({
                        phase: 'installing-marketplace'
                    });
                    (0, debug_js_1.logForDebugging)(`Installing marketplace ${marketplaceRepo}`);
                    await (0, marketplaceManager_js_1.addMarketplaceSource)({
                        source: 'github',
                    ...
                    (0, debug_js_1.logForDebugging)(`Marketplace ${marketplaceName} installed`);
                }
                    ...
                    (0, debug_js_1.logForDebugging)(`Refreshing marketplace ${marketplaceName}`);
                    await (0, marketplaceManager_js_1.refreshMarketplace)(marketplaceName, message_0 => {
                        setProgressMessage(message_0);
                    });
                    (0, marketplaceManager_js_1.clearMarketplacesCache)();
                    (0, cacheUtils_js_1.clearAllCaches)();
                    (0, debug_js_1.logForDebugging)(`Marketplace ${marketplaceName} refreshed`);
                }
                if (!pluginAlreadyInstalled) {
                    ...
                    (0, debug_js_1.logForDebugging)(`Installing plugin ${pluginId}`);
                    const result = await (0, pluginStartupCheck_js_1.installSelectedPlugins)([pluginId]);
                    if (result.failed.length > 0) {
                        const errorMsg = result.failed.map(f => `${f.name}: ${f.error}`).join(', ');
                        throw new Error(`Failed to install plugin: ${errorMsg}`);
                    }
                    (0, cacheUtils_js_1.clearAllCaches)();
                    (0, debug_js_1.logForDebugging)(`Plugin ${pluginId} installed`);
                }
                else {
                    // Plugin is installed, check if it's enabled
                    const { disabled } = await (0, pluginLoader_js_1.loadAllPlugins)();
                    ...
                    if (isDisabled) {
                        ...
                        (0, debug_js_1.logForDebugging)(`Enabling plugin ${pluginId}`);
                        const enableResult = await (0, pluginOperations_js_1.enablePluginOp)(pluginId);
                        if (!enableResult.success) {
                            throw new Error(`Failed to enable plugin: ${enableResult.message}`);
                        }
                        (0, cacheUtils_js_1.clearAllCaches)();
                        (0, debug_js_1.logForDebugging)(`Plugin ${pluginId} enabled`);
                    }
                }
                setState({
                    phase: 'ready'
    ...
    return React.createElement(ink_js_1.Box, { flexDirection: "column" },
        React.createElement(ink_js_1.Box, null,
            ...
            React.createElement(ink_js_1.Text, null, progressMessage || statusMessage)));
}
function ThinkbackMenu(t0) {
    const $ = (0, compiler_runtime_1.c)(19);
    const { onDone, onAction, skillDir, hasGenerated } = t0;
    const [hasSelected, setHasSelected] = (0, react_1.useState)(false);
    let t1;
    if ($[0] !== hasGenerated) {
        t1 = hasGenerated ? [{
    ...
    let t4;
    if ($[8] !== hasGenerated) {
        t4 = !hasGenerated && React.createElement(ink_js_1.Box, { flexDirection: "column" },
            React.createElement(ink_js_1.Text, null, "Relive your year of coding with Claude."),
            React.createElement(ink_js_1.Text, { dimColor: true }, "We'll create a personalized ASCII animation celebrating your journey."));
        $[8] = hasGenerated;
        $[9] = t4;
    }
    else {
    ...
    if ($[10] !== handleSelect || $[11] !== options) {
        t5 = React.createElement(select_js_1.Select, { options: options, onChange: handleSelect, visibleOptionCount: 5 });
        $[10] = handleSelect;
        $[11] = options;
        $[12] = t5;
    }
    ...
    let t6;
    if ($[13] !== t4 || $[14] !== t5) {
        t6 = React.createElement(ink_js_1.Box, { flexDirection: "column", gap: 1 },
            t4,
            t5);
        $[13] = t4;
        $[14] = t5;
    ...
    if ($[16] !== handleCancel || $[17] !== t6) {
        t7 = React.createElement(Dialog_js_1.Dialog, { title: "Think Back on 2025 with Claude Code", subtitle: "Generate your 2025 Claude Code Think Back (takes a few minutes to run)", onCancel: handleCancel, color: "claude" }, t6);
        $[16] = handleCancel;
        $[17] = t6;
        $[18] = t7;
    }
...
function ThinkbackFlow(t0) {
    ...
    const { onDone } = t0;
    const [installComplete, setInstallComplete] = (0, react_1.useState)(false);
    const [installError, setInstallError] = (0, react_1.useState)(null);
    const [skillDir, setSkillDir] = (0, react_1.useState)(null);
    const [hasGenerated, setHasGenerated] = (0, react_1.useState)(null);
    let t1;
        ...
        t3 = () => {
            if (installComplete && !skillDir && !installError) {
                getThinkbackSkillDir().then(dir => {
                    if (dir) {
                        (0, debug_js_1.logForDebugging)(`Thinkback skill directory: ${dir}`);
                        setSkillDir(dir);
                    }
                    else {
    ...
    if ($[9] !== skillDir) {
        t5 = () => {
            ...
            const dataPath = (0, path_1.join)(skillDir, "year_in_review.js");
            (0, file_js_1.pathExists)(dataPath).then(exists => {
                (0, debug_js_1.logForDebugging)(`Checking for ${dataPath}: ${exists ? "found" : "not found"}`);
    ...
    if (installError) {
        let t8;
        if ($[14] !== installError) {
            t8 = React.createElement(ink_js_1.Text, { color: "error" },
                "Error: ",
                installError);
            $[14] = installError;
            $[15] = t8;
        ...
        if ($[16] === Symbol.for("react.memo_cache_sentinel")) {
            t9 = React.createElement(ink_js_1.Text, { dimColor: true }, "Try running /plugin to manually install the think-back plugin.");
            $[16] = t9;
        }
        else {
            t9 = $[16];
        ...
        let t10;
        if ($[17] !== t8) {
            t10 = React.createElement(ink_js_1.Box, { flexDirection: "column" },
                t8,
                t9);
            $[17] = t8;
            $[18] = t10;
    ...
    if (!skillDir || hasGenerated === null) {
        ...
        if ($[21] === Symbol.for("react.memo_cache_sentinel")) {
            t8 = React.createElement(ink_js_1.Box, null,
                React.createElement(Spinner_js_1.Spinner, null),
                React.createElement(ink_js_1.Text, null, "Loading thinkback skill\u2026"));
            $[21] = t8;
        }
        else {
            t8 = $[21];
