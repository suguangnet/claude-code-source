"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.handleMcpjsonServerApprovals = handleMcpjsonServerApprovals;
const react_1 = __importDefault(require("react"));
const MCPServerApprovalDialog_js_1 = require("../components/MCPServerApprovalDialog.js");
const MCPServerMultiselectDialog_js_1 = require("../components/MCPServerMultiselectDialog.js");
const KeybindingProviderSetup_js_1 = require("../keybindings/KeybindingProviderSetup.js");
const AppState_js_1 = require("../state/AppState.js");
const config_js_1 = require("./mcp/config.js");
const utils_js_1 = require("./mcp/utils.js");
/**
 * Show MCP server approval dialogs for pending project servers.
 * Uses the provided Ink root to render (reusing the existing instance
 * from main.tsx instead of creating a separate one).
 */
async function handleMcpjsonServerApprovals(root) {
    const { servers: projectServers } = (0, config_js_1.getMcpConfigsByScope)('project');
    const pendingServers = Object.keys(projectServers).filter(serverName => (0, utils_js_1.getProjectMcpServerStatus)(serverName) === 'pending');
    if (pendingServers.length === 0) {
        return;
    }
    await new Promise(resolve => {
        const done = () => void resolve();
        if (pendingServers.length === 1 && pendingServers[0] !== undefined) {
            const serverName = pendingServers[0];
            root.render(react_1.default.createElement(AppState_js_1.AppStateProvider, null,
                react_1.default.createElement(KeybindingProviderSetup_js_1.KeybindingSetup, null,
                    react_1.default.createElement(MCPServerApprovalDialog_js_1.MCPServerApprovalDialog, { serverName: serverName, onDone: done }))));
        }
        else {
            root.render(react_1.default.createElement(AppState_js_1.AppStateProvider, null,
                react_1.default.createElement(KeybindingProviderSetup_js_1.KeybindingSetup, null,
                    react_1.default.createElement(MCPServerMultiselectDialog_js_1.MCPServerMultiselectDialog, { serverNames: pendingServers, onDone: done }))));
        }
    });
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
