"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.AssistantTextMessage = AssistantTextMessage;
const compiler_runtime_1 = require("react/compiler-runtime");
const react_1 = __importStar(require("react"));
const compact_js_1 = require("src/services/compact/compact.js");
const rateLimitMessages_js_1 = require("src/services/rateLimitMessages.js");
const figures_js_1 = require("../../constants/figures.js");
const ink_js_1 = require("../../ink.js");
const errors_js_1 = require("../../services/api/errors.js");
const messages_js_1 = require("../../utils/messages.js");
const contextWindowUpgradeCheck_js_1 = require("../../utils/model/contextWindowUpgradeCheck.js");
const model_js_1 = require("../../utils/model/model.js");
const macOsKeychainStorage_js_1 = require("../../utils/secureStorage/macOsKeychainStorage.js");
const CtrlOToExpand_js_1 = require("../CtrlOToExpand.js");
const InterruptedByUser_js_1 = require("../InterruptedByUser.js");
const Markdown_js_1 = require("../Markdown.js");
const MessageResponse_js_1 = require("../MessageResponse.js");
const messageActions_js_1 = require("../messageActions.js");
const RateLimitMessage_js_1 = require("./RateLimitMessage.js");
const MAX_API_ERROR_CHARS = 1000;
function InvalidApiKeyMessage() {
    const $ = (0, compiler_runtime_1.c)(2);
    let t0;
    if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
        t0 = (0, macOsKeychainStorage_js_1.isMacOsKeychainLocked)();
        $[0] = t0;
    }
    else {
        t0 = $[0];
    }
    const isKeychainLocked = t0;
    let t1;
    if ($[1] === Symbol.for("react.memo_cache_sentinel")) {
        t1 = react_1.default.createElement(MessageResponse_js_1.MessageResponse, null,
            react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
                react_1.default.createElement(ink_js_1.Text, { color: "error" }, errors_js_1.INVALID_API_KEY_ERROR_MESSAGE),
                isKeychainLocked && react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "\u00B7 Run in another terminal: security unlock-keychain")));
        $[1] = t1;
    }
    else {
        t1 = $[1];
    }
    return t1;
}
function AssistantTextMessage(t0) {
    const $ = (0, compiler_runtime_1.c)(34);
    const { param: t1, addMargin, shouldShowDot, verbose, onOpenRateLimitOptions } = t0;
    const { text } = t1;
    const isSelected = (0, react_1.useContext)(messageActions_js_1.MessageActionsSelectedContext);
    if ((0, messages_js_1.isEmptyMessageText)(text)) {
        return null;
    }
    if ((0, rateLimitMessages_js_1.isRateLimitErrorMessage)(text)) {
        let t2;
        if ($[0] !== onOpenRateLimitOptions || $[1] !== text) {
            t2 = react_1.default.createElement(RateLimitMessage_js_1.RateLimitMessage, { text: text, onOpenRateLimitOptions: onOpenRateLimitOptions });
            $[0] = onOpenRateLimitOptions;
            $[1] = text;
            $[2] = t2;
        }
        else {
            t2 = $[2];
        }
        return t2;
    }
    switch (text) {
        case messages_js_1.NO_RESPONSE_REQUESTED:
            {
                return null;
            }
        case errors_js_1.PROMPT_TOO_LONG_ERROR_MESSAGE:
            {
                let t2;
                if ($[3] === Symbol.for("react.memo_cache_sentinel")) {
                    t2 = (0, contextWindowUpgradeCheck_js_1.getUpgradeMessage)("warning");
                    $[3] = t2;
                }
                else {
                    t2 = $[3];
                }
                const upgradeHint = t2;
                let t3;
                if ($[4] === Symbol.for("react.memo_cache_sentinel")) {
                    t3 = react_1.default.createElement(MessageResponse_js_1.MessageResponse, { height: 1 },
                        react_1.default.createElement(ink_js_1.Text, { color: "error" },
                            "Context limit reached \u00B7 /compact or /clear to continue",
                            upgradeHint ? ` · ${upgradeHint}` : ""));
                    $[4] = t3;
                }
                else {
                    t3 = $[4];
                }
                return t3;
            }
        case errors_js_1.CREDIT_BALANCE_TOO_LOW_ERROR_MESSAGE:
            {
                let t2;
                if ($[5] === Symbol.for("react.memo_cache_sentinel")) {
                    t2 = react_1.default.createElement(MessageResponse_js_1.MessageResponse, { height: 1 },
                        react_1.default.createElement(ink_js_1.Text, { color: "error" }, "Credit balance too low \u00B7 Add funds: https://platform.claude.com/settings/billing"));
                    $[5] = t2;
                }
                else {
                    t2 = $[5];
                }
                return t2;
            }
        case errors_js_1.INVALID_API_KEY_ERROR_MESSAGE:
            {
                let t2;
                if ($[6] === Symbol.for("react.memo_cache_sentinel")) {
                    t2 = react_1.default.createElement(InvalidApiKeyMessage, null);
                    $[6] = t2;
                }
                else {
                    t2 = $[6];
                }
                return t2;
            }
        case errors_js_1.INVALID_API_KEY_ERROR_MESSAGE_EXTERNAL:
            {
                let t2;
                if ($[7] === Symbol.for("react.memo_cache_sentinel")) {
                    t2 = react_1.default.createElement(MessageResponse_js_1.MessageResponse, { height: 1 },
                        react_1.default.createElement(ink_js_1.Text, { color: "error" }, errors_js_1.INVALID_API_KEY_ERROR_MESSAGE_EXTERNAL));
                    $[7] = t2;
                }
                else {
                    t2 = $[7];
                }
                return t2;
            }
        case errors_js_1.ORG_DISABLED_ERROR_MESSAGE_ENV_KEY:
        case errors_js_1.ORG_DISABLED_ERROR_MESSAGE_ENV_KEY_WITH_OAUTH:
            {
                let t2;
                if ($[8] !== text) {
                    t2 = react_1.default.createElement(MessageResponse_js_1.MessageResponse, null,
                        react_1.default.createElement(ink_js_1.Text, { color: "error" }, text));
                    $[8] = text;
                    $[9] = t2;
                }
                else {
                    t2 = $[9];
                }
                return t2;
            }
        case errors_js_1.TOKEN_REVOKED_ERROR_MESSAGE:
            {
                let t2;
                if ($[10] === Symbol.for("react.memo_cache_sentinel")) {
                    t2 = react_1.default.createElement(MessageResponse_js_1.MessageResponse, { height: 1 },
                        react_1.default.createElement(ink_js_1.Text, { color: "error" }, errors_js_1.TOKEN_REVOKED_ERROR_MESSAGE));
                    $[10] = t2;
                }
                else {
                    t2 = $[10];
                }
                return t2;
            }
        case errors_js_1.API_TIMEOUT_ERROR_MESSAGE:
            {
                let t2;
                if ($[11] === Symbol.for("react.memo_cache_sentinel")) {
                    t2 = react_1.default.createElement(MessageResponse_js_1.MessageResponse, { height: 1 },
                        react_1.default.createElement(ink_js_1.Text, { color: "error" },
                            errors_js_1.API_TIMEOUT_ERROR_MESSAGE,
                            process.env.API_TIMEOUT_MS && react_1.default.createElement(react_1.default.Fragment, null,
                                " ",
                                "(API_TIMEOUT_MS=",
                                process.env.API_TIMEOUT_MS,
                                "ms, try increasing it)")));
                    $[11] = t2;
                }
                else {
                    t2 = $[11];
                }
                return t2;
            }
        case errors_js_1.CUSTOM_OFF_SWITCH_MESSAGE:
            {
                let t2;
                if ($[12] === Symbol.for("react.memo_cache_sentinel")) {
                    t2 = react_1.default.createElement(ink_js_1.Text, { color: "error" }, "We are experiencing high demand for Opus 4.");
                    $[12] = t2;
                }
                else {
                    t2 = $[12];
                }
                let t3;
                if ($[13] === Symbol.for("react.memo_cache_sentinel")) {
                    t3 = react_1.default.createElement(MessageResponse_js_1.MessageResponse, null,
                        react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", gap: 1 },
                            t2,
                            react_1.default.createElement(ink_js_1.Text, null,
                                "To continue immediately, use /model to switch to",
                                " ",
                                (0, model_js_1.renderModelName)((0, model_js_1.getDefaultSonnetModel)()),
                                " and continue coding.")));
                    $[13] = t3;
                }
                else {
                    t3 = $[13];
                }
                return t3;
            }
        case compact_js_1.ERROR_MESSAGE_USER_ABORT:
            {
                let t2;
                if ($[14] === Symbol.for("react.memo_cache_sentinel")) {
                    t2 = react_1.default.createElement(MessageResponse_js_1.MessageResponse, { height: 1 },
                        react_1.default.createElement(InterruptedByUser_js_1.InterruptedByUser, null));
                    $[14] = t2;
                }
                else {
                    t2 = $[14];
                }
                return t2;
            }
        default:
            {
                if ((0, errors_js_1.startsWithApiErrorPrefix)(text)) {
                    const truncated = !verbose && text.length > MAX_API_ERROR_CHARS;
                    const t2 = text === errors_js_1.API_ERROR_MESSAGE_PREFIX ? `${errors_js_1.API_ERROR_MESSAGE_PREFIX}: Please wait a moment and try again.` : truncated ? text.slice(0, MAX_API_ERROR_CHARS) + "\u2026" : text;
                    let t3;
                    if ($[15] !== t2) {
                        t3 = react_1.default.createElement(ink_js_1.Text, { color: "error" }, t2);
                        $[15] = t2;
                        $[16] = t3;
                    }
                    else {
                        t3 = $[16];
                    }
                    let t4;
                    if ($[17] !== truncated) {
                        t4 = truncated && react_1.default.createElement(CtrlOToExpand_js_1.CtrlOToExpand, null);
                        $[17] = truncated;
                        $[18] = t4;
                    }
                    else {
                        t4 = $[18];
                    }
                    let t5;
                    if ($[19] !== t3 || $[20] !== t4) {
                        t5 = react_1.default.createElement(MessageResponse_js_1.MessageResponse, null,
                            react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
                                t3,
                                t4));
                        $[19] = t3;
                        $[20] = t4;
                        $[21] = t5;
                    }
                    else {
                        t5 = $[21];
                    }
                    return t5;
                }
                const t2 = addMargin ? 1 : 0;
                const t3 = isSelected ? "messageActionsBackground" : undefined;
                let t4;
                if ($[22] !== isSelected || $[23] !== shouldShowDot) {
                    t4 = shouldShowDot && react_1.default.createElement(ink_js_1.NoSelect, { fromLeftEdge: true, minWidth: 2 },
                        react_1.default.createElement(ink_js_1.Text, { color: isSelected ? "suggestion" : "text" }, figures_js_1.BLACK_CIRCLE));
                    $[22] = isSelected;
                    $[23] = shouldShowDot;
                    $[24] = t4;
                }
                else {
                    t4 = $[24];
                }
                let t5;
                if ($[25] !== text) {
                    t5 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
                        react_1.default.createElement(Markdown_js_1.Markdown, null, text));
                    $[25] = text;
                    $[26] = t5;
                }
                else {
                    t5 = $[26];
                }
                let t6;
                if ($[27] !== t4 || $[28] !== t5) {
                    t6 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "row" },
                        t4,
                        t5);
                    $[27] = t4;
                    $[28] = t5;
                    $[29] = t6;
                }
                else {
                    t6 = $[29];
                }
                let t7;
                if ($[30] !== t2 || $[31] !== t3 || $[32] !== t6) {
                    t7 = react_1.default.createElement(ink_js_1.Box, { alignItems: "flex-start", flexDirection: "row", justifyContent: "space-between", marginTop: t2, width: "100%", backgroundColor: t3 }, t6);
                    $[30] = t2;
                    $[31] = t3;
                    $[32] = t6;
                    $[33] = t7;
                }
                else {
                    t7 = $[33];
                }
                return t7;
            }
    }
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
