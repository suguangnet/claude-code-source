...
const index_js_2 = require("../../../wizard/index.js");
const agentFileUtils_js_1 = require("../../agentFileUtils.js");
const ConfirmStep_js_1 = require("./ConfirmStep.js");
function ConfirmStepWrapper({ tools, existingAgents, onComplete }) {
    const { wizardData } = (0, index_js_2.useWizard)();
    const [saveError, setSaveError] = (0, react_1.useState)(null);
    const setAppState = (0, AppState_js_1.useSetAppState)();
    const saveAgent = (0, react_1.useCallback)(async (openInEditor) => {
        if (!wizardData?.finalAgent)
            return;
        try {
            await (0, agentFileUtils_js_1.saveAgentToFile)(wizardData.location, wizardData.finalAgent.agentType, wizardData.finalAgent.whenToUse, wizardData.finalAgent.tools, wizardData.finalAgent.getSystemPrompt(), true, wizardData.finalAgent.color, wizardData.finalAgent.model, wizardData.finalAgent.memory);
            setAppState(state => {
                if (!wizardData.finalAgent)
                    return state;
            ...
            if (openInEditor) {
                const filePath = (0, agentFileUtils_js_1.getNewAgentFilePath)({
                    source: wizardData.location,
                    agentType: wizardData.finalAgent.agentType
                });
                await (0, promptEditor_js_1.editFileInEditor)(filePath);
            }
            ...
            const message = openInEditor ? `Created agent: ${chalk_1.default.bold(wizardData.finalAgent.agentType)} and opened in editor. ` + `If you made edits, restart to load the latest version.` : `Created agent: ${chalk_1.default.bold(wizardData.finalAgent.agentType)}`;
            onComplete(message);
        }
        catch (err) {
    ...
    const handleSave = (0, react_1.useCallback)(() => saveAgent(false), [saveAgent]);
    const handleSaveAndEdit = (0, react_1.useCallback)(() => saveAgent(true), [saveAgent]);
    return react_1.default.createElement(ConfirmStep_js_1.ConfirmStep, { tools: tools, existingAgents: existingAgents, onSave: handleSave, onSaveAndEdit: handleSaveAndEdit, error: saveError });
}
