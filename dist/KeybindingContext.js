...
const KeybindingContext = (0, react_1.createContext)(null);
function KeybindingProvider(t0) {
    ...
    let t1;
    if ($[0] !== bindings) {
        t1 = (action, context) => (0, resolver_js_1.getBindingDisplayText)(action, context, bindings);
        $[0] = bindings;
        $[1] = t1;
    }
    else {
    ...
    if ($[7] !== bindings || $[8] !== pendingChordRef) {
        t4 = (input, key, contexts) => (0, resolver_js_1.resolveKeyWithChordState)(input, key, contexts, bindings, pendingChordRef.current);
        $[7] = bindings;
        $[8] = pendingChordRef;
        $[9] = t4;
    }
    ...
    if ($[21] !== children || $[22] !== value) {
        t6 = react_1.default.createElement(KeybindingContext.Provider, { value: value }, children);
        $[21] = children;
        $[22] = value;
        $[23] = t6;
    }
...
function useKeybindingContext() {
    const ctx = (0, react_1.useContext)(KeybindingContext);
    if (!ctx) {
