...
const loadOutputStylesDir_js_1 = require("../outputStyles/loadOutputStylesDir.js");
...
"\`${figures_1.default.star} Insight ─────────────────────────────────────\`
[2-3 key educational points]
\`─────────────────────────────────────────────────\`"

These insights should be included in the conversation, not in the codebase. You should generally focus on interesting insights that are specific to the codebase or the code you just wrote, rather than general programming concepts.`;
exports.DEFAULT_OUTPUT_STYLE_NAME = 'default';
exports.OUTPUT_STYLE_CONFIG = {
    [exports.DEFAULT_OUTPUT_STYLE_NAME]: null,
    Explanatory: {
        name: 'Explanatory',
        source: 'built-in',
...
### Request Format
\`\`\`
${figures_1.default.bullet} **Learn by Doing**
...
exports.getAllOutputStyles = (0, memoize_js_1.default)(async function getAllOutputStyles(cwd) {
    const customStyles = await (0, loadOutputStylesDir_js_1.getOutputStyleDirStyles)(cwd);
    const pluginStyles = await (0, loadPluginOutputStyles_js_1.loadPluginOutputStyles)();
...
function clearAllOutputStylesCache() {
    exports.getAllOutputStyles.cache?.clear?.();
}
async function getOutputStyleConfig() {
    const allStyles = await (0, exports.getAllOutputStyles)((0, cwd_js_1.getCwd)());
    ...
    if (firstForcedStyle) {
        if (forcedStyles.length > 1) {
            (0, debug_js_1.logForDebugging)(`Multiple plugins have forced output styles: ${forcedStyles.map(s => s.name).join(', ')}. Using: ${firstForcedStyle.name}`, { level: 'warn' });
        }
        (0, debug_js_1.logForDebugging)(`Using forced plugin output style: ${firstForcedStyle.name}`);
...
function hasCustomOutputStyle() {
    const style = (0, settings_js_1.getSettings_DEPRECATED)()?.outputStyle;
