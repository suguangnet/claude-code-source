...
const powershellDetection_js_1 = require("../../utils/shell/powershellDetection.js");
...
const shouldUseSandbox_js_1 = require("../BashTool/shouldUseSandbox.js");
const UI_js_1 = require("../BashTool/UI.js");
const utils_js_1 = require("../BashTool/utils.js");
...
const powershellPermissions_js_1 = require("./powershellPermissions.js");
...
function isSearchOrReadPowerShellCommand(command) {
    ...
    for (const part of parts) {
        ...
        const canonical = (0, readOnlyValidation_js_1.resolveToCanonical)(baseCommand);
...
function isAutobackgroundingAllowed(command) {
    ...
    if (!firstWord)
        return true;
    const canonical = (0, readOnlyValidation_js_1.resolveToCanonical)(firstWord);
...
exports.PowerShellTool = (0, Tool_js_1.buildTool)({
    ...
    async checkPermissions(input, context) {
        return await (0, powershellPermissions_js_1.powershellToolHasPermission)(input, context);
    },
    ...
    mapToolResultToToolResultBlockParam({ interrupted, stdout, stderr, isImage, persistedOutputPath, persistedOutputSize, backgroundTaskId, backgroundedByUser, assistantAutoBackgrounded }, toolUseID) {
        ...
        if (persistedOutputPath) {
            ...
            const preview = (0, toolResultStorage_js_1.generatePreview)(trimmed, toolResultStorage_js_1.PREVIEW_SIZE_BYTES);
    ...
    async call(input, toolUseContext, _canUseTool, _parentMessage, onProgress) {
        ...
        try {
            ...
            if (result.backgroundTaskId) {
                const bgExtracted = (0, claudeCodeHints_js_1.extractClaudeCodeHints)(result.stdout || '', input.command);
            ...
            const interpretation = (0, commandSemantics_js_1.interpretCommandResult)(input.command, result.code, processedStdout, result.stderr || '');
            ...
            let stdout = (0, utils_js_1.stripEmptyLines)(stdoutAccumulator.toString());
            ...
            const extracted = (0, claudeCodeHints_js_1.extractClaudeCodeHints)(stdout, input.command);
            ...
            let isImage = (0, utils_js_1.isImageOutput)(stdout);
            let compressedStdout = stdout;
            if (isImage) {
                const resized = await (0, utils_js_1.resizeShellImageOutput)(stdout, result.outputFilePath, persistedOutputSize);
                if (resized) {
...
async function* runPowerShellCommand({ input, abortController, setAppState, setToolJSX, preventCwdChanges, isMainThread, toolUseId, agentId }) {
    ...
    const timeoutMs = Math.min(timeout || (0, prompt_js_1.getDefaultTimeoutMs)(), (0, prompt_js_1.getMaxTimeoutMs)());
    let fullOutput = '';
    ...
    const powershellPath = await (0, powershellDetection_js_1.getCachedPowerShellPath)();
    if (!powershellPath) {
    ...
    let shellCommand;
    try {
        shellCommand = await (0, Shell_js_1.exec)(command, abortController.signal, 'powershell', {
            timeout: timeoutMs,
        ...
        return {
            stdout: '',
            stderr: `Failed to execute PowerShell command: ${(0, errors_js_1.errorMessage)(e)}`,
            code: 0,
            interrupted: false
        };
    }
