...
const claudeCodeHints_js_1 = require("../../utils/claudeCodeHints.js");
...
const hintRecommendation_js_1 = require("../../utils/plugins/hintRecommendation.js");
...
const TaskOutput_js_1 = require("../../utils/task/TaskOutput.js");
const terminal_js_1 = require("../../utils/terminal.js");
const toolResultStorage_js_1 = require("../../utils/toolResultStorage.js");
const UI_js_1 = require("../FileEditTool/UI.js");
const gitOperationTracking_js_1 = require("../shared/gitOperationTracking.js");
const bashPermissions_js_1 = require("./bashPermissions.js");
const commandSemantics_js_1 = require("./commandSemantics.js");
const prompt_js_1 = require("./prompt.js");
const readOnlyValidation_js_1 = require("./readOnlyValidation.js");
const sedEditParser_js_1 = require("./sedEditParser.js");
const shouldUseSandbox_js_1 = require("./shouldUseSandbox.js");
const toolName_js_1 = require("./toolName.js");
...
const fullInputSchema = (0, lazySchema_js_1.lazySchema)(() => v4_1.z.strictObject({
    command: v4_1.z.string().describe('The command to execute'),
    timeout: (0, semanticNumber_js_1.semanticNumber)(v4_1.z.number().optional()).describe(`Optional timeout in milliseconds (max ${(0, prompt_js_1.getMaxTimeoutMs)()})`),
...
const inputSchema = (0, lazySchema_js_1.lazySchema)(() => isBackgroundTasksDisabled ? fullInputSchema().omit({
...
function getCommandTypeForLogging(command) {
    const parts = (0, commands_js_1.splitCommand_DEPRECATED)(command);
    if (parts.length === 0)
        return 'other';
...
async function applySedEdit(simulatedEdit, toolUseContext, parentMessage) {
    ...
    const absoluteFilePath = (0, path_js_1.expandPath)(filePath);
    const fs = (0, fsOperations_js_1.getFsImplementation)();
    // Read original content for VS Code notification
    const encoding = (0, file_js_1.detectFileEncoding)(absoluteFilePath);
    let originalContent;
    try {
    ...
    if ((0, fileHistory_js_1.fileHistoryEnabled)() && parentMessage) {
        await (0, fileHistory_js_1.fileHistoryTrackEdit)(toolUseContext.updateFileHistoryState, absoluteFilePath, parentMessage.uuid);
    }
    // Detect line endings and write new content
    const endings = (0, file_js_1.detectLineEndings)(absoluteFilePath);
...
exports.BashTool = (0, Tool_js_1.buildTool)({
    ...
    async prompt() {
        return (0, prompt_js_1.getSimplePrompt)();
    },
    isConcurrencySafe(input) {
        return this.isReadOnly?.(input) ?? false;
    },
    isReadOnly(input) {
        const compoundCommandHasCd = (0, bashPermissions_js_1.commandHasAnyCd)(input.command);
        const result = (0, readOnlyValidation_js_1.checkReadOnlyConstraints)(input, compoundCommandHasCd);
    ...
    async preparePermissionMatcher({ command }) {
        ...
        const parsed = await (0, ast_js_1.parseForSecurity)(command);
        ...
        return pattern => {
            const prefix = (0, bashPermissions_js_1.permissionRuleExtractPrefix)(pattern);
    ...
    userFacingName(input) {
        ...
        if (input.command) {
            const sedInfo = (0, sedEditParser_js_1.parseSedEditCommand)(input.command);
            if (sedInfo) {
    ...
    getActivityDescription(input) {
        ...
        const desc = input.description ?? (0, format_js_1.truncate)(input.command, toolLimits_js_1.TOOL_SUMMARY_MAX_LENGTH);
        return `Running ${desc}`;
    },
    ...
    async checkPermissions(input, context) {
        return (0, bashPermissions_js_1.bashToolHasPermission)(input, context);
    },
    ...
    mapToolResultToToolResultBlockParam({ interrupted, stdout, stderr, isImage, backgroundTaskId, backgroundedByUser, assistantAutoBackgrounded, structuredContent, persistedOutputPath, persistedOutputSize }, toolUseID) {
        ...
        if (isImage) {
            const block = (0, utils_js_1.buildImageToolResult)(stdout, toolUseID);
            if (block)
                return block;
        }
        ...
        if (persistedOutputPath) {
            const preview = (0, toolResultStorage_js_1.generatePreview)(processedStdout, toolResultStorage_js_1.PREVIEW_SIZE_BYTES);
        ...
        let backgroundInfo = '';
        if (backgroundTaskId) {
            const outputPath = (0, diskOutput_js_1.getTaskOutputPath)(backgroundTaskId);
    ...
    async call(input, toolUseContext, _canUseTool, parentMessage, onProgress) {
        ...
        const stdoutAccumulator = new stringUtils_js_1.EndTruncatingAccumulator();
        ...
        try {
            ...
            const outputWithSbFailures = sandbox_adapter_js_1.SandboxManager.annotateStderrWithSandboxFailures(input.command, result.stdout || '');
        ...
        if (result.outputFilePath && result.outputTaskId) {
            try {
                const fileStat = await (0, promises_1.stat)(result.outputFilePath);
                persistedOutputSize = fileStat.size;
                await (0, toolResultStorage_js_1.ensureToolResultsDir)();
                const dest = (0, toolResultStorage_js_1.getToolResultPath)(result.outputTaskId, false);
                if (fileStat.size > MAX_PERSISTED_SIZE) {
                    await (0, promises_1.truncate)(result.outputFilePath, MAX_PERSISTED_SIZE);
                }
                try {
                    await (0, promises_1.link)(result.outputFilePath, dest);
                }
                catch {
                    await (0, promises_1.copyFile)(result.outputFilePath, dest);
                }
        ...
        const codeIndexingTool = (0, codeIndexing_js_1.detectCodeIndexingFromCommand)(input.command);
        if (codeIndexingTool) {
        ...
        let strippedStdout = (0, utils_js_1.stripEmptyLines)(stdout);
        ...
        const extracted = (0, claudeCodeHints_js_1.extractClaudeCodeHints)(strippedStdout, input.command);
        ...
        let isImage = (0, utils_js_1.isImageOutput)(strippedStdout);
        ...
        if (isImage) {
            const resized = await (0, utils_js_1.resizeShellImageOutput)(strippedStdout, result.outputFilePath, persistedOutputSize);
            if (resized) {
...
async function* runShellCommand({ input, abortController, setAppState, setToolJSX, preventCwdChanges, isMainThread, toolUseId, agentId }) {
    ...
    const timeoutMs = timeout || (0, prompt_js_1.getDefaultTimeoutMs)();
    let fullOutput = '';
    ...
    const shellCommand = await (0, Shell_js_1.exec)(command, abortController.signal, 'bash', {
        timeout: timeoutMs,
    ...
    async function spawnBackgroundTask() {
        const handle = await (0, LocalShellTask_js_1.spawnShellTask)({
            command,
    ...
    try {
        while (true) {
            ...
            if (!isBackgroundTasksDisabled && backgroundShellId === undefined && elapsedSeconds >= PROGRESS_THRESHOLD_MS / 1000 && setToolJSX) {
                ...
                setToolJSX({
                    jsx: React.createElement(UI_js_2.BackgroundHint, null),
