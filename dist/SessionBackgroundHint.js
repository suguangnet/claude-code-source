...
function SessionBackgroundHint(t0) {
    ...
    const [showSessionHint, setShowSessionHint] = (0, react_1.useState)(false);
    const handleDoublePress = (0, useDoublePress_js_1.useDoublePress)(setShowSessionHint, onBackgroundSession, _temp);
    let t1;
    if ($[0] !== appStateStore || $[1] !== handleDoublePress || $[2] !== isLoading || $[3] !== setAppState) {
        t1 = () => {
            ...
            if ((0, LocalShellTask_js_1.hasForegroundTasks)(state)) {
                (0, LocalShellTask_js_1.backgroundAll)(() => appStateStore.getState(), setAppState);
    ...
    const hasForeground = (0, AppState_js_1.useAppState)(LocalShellTask_js_1.hasForegroundTasks);
    let t2;
    ...
    const baseShortcut = (0, useShortcutDisplay_js_1.useShortcutDisplay)("task:background", "Task", "ctrl+b");
    const shortcut = env_js_1.env.terminal === "tmux" && baseShortcut === "ctrl+b" ? "ctrl+b ctrl+b" : baseShortcut;
    if (!isLoading || !showSessionHint) {
        return null;
    }
    let t5;
    if ($[8] !== shortcut) {
        t5 = React.createElement(ink_js_1.Box, { paddingLeft: 2 },
            React.createElement(ink_js_1.Text, { dimColor: true },
                React.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: shortcut, action: "background" })));
        $[8] = shortcut;
        $[9] = t5;
    }
    else {
