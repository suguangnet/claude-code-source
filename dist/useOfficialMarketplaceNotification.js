...
const officialMarketplaceStartupCheck_js_1 = require("../utils/plugins/officialMarketplaceStartupCheck.js");
const useStartupNotification_js_1 = require("./notifs/useStartupNotification.js");
/**
...
async function _temp() {
    const result = await (0, officialMarketplaceStartupCheck_js_1.checkAndInstallOfficialMarketplace)();
    const notifs = [];
    if (result.configSaveFailed) {
        (0, debug_js_1.logForDebugging)("Showing marketplace config save failure notification");
        notifs.push({
            key: "marketplace-config-save-failed",
            jsx: React.createElement(ink_js_1.Text, { color: "error" }, "Failed to save marketplace retry info \u00B7 Check ~/.claude.json permissions"),
            priority: "immediate",
            timeoutMs: 10000
        });
    }
    if (result.installed) {
        ...
        notifs.push({
            ...
            jsx: React.createElement(ink_js_1.Text, { color: "success" }, "\u2713 Anthropic marketplace installed \u00B7 /plugin to see available plugins"),
            priority: "immediate",
            timeoutMs: 7000
        });
    }
        ...
        if (result.skipped && result.reason === "unknown") {
            ...
            notifs.push({
                ...
                jsx: React.createElement(ink_js_1.Text, { color: "warning" }, "Failed to install Anthropic marketplace \u00B7 Will retry on next startup"),
                priority: "immediate",
                timeoutMs: 8000
            });
        }
