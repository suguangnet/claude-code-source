"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.call = call;
exports.Login = Login;
const compiler_runtime_1 = require("react/compiler-runtime");
const bun_bundle_1 = require("bun:bundle");
const React = __importStar(require("react"));
const state_js_1 = require("../../bootstrap/state.js");
const trustedDevice_js_1 = require("../../bridge/trustedDevice.js");
const ConfigurableShortcutHint_js_1 = require("../../components/ConfigurableShortcutHint.js");
const ConsoleOAuthFlow_js_1 = require("../../components/ConsoleOAuthFlow.js");
const Dialog_js_1 = require("../../components/design-system/Dialog.js");
const useMainLoopModel_js_1 = require("../../hooks/useMainLoopModel.js");
const ink_js_1 = require("../../ink.js");
const growthbook_js_1 = require("../../services/analytics/growthbook.js");
const index_js_1 = require("../../services/policyLimits/index.js");
const index_js_2 = require("../../services/remoteManagedSettings/index.js");
const messages_js_1 = require("../../utils/messages.js");
const bypassPermissionsKillswitch_js_1 = require("../../utils/permissions/bypassPermissionsKillswitch.js");
const user_js_1 = require("../../utils/user.js");
async function call(onDone, context) {
    return React.createElement(Login, { onDone: async (success) => {
            context.onChangeAPIKey();
            // Signature-bearing blocks (thinking, connector_text) are bound to the API key —
            // strip them so the new key doesn't reject stale signatures.
            context.setMessages(messages_js_1.stripSignatureBlocks);
            if (success) {
                // Post-login refresh logic. Keep in sync with onboarding in src/interactiveHelpers.tsx
                // Reset cost state when switching accounts
                (0, state_js_1.resetCostState)();
                // Refresh remotely managed settings after login (non-blocking)
                void (0, index_js_2.refreshRemoteManagedSettings)();
                // Refresh policy limits after login (non-blocking)
                void (0, index_js_1.refreshPolicyLimits)();
                // Clear user data cache BEFORE GrowthBook refresh so it picks up fresh credentials
                (0, user_js_1.resetUserCache)();
                // Refresh GrowthBook after login to get updated feature flags (e.g., for claude.ai MCPs)
                (0, growthbook_js_1.refreshGrowthBookAfterAuthChange)();
                // Clear any stale trusted device token from a previous account before
                // re-enrolling — prevents sending the old token on bridge calls while
                // the async enrollTrustedDevice() is in-flight.
                (0, trustedDevice_js_1.clearTrustedDeviceToken)();
                // Enroll as a trusted device for Remote Control (10-min fresh-session window)
                void (0, trustedDevice_js_1.enrollTrustedDevice)();
                // Reset killswitch gate checks and re-run with new org
                (0, bypassPermissionsKillswitch_js_1.resetBypassPermissionsCheck)();
                const appState = context.getAppState();
                void (0, bypassPermissionsKillswitch_js_1.checkAndDisableBypassPermissionsIfNeeded)(appState.toolPermissionContext, context.setAppState);
                if ((0, bun_bundle_1.feature)('TRANSCRIPT_CLASSIFIER')) {
                    (0, bypassPermissionsKillswitch_js_1.resetAutoModeGateCheck)();
                    void (0, bypassPermissionsKillswitch_js_1.checkAndDisableAutoModeIfNeeded)(appState.toolPermissionContext, context.setAppState, appState.fastMode);
                }
                // Increment authVersion to trigger re-fetching of auth-dependent data in hooks (e.g., MCP servers)
                context.setAppState(prev => ({
                    ...prev,
                    authVersion: prev.authVersion + 1
                }));
            }
            onDone(success ? 'Login successful' : 'Login interrupted');
        } });
}
function Login(props) {
    const $ = (0, compiler_runtime_1.c)(12);
    const mainLoopModel = (0, useMainLoopModel_js_1.useMainLoopModel)();
    let t0;
    if ($[0] !== mainLoopModel || $[1] !== props) {
        t0 = () => props.onDone(false, mainLoopModel);
        $[0] = mainLoopModel;
        $[1] = props;
        $[2] = t0;
    }
    else {
        t0 = $[2];
    }
    let t1;
    if ($[3] !== mainLoopModel || $[4] !== props) {
        t1 = () => props.onDone(true, mainLoopModel);
        $[3] = mainLoopModel;
        $[4] = props;
        $[5] = t1;
    }
    else {
        t1 = $[5];
    }
    let t2;
    if ($[6] !== props.startingMessage || $[7] !== t1) {
        t2 = React.createElement(ConsoleOAuthFlow_js_1.ConsoleOAuthFlow, { onDone: t1, startingMessage: props.startingMessage });
        $[6] = props.startingMessage;
        $[7] = t1;
        $[8] = t2;
    }
    else {
        t2 = $[8];
    }
    let t3;
    if ($[9] !== t0 || $[10] !== t2) {
        t3 = React.createElement(Dialog_js_1.Dialog, { title: "Login", onCancel: t0, color: "permission", inputGuide: _temp }, t2);
        $[9] = t0;
        $[10] = t2;
        $[11] = t3;
    }
    else {
        t3 = $[11];
    }
    return t3;
}
function _temp(exitState) {
    return exitState.pending ? React.createElement(ink_js_1.Text, null,
        "Press ",
        exitState.keyName,
        " again to exit") : React.createElement(ConfigurableShortcutHint_js_1.ConfigurableShortcutHint, { action: "confirm:no", context: "Confirmation", fallback: "Esc", description: "cancel" });
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
