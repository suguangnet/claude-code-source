...
const shellCompletion_js_1 = require("../utils/bash/shellCompletion.js");
...
const directoryCompletion_js_1 = require("../utils/suggestions/directoryCompletion.js");
const shellHistoryCompletion_js_1 = require("../utils/suggestions/shellHistoryCompletion.js");
...
const fileSuggestions_js_1 = require("./fileSuggestions.js");
const unifiedSuggestions_js_1 = require("./unifiedSuggestions.js");
...
async function generateBashSuggestions(input, cursorOffset) {
    try {
        ...
        const suggestions = await (0, shellCompletion_js_1.getShellCompletions)(input, cursorOffset, currentShellCompletionAbortController.signal);
        return suggestions;
    }
    catch {
 ...
 */
function useTypeahead({ commands, onInputChange, onSubmit, setCursorOffset, input, cursorOffset, mode, agents, setSuggestionsState, suggestionsState: { suggestions, selectedSuggestion, commandArgumentHint }, suppressSuggestions = false, markAccepted, onModeChange }) {
    const { addNotification } = (0, notifications_js_1.useNotifications)();
    const thinkingToggleShortcut = (0, useShortcutDisplay_js_1.useShortcutDisplay)('chat:thinkingToggle', 'Chat', 'alt+t');
    const [suggestionType, setSuggestionType] = (0, react_1.useState)('none');
    // Compute max column width from ALL commands once (not filtered results)
    // This prevents layout shift when filtering
    const allCommandsMaxWidth = (0, react_1.useMemo)(() => {
        const visibleCommands = commands.filter(cmd => !cmd.isHidden);
        if (visibleCommands.length === 0)
            return undefined;
        const maxLen = Math.max(...visibleCommands.map(cmd => (0, commands_js_1.getCommandName)(cmd).length));
        return maxLen + 6; // +1 for "/" prefix, +5 for padding
    }, [commands]);
    const [maxColumnWidth, setMaxColumnWidth] = (0, react_1.useState)(undefined);
    const mcpResources = (0, AppState_js_1.useAppState)(s => s.mcp.resources);
    ...
    const isViewingTeammate = (0, AppState_js_1.useAppState)(s => !!s.viewingAgentTaskId);
    ...
    const [inlineGhostText, setInlineGhostText] = (0, react_1.useState)(undefined);
    ...
    const syncPromptGhostText = (0, react_1.useMemo)(() => {
        if (mode !== 'prompt' || suppressSuggestions)
            return undefined;
        const midInputCommand = (0, commandSuggestions_js_1.findMidInputSlashCommand)(input, cursorOffset);
        if (!midInputCommand)
            return undefined;
        const match = (0, commandSuggestions_js_1.getBestCommandMatch)(midInputCommand.partialCommand, commands);
        if (!match)
            return undefined;
        return {
            text: match.suffix,
    ...
    const cursorOffsetRef = (0, react_1.useRef)(cursorOffset);
    cursorOffsetRef.current = cursorOffset;
    // Track the latest search token to discard stale results from slow async operations
    const latestSearchTokenRef = (0, react_1.useRef)(null);
    ...
    const fetchFileSuggestions = (0, react_1.useCallback)(async (searchToken, isAtSymbol = false) => {
        latestSearchTokenRef.current = searchToken;
        const combinedItems = await (0, unifiedSuggestions_js_1.generateUnifiedSuggestions)(searchToken, mcpResources, agents, isAtSymbol);
    ...
    (0, react_1.useEffect)(() => {
        ...
        return (0, fileSuggestions_js_1.onIndexBuildComplete)(() => {
    ...
    const debouncedFetchFileSuggestions = (0, usehooks_ts_1.useDebounceCallback)(fetchFileSuggestions, 50);
    const fetchSlackChannels = (0, react_1.useCallback)(async (partial) => {
        latestSlackTokenRef.current = partial;
        const channels = await (0, slackChannelSuggestions_js_1.getSlackChannelSuggestions)(store.getState().mcp.clients, partial);
    ...
    const debouncedFetchSlackChannels = (0, usehooks_ts_1.useDebounceCallback)(fetchSlackChannels, 150);
    ...
    const updateSuggestions = (0, react_1.useCallback)(async (value, inputCursorOffset) => {
        ...
        if (mode === 'prompt') {
            const midInputCommand = (0, commandSuggestions_js_1.findMidInputSlashCommand)(value, effectiveCursorOffset);
            if (midInputCommand) {
                const match = (0, commandSuggestions_js_1.getBestCommandMatch)(midInputCommand.partialCommand, commands);
                if (match) {
        ...
        if (mode === 'bash' && value.trim()) {
            ...
            const historyMatch = await (0, shellHistoryCompletion_js_1.getShellHistoryCompletion)(value);
        ...
        if (mode === 'prompt' && (0, commandSuggestions_js_1.isCommandInput)(value) && effectiveCursorOffset > 0) {
            ...
            if (parsedCommand && parsedCommand.commandName === 'add-dir' && parsedCommand.args) {
                ...
                const dirSuggestions = await (0, directoryCompletion_js_1.getDirectoryCompletions)(args);
            ...
            if (parsedCommand && parsedCommand.commandName === 'resume' && parsedCommand.args !== undefined && value.includes(' ')) {
                ...
                const matches = await (0, sessionStorage_js_1.searchSessionsByCustomTitle)(args, {
                    limit: 10
                });
        ...
        if (mode === 'prompt' && (0, commandSuggestions_js_1.isCommandInput)(value) && effectiveCursorOffset > 0 && !hasCommandWithArguments(isAtEndWithWhitespace, value)) {
            ...
            if (value.length > 1) {
                ...
                if (spaceIndex !== -1) {
                    const exactMatch = commands.find(cmd => (0, commands_js_1.getCommandName)(cmd) === commandName);
                    if (exactMatch || hasRealArguments) {
                        ...
                        // Priority 2: Progressive hint from argNames (show when trailing space)
                            ...
                            const typedArgs = (0, argumentSubstitution_js_1.parseArguments)(argsText);
                            commandArgumentHint = (0, argumentSubstitution_js_1.generateProgressiveArgumentHint)(exactMatch.argNames, typedArgs);
                        }
                        setSuggestionsState(() => ({
                            commandArgumentHint,
                            suggestions: [],
            ...
            const commandItems = (0, commandSuggestions_js_1.generateCommandSuggestions)(value, commands);
            setSuggestionsState(() => ({
                commandArgumentHint,
        ...
        if (hasAtSymbol && mode !== 'bash') {
            ...
            if (completionToken && completionToken.token.startsWith('@')) {
                ...
                if ((0, directoryCompletion_js_1.isPathLikeToken)(searchToken)) {
                    latestPathTokenRef.current = searchToken;
                    const pathSuggestions = await (0, directoryCompletion_js_1.getPathCompletions)(searchToken, {
                        maxResults: 10
                    });
                    // Discard stale results if a newer query was initiated while waiting
    ...
    const handleTab = (0, react_1.useCallback)(async () => {
        ...
        if (suggestions.length > 0) {
            ...
            if (suggestionType === 'command' && index < suggestions.length) {
                ...
                if (suggestion) {
                    ...
                    const isInCommandContext = (0, commandSuggestions_js_1.isCommandInput)(input);
                    let newInput;
                ...
                const commonPrefix = (0, fileSuggestions_js_1.findLongestCommonPrefix)(suggestions);
            ...
            if (mode === 'bash') {
                ...
                if (completionInfo) {
                    ...
                    suggestionItems = await (0, unifiedSuggestions_js_1.generateUnifiedSuggestions)(searchToken, mcpResources, agents, isAtSymbol);
                }
                else {
                    suggestionItems = [];
                }
    ...
    const handleAutocompleteAccept = (0, react_1.useCallback)(() => {
        void handleTab();
    }, [handleTab]);
    // Handler for autocomplete:dismiss - clears suggestions and prevents re-triggering
    const handleAutocompleteDismiss = (0, react_1.useCallback)(() => {
        debouncedFetchFileSuggestions.cancel();
        debouncedFetchSlackChannels.cancel();
        clearSuggestions();
    ...
    const handleAutocompletePrevious = (0, react_1.useCallback)(() => {
        setSuggestionsState(prev => ({
            ...prev,
    ...
    const handleAutocompleteNext = (0, react_1.useCallback)(() => {
        setSuggestionsState(prev => ({
            ...prev,
    ...
    const autocompleteHandlers = (0, react_1.useMemo)(() => ({
    ...
    const isModalOverlayActive = (0, overlayContext_js_1.useIsModalOverlayActive)();
    ...
    function acceptSuggestionText(text) {
        const detectedMode = (0, inputModes_js_1.getModeFromInput)(text);
        if (detectedMode !== 'prompt' && onModeChange) {
            onModeChange(detectedMode);
            const stripped = (0, inputModes_js_1.getValueFromInput)(text);
    ...
    (0, ink_js_2.useInput)((_input, _key, event) => {
        const kbEvent = new keyboard_event_js_1.KeyboardEvent(event.keypress);
