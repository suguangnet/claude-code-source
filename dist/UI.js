...
const FilePathLink_js_1 = require("../../components/FilePathLink.js");
...
function getAgentOutputTaskId(filePath) {
    const prefix = `${(0, diskOutput_js_1.getTaskOutputDir)()}/`;
    const suffix = '.output';
...
function renderToolUseMessage({ file_path, offset, limit, pages }, { verbose }) {
    if (!file_path) {
        return null;
    }
    ...
    const displayPath = verbose ? file_path : (0, file_js_1.getDisplayPath)(file_path);
    if (pages) {
        return React.createElement(React.Fragment, null,
            React.createElement(FilePathLink_js_1.FilePathLink, { filePath: file_path }, displayPath),
            ` · pages ${pages}`);
    }
    ...
    return React.createElement(FilePathLink_js_1.FilePathLink, { filePath: file_path }, displayPath);
}
function renderToolUseTag({ file_path }) {
...
function renderToolResultMessage(output) {
    ...
    switch (output.type) {
        case 'image':
            {
                ...
                const formattedSize = (0, format_js_1.formatFileSize)(originalSize);
                return React.createElement(MessageResponse_js_1.MessageResponse, { height: 1 },
        ...
        case 'notebook':
            {
                ...
                if (!cells || cells.length < 1) {
                    return React.createElement(ink_js_1.Text, { color: "error" }, "No cells found in notebook");
                }
                return React.createElement(MessageResponse_js_1.MessageResponse, { height: 1 },
                    React.createElement(ink_js_1.Text, null,
                        "Read ",
                        React.createElement(ink_js_1.Text, { bold: true }, cells.length),
                        " cells"));
            }
        case 'pdf':
            {
        ...
        case 'parts':
            {
                return React.createElement(MessageResponse_js_1.MessageResponse, { height: 1 },
                    React.createElement(ink_js_1.Text, null,
                        "Read ",
                        React.createElement(ink_js_1.Text, { bold: true }, output.file.count),
                        ' ',
        ...
        case 'text':
            {
                ...
                return React.createElement(MessageResponse_js_1.MessageResponse, { height: 1 },
                    React.createElement(ink_js_1.Text, null,
                        "Read ",
                        React.createElement(ink_js_1.Text, { bold: true }, numLines),
                        ' ',
        ...
        case 'file_unchanged':
            {
                return React.createElement(MessageResponse_js_1.MessageResponse, { height: 1 },
                    React.createElement(ink_js_1.Text, { dimColor: true }, "Unchanged since last read"));
            }
    }
}
function renderToolUseErrorMessage(result, { verbose }) {
    if (!verbose && typeof result === 'string') {
        ...
        if (result.includes(file_js_1.FILE_NOT_FOUND_CWD_NOTE)) {
            return React.createElement(MessageResponse_js_1.MessageResponse, null,
                React.createElement(ink_js_1.Text, { color: "error" }, "File not found"));
        }
        if ((0, messages_js_1.extractTag)(result, 'tool_use_error')) {
            return React.createElement(MessageResponse_js_1.MessageResponse, null,
                React.createElement(ink_js_1.Text, { color: "error" }, "Error reading file"));
        }
    }
    return React.createElement(FallbackToolUseErrorMessage_js_1.FallbackToolUseErrorMessage, { result: result, verbose: verbose });
}
