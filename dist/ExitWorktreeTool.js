...
async function countWorktreeChanges(worktreePath, originalHeadCommit) {
    const status = await (0, execFileNoThrow_js_1.execFileNoThrow)('git', [
        '-C',
        worktreePath,
        'status',
        '--porcelain',
    ...
    const changedFiles = (0, array_js_1.count)(status.stdout.split('\n'), l => l.trim() !== '');
    ...
    const revList = await (0, execFileNoThrow_js_1.execFileNoThrow)('git', [
        '-C',
        worktreePath,
        'rev-list',
        '--count',
...
exports.ExitWorktreeTool = (0, Tool_js_1.buildTool)({
    ...
    async prompt() {
        return (0, prompt_js_1.getExitWorktreeToolPrompt)();
    },
    get inputSchema() {
        return inputSchema();
    },
    ...
    async validateInput(input) {
        ...
        const session = (0, worktree_js_1.getCurrentWorktreeSession)();
        if (!session) {
            return {
                result: false,
    ...
    async call(input) {
        ...
        const projectRootIsWorktree = (0, state_js_1.getProjectRoot)() === (0, state_js_1.getOriginalCwd)();
        ...
        // action === 'remove'
        if (tmuxSessionName) {
            await (0, worktree_js_1.killTmuxSession)(tmuxSessionName);
        }
