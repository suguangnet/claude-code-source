"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.call = call;
const path_1 = require("path");
const react_1 = __importDefault(require("react"));
const state_js_1 = require("../../bootstrap/state.js");
const SandboxSettings_js_1 = require("../../components/sandbox/SandboxSettings.js");
const ink_js_1 = require("../../ink.js");
const platform_js_1 = require("../../utils/platform.js");
const sandbox_adapter_js_1 = require("../../utils/sandbox/sandbox-adapter.js");
const settings_js_1 = require("../../utils/settings/settings.js");
async function call(onDone, _context, args) {
    const settings = (0, settings_js_1.getSettings_DEPRECATED)();
    const themeName = settings.theme || 'light';
    const platform = (0, platform_js_1.getPlatform)();
    if (!sandbox_adapter_js_1.SandboxManager.isSupportedPlatform()) {
        // WSL1 users will see this since isSupportedPlatform returns false for WSL1
        const errorMessage = platform === 'wsl' ? 'Error: Sandboxing requires WSL2. WSL1 is not supported.' : 'Error: Sandboxing is currently only supported on macOS, Linux, and WSL2.';
        const message = (0, ink_js_1.color)('error', themeName)(errorMessage);
        onDone(message);
        return null;
    }
    // Check dependencies - get structured result with errors/warnings
    const depCheck = sandbox_adapter_js_1.SandboxManager.checkDependencies();
    // Check if platform is in enabledPlatforms list (undocumented enterprise setting)
    if (!sandbox_adapter_js_1.SandboxManager.isPlatformInEnabledList()) {
        const message = (0, ink_js_1.color)('error', themeName)(`Error: Sandboxing is disabled for this platform (${platform}) via the enabledPlatforms setting.`);
        onDone(message);
        return null;
    }
    // Check if sandbox settings are locked by higher-priority settings
    if (sandbox_adapter_js_1.SandboxManager.areSandboxSettingsLockedByPolicy()) {
        const message = (0, ink_js_1.color)('error', themeName)('Error: Sandbox settings are overridden by a higher-priority configuration and cannot be changed locally.');
        onDone(message);
        return null;
    }
    // Parse the arguments
    const trimmedArgs = args?.trim() || '';
    // If no args, show the interactive menu
    if (!trimmedArgs) {
        return react_1.default.createElement(SandboxSettings_js_1.SandboxSettings, { onComplete: onDone, depCheck: depCheck });
    }
    // Handle subcommands
    if (trimmedArgs) {
        const parts = trimmedArgs.split(' ');
        const subcommand = parts[0];
        if (subcommand === 'exclude') {
            // Handle exclude subcommand
            const commandPattern = trimmedArgs.slice('exclude '.length).trim();
            if (!commandPattern) {
                const message = (0, ink_js_1.color)('error', themeName)('Error: Please provide a command pattern to exclude (e.g., /sandbox exclude "npm run test:*")');
                onDone(message);
                return null;
            }
            // Remove quotes if present
            const cleanPattern = commandPattern.replace(/^["']|["']$/g, '');
            // Add to excludedCommands
            (0, sandbox_adapter_js_1.addToExcludedCommands)(cleanPattern);
            // Get the local settings path and make it relative to cwd
            const localSettingsPath = (0, settings_js_1.getSettingsFilePathForSource)('localSettings');
            const relativePath = localSettingsPath ? (0, path_1.relative)((0, state_js_1.getCwdState)(), localSettingsPath) : '.claude/settings.local.json';
            const message = (0, ink_js_1.color)('success', themeName)(`Added "${cleanPattern}" to excluded commands in ${relativePath}`);
            onDone(message);
            return null;
        }
        else {
            // Unknown subcommand
            const message = (0, ink_js_1.color)('error', themeName)(`Error: Unknown subcommand "${subcommand}". Available subcommand: exclude`);
            onDone(message);
            return null;
        }
    }
    // Should never reach here since we handle all cases above
    return null;
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
