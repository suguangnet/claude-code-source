...
function resumeHelpMessage(result) {
    switch (result.resultType) {
        case 'sessionNotFound':
            return `Session ${chalk_1.default.bold(result.arg)} was not found.`;
        case 'multipleMatches':
            return `Found ${result.count} sessions matching ${chalk_1.default.bold(result.arg)}. Please use /resume to pick a specific session.`;
    }
}
function ResumeError(t0) {
    ...
    let t3;
    if ($[3] !== args) {
        t3 = React.createElement(ink_js_1.Text, { dimColor: true },
    ...
    let t4;
    if ($[5] !== message) {
        t4 = React.createElement(MessageResponse_js_1.MessageResponse, null,
            React.createElement(ink_js_1.Text, null, message));
        $[5] = message;
        $[6] = t4;
    }
    else {
...
function ResumeCommand({ onDone, onResume }) {
    ...
    const insideModal = (0, modalContext_js_1.useIsInsideModal)();
    const loadLogs = React.useCallback(async (allProjects, paths) => {
        setLoading(true);
        try {
            const allLogs = allProjects ? await (0, sessionStorage_js_1.loadAllProjectsMessageLogs)() : await (0, sessionStorage_js_1.loadSameRepoMessageLogs)(paths);
            const resumable = filterResumableSessions(allLogs, (0, state_js_1.getSessionId)());
    ...
    React.useEffect(() => {
        async function init() {
            const paths_0 = await (0, getWorktreePaths_js_1.getWorktreePaths)((0, state_js_1.getOriginalCwd)());
    ...
    async function handleSelect(log) {
        const sessionId = (0, uuid_js_1.validateUuid)((0, sessionStorage_js_1.getSessionIdFromLog)(log));
        if (!sessionId) {
            onDone('Failed to resume conversation');
            return;
        }
        // Load full messages for lite logs
        const fullLog = (0, sessionStorage_js_1.isLiteLog)(log) ? await (0, sessionStorage_js_1.loadFullLog)(log) : log;
        // Check if this conversation is from a different directory
        const crossProjectCheck = (0, crossProjectResume_js_1.checkCrossProjectResume)(fullLog, showAllProjects, worktreePaths);
...
const call = async (onDone, context, args) => {
    ...
    const logs = await (0, sessionStorage_js_1.loadSameRepoMessageLogs)(worktreePaths);
    if (logs.length === 0) {
    ...
    const maybeSessionId = (0, uuid_js_1.validateUuid)(arg);
    if (maybeSessionId) {
        const matchingLogs = logs.filter(l => (0, sessionStorage_js_1.getSessionIdFromLog)(l) === maybeSessionId).sort((a, b) => b.modified.getTime() - a.modified.getTime());
        ...
        const directLog = await (0, sessionStorage_js_1.getLastSessionLog)(maybeSessionId);
        if (directLog) {
    ...
    if ((0, sessionStorage_js_1.isCustomTitleEnabled)()) {
        const titleMatches = await (0, sessionStorage_js_1.searchSessionsByCustomTitle)(arg, {
            exact: true
        });
