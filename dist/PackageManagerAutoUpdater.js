...
const packageManagers_js_1 = require("../utils/nativeInstaller/packageManagers.js");
...
function PackageManagerAutoUpdater(t0) {
    ...
    const [packageManager, setPackageManager] = (0, react_1.useState)("unknown");
    let t1;
    if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
        t1 = async () => {
            ...
            const [channel, pm] = await Promise.all([Promise.resolve((0, settings_js_1.getInitialSettings)()?.autoUpdatesChannel ?? "latest"), (0, packageManagers_js_1.getPackageManager)()]);
            setPackageManager(pm);
            let latest = await (0, autoUpdater_js_1.getLatestVersionFromGcs)(channel);
            const maxVersion = await (0, autoUpdater_js_1.getMaxVersion)();
            if (maxVersion && latest && (0, semver_js_1.gt)(latest, maxVersion)) {
                (0, debug_js_1.logForDebugging)(`PackageManagerAutoUpdater: maxVersion ${maxVersion} is set, capping update from ${latest} to ${maxVersion}`);
                if ((0, semver_js_1.gte)(MACRO.VERSION, maxVersion)) {
                    (0, debug_js_1.logForDebugging)(`PackageManagerAutoUpdater: current version ${MACRO.VERSION} is already at or above maxVersion ${maxVersion}, skipping update`);
                    setUpdateAvailable(false);
                    return;
                }
                latest = maxVersion;
            }
            const hasUpdate = latest && !(0, semver_js_1.gte)(MACRO.VERSION, latest) && !(0, autoUpdater_js_1.shouldSkipVersion)(latest);
            setUpdateAvailable(!!hasUpdate);
            if (hasUpdate) {
                (0, debug_js_1.logForDebugging)(`PackageManagerAutoUpdater: Update available ${MACRO.VERSION} -> ${latest}`);
            }
        };
        $[0] = t1;
    }
    ...
    let t4;
    if ($[3] !== verbose) {
        t4 = verbose && React.createElement(ink_js_1.Text, { dimColor: true, wrap: "truncate" },
            "currentVersion: ",
            MACRO.VERSION);
        $[3] = verbose;
        $[4] = t4;
    ...
    let t5;
    if ($[5] !== updateCommand) {
        t5 = React.createElement(ink_js_1.Text, { color: "warning", wrap: "truncate" },
            "Update available! Run: ",
            React.createElement(ink_js_1.Text, { bold: true }, updateCommand));
        $[5] = updateCommand;
        $[6] = t5;
    }
    else {
