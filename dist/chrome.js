"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.call = void 0;
const compiler_runtime_1 = require("react/compiler-runtime");
const react_1 = __importStar(require("react"));
const select_js_1 = require("../../components/CustomSelect/select.js");
const Dialog_js_1 = require("../../components/design-system/Dialog.js");
const ink_js_1 = require("../../ink.js");
const AppState_js_1 = require("../../state/AppState.js");
const auth_js_1 = require("../../utils/auth.js");
const browser_js_1 = require("../../utils/browser.js");
const common_js_1 = require("../../utils/claudeInChrome/common.js");
const setup_js_1 = require("../../utils/claudeInChrome/setup.js");
const config_js_1 = require("../../utils/config.js");
const env_js_1 = require("../../utils/env.js");
const envUtils_js_1 = require("../../utils/envUtils.js");
const CHROME_EXTENSION_URL = 'https://claude.ai/chrome';
const CHROME_PERMISSIONS_URL = 'https://clau.de/chrome/permissions';
const CHROME_RECONNECT_URL = 'https://clau.de/chrome/reconnect';
function ClaudeInChromeMenu(t0) {
    const $ = (0, compiler_runtime_1.c)(41);
    const { onDone, isExtensionInstalled: installed, configEnabled, isClaudeAISubscriber, isWSL } = t0;
    const mcpClients = (0, AppState_js_1.useAppState)(_temp);
    const [selectKey, setSelectKey] = (0, react_1.useState)(0);
    const [enabledByDefault, setEnabledByDefault] = (0, react_1.useState)(configEnabled ?? false);
    const [showInstallHint, setShowInstallHint] = (0, react_1.useState)(false);
    const [isExtensionInstalled, setIsExtensionInstalled] = (0, react_1.useState)(installed);
    let t1;
    if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
        t1 = false && (0, envUtils_js_1.isRunningOnHomespace)();
        $[0] = t1;
    }
    else {
        t1 = $[0];
    }
    const isHomespace = t1;
    let t2;
    if ($[1] !== mcpClients) {
        t2 = mcpClients.find(_temp2);
        $[1] = mcpClients;
        $[2] = t2;
    }
    else {
        t2 = $[2];
    }
    const chromeClient = t2;
    const isConnected = chromeClient?.type === "connected";
    let t3;
    if ($[3] === Symbol.for("react.memo_cache_sentinel")) {
        t3 = function openUrl(url) {
            if (isHomespace) {
                (0, browser_js_1.openBrowser)(url);
            }
            else {
                (0, common_js_1.openInChrome)(url);
            }
        };
        $[3] = t3;
    }
    else {
        t3 = $[3];
    }
    const openUrl = t3;
    let t4;
    if ($[4] !== enabledByDefault) {
        t4 = function handleAction(action) {
            bb22: switch (action) {
                case "install-extension":
                    {
                        setSelectKey(_temp3);
                        setShowInstallHint(true);
                        openUrl(CHROME_EXTENSION_URL);
                        break bb22;
                    }
                case "reconnect":
                    {
                        setSelectKey(_temp4);
                        (0, setup_js_1.isChromeExtensionInstalled)().then(installed_0 => {
                            setIsExtensionInstalled(installed_0);
                            if (installed_0) {
                                setShowInstallHint(false);
                            }
                        });
                        openUrl(CHROME_RECONNECT_URL);
                        break bb22;
                    }
                case "manage-permissions":
                    {
                        setSelectKey(_temp5);
                        openUrl(CHROME_PERMISSIONS_URL);
                        break bb22;
                    }
                case "toggle-default":
                    {
                        const newValue = !enabledByDefault;
                        (0, config_js_1.saveGlobalConfig)(current => ({
                            ...current,
                            claudeInChromeDefaultEnabled: newValue
                        }));
                        setEnabledByDefault(newValue);
                    }
            }
        };
        $[4] = enabledByDefault;
        $[5] = t4;
    }
    else {
        t4 = $[5];
    }
    const handleAction = t4;
    let options;
    if ($[6] !== enabledByDefault || $[7] !== isExtensionInstalled) {
        options = [];
        const requiresExtensionSuffix = isExtensionInstalled ? "" : " (requires extension)";
        if (!isExtensionInstalled && !isHomespace) {
            let t5;
            if ($[9] === Symbol.for("react.memo_cache_sentinel")) {
                t5 = {
                    label: "Install Chrome extension",
                    value: "install-extension"
                };
                $[9] = t5;
            }
            else {
                t5 = $[9];
            }
            options.push(t5);
        }
        let t5;
        if ($[10] === Symbol.for("react.memo_cache_sentinel")) {
            t5 = react_1.default.createElement(ink_js_1.Text, null, "Manage permissions");
            $[10] = t5;
        }
        else {
            t5 = $[10];
        }
        let t6;
        if ($[11] !== requiresExtensionSuffix) {
            t6 = {
                label: react_1.default.createElement(react_1.default.Fragment, null,
                    t5,
                    react_1.default.createElement(ink_js_1.Text, { dimColor: true }, requiresExtensionSuffix)),
                value: "manage-permissions"
            };
            $[11] = requiresExtensionSuffix;
            $[12] = t6;
        }
        else {
            t6 = $[12];
        }
        let t7;
        if ($[13] === Symbol.for("react.memo_cache_sentinel")) {
            t7 = react_1.default.createElement(ink_js_1.Text, null, "Reconnect extension");
            $[13] = t7;
        }
        else {
            t7 = $[13];
        }
        let t8;
        if ($[14] !== requiresExtensionSuffix) {
            t8 = {
                label: react_1.default.createElement(react_1.default.Fragment, null,
                    t7,
                    react_1.default.createElement(ink_js_1.Text, { dimColor: true }, requiresExtensionSuffix)),
                value: "reconnect"
            };
            $[14] = requiresExtensionSuffix;
            $[15] = t8;
        }
        else {
            t8 = $[15];
        }
        const t9 = `Enabled by default: ${enabledByDefault ? "Yes" : "No"}`;
        let t10;
        if ($[16] !== t9) {
            t10 = {
                label: t9,
                value: "toggle-default"
            };
            $[16] = t9;
            $[17] = t10;
        }
        else {
            t10 = $[17];
        }
        options.push(t6, t8, t10);
        $[6] = enabledByDefault;
        $[7] = isExtensionInstalled;
        $[8] = options;
    }
    else {
        options = $[8];
    }
    const isDisabled = isWSL || true && !isClaudeAISubscriber;
    let t5;
    if ($[18] !== onDone) {
        t5 = () => onDone();
        $[18] = onDone;
        $[19] = t5;
    }
    else {
        t5 = $[19];
    }
    let t6;
    if ($[20] === Symbol.for("react.memo_cache_sentinel")) {
        t6 = react_1.default.createElement(ink_js_1.Text, null, "Claude in Chrome works with the Chrome extension to let you control your browser directly from Claude Code. Navigate websites, fill forms, capture screenshots, record GIFs, and debug with console logs and network requests.");
        $[20] = t6;
    }
    else {
        t6 = $[20];
    }
    let t7;
    if ($[21] !== isWSL) {
        t7 = isWSL && react_1.default.createElement(ink_js_1.Text, { color: "error" }, "Claude in Chrome is not supported in WSL at this time.");
        $[21] = isWSL;
        $[22] = t7;
    }
    else {
        t7 = $[22];
    }
    let t8;
    if ($[23] !== isClaudeAISubscriber) {
        t8 = true && !isClaudeAISubscriber && react_1.default.createElement(ink_js_1.Text, { color: "error" }, "Claude in Chrome requires a claude.ai subscription.");
        $[23] = isClaudeAISubscriber;
        $[24] = t8;
    }
    else {
        t8 = $[24];
    }
    let t9;
    if ($[25] !== handleAction || $[26] !== isConnected || $[27] !== isDisabled || $[28] !== isExtensionInstalled || $[29] !== options || $[30] !== selectKey || $[31] !== showInstallHint) {
        t9 = !isDisabled && react_1.default.createElement(react_1.default.Fragment, null,
            !isHomespace && react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
                react_1.default.createElement(ink_js_1.Text, null,
                    "Status:",
                    " ",
                    isConnected ? react_1.default.createElement(ink_js_1.Text, { color: "success" }, "Enabled") : react_1.default.createElement(ink_js_1.Text, { color: "inactive" }, "Disabled")),
                react_1.default.createElement(ink_js_1.Text, null,
                    "Extension:",
                    " ",
                    isExtensionInstalled ? react_1.default.createElement(ink_js_1.Text, { color: "success" }, "Installed") : react_1.default.createElement(ink_js_1.Text, { color: "warning" }, "Not detected"))),
            react_1.default.createElement(select_js_1.Select, { key: selectKey, options: options, onChange: handleAction, hideIndexes: true }),
            showInstallHint && react_1.default.createElement(ink_js_1.Text, { color: "warning" },
                "Once installed, select ",
                "\"Reconnect extension\"",
                " to connect."),
            react_1.default.createElement(ink_js_1.Text, null,
                react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "Usage: "),
                react_1.default.createElement(ink_js_1.Text, null, "claude --chrome"),
                react_1.default.createElement(ink_js_1.Text, { dimColor: true }, " or "),
                react_1.default.createElement(ink_js_1.Text, null, "claude --no-chrome")),
            react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "Site-level permissions are inherited from the Chrome extension. Manage permissions in the Chrome extension settings to control which sites Claude can browse, click, and type on."));
        $[25] = handleAction;
        $[26] = isConnected;
        $[27] = isDisabled;
        $[28] = isExtensionInstalled;
        $[29] = options;
        $[30] = selectKey;
        $[31] = showInstallHint;
        $[32] = t9;
    }
    else {
        t9 = $[32];
    }
    let t10;
    if ($[33] === Symbol.for("react.memo_cache_sentinel")) {
        t10 = react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "Learn more: https://code.claude.com/docs/en/chrome");
        $[33] = t10;
    }
    else {
        t10 = $[33];
    }
    let t11;
    if ($[34] !== t7 || $[35] !== t8 || $[36] !== t9) {
        t11 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", gap: 1 },
            t6,
            t7,
            t8,
            t9,
            t10);
        $[34] = t7;
        $[35] = t8;
        $[36] = t9;
        $[37] = t11;
    }
    else {
        t11 = $[37];
    }
    let t12;
    if ($[38] !== t11 || $[39] !== t5) {
        t12 = react_1.default.createElement(Dialog_js_1.Dialog, { title: "Claude in Chrome (Beta)", onCancel: t5, color: "chromeYellow" }, t11);
        $[38] = t11;
        $[39] = t5;
        $[40] = t12;
    }
    else {
        t12 = $[40];
    }
    return t12;
}
function _temp5(k) {
    return k + 1;
}
function _temp4(k_0) {
    return k_0 + 1;
}
function _temp3(k_1) {
    return k_1 + 1;
}
function _temp2(c) {
    return c.name === common_js_1.CLAUDE_IN_CHROME_MCP_SERVER_NAME;
}
function _temp(s) {
    return s.mcp.clients;
}
const call = async function (onDone) {
    const isExtensionInstalled = await (0, setup_js_1.isChromeExtensionInstalled)();
    const config = (0, config_js_1.getGlobalConfig)();
    const isSubscriber = (0, auth_js_1.isClaudeAISubscriber)();
    const isWSL = env_js_1.env.isWslEnvironment();
    return react_1.default.createElement(ClaudeInChromeMenu, { onDone: onDone, isExtensionInstalled: isExtensionInstalled, configEnabled: config.claudeInChromeDefaultEnabled, isClaudeAISubscriber: isSubscriber, isWSL: isWSL });
};
exports.call = call;
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
