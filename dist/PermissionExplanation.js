...
const permissionExplainer_js_1 = require("../../utils/permissions/permissionExplainer.js");
...
function ShimmerLoadingText() {
    ...
    const [ref, glimmerIndex] = (0, useShimmerAnimation_js_1.useShimmerAnimation)("responding", LOADING_MESSAGE, false);
    let t0;
    if ($[0] !== glimmerIndex) {
        t0 = LOADING_MESSAGE.split("").map((char, index) => react_1.default.createElement(ShimmerChar_js_1.ShimmerChar, { key: index, char: char, index: index, glimmerIndex: glimmerIndex, messageColor: "inactive", shimmerColor: "text" }));
        $[0] = glimmerIndex;
        $[1] = t0;
    }
    else {
    ...
    let t1;
    if ($[2] !== t0) {
        t1 = react_1.default.createElement(ink_js_1.Text, null, t0);
        $[2] = t0;
        $[3] = t1;
    }
    else {
...
function ExplanationResult(t0) {
    ...
    const { promise } = t0;
    const explanation = (0, react_1.use)(promise);
    if (!explanation) {
        let t1;
        if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
            t1 = react_1.default.createElement(ink_js_1.Box, { marginTop: 1 },
                react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "Explanation unavailable"));
            $[0] = t1;
        }
        else {
            t1 = $[0];
    ...
    if ($[1] !== explanation.explanation) {
        t1 = react_1.default.createElement(ink_js_1.Text, null, explanation.explanation);
    ...
    if ($[3] !== explanation.reasoning) {
        t2 = react_1.default.createElement(ink_js_1.Box, { marginTop: 1 },
            react_1.default.createElement(ink_js_1.Text, null, explanation.reasoning));
    ...
    let t5;
    if ($[9] !== t3 || $[10] !== t4) {
        t5 = react_1.default.createElement(ink_js_1.Text, { color: t3 },
            t4,
            ":");
        $[9] = t3;
        $[10] = t4;
    ...
    if ($[17] !== t1 || $[18] !== t2 || $[19] !== t7) {
        t8 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: 1 },
            t1,
            t2,
            t7);
        $[17] = t1;
...
function PermissionExplainerContent(t0) {
    ...
    if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
        t1 = react_1.default.createElement(ink_js_1.Box, { marginTop: 1 },
            react_1.default.createElement(ShimmerLoadingText, null));
        $[0] = t1;
    }
    else {
        t1 = $[0];
    ...
    let t2;
    if ($[1] !== promise) {
        t2 = react_1.default.createElement(react_1.Suspense, { fallback: t1 },
            react_1.default.createElement(ExplanationResult, { promise: promise }));
        $[1] = promise;
        $[2] = t2;
    }
    else {
