...
const useCopyOnSelect_js_1 = require("../hooks/useCopyOnSelect.js");
...
function initAndLogWheelAccel() {
    const xtermJs = (0, terminal_js_1.isXtermJs)();
    const base = readScrollSpeedBase();
    (0, debug_js_1.logForDebugging)(`wheel accel: ${xtermJs ? 'decay (xterm.js)' : 'window (native)'} · base=${base} · TERM_PROGRAM=${process.env.TERM_PROGRAM ?? 'unset'}`);
 ...
 */
function ScrollKeybindingHandler({ scrollRef, isActive, onScroll, isModal = false }) {
    const selection = (0, use_selection_js_1.useSelection)();
    ...
    const wheelAccel = (0, react_1.useRef)(null);
    function showCopiedToast(text) {
        ...
        const path = (0, osc_js_1.getClipboardPath)();
        const n = text.length;
        let msg;
        switch (path) {
            case 'native':
    ...
    (0, useKeybinding_js_1.useKeybindings)({
        ...
        'scroll:lineUp': () => {
            ...
            if (!s_2 || s_2.getScrollHeight() <= s_2.getViewportHeight())
                return false;
            wheelAccel.current ?? (wheelAccel.current = initAndLogWheelAccel());
    ...
    (0, ink_js_1.useInput)((input_0, key_0, event_0) => {
...
function useDragToScroll(scrollRef, selection, isActive, onScroll) {
    ...
    const dirRef = (0, react_1.useRef)(0); // -1 scrolling up, +1 down, 0 idle
    // Survives stop() — reset only on drag-finish. See check() for semantics.
    const lastScrolledDirRef = (0, react_1.useRef)(0);
    const ticksRef = (0, react_1.useRef)(0);
