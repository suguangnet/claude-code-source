...
const teammateSelectHint_js_1 = require("./teammateSelectHint.js");
/**
...
function TeammateSpinnerLine({ teammate, isLast, isSelected, isForegrounded, allIdle, showPreview }) {
    const [randomVerb] = (0, react_1.useState)(() => teammate.spinnerVerb ?? (0, sample_js_1.default)((0, spinnerVerbs_js_1.getSpinnerVerbs)()));
    const [pastTenseVerb] = (0, react_1.useState)(() => teammate.pastTenseVerb ?? (0, sample_js_1.default)(turnCompletionVerbs_js_1.TURN_COMPLETION_VERBS));
    const isHighlighted = isSelected || isForegrounded;
    const treeChar = isHighlighted ? isLast ? '╘═' : '╞═' : isLast ? '└─' : '├─';
    const nameColor = (0, ink_js_2.toInkColor)(teammate.identity.color);
    const { columns } = (0, useTerminalSize_js_1.useTerminalSize)();
    // Track when teammate became idle (for "Idle for X..." display)
    const idleStartRef = (0, react_1.useRef)(null);
    // Freeze elapsed time when entering all-idle state
    const frozenDurationRef = (0, react_1.useRef)(null);
    ...
    const idleElapsedTime = (0, useElapsedTime_js_1.useElapsedTime)(idleStartRef.current ?? Date.now(), teammate.isIdle && !allIdle);
    ...
    if (allIdle && frozenDurationRef.current === null) {
        frozenDurationRef.current = (0, format_js_1.formatDuration)(Math.max(0, Date.now() - teammate.startTime - (teammate.totalPausedMs ?? 0)));
    }
    ...
    const fullNameWidth = (0, stringWidth_js_1.stringWidth)(fullAgentName);
    ...
    const statsText = ` · ${toolUseCount} tool ${toolUseCount === 1 ? 'use' : 'uses'} · ${(0, format_js_1.formatNumber)(tokenCount)} tokens`;
    const statsWidth = (0, stringWidth_js_1.stringWidth)(statsText);
    const selectHintText = ` · ${teammateSelectHint_js_1.TEAMMATE_SELECT_HINT}`;
    const selectHintWidth = (0, stringWidth_js_1.stringWidth)(selectHintText);
    const viewHintText = ' · enter to view';
    const viewHintWidth = (0, stringWidth_js_1.stringWidth)(viewHintText);
    ...
    const activityText = (() => {
        ...
        if (activities && activities.length > 0) {
            const summary = (0, collapseReadSearch_js_1.summarizeRecentActivities)(activities);
            if (summary)
    ...
    const renderStatus = () => {
        if (teammate.shutdownRequested) {
            return React.createElement(ink_js_1.Text, { dimColor: true }, "[stopping]");
        }
        if (teammate.awaitingPlanApproval) {
            return React.createElement(ink_js_1.Text, { color: "warning" }, "[awaiting approval]");
        }
        if (teammate.isIdle) {
            if (allIdle) {
                return React.createElement(ink_js_1.Text, { dimColor: true },
                    pastTenseVerb,
                    " for ",
                    displayTime);
            }
        ...
        return React.createElement(ink_js_1.Text, { dimColor: true }, activityText?.endsWith('…') ? activityText : `${activityText}…`);
    };
    ...
    return React.createElement(ink_js_1.Box, { flexDirection: "column" },
        React.createElement(ink_js_1.Box, { paddingLeft: 3 },
            React.createElement(ink_js_1.Text, { color: isSelected ? 'suggestion' : undefined, bold: isSelected }, isSelected ? figures_1.default.pointer : ' '),
            React.createElement(ink_js_1.Text, { dimColor: !isSelected },
                treeChar,
                " "),
            showName && React.createElement(ink_js_1.Text, { color: isSelected ? 'suggestion' : nameColor },
                "@",
                teammate.identity.agentName),
            showName && React.createElement(ink_js_1.Text, { dimColor: !isSelected }, ": "),
            renderStatus(),
            showStats && React.createElement(ink_js_1.Text, { dimColor: true },
                ' ',
                "\u00B7 ",
                toolUseCount,
                " tool ",
                ...
                " \u00B7",
                ' ',
                (0, format_js_1.formatNumber)(tokenCount),
                " tokens"),
            showSelectHint && React.createElement(ink_js_1.Text, { dimColor: true },
                " \u00B7 ",
                teammateSelectHint_js_1.TEAMMATE_SELECT_HINT),
            showViewHint && React.createElement(ink_js_1.Text, { dimColor: true }, " \u00B7 enter to view")),
        previewLines.map((line, idx) => React.createElement(ink_js_1.Box, { key: idx, paddingLeft: 3 },
            React.createElement(ink_js_1.Text, { dimColor: true }, " "),
            React.createElement(ink_js_1.Text, { dimColor: true },
                previewTreeChar,
                " "),
            React.createElement(ink_js_1.Text, { dimColor: true }, line))));
}
