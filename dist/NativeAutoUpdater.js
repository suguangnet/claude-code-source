...
const useUpdateNotification_js_1 = require("../hooks/useUpdateNotification.js");
const ink_js_1 = require("../ink.js");
const autoUpdater_js_1 = require("../utils/autoUpdater.js");
...
function NativeAutoUpdater({ isUpdating, onChangeIsUpdating, onAutoUpdaterResult, autoUpdaterResult, showSuccessMessage, verbose }) {
    const [versions, setVersions] = (0, react_1.useState)({});
    const [maxVersionIssue, setMaxVersionIssue] = (0, react_1.useState)(null);
    const updateSemver = (0, useUpdateNotification_js_1.useUpdateNotification)(autoUpdaterResult?.version);
    ...
    const isUpdatingRef = (0, react_1.useRef)(isUpdating);
    ...
    const checkForUpdates = React.useCallback(async () => {
        ...
        try {
            ...
            if (maxVersion && (0, semver_js_1.gt)(MACRO.VERSION, maxVersion)) {
                const msg = await (0, autoUpdater_js_1.getMaxVersionMessage)();
                setMaxVersionIssue(msg ?? 'affects your version');
            }
            const result = await (0, index_js_2.installLatest)(channel);
    ...
    return React.createElement(ink_js_1.Box, { flexDirection: "row", gap: 1 },
        verbose && React.createElement(ink_js_1.Text, { dimColor: true, wrap: "truncate" },
            "current: ",
            versions.current,
            " \u00B7 ",
            channel,
            ": ",
            versions.latest),
        isUpdating ? React.createElement(ink_js_1.Box, null,
            React.createElement(ink_js_1.Text, { dimColor: true, wrap: "truncate" }, "Checking for updates")) : autoUpdaterResult?.status === 'success' && showSuccessMessage && updateSemver && React.createElement(ink_js_1.Text, { color: "success", wrap: "truncate" }, "\u2713 Update installed \u00B7 Restart to update"),
        autoUpdaterResult?.status === 'install_failed' && React.createElement(ink_js_1.Text, { color: "error", wrap: "truncate" },
            "\u2717 Auto-update failed \u00B7 Try ",
            React.createElement(ink_js_1.Text, { bold: true }, "/status")),
        maxVersionIssue && "external" === 'ant' && React.createElement(ink_js_1.Text, { color: "warning" },
            "\u26A0 Known issue: ",
            maxVersionIssue,
            " \u00B7 Run",
            ' ',
            React.createElement(ink_js_1.Text, { bold: true }, "claude rollback --safe"),
            " to downgrade"));
}
