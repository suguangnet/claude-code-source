...
const bridgeMessaging_js_1 = require("../bridge/bridgeMessaging.js");
const RemoteSessionManager_js_1 = require("../remote/RemoteSessionManager.js");
 ...
 */
function useRemoteSession({ config, setMessages, setIsLoading, onInit, setToolUseConfirmQueue, tools, setStreamingToolUses, setStreamMode, setInProgressToolUseIDs, }) {
    const isRemoteMode = !!config;
    const setAppState = (0, AppState_js_1.useSetAppState)();
    const setConnStatus = (0, react_1.useCallback)((s) => setAppState(prev => prev.remoteConnectionStatus === s
        ? prev
    ...
    const runningTaskIdsRef = (0, react_1.useRef)(new Set());
    const writeTaskCount = (0, react_1.useCallback)(() => {
    ...
    const responseTimeoutRef = (0, react_1.useRef)(null);
    ...
    const isCompactingRef = (0, react_1.useRef)(false);
    ...
    const hasUpdatedTitleRef = (0, react_1.useRef)(false);
    ...
    const sentUUIDsRef = (0, react_1.useRef)(new bridgeMessaging_js_1.BoundedUUIDSet(50));
    ...
    (0, react_1.useEffect)(() => {
        ...
        (0, debug_js_1.logForDebugging)(`[useRemoteSession] Initializing for session ${config.sessionId}`);
        const manager = new RemoteSessionManager_js_1.RemoteSessionManager(config, {
            onMessage: sdkMessage => {
                ...
                (0, debug_js_1.logForDebugging)(`[useRemoteSession] Received ${parts.join(' ')}`);
                ...
                if (sdkMessage.type === 'user' &&
                    ...
                    (0, debug_js_1.logForDebugging)(`[useRemoteSession] Dropping echoed user message ${sdkMessage.uuid}`);
                    return;
                }
                ...
                if (sdkMessage.type === 'system' &&
                    ...
                    onInit) {
                    (0, debug_js_1.logForDebugging)(`[useRemoteSession] Init received with ${sdkMessage.slash_commands.length} slash commands`);
                ...
                const converted = (0, sdkMessageAdapter_js_1.convertSDKMessage)(sdkMessage, config.viewerOnly
                ...
                if (converted.type === 'message') {
                    ...
                    if (setStreamingToolUses && setStreamMode) {
                        (0, messages_js_1.handleMessageFromStream)(converted.event, message => setMessages(prev => [...prev, message]), () => {
            ...
            onPermissionRequest: (request, requestId) => {
                (0, debug_js_1.logForDebugging)(`[useRemoteSession] Permission request for tool: ${request.tool_name}`);
            ...
            onPermissionCancelled: (requestId, toolUseId) => {
                (0, debug_js_1.logForDebugging)(`[useRemoteSession] Permission request cancelled: ${requestId}`);
            ...
            onError: error => {
                (0, debug_js_1.logForDebugging)(`[useRemoteSession] Error: ${error.message}`);
            },
        });
    ...
    const sendMessage = (0, react_1.useCallback)(async (content, opts) => {
        ...
        if (!hasUpdatedTitleRef.current &&
            ...
            if (description) {
                ...
                void (0, sessionTitle_js_1.generateSessionTitle)(description, new AbortController().signal).then(title => {
                    void (0, api_js_1.updateSessionTitle)(sessionId, title ?? (0, format_js_1.truncateToWidth)(description, 75));
                });
            }
        }
        ...
        if (!config?.viewerOnly) {
            ...
            responseTimeoutRef.current = setTimeout((setMessages, manager) => {
                ...
                const warningMessage = (0, messages_js_1.createSystemMessage)('Remote session may be unresponsive. Attempting to reconnect…', 'warning');
