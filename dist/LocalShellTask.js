...
const LocalAgentTask_js_1 = require("../LocalAgentTask/LocalAgentTask.js");
const LocalMainSessionTask_js_1 = require("../LocalMainSessionTask.js");
const guards_js_1 = require("./guards.js");
const killShellTasks_js_1 = require("./killShellTasks.js");
...
function startStallWatchdog(taskId, description, kind, toolUseId, agentId) {
    ...
    const timer = setInterval(() => {
        void (0, promises_1.stat)(outputPath).then(s => {
            if (s.size > lastSize) {
                lastSize = s.size;
                lastGrowth = Date.now();
                return;
            ...
            if (Date.now() - lastGrowth < STALL_THRESHOLD_MS)
                return;
            void (0, fsOperations_js_1.tailFile)(outputPath, STALL_TAIL_BYTES).then(({ content }) => {
                if (cancelled)
                    return;
                ...
                const summary = `${exports.BACKGROUND_BASH_SUMMARY_PREFIX}"${description}" appears to be waiting for interactive input`;
...
<${xml_js_1.SUMMARY_TAG}>${(0, xml_js_2.escapeXml)(summary)}</${xml_js_1.SUMMARY_TAG}>
...
function enqueueShellNotification(taskId, description, status, exitCode, setAppState, toolUseId, kind = 'bash', agentId) {
    ...
    if ((0, bun_bundle_1.feature)('MONITOR_TOOL') && kind === 'monitor') {
        ...
        switch (status) {
            case 'completed':
                summary = `${exports.BACKGROUND_BASH_SUMMARY_PREFIX}"${description}" completed${exitCode !== undefined ? ` (exit code ${exitCode})` : ''}`;
                break;
            case 'failed':
                summary = `${exports.BACKGROUND_BASH_SUMMARY_PREFIX}"${description}" failed${exitCode !== undefined ? ` with exit code ${exitCode}` : ''}`;
                break;
            case 'killed':
                summary = `${exports.BACKGROUND_BASH_SUMMARY_PREFIX}"${description}" was stopped`;
                break;
        }
    }
    const outputPath = (0, diskOutput_js_1.getTaskOutputPath)(taskId);
...
function markTaskNotified(taskId, setAppState) {
    (0, framework_js_1.updateTaskState)(taskId, setAppState, t => t.notified ? t : {
        ...t,
        notified: true
    });
}
