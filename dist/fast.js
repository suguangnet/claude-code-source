...
const FastIcon_js_1 = require("../../components/FastIcon.js");
...
function applyFastMode(enable, setAppState) {
    ...
    if (enable) {
        setAppState(prev => {
            ...
            const needsModelSwitch = !(0, fastMode_js_1.isFastModeSupportedByModel)(prev.mainLoopModel);
            return {
                ...prev,
                ...(needsModelSwitch ? {
...
function FastModePicker(t0) {
    ...
    const model = (0, AppState_js_1.useAppState)(_temp);
    const initialFastMode = (0, AppState_js_1.useAppState)(_temp2);
    const setAppState = (0, AppState_js_1.useSetAppState)();
    const [enableFastMode, setEnableFastMode] = (0, react_1.useState)(initialFastMode ?? false);
    let t1;
    ...
    if ($[2] !== enableFastMode || $[3] !== isUnavailable || $[4] !== model || $[5] !== onDone || $[6] !== setAppState) {
        t3 = function handleConfirm() {
            ...
            if (enableFastMode) {
                const fastIcon = (0, FastIcon_js_1.getFastIconString)(enableFastMode);
                const modelUpdated = !(0, fastMode_js_1.isFastModeSupportedByModel)(model) ? ` · model set to ${fastMode_js_1.FAST_MODE_MODEL_DISPLAY}` : "";
    ...
    if ($[8] !== initialFastMode || $[9] !== isUnavailable || $[10] !== onDone || $[11] !== setAppState) {
        t4 = function handleCancel() {
            ...
            const message = initialFastMode ? `${(0, FastIcon_js_1.getFastIconString)()} Kept Fast mode ON` : "Kept Fast mode OFF";
            onDone(message, {
                display: "system"
            });
        };
    ...
    if ($[19] === Symbol.for("react.memo_cache_sentinel")) {
        t8 = React.createElement(ink_js_1.Text, null,
            React.createElement(FastIcon_js_1.FastIcon, { cooldown: isCooldown }),
    ...
    let t9;
    if ($[20] !== isUnavailable) {
        t9 = exitState => exitState.pending ? React.createElement(ink_js_1.Text, null,
            "Press ",
            exitState.keyName,
            " again to exit") : isUnavailable ? React.createElement(ink_js_1.Text, null, "Esc to cancel") : React.createElement(ink_js_1.Text, null, "Tab to toggle \u00B7 Enter to confirm \u00B7 Esc to cancel");
        $[20] = isUnavailable;
        $[21] = t9;
    }
    else {
    ...
    if ($[22] !== enableFastMode || $[23] !== unavailableReason) {
        t10 = unavailableReason ? React.createElement(ink_js_1.Box, { marginLeft: 2 },
            React.createElement(ink_js_1.Text, { color: "error" }, unavailableReason)) : React.createElement(React.Fragment, null,
            React.createElement(ink_js_1.Box, { flexDirection: "column", gap: 0, marginLeft: 2 },
                React.createElement(ink_js_1.Box, { flexDirection: "row", gap: 2 },
                    React.createElement(ink_js_1.Text, { bold: true }, "Fast mode"),
                    React.createElement(ink_js_1.Text, { color: enableFastMode ? "fastMode" : undefined, bold: enableFastMode }, enableFastMode ? "ON " : "OFF"),
                    React.createElement(ink_js_1.Text, { dimColor: true }, pricing))),
            isCooldown && runtimeState.status === "cooldown" && React.createElement(ink_js_1.Box, { marginLeft: 2 },
                React.createElement(ink_js_1.Text, { color: "warning" },
                    ...
                    " \xB7 resets in ",
                    (0, format_js_1.formatDuration)(runtimeState.resetAt - Date.now(), {
    ...
    if ($[25] === Symbol.for("react.memo_cache_sentinel")) {
        t11 = React.createElement(ink_js_1.Text, { dimColor: true },
            "Learn more:",
            " ",
            React.createElement(ink_js_1.Link, { url: "https://code.claude.com/docs/en/fast-mode" }, "https://code.claude.com/docs/en/fast-mode"));
        $[25] = t11;
    }
    else {
        t11 = $[25];
    ...
    if ($[26] !== handleCancel || $[27] !== t10 || $[28] !== t9) {
        t12 = React.createElement(Dialog_js_1.Dialog, { title: title, subtitle: `High-speed mode for ${fastMode_js_1.FAST_MODE_MODEL_DISPLAY}. Billed as extra usage at a premium rate. Separate rate limits apply.`, onCancel: handleCancel, color: "fastMode", inputGuide: t9 },
            t10,
            t11);
        $[26] = handleCancel;
        $[27] = t10;
...
async function handleFastModeShortcut(enable, getAppState, setAppState) {
    const unavailableReason = (0, fastMode_js_1.getFastModeUnavailableReason)();
    ...
    if (enable) {
        const fastIcon = (0, FastIcon_js_1.getFastIconString)(true);
        const modelUpdated = !(0, fastMode_js_1.isFastModeSupportedByModel)(mainLoopModel) ? ` · model set to ${fastMode_js_1.FAST_MODE_MODEL_DISPLAY}` : '';
        const pricing = (0, modelCost_js_1.formatModelPricing)((0, modelCost_js_1.getOpus46CostTier)(true));
...
async function call(onDone, context, args) {
    ...
    await (0, fastMode_js_1.prefetchFastModeStatus)();
