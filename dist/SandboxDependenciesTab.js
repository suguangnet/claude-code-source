"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SandboxDependenciesTab = SandboxDependenciesTab;
const compiler_runtime_1 = require("react/compiler-runtime");
const react_1 = __importDefault(require("react"));
const ink_js_1 = require("../../ink.js");
const platform_js_1 = require("../../utils/platform.js");
function SandboxDependenciesTab(t0) {
    const $ = (0, compiler_runtime_1.c)(24);
    const { depCheck } = t0;
    let t1;
    if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
        t1 = (0, platform_js_1.getPlatform)();
        $[0] = t1;
    }
    else {
        t1 = $[0];
    }
    const platform = t1;
    const isMac = platform === "macos";
    let t2;
    if ($[1] !== depCheck.errors) {
        t2 = depCheck.errors.some(_temp);
        $[1] = depCheck.errors;
        $[2] = t2;
    }
    else {
        t2 = $[2];
    }
    const rgMissing = t2;
    let t3;
    if ($[3] !== depCheck.errors) {
        t3 = depCheck.errors.some(_temp2);
        $[3] = depCheck.errors;
        $[4] = t3;
    }
    else {
        t3 = $[4];
    }
    const bwrapMissing = t3;
    let t4;
    if ($[5] !== depCheck.errors) {
        t4 = depCheck.errors.some(_temp3);
        $[5] = depCheck.errors;
        $[6] = t4;
    }
    else {
        t4 = $[6];
    }
    const socatMissing = t4;
    const seccompMissing = depCheck.warnings.length > 0;
    let t5;
    if ($[7] !== bwrapMissing || $[8] !== depCheck.errors || $[9] !== rgMissing || $[10] !== seccompMissing || $[11] !== socatMissing) {
        const otherErrors = depCheck.errors.filter(_temp4);
        const rgInstallHint = isMac ? "brew install ripgrep" : "apt install ripgrep";
        let t6;
        if ($[13] === Symbol.for("react.memo_cache_sentinel")) {
            t6 = isMac && react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
                react_1.default.createElement(ink_js_1.Text, null,
                    "seatbelt: ",
                    react_1.default.createElement(ink_js_1.Text, { color: "success" }, "built-in (macOS)")));
            $[13] = t6;
        }
        else {
            t6 = $[13];
        }
        let t7;
        let t8;
        if ($[14] !== rgMissing) {
            t7 = react_1.default.createElement(ink_js_1.Text, null,
                "ripgrep (rg):",
                " ",
                rgMissing ? react_1.default.createElement(ink_js_1.Text, { color: "error" }, "not found") : react_1.default.createElement(ink_js_1.Text, { color: "success" }, "found"));
            t8 = rgMissing && react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                "  ",
                "\u00B7 ",
                rgInstallHint);
            $[14] = rgMissing;
            $[15] = t7;
            $[16] = t8;
        }
        else {
            t7 = $[15];
            t8 = $[16];
        }
        let t9;
        if ($[17] !== t7 || $[18] !== t8) {
            t9 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
                t7,
                t8);
            $[17] = t7;
            $[18] = t8;
            $[19] = t9;
        }
        else {
            t9 = $[19];
        }
        let t10;
        if ($[20] !== bwrapMissing || $[21] !== seccompMissing || $[22] !== socatMissing) {
            t10 = !isMac && react_1.default.createElement(react_1.default.Fragment, null,
                react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
                    react_1.default.createElement(ink_js_1.Text, null,
                        "bubblewrap (bwrap):",
                        " ",
                        bwrapMissing ? react_1.default.createElement(ink_js_1.Text, { color: "error" }, "not installed") : react_1.default.createElement(ink_js_1.Text, { color: "success" }, "installed")),
                    bwrapMissing && react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                        "  ",
                        "\u00B7 apt install bubblewrap")),
                react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
                    react_1.default.createElement(ink_js_1.Text, null,
                        "socat:",
                        " ",
                        socatMissing ? react_1.default.createElement(ink_js_1.Text, { color: "error" }, "not installed") : react_1.default.createElement(ink_js_1.Text, { color: "success" }, "installed")),
                    socatMissing && react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                        "  ",
                        "\u00B7 apt install socat")),
                react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
                    react_1.default.createElement(ink_js_1.Text, null,
                        "seccomp filter:",
                        " ",
                        seccompMissing ? react_1.default.createElement(ink_js_1.Text, { color: "warning" }, "not installed") : react_1.default.createElement(ink_js_1.Text, { color: "success" }, "installed"),
                        seccompMissing && react_1.default.createElement(ink_js_1.Text, { dimColor: true }, " (required to block unix domain sockets)")),
                    seccompMissing && react_1.default.createElement(ink_js_1.Box, { flexDirection: "column" },
                        react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                            "  ",
                            "\u00B7 npm install -g @anthropic-ai/sandbox-runtime"),
                        react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                            "  ",
                            "\u00B7 or copy vendor/seccomp/* from sandbox-runtime and set"),
                        react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                            "    ",
                            "sandbox.seccomp.bpfPath and applyPath in settings.json"))));
            $[20] = bwrapMissing;
            $[21] = seccompMissing;
            $[22] = socatMissing;
            $[23] = t10;
        }
        else {
            t10 = $[23];
        }
        t5 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", paddingY: 1, gap: 1 },
            t6,
            t9,
            t10,
            otherErrors.map(_temp5));
        $[7] = bwrapMissing;
        $[8] = depCheck.errors;
        $[9] = rgMissing;
        $[10] = seccompMissing;
        $[11] = socatMissing;
        $[12] = t5;
    }
    else {
        t5 = $[12];
    }
    return t5;
}
function _temp5(err) {
    return react_1.default.createElement(ink_js_1.Text, { key: err, color: "error" }, err);
}
function _temp4(e_2) {
    return !e_2.includes("ripgrep") && !e_2.includes("bwrap") && !e_2.includes("socat");
}
function _temp3(e_1) {
    return e_1.includes("socat");
}
function _temp2(e_0) {
    return e_0.includes("bwrap");
}
function _temp(e) {
    return e.includes("ripgrep");
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
