"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CreateAgentWizard = CreateAgentWizard;
const compiler_runtime_1 = require("react/compiler-runtime");
const react_1 = __importDefault(require("react"));
const paths_js_1 = require("../../../memdir/paths.js");
const index_js_1 = require("../../wizard/index.js");
const ColorStep_js_1 = require("./wizard-steps/ColorStep.js");
const ConfirmStepWrapper_js_1 = require("./wizard-steps/ConfirmStepWrapper.js");
const DescriptionStep_js_1 = require("./wizard-steps/DescriptionStep.js");
const GenerateStep_js_1 = require("./wizard-steps/GenerateStep.js");
const LocationStep_js_1 = require("./wizard-steps/LocationStep.js");
const MemoryStep_js_1 = require("./wizard-steps/MemoryStep.js");
const MethodStep_js_1 = require("./wizard-steps/MethodStep.js");
const ModelStep_js_1 = require("./wizard-steps/ModelStep.js");
const PromptStep_js_1 = require("./wizard-steps/PromptStep.js");
const ToolsStep_js_1 = require("./wizard-steps/ToolsStep.js");
const TypeStep_js_1 = require("./wizard-steps/TypeStep.js");
function CreateAgentWizard(t0) {
    const $ = (0, compiler_runtime_1.c)(17);
    const { tools, existingAgents, onComplete, onCancel } = t0;
    let t1;
    if ($[0] !== existingAgents) {
        t1 = () => react_1.default.createElement(TypeStep_js_1.TypeStep, { existingAgents: existingAgents });
        $[0] = existingAgents;
        $[1] = t1;
    }
    else {
        t1 = $[1];
    }
    let t2;
    if ($[2] !== tools) {
        t2 = () => react_1.default.createElement(ToolsStep_js_1.ToolsStep, { tools: tools });
        $[2] = tools;
        $[3] = t2;
    }
    else {
        t2 = $[3];
    }
    let t3;
    if ($[4] === Symbol.for("react.memo_cache_sentinel")) {
        t3 = (0, paths_js_1.isAutoMemoryEnabled)() ? [MemoryStep_js_1.MemoryStep] : [];
        $[4] = t3;
    }
    else {
        t3 = $[4];
    }
    let t4;
    if ($[5] !== existingAgents || $[6] !== onComplete || $[7] !== tools) {
        t4 = () => react_1.default.createElement(ConfirmStepWrapper_js_1.ConfirmStepWrapper, { tools: tools, existingAgents: existingAgents, onComplete: onComplete });
        $[5] = existingAgents;
        $[6] = onComplete;
        $[7] = tools;
        $[8] = t4;
    }
    else {
        t4 = $[8];
    }
    let t5;
    if ($[9] !== t1 || $[10] !== t2 || $[11] !== t4) {
        t5 = [LocationStep_js_1.LocationStep, MethodStep_js_1.MethodStep, GenerateStep_js_1.GenerateStep, t1, PromptStep_js_1.PromptStep, DescriptionStep_js_1.DescriptionStep, t2, ModelStep_js_1.ModelStep, ColorStep_js_1.ColorStep, ...t3, t4];
        $[9] = t1;
        $[10] = t2;
        $[11] = t4;
        $[12] = t5;
    }
    else {
        t5 = $[12];
    }
    const steps = t5;
    let t6;
    if ($[13] === Symbol.for("react.memo_cache_sentinel")) {
        t6 = {};
        $[13] = t6;
    }
    else {
        t6 = $[13];
    }
    let t7;
    if ($[14] !== onCancel || $[15] !== steps) {
        t7 = react_1.default.createElement(index_js_1.WizardProvider, { steps: steps, initialData: t6, onComplete: _temp, onCancel: onCancel, title: "Create new agent", showStepCounter: false });
        $[14] = onCancel;
        $[15] = steps;
        $[16] = t7;
    }
    else {
        t7 = $[16];
    }
    return t7;
}
function _temp() { }
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
