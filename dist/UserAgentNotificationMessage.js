...
function UserAgentNotificationMessage(t0) {
    ...
    let t3;
    if ($[2] !== text) {
        const status = (0, messages_js_1.extractTag)(text, "status");
    ...
    let t5;
    if ($[4] !== color) {
        t5 = React.createElement(ink_js_1.Text, { color: color }, figures_js_1.BLACK_CIRCLE);
        $[4] = color;
        $[5] = t5;
    }
    else {
    ...
    let t7;
    if ($[9] !== t4 || $[10] !== t6) {
        t7 = React.createElement(ink_js_1.Box, { marginTop: t4 }, t6);
        $[9] = t4;
        $[10] = t6;
        $[11] = t7;
    }
