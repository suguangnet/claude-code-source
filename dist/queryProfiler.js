...
const ENABLED = (0, envUtils_js_1.isEnvTruthy)(process.env.CLAUDE_CODE_PROFILE_QUERY);
...
function getQueryProfileReport() {
    ...
    if (firstChunkTime > 0) {
        ...
        lines.push(`Total TTFT: ${(0, profilerBase_js_1.formatMs)(firstChunkTime)}ms`);
        lines.push(`  - Pre-request overhead: ${(0, profilerBase_js_1.formatMs)(preRequestOverhead)}ms (${preRequestPercent}%)`);
        lines.push(`  - Network latency: ${(0, profilerBase_js_1.formatMs)(networkLatency)}ms (${networkPercent}%)`);
    }
    else {
        lines.push(`Total time: ${(0, profilerBase_js_1.formatMs)(totalTime)}ms`);
    }
    // Add phase summary
...
function getPhaseSummary(marks, baselineTime) {
    ...
    for (const phase of phases) {
        ...
        if (startTime !== undefined && endTime !== undefined) {
            ...
            lines.push(`  ${phase.name.padEnd(22)} ${(0, profilerBase_js_1.formatMs)(duration).padStart(10)}ms ${bar}`);
        }
    }
    ...
    if (apiRequestSent !== undefined) {
        lines.push('');
        lines.push(`  ${'Total pre-API overhead'.padEnd(22)} ${(0, profilerBase_js_1.formatMs)(apiRequestSent).padStart(10)}ms`);
    }
    return lines.join('\n');
}
/**
