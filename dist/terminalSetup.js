...
const completionCache_js_1 = require("../../utils/completionCache.js");
...
function getNativeCSIuTerminalDisplayName() {
    ...
    return NATIVE_CSIU_TERMINALS[env_js_1.env.terminal] ?? null;
}
/**
...
async function setupTerminal(theme) {
    ...
    (0, config_js_1.saveGlobalConfig)(current => {
        if (['vscode', 'cursor', 'windsurf', 'alacritty', 'zed'].includes(env_js_1.env.terminal ?? '')) {
    ...
    if ("external" === 'ant') {
        result += await (0, completionCache_js_1.setupShellCompletion)(theme);
    }
    return result;
}
...
async function call(onDone, context, _args) {
    if (env_js_1.env.terminal && env_js_1.env.terminal in NATIVE_CSIU_TERMINALS) {
        const message = `Shift+Enter is natively supported in ${NATIVE_CSIU_TERMINALS[env_js_1.env.terminal]}.
...
No configuration needed. Just use Shift+Enter to add newlines.`;
    ...
    if (!shouldOfferTerminalSetup()) {
        const terminalName = env_js_1.env.terminal || 'your current terminal';
...
${chalk_1.default.dim('Note: You can already use backslash (\\\\) + return to add newlines.')}
...
${chalk_1.default.dim('Note: iTerm2, WezTerm, Ghostty, Kitty, and Warp support Shift+Enter natively.')}`;
        onDone(message);
        return null;
    }
...
async function installBindingsForVSCodeTerminal(editor = 'VSCode', theme) {
    ...
    if (isVSCodeRemoteSSH()) {
        return `${(0, ink_js_1.color)('warning', theme)(`Cannot install keybindings from a remote ${editor} session.`)}${EOL}${EOL}${editor} keybindings must be installed on your local machine, not the remote server.${EOL}${EOL}To install the Shift+Enter keybinding:${EOL}1. Open ${editor} on your local machine (not connected to remote)${EOL}2. Open the Command Palette (Cmd/Ctrl+Shift+P) → "Preferences: Open Keyboard Shortcuts (JSON)"${EOL}3. Add this keybinding (the file must be a JSON array):${EOL}${EOL}${chalk_1.default.dim(`[
  {
    "key": "shift+enter",
    ...
    const userDirPath = (0, path_1.join)((0, os_1.homedir)(), (0, os_1.platform)() === 'win32' ? (0, path_1.join)('AppData', 'Roaming', editorDir, 'User') : (0, os_1.platform)() === 'darwin' ? (0, path_1.join)('Library', 'Application Support', editorDir, 'User') : (0, path_1.join)('.config', editorDir, 'User'));
    const keybindingsPath = (0, path_1.join)(userDirPath, 'keybindings.json');
    try {
        // Ensure user directory exists (idempotent with recursive)
        await (0, promises_1.mkdir)(userDirPath, {
            recursive: true
        });
        ...
        try {
            content = await (0, promises_1.readFile)(keybindingsPath, {
                encoding: 'utf-8'
            });
            fileExists = true;
            keybindings = (0, json_js_1.safeParseJSONC)(content) ?? [];
        }
        catch (e) {
        ...
        if (fileExists) {
            const randomSha = (0, crypto_1.randomBytes)(4).toString('hex');
            const backupPath = `${keybindingsPath}.${randomSha}.bak`;
            try {
                await (0, promises_1.copyFile)(keybindingsPath, backupPath);
            }
            catch {
                return `${(0, ink_js_1.color)('warning', theme)(`Error backing up existing ${editor} terminal keybindings. Bailing out.`)}${EOL}${chalk_1.default.dim(`See ${formatPathLink(keybindingsPath)}`)}${EOL}${chalk_1.default.dim(`Backup path: ${formatPathLink(backupPath)}`)}${EOL}`;
            }
        }
        ...
        if (existingBinding) {
            return `${(0, ink_js_1.color)('warning', theme)(`Found existing ${editor} terminal Shift+Enter key binding. Remove it to continue.`)}${EOL}${chalk_1.default.dim(`See ${formatPathLink(keybindingsPath)}`)}${EOL}`;
        }
        ...
        const updatedContent = (0, json_js_1.addItemToJSONCArray)(content, newKeybinding);
        // Write the updated content back to the file
        await (0, promises_1.writeFile)(keybindingsPath, updatedContent, {
            encoding: 'utf-8'
        });
        return `${(0, ink_js_1.color)('success', theme)(`Installed ${editor} terminal Shift+Enter key binding`)}${EOL}${chalk_1.default.dim(`See ${formatPathLink(keybindingsPath)}`)}${EOL}`;
    }
    catch (error) {
...
async function enableOptionAsMetaForProfile(profileName) {
    ...
    const { code: addCode } = await (0, execFileNoThrow_js_1.execFileNoThrow)('/usr/libexec/PlistBuddy', ['-c', `Add :'Window Settings':'${profileName}':useOptionAsMetaKey bool true`, (0, appleTerminalBackup_js_1.getTerminalPlistPath)()]);
    // If adding fails (likely because it already exists), try setting it instead
    if (addCode !== 0) {
        const { code: setCode } = await (0, execFileNoThrow_js_1.execFileNoThrow)('/usr/libexec/PlistBuddy', ['-c', `Set :'Window Settings':'${profileName}':useOptionAsMetaKey true`, (0, appleTerminalBackup_js_1.getTerminalPlistPath)()]);
        if (setCode !== 0) {
            (0, log_js_1.logError)(new Error(`Failed to enable Option as Meta key for Terminal.app profile: ${profileName}`));
            return false;
        }
    }
    return true;
...
async function disableAudioBellForProfile(profileName) {
    // First try to add the property (in case it doesn't exist)
    // Quote the profile name to handle names with spaces (e.g., "Man Page", "Red Sands")
    const { code: addCode } = await (0, execFileNoThrow_js_1.execFileNoThrow)('/usr/libexec/PlistBuddy', ['-c', `Add :'Window Settings':'${profileName}':Bell bool false`, (0, appleTerminalBackup_js_1.getTerminalPlistPath)()]);
    // If adding fails (likely because it already exists), try setting it instead
    if (addCode !== 0) {
        const { code: setCode } = await (0, execFileNoThrow_js_1.execFileNoThrow)('/usr/libexec/PlistBuddy', ['-c', `Set :'Window Settings':'${profileName}':Bell false`, (0, appleTerminalBackup_js_1.getTerminalPlistPath)()]);
        if (setCode !== 0) {
            (0, log_js_1.logError)(new Error(`Failed to disable audio bell for Terminal.app profile: ${profileName}`));
            return false;
        }
    }
    return true;
...
async function enableOptionAsMetaForTerminal(theme) {
    try {
        ...
        const backupPath = await (0, appleTerminalBackup_js_1.backupTerminalPreferences)();
        if (!backupPath) {
        ...
        const { stdout: defaultProfile, code: readCode } = await (0, execFileNoThrow_js_1.execFileNoThrow)('defaults', ['read', 'com.apple.Terminal', 'Default Window Settings']);
        if (readCode !== 0 || !defaultProfile.trim()) {
            throw new Error('Failed to read default Terminal.app profile');
        }
        const { stdout: startupProfile, code: startupCode } = await (0, execFileNoThrow_js_1.execFileNoThrow)('defaults', ['read', 'com.apple.Terminal', 'Startup Window Settings']);
        ...
        return `${(0, ink_js_1.color)('success', theme)(`Configured Terminal.app settings:`)}${EOL}${(0, ink_js_1.color)('success', theme)('- Enabled "Use Option as Meta key"')}${EOL}${(0, ink_js_1.color)('success', theme)('- Switched to visual bell')}${EOL}${chalk_1.default.dim('Option+Enter will now enter a newline.')}${EOL}${chalk_1.default.dim('You must restart Terminal.app for changes to take effect.', theme)}${EOL}`;
    }
    catch (error) {
        (0, log_js_1.logError)(error);
        // Attempt to restore from backup
        const restoreResult = await (0, appleTerminalBackup_js_1.checkAndRestoreTerminalBackup)();
...
chars = "\\u001B\\r"`;
    ...
    for (const path of configPaths) {
        try {
            configContent = await (0, promises_1.readFile)(path, {
                encoding: 'utf-8'
            });
            configPath = path;
            configExists = true;
    ...
    try {
        if (configExists) {
            ...
            if (configContent.includes('mods = "Shift"') && configContent.includes('key = "Return"')) {
                return `${(0, ink_js_1.color)('warning', theme)('Found existing Alacritty Shift+Enter key binding. Remove it to continue.')}${EOL}${chalk_1.default.dim(`See ${formatPathLink(configPath)}`)}${EOL}`;
            }
            // Create backup
            const randomSha = (0, crypto_1.randomBytes)(4).toString('hex');
            ...
            try {
                ...
                return `${(0, ink_js_1.color)('warning', theme)('Error backing up existing Alacritty config. Bailing out.')}${EOL}${chalk_1.default.dim(`See ${formatPathLink(configPath)}`)}${EOL}${chalk_1.default.dim(`Backup path: ${formatPathLink(backupPath)}`)}${EOL}`;
            }
        }
        else {
            // Ensure config directory exists (idempotent with recursive)
            await (0, promises_1.mkdir)((0, path_1.dirname)(configPath), {
                recursive: true
            });
        }
        ...
        await (0, promises_1.writeFile)(configPath, updatedContent, {
            encoding: 'utf-8'
        });
        return `${(0, ink_js_1.color)('success', theme)('Installed Alacritty Shift+Enter key binding')}${EOL}${(0, ink_js_1.color)('success', theme)('You may need to restart Alacritty for changes to take effect')}${EOL}${chalk_1.default.dim(`See ${formatPathLink(configPath)}`)}${EOL}`;
    }
    catch (error) {
        (0, log_js_1.logError)(error);
...
async function installBindingsForZed(theme) {
    ...
    const zedDir = (0, path_1.join)((0, os_1.homedir)(), '.config', 'zed');
    const keymapPath = (0, path_1.join)(zedDir, 'keymap.json');
    try {
        // Ensure zed directory exists (idempotent with recursive)
        await (0, promises_1.mkdir)(zedDir, {
            recursive: true
        });
        ...
        let fileExists = false;
        try {
            keymapContent = await (0, promises_1.readFile)(keymapPath, {
                encoding: 'utf-8'
            });
            fileExists = true;
        }
        ...
        if (fileExists) {
            ...
            if (keymapContent.includes('shift-enter')) {
                return `${(0, ink_js_1.color)('warning', theme)('Found existing Zed Shift+Enter key binding. Remove it to continue.')}${EOL}${chalk_1.default.dim(`See ${formatPathLink(keymapPath)}`)}${EOL}`;
            }
            // Create backup
            const randomSha = (0, crypto_1.randomBytes)(4).toString('hex');
            const backupPath = `${keymapPath}.${randomSha}.bak`;
            try {
                await (0, promises_1.copyFile)(keymapPath, backupPath);
            }
            catch {
                return `${(0, ink_js_1.color)('warning', theme)('Error backing up existing Zed keymap. Bailing out.')}${EOL}${chalk_1.default.dim(`See ${formatPathLink(keymapPath)}`)}${EOL}${chalk_1.default.dim(`Backup path: ${formatPathLink(backupPath)}`)}${EOL}`;
            }
        }
        ...
        await (0, promises_1.writeFile)(keymapPath, (0, slowOperations_js_1.jsonStringify)(keymap, null, 2) + '\n', {
            encoding: 'utf-8'
        });
        return `${(0, ink_js_1.color)('success', theme)('Installed Zed Shift+Enter key binding')}${EOL}${chalk_1.default.dim(`See ${formatPathLink(keymapPath)}`)}${EOL}`;
    }
    catch (error) {
        (0, log_js_1.logError)(error);
