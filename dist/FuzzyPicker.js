...
const geometry_js_1 = require("../../ink/layout/geometry.js");
const ink_js_1 = require("../../ink.js");
const SearchBox_js_1 = require("../SearchBox.js");
const Byline_js_1 = require("./Byline.js");
const KeyboardShortcutHint_js_1 = require("./KeyboardShortcutHint.js");
const ListItem_js_1 = require("./ListItem.js");
...
function FuzzyPicker({ title, placeholder = 'Type to search…', initialQuery, items, getKey, renderItem, renderPreview, previewPosition = 'bottom', visibleCount: requestedVisible = DEFAULT_VISIBLE, direction = 'down', onQueryChange, onSelect, onTab, onShiftTab, onFocus, onCancel, emptyMessage = 'No results', matchLabel, selectAction = 'select', extraHints }) {
    ...
    const step = (delta) => {
        setFocusedIndex(i => (0, geometry_js_1.clamp)(i + delta, 0, items.length - 1));
    };
    ...
    const { query, cursorOffset } = (0, useSearchInput_js_1.useSearchInput)({
        isActive: true,
        onExit: () => { },
        onCancel,
        initialQuery,
    ...
    (0, react_1.useEffect)(() => {
        setFocusedIndex(i => (0, geometry_js_1.clamp)(i, 0, items.length - 1));
    }, [items.length]);
    ...
    const windowStart = (0, geometry_js_1.clamp)(focusedIndex - visibleCount + 1, 0, items.length - visibleCount);
    ...
    const searchBox = React.createElement(SearchBox_js_1.SearchBox, { query: query, cursorOffset: cursorOffset, placeholder: placeholder, isFocused: true, isTerminalFocused: isTerminalFocused });
    ...
    const preview = renderPreview && focused ? React.createElement(ink_js_1.Box, { flexDirection: "column", flexGrow: 1 }, renderPreview(focused)) : null;
    ...
    const listGroup = renderPreview && previewPosition === 'right' ? React.createElement(ink_js_1.Box, { flexDirection: "row", gap: 2, height: visibleCount + (matchLabel ? 1 : 0) },
        React.createElement(ink_js_1.Box, { flexDirection: "column", flexShrink: 0 },
            listBlock,
            matchLabel && React.createElement(ink_js_1.Text, { dimColor: true }, matchLabel)),
        preview ?? React.createElement(ink_js_1.Box, { flexGrow: 1 })) :
    ...
    return React.createElement(Pane_js_1.Pane, { color: "permission" },
        React.createElement(ink_js_1.Box, { flexDirection: "column", gap: 1, tabIndex: 0, autoFocus: true, onKeyDown: handleKeyDown },
            React.createElement(ink_js_1.Text, { bold: true, color: "permission" }, title),
            inputAbove && searchBox,
            listGroup,
            !inputAbove && searchBox,
            React.createElement(ink_js_1.Text, { dimColor: true },
                React.createElement(Byline_js_1.Byline, null,
                    React.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "\u2191/\u2193", action: compact ? 'nav' : 'navigate' }),
                    React.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "Enter", action: compact ? firstWord(selectAction) : selectAction }),
                    onTab && React.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "Tab", action: onTab.action }),
                    onShiftTab && !compact && React.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "shift+tab", action: onShiftTab.action }),
...
function List(t0) {
    ...
    if (visible.length === 0) {
        let t1;
        if ($[0] !== emptyText) {
            t1 = React.createElement(ink_js_1.Text, { dimColor: true }, emptyText);
            $[0] = emptyText;
            $[1] = t1;
        }
        else {
        ...
        if ($[2] !== t1 || $[3] !== visibleCount) {
            t2 = React.createElement(ink_js_1.Box, { height: visibleCount, flexShrink: 0 }, t1);
            $[2] = t1;
            $[3] = visibleCount;
            $[4] = t2;
        }
    ...
    if ($[5] !== direction || $[6] !== focusedIndex || $[7] !== getKey || $[8] !== renderItem || $[9] !== total || $[10] !== visible || $[11] !== visibleCount || $[12] !== windowStart) {
        ...
        if ($[14] !== direction || $[15] !== focusedIndex || $[16] !== getKey || $[17] !== renderItem || $[18] !== total || $[19] !== visible.length || $[20] !== visibleCount || $[21] !== windowStart) {
            t2 = (item, i) => {
                ...
                return React.createElement(ListItem_js_1.ListItem, { key: getKey(item), isFocused: isFocused, showScrollUp: direction === "up" ? atHighEdge : atLowEdge, showScrollDown: direction === "up" ? atLowEdge : atHighEdge, styled: false }, renderItem(item, isFocused));
            };
            $[14] = direction;
            $[15] = focusedIndex;
            $[16] = getKey;
    ...
    if ($[23] !== rows || $[24] !== t2 || $[25] !== visibleCount) {
        t3 = React.createElement(ink_js_1.Box, { height: visibleCount, flexShrink: 0, flexDirection: t2 }, rows);
        $[23] = rows;
        $[24] = t2;
        $[25] = visibleCount;
        $[26] = t3;
