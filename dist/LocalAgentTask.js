    ...
    for (const content of message.message.content) {
        if (content.type === 'tool_use') {
            ...
            if (content.name !== SyntheticOutputTool_js_1.SYNTHETIC_OUTPUT_TOOL_NAME) {
                ...
                const classification = tools ? (0, collapseReadSearch_js_1.getToolSearchOrReadInfo)(content.name, input, tools) : undefined;
 ...
 */
function enqueueAgentNotification({ taskId, description, status, error, setAppState, finalMessage, usage, toolUseId, worktreePath, worktreeBranch }) {
    ...
    const worktreeSection = worktreePath ? `\n<${xml_js_1.WORKTREE_TAG}><${xml_js_1.WORKTREE_PATH_TAG}>${worktreePath}</${xml_js_1.WORKTREE_PATH_TAG}>${worktreeBranch ? `<${xml_js_1.WORKTREE_BRANCH_TAG}>${worktreeBranch}</${xml_js_1.WORKTREE_BRANCH_TAG}>` : ''}</${xml_js_1.WORKTREE_TAG}>` : '';
...
<${xml_js_1.SUMMARY_TAG}>${summary}</${xml_js_1.SUMMARY_TAG}>${resultSection}${usageSection}${worktreeSection}
...
function killAsyncAgent(taskId, setAppState) {
    ...
    (0, framework_js_1.updateTaskState)(taskId, setAppState, task => {
        ...
        return {
            ...
            status: 'killed',
            endTime: Date.now(),
            evictAfter: task.retain ? undefined : Date.now() + framework_js_1.PANEL_GRACE_MS,
 ...
 */
function registerAsyncAgent({ agentId, description, prompt, selectedAgent, setAppState, parentAbortController, toolUseId }) {
    ...
    const abortController = parentAbortController ? (0, abortController_js_1.createChildAbortController)(parentAbortController) : (0, abortController_js_1.createAbortController)();
    const taskState = {
 ...
 */
function registerAgentForeground({ agentId, description, prompt, selectedAgent, setAppState, autoBackgroundMs, toolUseId }) {
    void (0, diskOutput_js_1.initTaskOutputAsSymlink)(agentId, (0, sessionStorage_js_1.getAgentTranscriptPath)((0, ids_js_1.asAgentId)(agentId)));
