...
function getVisibleAgentTasks(tasks) {
    return Object.values(tasks).filter((t) => (0, LocalAgentTask_js_1.isPanelAgentTask)(t) && t.evictAfter !== 0).sort((a, b) => a.startTime - b.startTime);
}
function CoordinatorTaskPanel() {
    ...
    const agentNameRegistry = (0, AppState_js_1.useAppState)(s_1 => s_1.agentNameRegistry);
    const coordinatorTaskIndex = (0, AppState_js_1.useAppState)(s_2 => s_2.coordinatorTaskIndex);
    const tasksSelected = (0, AppState_js_1.useAppState)(s_3 => s_3.footerSelection === 'tasks');
    ...
    const hasTasks = Object.values(tasks).some(LocalAgentTask_js_1.isPanelAgentTask);
    ...
    React.useEffect(() => {
        ...
        const interval = setInterval((tasksRef_0, setAppState_0, setTick_0) => {
            ...
            for (const t of Object.values(tasksRef_0.current)) {
                if ((0, LocalAgentTask_js_1.isPanelAgentTask)(t) && (t.evictAfter ?? Infinity) <= now) {
    ...
    return React.createElement(ink_js_1.Box, { flexDirection: "column", marginTop: 1 },
        React.createElement(MainLine, { isSelected: selectedIndex === 0, isViewed: viewingAgentTaskId === undefined, onClick: () => (0, teammateViewHelpers_js_1.exitTeammateView)(setAppState) }),
        visibleTasks.map((task, i) => React.createElement(AgentLine, { key: task.id, task: task, name: nameByAgentId.get(task.id), isSelected: selectedIndex === i + 1, isViewed: viewingAgentTaskId === task.id, onClick: () => (0, teammateViewHelpers_js_1.enterTeammateView)(task.id, setAppState) })));
}
/**
...
function MainLine(t0) {
    ...
    const prefix = isSelected || hover ? figures_1.default.pointer + " " : "  ";
    const bullet = isViewed ? figures_js_1.BLACK_CIRCLE : figures_1.default.circle;
    let t1;
    let t2;
    ...
    if ($[2] !== bullet || $[3] !== isViewed || $[4] !== prefix || $[5] !== t3) {
        t4 = React.createElement(ink_js_1.Text, { dimColor: t3, bold: isViewed },
            prefix,
            bullet,
            " main");
        $[2] = bullet;
    ...
    let t5;
    if ($[7] !== onClick || $[8] !== t4) {
        t5 = React.createElement(ink_js_1.Box, { onClick: onClick, onMouseEnter: t1, onMouseLeave: t2 }, t4);
        $[7] = onClick;
        $[8] = t4;
        $[9] = t5;
    }
...
function AgentLine(t0) {
    ...
    const [hover, setHover] = React.useState(false);
    const isRunning = !(0, taskStatusUtils_js_1.isTerminalStatus)(task.status);
    ...
    const arrow = lastActivity ? figures_1.default.arrowDown : figures_1.default.arrowUp;
    let t2;
    if ($[2] !== arrow || $[3] !== tokenCount) {
        t2 = tokenCount !== undefined && tokenCount > 0 ? ` · ${arrow} ${(0, format_js_1.formatNumber)(tokenCount)} tokens` : "";
        $[2] = arrow;
        $[3] = tokenCount;
        $[4] = t2;
    }
    ...
    const prefix = highlighted ? figures_1.default.pointer + " " : "  ";
    const bullet = isViewed ? figures_js_1.BLACK_CIRCLE : figures_1.default.circle;
    const dim = !highlighted && !isViewed;
    const sep = isRunning ? figures_js_1.PLAY_ICON : figures_js_1.PAUSE_ICON;
    ...
    const availableForDesc = columns - (0, stringWidth_js_1.stringWidth)(prefix) - (0, stringWidth_js_1.stringWidth)(`${bullet} `) - (0, stringWidth_js_1.stringWidth)(namePart) - (0, stringWidth_js_1.stringWidth)(suffixPart);
    ...
    if ($[8] !== name) {
        t5 = name && React.createElement(React.Fragment, null,
            React.createElement(ink_js_1.Text, { dimColor: false, bold: true }, name),
            ": ");
        $[8] = name;
        $[9] = t5;
    }
    ...
    if ($[10] !== queuedCount || $[11] !== queuedText) {
        t6 = queuedCount > 0 && React.createElement(ink_js_1.Text, { color: "warning" }, queuedText);
        $[10] = queuedCount;
        $[11] = queuedText;
        $[12] = t6;
    }
    ...
    let t7;
    if ($[13] !== hintPart) {
        t7 = hintPart && React.createElement(ink_js_1.Text, { dimColor: true }, hintPart);
        $[13] = hintPart;
        $[14] = t7;
    }
    else {
    ...
    if ($[15] !== bullet || $[16] !== dim || $[17] !== elapsed || $[18] !== isViewed || $[19] !== prefix || $[20] !== sep || $[21] !== t5 || $[22] !== t6 || $[23] !== t7 || $[24] !== tokenText || $[25] !== truncated) {
        t8 = React.createElement(ink_js_1.Text, { dimColor: dim, bold: isViewed },
            prefix,
            bullet,
            " ",
            t5,
    ...
    if ($[29] !== line || $[30] !== onClick) {
        t11 = React.createElement(ink_js_1.Box, { onClick: onClick, onMouseEnter: t9, onMouseLeave: t10 }, line);
        $[29] = line;
        $[30] = onClick;
        $[31] = t11;
    }
