...
function FileEditToolUpdatedMessage(t0) {
    ...
    if ($[0] !== numAdditions) {
        t1 = numAdditions > 0 ? React.createElement(React.Fragment, null,
            "Added ",
            React.createElement(ink_js_1.Text, { bold: true }, numAdditions),
            " ",
    ...
    if ($[2] !== numAdditions || $[3] !== numRemovals) {
        t3 = numRemovals > 0 ? React.createElement(React.Fragment, null,
            numAdditions === 0 ? "R" : "r",
            "emoved ",
            React.createElement(ink_js_1.Text, { bold: true }, numRemovals),
            " ",
    ...
    if (previewHint) {
        if (style !== "condensed" && !verbose) {
            let t5;
            if ($[9] !== previewHint) {
                t5 = React.createElement(MessageResponse_js_1.MessageResponse, null,
                    React.createElement(ink_js_1.Text, { dimColor: true }, previewHint));
                $[9] = previewHint;
                $[10] = t5;
            }
            else {
    ...
    let t5;
    if ($[11] !== text) {
        t5 = React.createElement(ink_js_1.Text, null, text);
        $[11] = text;
        $[12] = t5;
    }
    else {
    ...
    if ($[13] !== fileContent || $[14] !== filePath || $[15] !== firstLine || $[16] !== structuredPatch || $[17] !== t6) {
        t7 = React.createElement(StructuredDiffList_js_1.StructuredDiffList, { hunks: structuredPatch, dim: false, width: t6, filePath: filePath, firstLine: firstLine, fileContent: fileContent });
        $[13] = fileContent;
        $[14] = filePath;
        $[15] = firstLine;
        $[16] = structuredPatch;
