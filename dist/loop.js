...
Parse the input below into \`[interval] <prompt…>\` and schedule it with ${prompt_js_1.CRON_CREATE_TOOL_NAME}.
...
If the resulting prompt is empty, show usage \`/loop [interval] <prompt>\` and stop — do not call ${prompt_js_1.CRON_CREATE_TOOL_NAME}.
...
1. Call ${prompt_js_1.CRON_CREATE_TOOL_NAME} with:
   - \`cron\`: the expression from the table above
   - \`prompt\`: the parsed prompt from above, verbatim (slash commands are passed through unchanged)
   - \`recurring\`: \`true\`
2. Briefly confirm: what's scheduled, the cron expression, the human-readable cadence, that recurring tasks auto-expire after ${prompt_js_1.DEFAULT_MAX_AGE_DAYS} days, and that they can cancel sooner with ${prompt_js_1.CRON_DELETE_TOOL_NAME} (include the job ID).
