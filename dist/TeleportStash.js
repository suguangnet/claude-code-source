"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TeleportStash = TeleportStash;
const figures_1 = __importDefault(require("figures"));
const react_1 = __importStar(require("react"));
const ink_js_1 = require("../ink.js");
const debug_js_1 = require("../utils/debug.js");
const git_js_1 = require("../utils/git.js");
const index_js_1 = require("./CustomSelect/index.js");
const Dialog_js_1 = require("./design-system/Dialog.js");
const Spinner_js_1 = require("./Spinner.js");
function TeleportStash({ onStashAndContinue, onCancel }) {
    const [gitFileStatus, setGitFileStatus] = (0, react_1.useState)(null);
    const changedFiles = gitFileStatus !== null ? [...gitFileStatus.tracked, ...gitFileStatus.untracked] : [];
    const [loading, setLoading] = (0, react_1.useState)(true);
    const [stashing, setStashing] = (0, react_1.useState)(false);
    const [error, setError] = (0, react_1.useState)(null);
    // Load changed files on mount
    (0, react_1.useEffect)(() => {
        const loadChangedFiles = async () => {
            try {
                const fileStatus = await (0, git_js_1.getFileStatus)();
                setGitFileStatus(fileStatus);
            }
            catch (err) {
                const errorMessage = err instanceof Error ? err.message : String(err);
                (0, debug_js_1.logForDebugging)(`Error getting changed files: ${errorMessage}`, {
                    level: 'error'
                });
                setError('Failed to get changed files');
            }
            finally {
                setLoading(false);
            }
        };
        void loadChangedFiles();
    }, []);
    const handleStash = async () => {
        setStashing(true);
        try {
            (0, debug_js_1.logForDebugging)('Stashing changes before teleport...');
            const success = await (0, git_js_1.stashToCleanState)('Teleport auto-stash');
            if (success) {
                (0, debug_js_1.logForDebugging)('Successfully stashed changes');
                onStashAndContinue();
            }
            else {
                setError('Failed to stash changes');
            }
        }
        catch (err_0) {
            const errorMessage_0 = err_0 instanceof Error ? err_0.message : String(err_0);
            (0, debug_js_1.logForDebugging)(`Error stashing changes: ${errorMessage_0}`, {
                level: 'error'
            });
            setError('Failed to stash changes');
        }
        finally {
            setStashing(false);
        }
    };
    const handleSelectChange = (value) => {
        if (value === 'stash') {
            void handleStash();
        }
        else {
            onCancel();
        }
    };
    if (loading) {
        return react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", padding: 1 },
            react_1.default.createElement(ink_js_1.Box, { marginBottom: 1 },
                react_1.default.createElement(Spinner_js_1.Spinner, null),
                react_1.default.createElement(ink_js_1.Text, null,
                    " Checking git status",
                    figures_1.default.ellipsis)));
    }
    if (error) {
        return react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", padding: 1 },
            react_1.default.createElement(ink_js_1.Text, { bold: true, color: "error" },
                "Error: ",
                error),
            react_1.default.createElement(ink_js_1.Box, { marginTop: 1 },
                react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "Press "),
                react_1.default.createElement(ink_js_1.Text, { bold: true }, "Escape"),
                react_1.default.createElement(ink_js_1.Text, { dimColor: true }, " to cancel")));
    }
    const showFileCount = changedFiles.length > 8;
    return react_1.default.createElement(Dialog_js_1.Dialog, { title: "Working Directory Has Changes", onCancel: onCancel },
        react_1.default.createElement(ink_js_1.Text, null, "Teleport will switch git branches. The following changes were found:"),
        react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", paddingLeft: 2 }, changedFiles.length > 0 ? showFileCount ? react_1.default.createElement(ink_js_1.Text, null,
            changedFiles.length,
            " files changed") : changedFiles.map((file, index) => react_1.default.createElement(ink_js_1.Text, { key: index }, file)) : react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "No changes detected")),
        react_1.default.createElement(ink_js_1.Text, null, "Would you like to stash these changes and continue with teleport?"),
        stashing ? react_1.default.createElement(ink_js_1.Box, null,
            react_1.default.createElement(Spinner_js_1.Spinner, null),
            react_1.default.createElement(ink_js_1.Text, null, " Stashing changes...")) : react_1.default.createElement(index_js_1.Select, { options: [{
                    label: 'Stash changes and continue',
                    value: 'stash'
                }, {
                    label: 'Exit',
                    value: 'exit'
                }], onChange: handleSelectChange }));
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
