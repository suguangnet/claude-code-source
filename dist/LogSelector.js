...
const SearchBox_js_1 = require("./SearchBox.js");
const SessionPreview_js_1 = require("./SessionPreview.js");
const Spinner_js_1 = require("./Spinner.js");
const TagTabs_js_1 = require("./TagTabs.js");
const TextInput_js_1 = __importDefault(require("./TextInput.js"));
const TreeSelect_js_1 = require("./ui/TreeSelect.js");
...
function buildLogLabel(log, maxLabelWidth, options) {
    ...
    const truncatedSummary = normalizeAndTruncateToWidth((0, log_js_1.getLogDisplayTitle)(log), maxSummaryWidth);
...
function buildLogMetadata(log, options) {
    ...
    const baseMetadata = (0, format_js_1.formatLogMetadata)(log);
...
function LogSelector(t0) {
    const $ = (0, compiler_runtime_1.c)(247);
    ...
    const terminalSize = (0, useTerminalSize_js_1.useTerminalSize)();
    const columns = forceWidth === undefined ? terminalSize.columns : forceWidth;
    const exitState = (0, useExitOnCtrlCDWithKeybindings_js_1.useExitOnCtrlCDWithKeybindings)(onCancel);
    ...
    let t5;
    if ($[3] !== theme.warning) {
        t5 = text => (0, colorize_js_1.applyColor)(text, theme.warning);
        $[3] = theme.warning;
        $[4] = t5;
    }
    else {
    ...
    const [currentBranch, setCurrentBranch] = react_1.default.useState(null);
    const [branchFilterEnabled, setBranchFilterEnabled] = react_1.default.useState(false);
    const [showAllWorktrees, setShowAllWorktrees] = react_1.default.useState(false);
    const [hasMultipleWorktrees, setHasMultipleWorktrees] = react_1.default.useState(false);
    let t6;
    ...
    const currentCwd = t6;
    const [renameValue, setRenameValue] = react_1.default.useState("");
    const [renameCursorOffset, setRenameCursorOffset] = react_1.default.useState(0);
    let t7;
    ...
    const [expandedGroupSessionIds, setExpandedGroupSessionIds] = react_1.default.useState(t7);
    const [focusedNode, setFocusedNode] = react_1.default.useState(null);
    const [focusedIndex, setFocusedIndex] = react_1.default.useState(1);
    const [viewMode, setViewMode] = react_1.default.useState("list");
    const [previewLog, setPreviewLog] = react_1.default.useState(null);
    const prevFocusedIdRef = react_1.default.useRef(null);
    const [selectedTagIndex, setSelectedTagIndex] = react_1.default.useState(0);
    let t8;
    ...
    const [agenticSearchState, setAgenticSearchState] = react_1.default.useState(t8);
    const [isAgenticSearchOptionFocused, setIsAgenticSearchOptionFocused] = react_1.default.useState(false);
    const agenticSearchAbortRef = react_1.default.useRef(null);
    ...
    const { query: searchQuery, setQuery: setSearchQuery, cursorOffset: searchCursorOffset } = (0, useSearchInput_js_1.useSearchInput)(t14);
    const deferredSearchQuery = react_1.default.useDeferredValue(searchQuery);
    const [debouncedDeepSearchQuery, setDebouncedDeepSearchQuery] = react_1.default.useState("");
    let t15;
    let t16;
    ...
    const [deepSearchResults, setDeepSearchResults] = react_1.default.useState(null);
    const [isSearching, setIsSearching] = react_1.default.useState(false);
    let t17;
    let t18;
    if ($[17] === Symbol.for("react.memo_cache_sentinel")) {
        t17 = () => {
            (0, git_js_1.getBranch)().then(branch => setCurrentBranch(branch));
            (0, getWorktreePaths_js_1.getWorktreePaths)(currentCwd).then(paths => {
    ...
    bb0: {
        ...
        if ($[39] !== baseFilteredLogs || $[40] !== searchQuery) {
            ...
            t23 = baseFilteredLogs.filter(log_5 => {
                const displayedTitle = (0, log_js_1.getLogDisplayTitle)(log_5).toLowerCase();
    ...
    bb3: {
        ...
        if ($[73] !== displayedLogs || $[74] !== highlightColor || $[75] !== maxLabelWidth || $[76] !== showAllProjects || $[77] !== snippets) {
            ...
            if ($[79] !== highlightColor || $[80] !== maxLabelWidth || $[81] !== showAllProjects || $[82] !== snippets) {
                t32 = (log_9, index_0) => {
                    const rawSummary = (0, log_js_1.getLogDisplayTitle)(log_9);
                    ...
                    const baseDescription = (0, format_js_1.formatLogMetadata)(log_9);
    ...
    if ($[84] !== displayedLogs || $[85] !== expandedGroupSessionIds || $[86] !== focusedLog) {
        t31 = () => {
            ...
            const sessionId_0 = (0, sessionStorage_js_1.getSessionIdFromLog)(focusedLog);
            if (!sessionId_0) {
                return "";
            }
            const sessionLogs = displayedLogs.filter(log_10 => (0, sessionStorage_js_1.getSessionIdFromLog)(log_10) === sessionId_0);
    ...
    if ($[88] !== focusedLog || $[89] !== onLogsChanged || $[90] !== renameValue) {
        t32 = async () => {
            const sessionId_1 = focusedLog ? (0, sessionStorage_js_1.getSessionIdFromLog)(focusedLog) : undefined;
            ...
            if (renameValue.trim()) {
                await (0, sessionStorage_js_1.saveCustomTitle)(sessionId_1, renameValue.trim(), focusedLog.fullPath);
    ...
    const prevAgenticStatusRef = react_1.default.useRef(agenticSearchState.status);
    let t40;
    ...
    if ($[118] !== displayedLogs) {
        t43 = node => {
            setFocusedNode(node);
            const index_2 = displayedLogs.findIndex(log_12 => (0, sessionStorage_js_1.getSessionIdFromLog)(log_12) === (0, sessionStorage_js_1.getSessionIdFromLog)(node.value.log));
            if (index_2 >= 0) {
    ...
    if (viewMode === "preview" && previewLog && isResumeWithRenameEnabled) {
        ...
        if ($[161] !== onSelect || $[162] !== previewLog) {
            t58 = react_1.default.createElement(SessionPreview_js_1.SessionPreview, { log: previewLog, onExit: t57, onSelect: onSelect });
            $[161] = onSelect;
            $[162] = previewLog;
            $[163] = t58;
        }
    ...
    if ($[164] === Symbol.for("react.memo_cache_sentinel")) {
        t58 = react_1.default.createElement(ink_js_1.Box, { flexShrink: 0 },
            react_1.default.createElement(Divider_js_1.Divider, { color: "suggestion" }));
        $[164] = t58;
    }
    else {
        t58 = $[164];
    ...
    if ($[165] === Symbol.for("react.memo_cache_sentinel")) {
        t59 = react_1.default.createElement(ink_js_1.Box, { flexShrink: 0 },
    ...
    if ($[166] !== columns || $[167] !== displayedLogs.length || $[168] !== effectiveTagIndex || $[169] !== focusedIndex || $[170] !== hasTags || $[171] !== showAllProjects || $[172] !== tagTabs || $[173] !== viewMode || $[174] !== visibleCount) {
        t60 = hasTags ? react_1.default.createElement(TagTabs_js_1.TagTabs, { tabs: tagTabs, selectedIndex: effectiveTagIndex, availableWidth: columns, showAllProjects: showAllProjects }) : react_1.default.createElement(ink_js_1.Box, { flexShrink: 0 },
            react_1.default.createElement(ink_js_1.Text, { bold: true, color: "suggestion" },
                "Resume Session",
                viewMode === "list" && displayedLogs.length > visibleCount && react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                    " ",
                    "(",
                    focusedIndex,
                    " of ",
    ...
    if ($[176] !== isTerminalFocused || $[177] !== searchCursorOffset || $[178] !== searchQuery || $[179] !== t61) {
        t62 = react_1.default.createElement(SearchBox_js_1.SearchBox, { query: searchQuery, isFocused: t61, isTerminalFocused: isTerminalFocused, cursorOffset: searchCursorOffset });
    ...
    if ($[181] !== filterIndicators || $[182] !== viewMode) {
        t63 = filterIndicators.length > 0 && viewMode !== "search" && react_1.default.createElement(ink_js_1.Box, { flexShrink: 0, paddingLeft: 2 },
            react_1.default.createElement(ink_js_1.Text, { dimColor: true },
                react_1.default.createElement(Byline_js_1.Byline, null, filterIndicators)));
    ...
    if ($[184] === Symbol.for("react.memo_cache_sentinel")) {
        t64 = react_1.default.createElement(ink_js_1.Box, { flexShrink: 0 },
            react_1.default.createElement(ink_js_1.Text, null, " "));
        $[184] = t64;
    }
    else {
    ...
    if ($[185] !== agenticSearchState.status) {
        t65 = agenticSearchState.status === "searching" && react_1.default.createElement(ink_js_1.Box, { paddingLeft: 1, flexShrink: 0 },
            react_1.default.createElement(Spinner_js_1.Spinner, null),
            react_1.default.createElement(ink_js_1.Text, null, " Searching\u2026"));
    ...
    if ($[187] !== agenticSearchState.results || $[188] !== agenticSearchState.status) {
        t66 = agenticSearchState.status === "results" && agenticSearchState.results.length > 0 && react_1.default.createElement(ink_js_1.Box, { paddingLeft: 1, marginBottom: 1, flexShrink: 0 },
            react_1.default.createElement(ink_js_1.Text, { dimColor: true, italic: true }, "Claude found these results:"));
    ...
    if ($[190] !== agenticSearchState.results || $[191] !== agenticSearchState.status || $[192] !== filteredLogs) {
        t67 = agenticSearchState.status === "results" && agenticSearchState.results.length === 0 && filteredLogs.length === 0 && react_1.default.createElement(ink_js_1.Box, { paddingLeft: 1, marginBottom: 1, flexShrink: 0 },
            react_1.default.createElement(ink_js_1.Text, { dimColor: true, italic: true }, "No matching sessions found."));
    ...
    if ($[194] !== agenticSearchState.status || $[195] !== filteredLogs) {
        t68 = agenticSearchState.status === "error" && filteredLogs.length === 0 && react_1.default.createElement(ink_js_1.Box, { paddingLeft: 1, marginBottom: 1, flexShrink: 0 },
            react_1.default.createElement(ink_js_1.Text, { dimColor: true, italic: true }, "No matching sessions found."));
    ...
    if ($[197] !== agenticSearchState.status || $[198] !== isAgenticSearchOptionFocused || $[199] !== onAgenticSearch || $[200] !== searchQuery) {
        t69 = Boolean(searchQuery.trim()) && onAgenticSearch && false && agenticSearchState.status !== "searching" && agenticSearchState.status !== "results" && agenticSearchState.status !== "error" && react_1.default.createElement(ink_js_1.Box, { flexShrink: 0, flexDirection: "column" },
            react_1.default.createElement(ink_js_1.Box, { flexDirection: "row", gap: 1 },
                react_1.default.createElement(ink_js_1.Text, { color: isAgenticSearchOptionFocused ? "suggestion" : undefined }, isAgenticSearchOptionFocused ? figures_1.default.pointer : " "),
                react_1.default.createElement(ink_js_1.Text, { color: isAgenticSearchOptionFocused ? "suggestion" : undefined, bold: isAgenticSearchOptionFocused }, "Search deeply using Claude \u2192")),
            react_1.default.createElement(ink_js_1.Box, { height: 1 }));
    ...
    if ($[202] !== agenticSearchState.status || $[203] !== branchFilterEnabled || $[204] !== columns || $[205] !== displayedLogs || $[206] !== expandedGroupSessionIds || $[207] !== flatOptions || $[208] !== focusedLog || $[209] !== focusedNode?.id || $[210] !== handleFlatOptionsSelectFocus || $[211] !== handleRenameSubmit || $[212] !== handleTreeSelectFocus || $[213] !== isAgenticSearchOptionFocused || $[214] !== onCancel || $[215] !== onSelect || $[216] !== renameCursorOffset || $[217] !== renameValue || $[218] !== treeNodes || $[219] !== viewMode || $[220] !== visibleCount) {
        t70 = agenticSearchState.status === "searching" ? null : viewMode === "rename" && focusedLog ? react_1.default.createElement(ink_js_1.Box, { paddingLeft: 2, flexDirection: "column" },
            react_1.default.createElement(ink_js_1.Text, { bold: true }, "Rename session:"),
            react_1.default.createElement(ink_js_1.Box, { paddingTop: 1 },
                react_1.default.createElement(TextInput_js_1.default, { value: renameValue, onChange: setRenameValue, onSubmit: handleRenameSubmit, placeholder: (0, log_js_1.getLogDisplayTitle)(focusedLog, "Enter new session name"), columns: columns, cursorOffset: renameCursorOffset, onChangeCursorOffset: setRenameCursorOffset, showCursor: true }))) : isResumeWithRenameEnabled ? react_1.default.createElement(TreeSelect_js_1.TreeSelect, { nodes: treeNodes, onSelect: node_0 => {
            ...
            }, onUpFromFirstItem: enterSearchMode }) : react_1.default.createElement(select_js_1.Select, { options: flatOptions, onChange: value_0 => {
    ...
    if ($[222] !== agenticSearchState.status || $[223] !== currentBranch || $[224] !== exitState.keyName || $[225] !== exitState.pending || $[226] !== getExpandCollapseHint || $[227] !== hasMultipleWorktrees || $[228] !== isAgenticSearchOptionFocused || $[229] !== isSearching || $[230] !== onToggleAllProjects || $[231] !== showAllProjects || $[232] !== showAllWorktrees || $[233] !== viewMode) {
        t71 = react_1.default.createElement(ink_js_1.Box, { paddingLeft: 2 }, exitState.pending ? react_1.default.createElement(ink_js_1.Text, { dimColor: true },
            "Press ",
            exitState.keyName,
            " again to exit") : viewMode === "rename" ? react_1.default.createElement(ink_js_1.Text, { dimColor: true },
            react_1.default.createElement(Byline_js_1.Byline, null,
                react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "Enter", action: "save" }),
                react_1.default.createElement(ConfigurableShortcutHint_js_1.ConfigurableShortcutHint, { action: "confirm:no", context: "Confirmation", fallback: "Esc", description: "cancel" }))) : agenticSearchState.status === "searching" ? react_1.default.createElement(ink_js_1.Text, { dimColor: true },
            react_1.default.createElement(Byline_js_1.Byline, null,
                react_1.default.createElement(ink_js_1.Text, null, "Searching with Claude\u2026"),
                react_1.default.createElement(ConfigurableShortcutHint_js_1.ConfigurableShortcutHint, { action: "confirm:no", context: "Confirmation", fallback: "Esc", description: "cancel" }))) : isAgenticSearchOptionFocused ? react_1.default.createElement(ink_js_1.Text, { dimColor: true },
            react_1.default.createElement(Byline_js_1.Byline, null,
                react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "Enter", action: "search" }),
                react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "\u2193", action: "skip" }),
                react_1.default.createElement(ConfigurableShortcutHint_js_1.ConfigurableShortcutHint, { action: "confirm:no", context: "Confirmation", fallback: "Esc", description: "cancel" }))) : viewMode === "search" ? react_1.default.createElement(ink_js_1.Text, { dimColor: true },
            react_1.default.createElement(Byline_js_1.Byline, null,
                react_1.default.createElement(ink_js_1.Text, null, isSearching && false ? "Searching\u2026" : "Type to Search"),
                react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "Enter", action: "select" }),
                react_1.default.createElement(ConfigurableShortcutHint_js_1.ConfigurableShortcutHint, { action: "confirm:no", context: "Confirmation", fallback: "Esc", description: "clear" }))) : react_1.default.createElement(ink_js_1.Text, { dimColor: true },
            react_1.default.createElement(Byline_js_1.Byline, null,
                onToggleAllProjects && react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "Ctrl+A", action: `show ${showAllProjects ? "current dir" : "all projects"}` }),
                currentBranch && react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "Ctrl+B", action: "toggle branch" }),
                hasMultipleWorktrees && react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "Ctrl+W", action: `show ${showAllWorktrees ? "current worktree" : "all worktrees"}` }),
                react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "Ctrl+V", action: "preview" }),
                react_1.default.createElement(KeyboardShortcutHint_js_1.KeyboardShortcutHint, { shortcut: "Ctrl+R", action: "rename" }),
                react_1.default.createElement(ink_js_1.Text, null, "Type to search"),
                ...
                getExpandCollapseHint() && react_1.default.createElement(ink_js_1.Text, null, getExpandCollapseHint()))));
    ...
    if ($[235] !== t57 || $[236] !== t60 || $[237] !== t62 || $[238] !== t63 || $[239] !== t65 || $[240] !== t66 || $[241] !== t67 || $[242] !== t68 || $[243] !== t69 || $[244] !== t70 || $[245] !== t71) {
        t72 = react_1.default.createElement(ink_js_1.Box, { flexDirection: "column", height: t57 },
            t58,
            t59,
            t60,
            t62,
...
function _temp2(log_1) {
    ...
    const logSessionId = (0, sessionStorage_js_1.getSessionIdFromLog)(log_1);
    ...
    const fromMessages = (0, sessionStorage_js_1.getFirstMeaningfulUserMessageTextContent)(log_1.messages);
    if (fromMessages) {
        return true;
    }
