...
const voiceKeyterms_js_1 = require("../services/voiceKeyterms.js");
const voiceStreamSTT_js_1 = require("../services/voiceStreamSTT.js");
...
function useVoice({ onTranscript, onError, enabled, focusMode, }) {
    const [state, setState] = (0, react_1.useState)('idle');
    const stateRef = (0, react_1.useRef)('idle');
    const connectionRef = (0, react_1.useRef)(null);
    const accumulatedRef = (0, react_1.useRef)('');
    const onTranscriptRef = (0, react_1.useRef)(onTranscript);
    const onErrorRef = (0, react_1.useRef)(onError);
    const cleanupTimerRef = (0, react_1.useRef)(null);
    const releaseTimerRef = (0, react_1.useRef)(null);
    ...
    // a false release.
    const seenRepeatRef = (0, react_1.useRef)(false);
    const repeatFallbackTimerRef = (0, react_1.useRef)(null);
    ...
    const focusTriggeredRef = (0, react_1.useRef)(false);
    // Timer that tears down the session after prolonged silence in focus mode.
    const focusSilenceTimerRef = (0, react_1.useRef)(null);
    ...
    const silenceTimedOutRef = (0, react_1.useRef)(false);
    const recordingStartRef = (0, react_1.useRef)(0);
    ...
    const fullAudioRef = (0, react_1.useRef)([]);
    const silentDropRetriedRef = (0, react_1.useRef)(false);
    ...
    const attemptGenRef = (0, react_1.useRef)(0);
    ...
    const focusFlushedCharsRef = (0, react_1.useRef)(0);
    ...
    const hasAudioSignalRef = (0, react_1.useRef)(false);
    ...
    const audioLevelsRef = (0, react_1.useRef)([]);
    const isFocused = (0, use_terminal_focus_js_1.useTerminalFocus)();
    const setVoiceState = (0, voice_js_1.useSetVoiceState)();
    ...
    function finishRecording() {
        ...
        void finalizePromise
            ...
            if (finalizeSource === 'no_data_timeout' &&
                ...
                (0, debug_js_1.logForDebugging)(`[voice] Silent-drop detected (no_data_timeout, ${String(fullAudioRef.current.length)} chunks); replaying on fresh connection`);
                ...
                if (isStale())
                    return;
                const stt = normalizeLanguageForSTT((0, settings_js_1.getInitialSettings)().language);
                const keyterms = await (0, voiceKeyterms_js_1.getVoiceKeyterms)();
                if (isStale())
                    return;
            ...
            (0, debug_js_1.logForDebugging)(`[voice] Final transcript assembled (${String(text.length)} chars): "${text.slice(0, 200)}"`);
            ...
            if (text) {
                (0, debug_js_1.logForDebugging)(`[voice] Injecting transcript (${String(text.length)} chars)`);
    ...
    (0, react_1.useEffect)(() => {
        if (enabled && !voiceModule) {
            void Promise.resolve().then(() => __importStar(require('../services/voice.js'))).then(mod => {
                voiceModule = mod;
            });
        }
    }, [enabled]);
    ...
    async function startRecordingSession() {
        ...
        if (!availability.available) {
            (0, debug_js_1.logForDebugging)(`[voice] Recording not available: ${availability.reason ?? 'unknown'}`);
        ...
        const rawLanguage = (0, settings_js_1.getInitialSettings)().language;
            ...
            void (0, voiceStreamSTT_js_1.connectVoiceStream)({
                onTranscript: (text, isFinal) => {
                    if (isStale())
                        return;
                    sawTranscript = true;
                    (0, debug_js_1.logForDebugging)(`[voice] onTranscript: isFinal=${String(isFinal)} text="${text}"`);
                    if (isFinal && text.trim()) {
                        if (focusTriggeredRef.current) {
                            ...
                            (0, debug_js_1.logForDebugging)(`[voice] Focus mode: flushing final transcript immediately: "${text.trim()}"`);
                            ...
                            (0, debug_js_1.logForDebugging)(`[voice] Accumulated final transcript: "${accumulatedRef.current}"`);
                ...
                onError: (error, opts) => {
                    if (isStale()) {
                        (0, debug_js_1.logForDebugging)(`[voice] ignoring onError from stale session: ${error}`);
                        return;
                    }
                    ...
                    if (attemptGenRef.current !== myAttemptGen) {
                        (0, debug_js_1.logForDebugging)(`[voice] ignoring stale onError from superseded attempt: ${error}`);
                        return;
                    }
                    ...
                    if (!opts?.fatal &&
                        ...
                        if (!retryUsedRef.current) {
                            ...
                            (0, debug_js_1.logForDebugging)(`[voice] early voice_stream error (pre-transcript), retrying once: ${error}`);
                    ...
                    attemptGenRef.current++;
                    (0, log_js_1.logError)(new Error(`[voice] voice_stream error: ${error}`));
                ...
                onReady: conn => {
                    ...
                    if (audioBuffer.length > 0) {
                        ...
                        (0, debug_js_1.logForDebugging)(`[voice] onReady: flushing ${String(audioBuffer.length)} buffered chunks (${String(totalBytes)} bytes) as ${String(slices.length)} coalesced frame(s)`);
    ...
    const handleKeyEvent = (0, react_1.useCallback)((fallbackMs = REPEAT_FALLBACK_MS) => {
