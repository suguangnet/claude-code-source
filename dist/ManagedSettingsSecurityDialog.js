"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ManagedSettingsSecurityDialog = ManagedSettingsSecurityDialog;
const compiler_runtime_1 = require("react/compiler-runtime");
const react_1 = __importDefault(require("react"));
const useExitOnCtrlCDWithKeybindings_js_1 = require("../../hooks/useExitOnCtrlCDWithKeybindings.js");
const ink_js_1 = require("../../ink.js");
const useKeybinding_js_1 = require("../../keybindings/useKeybinding.js");
const index_js_1 = require("../CustomSelect/index.js");
const PermissionDialog_js_1 = require("../permissions/PermissionDialog.js");
const utils_js_1 = require("./utils.js");
function ManagedSettingsSecurityDialog(t0) {
    const $ = (0, compiler_runtime_1.c)(26);
    const { settings, onAccept, onReject } = t0;
    const dangerous = (0, utils_js_1.extractDangerousSettings)(settings);
    const settingsList = (0, utils_js_1.formatDangerousSettingsList)(dangerous);
    const exitState = (0, useExitOnCtrlCDWithKeybindings_js_1.useExitOnCtrlCDWithKeybindings)();
    let t1;
    if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
        t1 = {
            context: "Confirmation"
        };
        $[0] = t1;
    }
    else {
        t1 = $[0];
    }
    (0, useKeybinding_js_1.useKeybinding)("confirm:no", onReject, t1);
    let t2;
    if ($[1] !== onAccept || $[2] !== onReject) {
        t2 = function onChange(value) {
            if (value === "exit") {
                onReject();
                return;
            }
            onAccept();
        };
        $[1] = onAccept;
        $[2] = onReject;
        $[3] = t2;
    }
    else {
        t2 = $[3];
    }
    const onChange = t2;
    const T0 = PermissionDialog_js_1.PermissionDialog;
    const t3 = "warning";
    const t4 = "warning";
    const t5 = "Managed settings require approval";
    const T1 = ink_js_1.Box;
    const t6 = "column";
    const t7 = 1;
    const t8 = 1;
    let t9;
    if ($[4] === Symbol.for("react.memo_cache_sentinel")) {
        t9 = react_1.default.createElement(ink_js_1.Text, null, "Your organization has configured managed settings that could allow execution of arbitrary code or interception of your prompts and responses.");
        $[4] = t9;
    }
    else {
        t9 = $[4];
    }
    const T2 = ink_js_1.Box;
    const t10 = "column";
    let t11;
    if ($[5] === Symbol.for("react.memo_cache_sentinel")) {
        t11 = react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "Settings requiring approval:");
        $[5] = t11;
    }
    else {
        t11 = $[5];
    }
    const t12 = settingsList.map(_temp);
    let t13;
    if ($[6] !== T2 || $[7] !== t11 || $[8] !== t12) {
        t13 = react_1.default.createElement(T2, { flexDirection: t10 },
            t11,
            t12);
        $[6] = T2;
        $[7] = t11;
        $[8] = t12;
        $[9] = t13;
    }
    else {
        t13 = $[9];
    }
    let t14;
    if ($[10] === Symbol.for("react.memo_cache_sentinel")) {
        t14 = react_1.default.createElement(ink_js_1.Text, null, "Only accept if you trust your organization's IT administration and expect these settings to be configured.");
        $[10] = t14;
    }
    else {
        t14 = $[10];
    }
    let t15;
    if ($[11] === Symbol.for("react.memo_cache_sentinel")) {
        t15 = [{
                label: "Yes, I trust these settings",
                value: "accept"
            }, {
                label: "No, exit Claude Code",
                value: "exit"
            }];
        $[11] = t15;
    }
    else {
        t15 = $[11];
    }
    let t16;
    if ($[12] !== onChange) {
        t16 = react_1.default.createElement(index_js_1.Select, { options: t15, onChange: value_0 => onChange(value_0), onCancel: () => onChange("exit") });
        $[12] = onChange;
        $[13] = t16;
    }
    else {
        t16 = $[13];
    }
    let t17;
    if ($[14] !== exitState.keyName || $[15] !== exitState.pending) {
        t17 = react_1.default.createElement(ink_js_1.Text, { dimColor: true }, exitState.pending ? react_1.default.createElement(react_1.default.Fragment, null,
            "Press ",
            exitState.keyName,
            " again to exit") : react_1.default.createElement(react_1.default.Fragment, null, "Enter to confirm \u00B7 Esc to exit"));
        $[14] = exitState.keyName;
        $[15] = exitState.pending;
        $[16] = t17;
    }
    else {
        t17 = $[16];
    }
    let t18;
    if ($[17] !== T1 || $[18] !== t13 || $[19] !== t16 || $[20] !== t17 || $[21] !== t9) {
        t18 = react_1.default.createElement(T1, { flexDirection: t6, gap: t7, paddingTop: t8 },
            t9,
            t13,
            t14,
            t16,
            t17);
        $[17] = T1;
        $[18] = t13;
        $[19] = t16;
        $[20] = t17;
        $[21] = t9;
        $[22] = t18;
    }
    else {
        t18 = $[22];
    }
    let t19;
    if ($[23] !== T0 || $[24] !== t18) {
        t19 = react_1.default.createElement(T0, { color: t3, titleColor: t4, title: t5 }, t18);
        $[23] = T0;
        $[24] = t18;
        $[25] = t19;
    }
    else {
        t19 = $[25];
    }
    return t19;
}
function _temp(item, index) {
    return react_1.default.createElement(ink_js_1.Box, { key: index, paddingLeft: 2 },
        react_1.default.createElement(ink_js_1.Text, null,
            react_1.default.createElement(ink_js_1.Text, { dimColor: true }, "\u00B7 "),
            react_1.default.createElement(ink_js_1.Text, null, item)));
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
