...
function useKeybindingWarnings(warnings, isReload) {
    ...
    if ($[0] !== addNotification || $[1] !== removeNotification || $[2] !== warnings) {
        t0 = () => {
            ...
            const errorCount = (0, array_js_1.count)(warnings, _temp);
            const warnCount = (0, array_js_1.count)(warnings, _temp2);
            let message;
            if (errorCount > 0 && warnCount > 0) {
                message = `Found ${errorCount} keybinding ${(0, stringUtils_js_1.plural)(errorCount, "error")} and ${warnCount} ${(0, stringUtils_js_1.plural)(warnCount, "warning")}`;
            }
            else {
                if (errorCount > 0) {
                    message = `Found ${errorCount} keybinding ${(0, stringUtils_js_1.plural)(errorCount, "error")}`;
                }
                else {
                    message = `Found ${warnCount} keybinding ${(0, stringUtils_js_1.plural)(warnCount, "warning")}`;
                }
            }
...
function KeybindingSetup({ children }) {
    ...
    const [{ bindings, warnings }, setLoadResult] = (0, react_1.useState)(() => {
        const result = (0, loadUserBindings_js_1.loadKeybindingsSyncWithWarnings)();
        (0, debug_js_1.logForDebugging)(`[keybindings] KeybindingSetup initialized with ${result.bindings.length} bindings, ${result.warnings.length} warnings`);
        return result;
    });
    ...
    const pendingChordRef = (0, react_1.useRef)(null);
    const [pendingChord, setPendingChordState] = (0, react_1.useState)(null);
    const chordTimeoutRef = (0, react_1.useRef)(null);
    // Handler registry for action callbacks (used by ChordInterceptor to invoke handlers)
    const handlerRegistryRef = (0, react_1.useRef)(new Map());
    ...
    const activeContextsRef = (0, react_1.useRef)(new Set());
    const registerActiveContext = (0, react_1.useCallback)((context) => {
        activeContextsRef.current.add(context);
    }, []);
    const unregisterActiveContext = (0, react_1.useCallback)((context_0) => {
        activeContextsRef.current.delete(context_0);
    }, []);
    // Clear chord timeout when component unmounts or chord changes
    const clearChordTimeout = (0, react_1.useCallback)(() => {
    ...
    const setPendingChord = (0, react_1.useCallback)((pending) => {
        clearChordTimeout();
        if (pending !== null) {
    ...
    (0, react_1.useEffect)(() => {
        ...
        const unsubscribe = (0, loadUserBindings_js_1.subscribeToKeybindingChanges)(result_0 => {
            ...
            (0, debug_js_1.logForDebugging)(`[keybindings] Reloaded: ${result_0.bindings.length} bindings, ${result_0.warnings.length} warnings`);
        });
        return () => {
            unsubscribe();
            clearChordTimeout();
...
function ChordInterceptor(t0) {
    ...
    if ($[0] !== activeContexts || $[1] !== bindings || $[2] !== handlerRegistryRef || $[3] !== pendingChordRef || $[4] !== setPendingChord) {
        t1 = (input, key, event) => {
            ...
            const result = (0, resolver_js_1.resolveKeyWithChordState)(input, key, contexts, bindings, pendingChordRef.current);
